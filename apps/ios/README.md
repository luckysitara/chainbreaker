# Chainbreaker iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Chainbreaker Gateway as a `role: node`.
...
- Beta release uses canonical `ai.chainbreaker.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `ChainbreakerPushTransport=relay`, `ChainbreakerPushDistribution=official`, and `ChainbreakerPushAPNsEnvironment=production`.
...
- `CHAINBREAKER_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
- `apps/ios/Sources/Chainbreaker.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `ChainbreakerPushTransport=direct` and `ChainbreakerPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `ChainbreakerPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `chainbreaker.json`. `CHAINBREAKER_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Chainbreaker official builds and to ensure a

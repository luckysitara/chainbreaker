...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.chainbreaker.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Chainbreaker", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Chainbreaker gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Chainbreaker.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "ChainbreakerShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Chainbreaker is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Chainbreaker Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("chainbreaker-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in

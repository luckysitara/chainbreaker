...
@Suite(.serialized) struct VoiceWakeManagerStateTests {
    ...
    @Test @MainActor func handleRecognitionCallbackDispatchesCommand() async {
        let manager = VoiceWakeManager()
        manager.triggerWords = ["chainbreaker"]
        manager.isEnabled = true
        ...
        let transcript = "chainbreaker hello"
        let triggerRange = transcript.range(of: "chainbreaker")!
        let helloRange = transcript.range(of: "hello")!
        let segments = [
            WakeWordSegment(text: "chainbreaker", start: 0.0, duration: 0.2, range: triggerRange),

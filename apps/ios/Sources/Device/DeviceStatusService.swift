...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> ChainbreakerDeviceStatusPayload {
        ...
        return ChainbreakerDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> ChainbreakerDeviceInfoPayload {
        ...
        return ChainbreakerDeviceInfoPayload(
    ...
    private func batteryStatus() -> ChainbreakerBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: ChainbreakerBatteryState = switch device.batteryState {
        ...
        return ChainbreakerBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> ChainbreakerThermalStatusPayload {
        let state: ChainbreakerThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return ChainbreakerThermalStatusPayload(state: state)
    }

    private func storageStatus() -> ChainbreakerStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return ChainbreakerStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }

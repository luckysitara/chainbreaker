...
final class MotionService: MotionServicing {
    func activities(params: ChainbreakerMotionActivityParams) async throws -> ChainbreakerMotionActivityPayload {
        ...
        let mapped: [ChainbreakerMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        ChainbreakerMotionActivityEntry(
        ...
        return ChainbreakerMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ChainbreakerPedometerParams) async throws -> ChainbreakerPedometerPayload {
        ...
        let payload: ChainbreakerPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ChainbreakerPedometerPayload(

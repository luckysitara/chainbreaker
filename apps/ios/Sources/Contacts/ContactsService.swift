...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ChainbreakerContactsSearchParams) async throws -> ChainbreakerContactsSearchPayload {
        ...
        return ChainbreakerContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ChainbreakerContactsAddParams) async throws -> ChainbreakerContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ChainbreakerContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ChainbreakerContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ChainbreakerContactPayload {
        ChainbreakerContactPayload(

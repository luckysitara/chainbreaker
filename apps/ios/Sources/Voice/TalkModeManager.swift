...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__CHAINBREAKER_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ChainbreakerTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.chainbreaker", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ChainbreakerTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ChainbreakerTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return ChainbreakerTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ChainbreakerTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ChainbreakerTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ChainbreakerTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ChainbreakerTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ChainbreakerTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ChainbreakerTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: ChainbreakerAgentEventPayload.self
            ) else {
                continue
            }

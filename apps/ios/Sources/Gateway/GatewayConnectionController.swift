...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [ChainbreakerCapability.canvas.rawValue, ChainbreakerCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(ChainbreakerCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ChainbreakerCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ChainbreakerLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ChainbreakerCapability.location.rawValue) }

        caps.append(ChainbreakerCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(ChainbreakerCapability.watch.rawValue)
        }
        caps.append(ChainbreakerCapability.photos.rawValue)
        caps.append(ChainbreakerCapability.contacts.rawValue)
        caps.append(ChainbreakerCapability.calendar.rawValue)
        caps.append(ChainbreakerCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(ChainbreakerCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            ChainbreakerScreenCommand.record.rawValue,
            ChainbreakerSystemCommand.notify.rawValue,
            ChainbreakerChatCommand.push.rawValue,
        ...
        if caps.contains(ChainbreakerCapability.camera.rawValue) {
            commands.append(ChainbreakerCameraCommand.list.rawValue)
            commands.append(ChainbreakerCameraCommand.snap.rawValue)
            commands.append(ChainbreakerCameraCommand.clip.rawValue)
        }
        if caps.contains(ChainbreakerCapability.location.rawValue) {
            commands.append(ChainbreakerLocationCommand.get.rawValue)
        }
        if caps.contains(ChainbreakerCapability.device.rawValue) {
            commands.append(ChainbreakerDeviceCommand.status.rawValue)
            commands.append(ChainbreakerDeviceCommand.info.rawValue)
        }
        if caps.contains(ChainbreakerCapability.watch.rawValue) {
            commands.append(ChainbreakerWatchCommand.status.rawValue)
            commands.append(ChainbreakerWatchCommand.notify.rawValue)
        }
        if caps.contains(ChainbreakerCapability.photos.rawValue) {
            commands.append(ChainbreakerPhotosCommand.latest.rawValue)
        }
        if caps.contains(ChainbreakerCapability.contacts.rawValue) {
            commands.append(ChainbreakerContactsCommand.search.rawValue)
            commands.append(ChainbreakerContactsCommand.add.rawValue)
        }
        if caps.contains(ChainbreakerCapability.calendar.rawValue) {
            commands.append(ChainbreakerCalendarCommand.events.rawValue)
            commands.append(ChainbreakerCalendarCommand.add.rawValue)
        }
        if caps.contains(ChainbreakerCapability.reminders.rawValue) {
            commands.append(ChainbreakerRemindersCommand.list.rawValue)
            commands.append(ChainbreakerRemindersCommand.add.rawValue)
        }
        if caps.contains(ChainbreakerCapability.motion.rawValue) {
            commands.append(ChainbreakerMotionCommand.activity.rawValue)
            commands.append(ChainbreakerMotionCommand.pedometer.rawValue)
        }

...
final class RemindersService: RemindersServicing {
    func list(params: ChainbreakerRemindersListParams) async throws -> ChainbreakerRemindersListPayload {
        ...
        let payload: [ChainbreakerReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return ChainbreakerReminderPayload(
        ...
        return ChainbreakerRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ChainbreakerRemindersAddParams) async throws -> ChainbreakerRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ChainbreakerReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ChainbreakerRemindersAddPayload(reminder: payload)
    }

...
@MainActor
final class ChainbreakerAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.chainbreaker.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.chainbreaker.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.chainbreaker.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "chainbreaker.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "chainbreaker.watch.promptId"
    static let sessionKeyKey = "chainbreaker.watch.sessionKey"
    static let actionPrimaryIDKey = "chainbreaker.watch.action.primary.id"
    static let actionPrimaryLabelKey = "chainbreaker.watch.action.primary.label"
    static let actionSecondaryIDKey = "chainbreaker.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "chainbreaker.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "chainbreaker.watch.action.primary"
    static let actionSecondaryIdentifier = "chainbreaker.watch.action.secondary"
    static let actionIdentifierPrefix = "chainbreaker.watch.action."
    static let actionIDKeyPrefix = "chainbreaker.watch.action.id."
    static let actionLabelKeyPrefix = "chainbreaker.watch.action.label."
    static let categoryPrefix = "chainbreaker.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ChainbreakerWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ChainbreakerWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ChainbreakerWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Chainbreaker" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ChainbreakerWatchAction]) -> [UNNotificationAction] {
...
@main
struct ChainbreakerApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(ChainbreakerAppDelegate.self) private var appDelegate
...
extension ChainbreakerApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Chainbreaker: installing uncaught exception handler")

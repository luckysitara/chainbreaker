...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = ChainbreakerLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = ChainbreakerLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.chainbreaker.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
                ...
                Section {
                    DisclosureGroup(isExpanded: self.$gatewayExpanded) {
                        if !self.isGatewayConnected {
                            Text(
                                "1. Open a chat with your Chainbreaker agent and send /pair\n"
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        self.featureToggle(
                            ...
                            help: "Enables voice conversation mode with your connected Chainbreaker agent.") { newValue in
                        ...
                        self.featureToggle(
                            ...
                            help: "Allows the gateway to request photos or short video clips "
                                + "while Chainbreaker is foregrounded."
                        )
                        ...
                        HStack(spacing: 8) {
                            ...
                            Button {
                                self.activeFeatureHelp = FeatureHelp(
                                    ...
                                    message: "Controls location permissions for Chainbreaker. "
                        ...
                        Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                            Text("Off").tag(ChainbreakerLocationMode.off.rawValue)
                            Text("While Using").tag(ChainbreakerLocationMode.whileUsing.rawValue)
                            Text("Always").tag(ChainbreakerLocationMode.always.rawValue)
                        }
                        .labelsHidden()
                        ...
                        self.featureToggle(
                            ...
                            help: "Keeps the screen awake while Chainbreaker is open.")
                    ...
                    DisclosureGroup("Device Info") {
                        ...
                        LabeledContent("Chainbreaker", value: DeviceInfoHelper.openClawVersionString())
                    }
                }
            }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = ChainbreakerLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Go back to your Chainbreaker chat and run /pair approve, then tap Connect again."
        }

...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.chainbreaker.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.chainbreaker.ios", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.chainbreaker.ios", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.chainbreaker.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.chainbreaker.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = ChainbreakerCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = ChainbreakerCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = ChainbreakerCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = ChainbreakerCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = ChainbreakerCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: ChainbreakerLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(ChainbreakerGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ChainbreakerNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ChainbreakerNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ChainbreakerNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(ChainbreakerLocationGetParams.self, from: req.paramsJSON)) ??
            ChainbreakerLocationGetParams()
        ...
        let payload = ChainbreakerLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(ChainbreakerCanvasPresentParams.self, from: req.paramsJSON)) ??
                ChainbreakerCanvasPresentParams()
        ...
        case ChainbreakerCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ChainbreakerCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(ChainbreakerCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ChainbreakerCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(ChainbreakerCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ChainbreakerCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(ChainbreakerCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case ChainbreakerCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.chainbreakerA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing chainbreakerA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case ChainbreakerCanvasA2UICommand.push.rawValue, ChainbreakerCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [ChainbreakerKit.AnyCodable]
            if command == ChainbreakerCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(ChainbreakerCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try ChainbreakerCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(ChainbreakerCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try ChainbreakerCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.chainbreakerA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ChainbreakerCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(ChainbreakerCameraSnapParams.self, from: req.paramsJSON)) ??
                ChainbreakerCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ChainbreakerCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(ChainbreakerCameraClipParams.self, from: req.paramsJSON)) ??
                ChainbreakerCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ChainbreakerScreenRecordParams.self, from: req.paramsJSON)) ??
            ChainbreakerScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ChainbreakerSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ChainbreakerNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ChainbreakerNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ChainbreakerNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ChainbreakerChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ChainbreakerNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = ChainbreakerChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ChainbreakerPhotosLatestParams.self, from: req.paramsJSON)) ??
            ChainbreakerPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(ChainbreakerContactsSearchParams.self, from: req.paramsJSON)) ??
                ChainbreakerContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerContactsCommand.add.rawValue:
            let params = try Self.decodeParams(ChainbreakerContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(ChainbreakerCalendarEventsParams.self, from: req.paramsJSON)) ??
                ChainbreakerCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(ChainbreakerCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(ChainbreakerRemindersListParams.self, from: req.paramsJSON)) ??
                ChainbreakerRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(ChainbreakerRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(ChainbreakerMotionActivityParams.self, from: req.paramsJSON)) ??
                ChainbreakerMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(ChainbreakerPedometerParams.self, from: req.paramsJSON)) ??
                ChainbreakerPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([ChainbreakerLocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            ChainbreakerCanvasCommand.present.rawValue,
            ChainbreakerCanvasCommand.hide.rawValue,
            ChainbreakerCanvasCommand.navigate.rawValue,
            ChainbreakerCanvasCommand.evalJS.rawValue,
            ChainbreakerCanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            ChainbreakerCanvasA2UICommand.reset.rawValue,
            ChainbreakerCanvasA2UICommand.push.rawValue,
            ChainbreakerCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([ChainbreakerScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([ChainbreakerSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([ChainbreakerChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            ChainbreakerWatchCommand.status.rawValue,
            ChainbreakerWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([ChainbreakerPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            ChainbreakerRemindersCommand.list.rawValue,
            ChainbreakerRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            ChainbreakerTalkCommand.pttStart.rawValue,
            ChainbreakerTalkCommand.pttStop.rawValue,
            ChainbreakerTalkCommand.pttCancel.rawValue,
            ChainbreakerTalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ChainbreakerWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = ChainbreakerWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ChainbreakerWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(ChainbreakerWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = ChainbreakerWatchNotifyPayload(
    ...
    func locationMode() -> ChainbreakerLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return ChainbreakerLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to Chainbreaker."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to Chainbreaker."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "chainbreaker-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "Chainbreaker Share Self-Test",
            url: URL(string: "https://chainbreaker.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.chainbreakerPushKind(userInfo)
        let receivedMessage =
    ...
    private static func chainbreakerPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["chainbreaker"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["chainbreaker"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }

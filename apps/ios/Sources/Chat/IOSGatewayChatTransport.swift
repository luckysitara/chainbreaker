...
struct IOSGatewayChatTransport: ChainbreakerChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.chainbreaker", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> ChainbreakerChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ChainbreakerChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ChainbreakerChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ChainbreakerChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ChainbreakerChatAttachmentPayload]) async throws -> ChainbreakerChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ChainbreakerChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ChainbreakerChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(ChainbreakerGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ChainbreakerGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ChainbreakerChatEventPayload.self)
                        {

import Foundation
import Testing
@testable import ChainbreakerKit
...
struct GatewayNodeSessionTests {
    ...
    func scannedSetupCodePrefersBootstrapAuthOverStoredDeviceToken() async throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["CHAINBREAKER_STATE_DIR"]
        setenv("CHAINBREAKER_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("CHAINBREAKER_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("CHAINBREAKER_STATE_DIR")
            }
        ...
        let options = GatewayConnectOptions(
            ...
            commands: [],
            permissions: [:],
            clientId: "chainbreaker-ios-test",
            clientMode: "ui",
    ...
    func normalizeCanvasHostUrlPreservesExplicitSecureCanvasPort() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "https://canvas.example.com:9443/__chainbreaker__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com")!)

        #expect(normalized == "https://canvas.example.com:9443/__chainbreaker__/cap/token")
    }

    @Test
    func normalizeCanvasHostUrlBackfillsGatewayHostForLoopbackCanvas() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "http://127.0.0.1:18789/__chainbreaker__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com:7443")!)

        #expect(normalized == "https://gateway.example.com:7443/__chainbreaker__/cap/token")
    }

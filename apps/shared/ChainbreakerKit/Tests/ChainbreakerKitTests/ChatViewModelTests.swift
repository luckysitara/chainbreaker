...
import Foundation
import Testing
@testable import ChainbreakerChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> ChainbreakerChatHistoryPayload
{
    ChainbreakerChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> ChainbreakerChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> ChainbreakerChatSessionEntry
{
    ChainbreakerChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> ChainbreakerChatModelChoice {
    ChainbreakerChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [ChainbreakerChatHistoryPayload],
    sessionsResponses: [ChainbreakerChatSessionsListResponse] = [],
    modelResponses: [[ChainbreakerChatModelChoice]] = [],
    ...
    -> (TestChatTransport, ChainbreakerChatViewModel)
{
...
private func sendUserMessage(_ vm: ChainbreakerChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, ChainbreakerChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ChainbreakerChatHistoryPayload]
    private let sessionsResponses: [ChainbreakerChatSessionsListResponse]
    private let modelResponses: [[ChainbreakerChatModelChoice]]
    ...
    private let stream: AsyncStream<ChainbreakerChatTransportEvent>
    private let continuation: AsyncStream<ChainbreakerChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<ChainbreakerChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ChainbreakerChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ChainbreakerChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [ChainbreakerChatAttachmentPayload]) async throws -> ChainbreakerChatSendResponse
    {
        ...
        return ChainbreakerChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ChainbreakerChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ChainbreakerChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ChainbreakerChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = ChainbreakerChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = ChainbreakerChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: ChainbreakerChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = ChainbreakerChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ChainbreakerChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(ChainbreakerChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == ChainbreakerChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = ChainbreakerChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ChainbreakerChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = ChainbreakerChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = ChainbreakerChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = ChainbreakerChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { ChainbreakerChatViewModel(sessionKey: "main", transport: transport) }

...
@MainActor
struct ChainbreakerChatComposer: View {
    private static let menuThinkingLevels = ["off", "low", "medium", "high"]

    @Bindable var viewModel: ChainbreakerChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ChainbreakerChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ChainbreakerChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(ChainbreakerChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ChainbreakerPendingAttachment.id)
                { (att: ChainbreakerPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ChainbreakerPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(ChainbreakerChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ChainbreakerChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ChainbreakerChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ChainbreakerChatTheme.subtleCard)
        .clipShape(Capsule())
    }

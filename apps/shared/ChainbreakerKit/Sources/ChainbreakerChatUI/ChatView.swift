...
@MainActor
public struct ChainbreakerChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: ChainbreakerChatViewModel
    ...
    public init(
        viewModel: ChainbreakerChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                ChainbreakerChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [ChainbreakerChatMessage] {
        let base: [ChainbreakerChatMessage]
    ...
    private func mergeToolResults(in messages: [ChainbreakerChatMessage]) -> [ChainbreakerChatMessage] {
        ...
        for message in messages {
            ...
            let merged = ChainbreakerChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: ChainbreakerChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: ChainbreakerChatMessage) -> Bool {
    ...
    private func primaryText(in message: ChainbreakerChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: ChainbreakerChatMessage) -> Bool {
    ...
    private func toolCalls(in message: ChainbreakerChatMessage) -> [ChainbreakerChatMessageContent] {
    ...
    private func inlineToolResults(in message: ChainbreakerChatMessage) -> [ChainbreakerChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: ChainbreakerChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: ChainbreakerChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(ChainbreakerChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)

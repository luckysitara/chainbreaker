...
private let chatUILogger = Logger(subsystem: "ai.chainbreaker", category: "ChainbreakerChatUI")

@MainActor
@Observable
public final class ChainbreakerChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [ChainbreakerChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [ChainbreakerChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [ChainbreakerPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ChainbreakerChatPendingToolCall] = []
    public private(set) var sessions: [ChainbreakerChatSessionEntry] = []
    private let transport: any ChainbreakerChatTransport
    private var sessionDefaults: ChainbreakerChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: ChainbreakerChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any ChainbreakerChatTransport,
    ...
    public var sessionChoices: [ChainbreakerChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [ChainbreakerChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ChainbreakerPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ChainbreakerChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ChainbreakerChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: ChainbreakerChatMessage) -> ChainbreakerChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ChainbreakerChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ChainbreakerChatMessageContent(
                type: content.type,
                text: cleaned,
        ...
        return ChainbreakerChatMessage(
            id: message.id,
            role: message.role,
    ...
    private static func messageContentFingerprint(for message: ChainbreakerChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: ChainbreakerChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: ChainbreakerChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [ChainbreakerChatMessage],
        incoming: [ChainbreakerChatMessage]) -> [ChainbreakerChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ChainbreakerChatMessage]) -> [ChainbreakerChatMessage] {
        var result: [ChainbreakerChatMessage] = []
    ...
    private static func dedupeKey(for message: ChainbreakerChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ChainbreakerChatMessageContent] = [
            ChainbreakerChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ChainbreakerChatAttachmentPayload in
            ChainbreakerChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ChainbreakerChatSessionEntry {
        ChainbreakerChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = ChainbreakerChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: ChainbreakerChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ChainbreakerChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ChainbreakerAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ChainbreakerChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ChainbreakerPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)

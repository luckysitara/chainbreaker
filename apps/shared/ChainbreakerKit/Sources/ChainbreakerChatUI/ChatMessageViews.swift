...
struct ChatMessageBubble: View {
    let message: ChainbreakerChatMessage
    let style: ChainbreakerChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ChainbreakerChatTheme.userText : ChainbreakerChatTheme.assistantText
    ...
    private var inlineAttachments: [ChainbreakerChatMessageContent] {
    ...
    private var toolCalls: [ChainbreakerChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ChainbreakerChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ChainbreakerChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ChainbreakerChatTheme.onboardingAssistantBubble
        }
        return ChainbreakerChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ChainbreakerChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ChainbreakerChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ChainbreakerChatTheme.userText : ChainbreakerChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ChainbreakerChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ChainbreakerChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ChainbreakerChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ChainbreakerChatTheme.assistantText)
            }
        }
    }
}

...
enum ChainbreakerChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ChainbreakerChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ChainbreakerChatTheme.onboardingAssistantBubble"),
...
enum ChainbreakerPlatformImageFactory {
    static func image(_ image: ChainbreakerPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)

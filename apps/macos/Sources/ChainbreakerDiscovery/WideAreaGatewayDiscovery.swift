...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) -> [WideAreaGatewayBeacon]
    {
        ...
        for raw in ptrLines {
            ...
            let suffix = "._chainbreaker-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = ChainbreakerBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_chainbreaker-gw._tcp.\(domainTrimmed)"

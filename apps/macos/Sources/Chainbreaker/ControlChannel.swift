...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ChainbreakerProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.chainbreaker", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ChainbreakerKit.AnyCodable]()) {
                $0[$1.key] = ChainbreakerKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ChainbreakerProtocol.AnyCodable?) -> [String: ChainbreakerProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ChainbreakerProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ChainbreakerKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ChainbreakerProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("chainbreaker.control.heartbeat")
    static let controlAgentEvent = Notification.Name("chainbreaker.control.agent")
}

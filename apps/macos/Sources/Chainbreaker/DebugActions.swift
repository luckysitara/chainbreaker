...
enum DebugActions {
    private static let verboseDefaultsKey = "chainbreaker.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "chainbreaker.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ChainbreakerPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Chainbreaker", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ChainbreakerPaths.configURL
        guard

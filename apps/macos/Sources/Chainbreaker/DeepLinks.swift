...
private let deepLinkLogger = Logger(subsystem: "ai.chainbreaker", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Chainbreaker is paused", message: "Unpause Chainbreaker to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Chainbreaker agent?", message: body) else { return }
        }

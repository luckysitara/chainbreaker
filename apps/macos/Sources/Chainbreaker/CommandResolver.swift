...
enum CommandResolver {
    private static let projectRootDefaultsKey = "chainbreaker.gatewayProjectRootPath"
    private static let helperName = "chainbreaker"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let chainbreakerEntry = root.appendingPathComponent("chainbreaker.mjs").path
        if FileManager().isReadableFile(atPath: chainbreakerEntry) { return chainbreakerEntry }
        let binEntry = root.appendingPathComponent("bin/chainbreaker.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__CHAINBREAKER_ERR__' >&2
        \(message)
        __CHAINBREAKER_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/chainbreaker")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let chainbreakerPaths = self.chainbreakerManagedPaths(home: home)
        if !chainbreakerPaths.isEmpty {
            extras.insert(contentsOf: chainbreakerPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + chainbreakerPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func chainbreakerManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".chainbreaker"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func chainbreakerExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectChainbreakerExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("chainbreaker.mjs").path,
            root.appendingPathComponent("bin/chainbreaker.js").path,
        ]
    ...
    static func hasAnyChainbreakerInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.chainbreakerExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func chainbreakerNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let chainbreakerPath = self.projectChainbreakerExecutable(projectRoot: root) {
            return [chainbreakerPath, subcommand] + extraArgs
        }
        if let chainbreakerPath = self.chainbreakerExecutable(searchPaths: searchPaths) {
            return [chainbreakerPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "chainbreaker", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            chainbreaker entrypoint missing (looked for dist/index.js or chainbreaker.mjs); run pnpm build.
            """
    ...
    static func chainbreakerCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real chainbreaker CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/chainbreaker"
        ...
        if command -v chainbreaker >/dev/null 2>&1; then
          CLI="$(command -v chainbreaker)"
          chainbreaker \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/chainbreaker.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/chainbreaker.mjs"
            node "$PRJ/chainbreaker.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/chainbreaker.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/chainbreaker.js"
            node "$PRJ/bin/chainbreaker.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent chainbreaker"
          pnpm --silent chainbreaker \(quotedArgs);
        else
          echo "chainbreaker CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ChainbreakerConfigFile.loadDict()

...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.chainbreaker/chainbreaker.json. Prefer CHAINBREAKER_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.chainbreaker/chainbreaker.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.chainbreaker/chainbreaker.json. Restart the gateway to apply."
        }

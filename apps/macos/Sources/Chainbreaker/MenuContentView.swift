...
/// Menu contents for the Chainbreaker menu bar extra.
struct MenuContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            self.debugMenu
            Button("About Chainbreaker") { self.open(tab: .about) }
    ...
    private var connectionLabel: String {
        ...
        case .unconfigured:
            "Chainbreaker Not Configured"
        case .remote:
            "Remote Chainbreaker Active"
        case .local:
            "Chainbreaker Active"
        }
    }
    ...
    private func open(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .chainbreakerSelectSettingsTab, object: tab)
        }
    }

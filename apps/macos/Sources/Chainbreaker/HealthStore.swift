...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "ai.chainbreaker", category: "health")
    ...
    var summaryLine: String {
        ...
        if link.summary.linked != true {
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return "\(fallbackLabel) \(fallbackState) · Not linked — run chainbreaker login"
            }
            return "Not linked — run chainbreaker login"
        }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (\(host)) isn’t listening — restart Chainbreaker to bring it back."
            }

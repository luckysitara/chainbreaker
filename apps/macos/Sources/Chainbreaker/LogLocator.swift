...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CHAINBREAKER_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/chainbreaker")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("chainbreaker-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("chainbreaker-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/chainbreaker/ (rolling or stdout), or nil if none exist.

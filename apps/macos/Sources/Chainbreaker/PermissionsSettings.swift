    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so Chainbreaker can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = ChainbreakerLocationMode.off.rawValue
    ...
    private var locationMode: ChainbreakerLocationMode {
        ChainbreakerLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: ChainbreakerLocationMode) async -> Bool {

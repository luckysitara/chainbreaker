...
private let webChatSwiftLogger = Logger(subsystem: "ai.chainbreaker", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "chainbreaker.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: ChainbreakerChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> ChainbreakerChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(ChainbreakerChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            ChainbreakerChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return ChainbreakerChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ChainbreakerChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ChainbreakerGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    ChainbreakerChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ChainbreakerAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: ChainbreakerProtocol.ModelChoice) -> ChainbreakerChatModelChoice {
        ChainbreakerChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ChainbreakerChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ChainbreakerChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ChainbreakerChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: ChainbreakerChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "Chainbreaker Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ChainbreakerChatView>) -> NSViewController
    {

...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.chainbreaker.mac`) even if the shipped
    ...
    {
        if environment["CHAINBREAKER_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "chainbreaker.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "chainbreaker.nixMode") { return true }

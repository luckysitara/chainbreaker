...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.chainbreaker", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ChainbreakerSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ChainbreakerSessionPreviewEntry,
    ...
    private static func cache(payload: ChainbreakerSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ChainbreakerChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ChainbreakerKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ChainbreakerKit.AnyCodable]) -> [ChainbreakerChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ChainbreakerChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ChainbreakerChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ChainbreakerChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ChainbreakerChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ChainbreakerChatMessage) -> String? {

...
enum ChainbreakerLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ChainbreakerOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ChainbreakerFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ChainbreakerLogging.bootstrapIfNeeded()
        let label = ChainbreakerLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ChainbreakerOSLogHandler: AppLogLevelBackedHandler {
...
struct ChainbreakerFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ChainbreakerLogging.parseLabel(self.label)

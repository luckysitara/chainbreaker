...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: ChainbreakerProtocol.AnyCodable(["ok": ChainbreakerProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ChainbreakerProtocol.AnyCodable(["ok": ChainbreakerProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = ChainbreakerProtocol.AnyCodable([
            "runId": ChainbreakerProtocol.AnyCodable("run-1"),
            "sessionKey": ChainbreakerProtocol.AnyCodable("main"),
            "state": ChainbreakerProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ChainbreakerProtocol.AnyCodable(["a": ChainbreakerProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)

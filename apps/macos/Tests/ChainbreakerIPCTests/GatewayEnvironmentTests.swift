...
struct GatewayEnvironmentTests {
    @Test func `semver parses common forms`() {
        ...
        // Product-prefixed output from `chainbreaker --version` should NOT parse as semver
        // (the prefix must be stripped by the caller, not the parser).
        #expect(Semver.parse("Chainbreaker 2026.3.23-1") == nil)
    }

    @Test func `gateway version output strips product prefix before parsing`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("  Chainbreaker 2026.3.23-1 \n")
    ...
    @Test func `gateway port defaults and respects override`() async {
        ...
        await TestIsolation.withIsolatedState(
            env: ["CHAINBREAKER_CONFIG_PATH": configPath],

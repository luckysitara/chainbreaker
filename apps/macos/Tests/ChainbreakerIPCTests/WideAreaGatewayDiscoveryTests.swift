...
import Foundation
import Testing
@testable import ChainbreakerDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("CHAINBREAKER_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("CHAINBREAKER_WIDE_AREA_DOMAIN", "chainbreaker.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("CHAINBREAKER_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("CHAINBREAKER_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._chainbreaker-gw._tcp.chainbreaker.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.chainbreaker.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Chainbreaker)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/chainbreaker/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Chainbreaker)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/chainbreaker/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._chainbreaker-gw._tcp.chainbreaker.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {

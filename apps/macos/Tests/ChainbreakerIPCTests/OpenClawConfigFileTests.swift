...
@Suite(.serialized)
struct ChainbreakerConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("chainbreaker-config-\(UUID().uuidString)")
            .appendingPathComponent("chainbreaker.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["CHAINBREAKER_CONFIG_PATH": override]) {
            #expect(ChainbreakerConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["CHAINBREAKER_CONFIG_PATH": override]) {
            ChainbreakerConfigFile.saveDict([
                "gateway": [
                    "remote": [
            ...
            #expect(ChainbreakerConfigFile.remoteGatewayPort() == 19999)
            #expect(ChainbreakerConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(ChainbreakerConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(ChainbreakerConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["CHAINBREAKER_CONFIG_PATH": override]) {
            ...
            ChainbreakerConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = ChainbreakerConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["CHAINBREAKER_CONFIG_PATH": override]) {
            ...
            ChainbreakerConfigFile.clearRemoteGatewayUrl()
            let root = ChainbreakerConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("chainbreaker-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "CHAINBREAKER_CONFIG_PATH": nil,
            "CHAINBREAKER_STATE_DIR": dir,
        ]) {
            #expect(ChainbreakerConfigFile.stateDirURL().path == dir)
            #expect(ChainbreakerConfigFile.url().path == "\(dir)/chainbreaker.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("chainbreaker-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("chainbreaker.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "CHAINBREAKER_STATE_DIR": stateDir.path,
            "CHAINBREAKER_CONFIG_PATH": configPath.path,
        ]) {
            ChainbreakerConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-chainbreaker-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            _ = ChainbreakerConfigFile.loadDict()
            ...
            let loaded = ChainbreakerConfigFile.loadDict()

...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: ChainbreakerProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ChainbreakerProtocol.AnyCodable] = [
            "presence": .init([ChainbreakerProtocol.AnyCodable(entry)]),
        ]
        let payload = ChainbreakerProtocol.AnyCodable(payloadMap)

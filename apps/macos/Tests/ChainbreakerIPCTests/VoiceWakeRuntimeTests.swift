...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "chainbreaker"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["chainbreaker"]
        let text = "hey chainbreaker"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey chainbreaker do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["chainbreaker"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "chainbreaker", start: 0.2, duration: 0.1, range: transcript.range(of: "chainbreaker")),

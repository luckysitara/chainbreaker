...
private const val NOTIFICATION_CHANNEL_BASE_ID = "chainbreaker.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Chainbreaker Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Chainbreaker Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Chainbreaker Active")
      }

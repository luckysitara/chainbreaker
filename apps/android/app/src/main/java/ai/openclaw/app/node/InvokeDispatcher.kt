...
import ai.chainbreaker.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      ChainbreakerCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      ChainbreakerCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      ChainbreakerCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      ChainbreakerCanvasA2UICommand.Push.rawValue, ChainbreakerCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      ChainbreakerCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      ChainbreakerCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      ChainbreakerCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      ChainbreakerLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      ChainbreakerDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      ChainbreakerDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      ChainbreakerDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      ChainbreakerDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      ChainbreakerNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      ChainbreakerNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      ChainbreakerSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.chainbreaker.app.protocol.ChainbreakerPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      ChainbreakerContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      ChainbreakerContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      ChainbreakerCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      ChainbreakerCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      ChainbreakerMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      ChainbreakerMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      ChainbreakerSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      ChainbreakerSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      ChainbreakerCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)

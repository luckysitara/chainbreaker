...
val androidStoreFile = providers.gradleProperty("CHAINBREAKER_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("CHAINBREAKER_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("CHAINBREAKER_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("CHAINBREAKER_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set CHAINBREAKER_ANDROID_STORE_FILE, " +
            "CHAINBREAKER_ANDROID_STORE_PASSWORD, CHAINBREAKER_ANDROID_KEY_ALIAS, and " +
            "CHAINBREAKER_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/ChainbreakerKit/Sources/ChainbreakerKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.chainbreaker.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "CHAINBREAKER_ENABLE_SMS", "false")
            buildConfigField("boolean", "CHAINBREAKER_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "CHAINBREAKER_ENABLE_SMS", "true")
            buildConfigField("boolean", "CHAINBREAKER_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "chainbreaker-$versionName-$buildType.apk"
                    } else {
                        "chainbreaker-$versionName-$flavorName-$buildType.apk"
                    }

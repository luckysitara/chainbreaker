...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/chainbreaker`)
...
If the page fails to load, start the Gateway first: `chainbreaker gateway`.
...
created. Re-run `chainbreaker devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `chainbreaker devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.chainbreaker/chainbreaker.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop chainbreaker`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Chainbreaker
...
Chainbreaker **blocks** Control UI connections without device identity.

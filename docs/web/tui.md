...
```bash
chainbreaker tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `chainbreaker tui --deliver`
...
- Local shell commands receive `CHAINBREAKER_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `CHAINBREAKER_THEME=light` before launching `chainbreaker tui`.
- To force the original dark palette instead, set `CHAINBREAKER_THEME=dark`.
...
- Check the Gateway logs: `chainbreaker logs --follow`.
- Confirm the agent can run: `chainbreaker status` and `chainbreaker models status`.
...
- No agents in picker: check `chainbreaker agents list` and your routing config.

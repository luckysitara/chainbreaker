...
- `sessionTarget: "isolated"` or `sessionTarget: "session:..."` creates a visible task that shows up in `chainbreaker tasks` with delivery notifications.
...
- Jobs persist under `~/.chainbreaker/cron/` so restarts don’t lose schedules.
...
- For upgrades, `chainbreaker doctor --fix` can normalize legacy cron store fields, including old top-level delivery hints such as `threadId`.
...
chainbreaker cron list
chainbreaker cron run <job-id>
chainbreaker cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.chainbreaker/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `chainbreaker cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Chainbreaker applies a
...
Main-session cron jobs create [background task](/automation/tasks) records with `silent` notify policy (no notifications by default). They appear in `chainbreaker tasks list` but do not generate delivery messages.
...
These detached runs create [background task](/automation/tasks) records visible in `chainbreaker tasks` and subject to task audit and maintenance.
...
If `delivery` is omitted for isolated jobs, Chainbreaker defaults to `announce`.
...
- CLI equivalents: `chainbreaker cron add --light-context ...` and `chainbreaker cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `chainbreaker cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.chainbreaker/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.chainbreaker/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Chainbreaker classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.chainbreaker/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, Chainbreaker archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `chainbreaker cron runs` and adjust retention before logs become large
...
chainbreaker cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
chainbreaker cron edit <jobId> --agent ops
chainbreaker cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
chainbreaker cron run <jobId>
chainbreaker cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `chainbreaker cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
chainbreaker cron edit <jobId> \
...
```bash
chainbreaker cron edit <jobId> --exact
```
...
```bash
chainbreaker system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`chainbreaker system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `CHAINBREAKER_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Chainbreaker applies exponential retry backoff for recurring jobs after consecutive errors:

...
- **Cron (isolated)** is a scheduled **background run** — creates a task record tracked in `chainbreaker tasks`.
...
- **Task tracking**: isolated jobs create [background task](/automation/tasks) records visible in `chainbreaker tasks` and `chainbreaker tasks audit`.
...
chainbreaker cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
chainbreaker cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
chainbreaker cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```
...
| [Tasks](/automation/tasks) | No task record                  | Task record (silent)     | Task record (visible in `chainbreaker tasks`)   |

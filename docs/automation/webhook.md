...
read_when:
  ...
  - Wiring external systems into Chainbreaker
title: "Webhooks"
---
...
```json5
{
  hooks: {
    enabled: true,
    token: "${CHAINBREAKER_HOOKS_TOKEN}",
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Chainbreaker config directory (typically `~/.chainbreaker/hooks/transforms`).
...
- `chainbreaker webhooks gmail setup` writes `hooks.gmail` config for `chainbreaker webhooks gmail run`.

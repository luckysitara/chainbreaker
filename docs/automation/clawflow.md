...
- Use `chainbreaker flows list`, `chainbreaker flows show`, and `chainbreaker flows cancel` to inspect or manage flows.

## Quick start

```bash
chainbreaker flows list
chainbreaker flows show <flow-id-or-owner-session>
chainbreaker flows cancel <flow-id-or-owner-session>
```
...
- `chainbreaker flows list` shows active and recent flows
- `chainbreaker flows show <lookup>` shows one flow and its linked tasks
- `chainbreaker flows cancel <lookup>` cancels the flow and any active child tasks

...
- `chainbreaker tasks list` shows all tasks; `chainbreaker tasks audit` surfaces issues.
...
chainbreaker tasks list

# Filter by runtime or status
chainbreaker tasks list --runtime acp
chainbreaker tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
chainbreaker tasks show <lookup>

# Cancel a running task (kills the child session)
chainbreaker tasks cancel <lookup>

# Change notification policy for a task
chainbreaker tasks notify <lookup> state_changes

# Run a health audit
chainbreaker tasks audit
```
...
| CLI operations         | `cli`        | `chainbreaker agent` commands that run through the gateway | `done_only`           |
...
| `cancelled` | Stopped by the operator via `chainbreaker tasks cancel`                    |
...
When a task reaches a terminal state, Chainbreaker notifies you. There are two delivery paths:
...
```bash
chainbreaker tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
chainbreaker tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
chainbreaker tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `chainbreaker status` when issues are detected.
...
For the full operator ledger, use the CLI: `chainbreaker tasks list`.

## Status integration (task pressure)

`chainbreaker status` includes an at-a-glance task summary:
...
```
$CHAINBREAKER_STATE_DIR/tasks/runs.sqlite
```
...
A cron job **definition** lives in `~/.chainbreaker/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.

...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `chainbreaker hooks`, while hook-pack installation and updates now go through `chainbreaker plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Chainbreaker. See [Webhook Hooks](/automation/webhook) or use `chainbreaker webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `chainbreaker hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but Chainbreaker keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend Chainbreaker's behavior without modifying core code

## Getting Started

### Bundled Hooks

Chainbreaker ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.chainbreaker/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.chainbreaker/logs/commands.log`
- **🚀 boot-md**: Runs `BOOT.md` when the gateway starts (requires internal hooks enabled)

List available hooks:

```bash
chainbreaker hooks list
```

Enable a hook:

```bash
chainbreaker hooks enable session-memory
```

Check hook status:

```bash
chainbreaker hooks check
```

Get detailed information:

```bash
chainbreaker hooks info session-memory
```

### Onboarding

During onboarding (`chainbreaker onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so Chainbreaker requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with Chainbreaker; located at `<chainbreaker>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.chainbreaker/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `chainbreaker.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.chainbreaker/hooks/<id>`.
Each `chainbreaker.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `chainbreaker plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.chainbreaker.ai/automation/hooks#my-hook
metadata:
  { "chainbreaker": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.chainbreaker` object supports:
...
```typescript
{
  ...
  cfg: ChainbreakerConfig            // Current gateway config
}
```
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Chainbreaker persists them.
...
Runs after the built-in install security scan and before installation continues. Chainbreaker fires this hook for interactive skill installs as well as plugin bundle, package, and single-file installs.

Default behavior differs by target type:

- Plugin installs fail closed on built-in scan `critical` findings and scan errors unless the operator explicitly uses `chainbreaker plugins install --dangerously-force-unsafe-install`.
...
- **`origin`**: Normalized install origin when available (for example `chainbreaker-bundled`, `chainbreaker-workspace`, `plugin-bundle`, `plugin-package`, or `plugin-file`)
...
```json
{
  ...
  "sourcePath": "/var/folders/.../chainbreaker-plugin-acme-audit/package",
  "sourcePathKind": "directory",
  "origin": "plugin-package",
  "request": {
    "kind": "plugin-npm",
    "mode": "install",
    "requestedSpecifier": "@acme/chainbreaker-plugin-audit@1.4.2"
  },
  "builtinScan": {
    "status": "ok",
    "scannedFiles": 12,
  ...
  "plugin": {
    "pluginId": "acme-audit",
    "contentType": "package",
    "packageName": "@acme/chainbreaker-plugin-audit",
...
- **Managed hooks** (`~/.chainbreaker/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.chainbreaker/hooks/my-hook
cd ~/.chainbreaker/hooks/my-hook
```
...
metadata: { "chainbreaker": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# Enable it
chainbreaker hooks enable my-hook
...
chainbreaker hooks list --eligible

# Verbose output (show missing requirements)
chainbreaker hooks list --verbose

# JSON output
chainbreaker hooks list --json
```
...
# JSON output
chainbreaker hooks info session-memory --json
```
...
# Disable a hook
chainbreaker hooks disable command-logger
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.chainbreaker/workspace`)
...
```bash
chainbreaker hooks enable bootstrap-extra-files
```
...
**Output**: `~/.chainbreaker/logs/commands.log`
...
tail -n 20 ~/.chainbreaker/logs/commands.log

# Pretty-print with jq
cat ~/.chainbreaker/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.chainbreaker/logs/commands.log | jq .
```

**Enable**:

```bash
chainbreaker hooks enable command-logger
```
...
```bash
chainbreaker hooks enable boot-md
```
...
```yaml
metadata: { "chainbreaker": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "chainbreaker": { "events": ["command"] } } # General - more overhead
```
...
```bash
chainbreaker hooks info my-hook
```
...
# Other platforms
tail -f ~/.chainbreaker/gateway.log
```
...
1. Check directory structure:
   ...
   ```bash
   ls -la ~/.chainbreaker/hooks/my-hook/
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.chainbreaker/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.chainbreaker/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.chainbreaker/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/chainbreaker/chainbreaker/tree/main/src/hooks/bundled)

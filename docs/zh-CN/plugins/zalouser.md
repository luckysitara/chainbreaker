---
read_when:
  - 你想在 Chainbreaker 中支持 Zalo Personal（非官方）
...
通过插件为 Chainbreaker 提供 Zalo Personal 支持，使用 `zca-cli` 自动化普通 Zalo 用户账户。
...
```bash
chainbreaker plugins install ./extensions/zalouser

...
通过插件为 Chainbreaker 提供语音通话。支持出站通知和带有入站策略的多轮对话。
...
- 使用 `chainbreaker voicecall ...` 或 `voice_call` 工具
...
```bash
chainbreaker plugins install ./extensions/voice-call

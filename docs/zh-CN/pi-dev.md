---
read_when:
  - 在 Chainbreaker 中处理 Pi 集成代码或测试时
...
本指南总结了在 Chainbreaker 中处理 Pi 集成时的一套合理工作流。
...
状态存储在 Chainbreaker 状态目录下。默认路径是 `~/.chainbreaker`。如果设置了 `CHAINBREAKER_STATE_DIR`，则改用该目录。

如需重置全部内容：

- `chainbreaker.json` 用于配置

...
- `~/Library/Application Support/Chainbreaker/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `chainbreaker-canvas://<session>/<path>`

示例：

- `chainbreaker-canvas://main/` → `<canvasRoot>/main/index.html`
- `chainbreaker-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `chainbreaker-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
chainbreaker nodes canvas present --node <id>
chainbreaker nodes canvas navigate --node <id> --url "/"
chainbreaker nodes canvas eval --node <id> --js "document.title"
chainbreaker nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__chainbreaker__/a2ui/
```
...
chainbreaker nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
chainbreaker nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "chainbreaker://agent?message=Review%20this%20design";
```

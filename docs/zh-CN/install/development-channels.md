...
当你使用 `--channel` **显式**切换渠道时，Chainbreaker 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/chainbreaker`，可通过 `CHAINBREAKER_GIT_DIR` 覆盖），
...
当你使用 `chainbreaker update` 切换渠道时，Chainbreaker 还会同步插件来源：

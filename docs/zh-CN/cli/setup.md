...
read_when:
  ...
  - 你想设置默认工作区路径
summary: "`chainbreaker setup` 的 CLI 参考（初始化配置 + 工作区）"
title: setup
x-i18n:
...
初始化 `~/.chainbreaker/chainbreaker.json` 和智能体工作区。
...
```bash
chainbreaker setup
chainbreaker setup --workspace ~/.chainbreaker/workspace
```

通过 setup 运行新手引导：

```bash
chainbreaker setup --wizard
```

...
read_when:
  ...
  - 你想调试插件加载失败问题
summary: "`chainbreaker plugins` 的 CLI 参考（列出、安装、启用/禁用、诊断）"
title: plugins
x-i18n:
...
内置插件随 Chainbreaker 一起发布，但默认禁用。使用 `plugins enable` 来激活它们。

所有插件必须提供 `chainbreaker.plugin.json` 文件，其中包含内联 JSON Schema（`configSchema`，即使为空）。缺少或无效的清单或模式会阻止插件加载并导致配置验证失败。
...
chainbreaker plugins update <id>
chainbreaker plugins update --all
chainbreaker plugins update <id> --dry-run
```

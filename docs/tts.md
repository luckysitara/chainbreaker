...
Chainbreaker can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Chainbreaker can send audio.
...
When `messages.tts.provider` is unset, Chainbreaker picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `chainbreaker.json`.
...
- If `provider` is **unset**, Chainbreaker uses the first configured speech provider in registry auto-select order.
...
`~/.chainbreaker/settings/tts.json`, override with `CHAINBREAKER_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Chainbreaker retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, Chainbreaker:
...
Discord note: `/tts` is a built-in Discord command, so Chainbreaker registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from Chainbreaker
```

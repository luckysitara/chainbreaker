---
summary: "Optional Docker-based setup and onboarding for Chainbreaker"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Chainbreaker on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export CHAINBREAKER_IMAGE="ghcr.io/chainbreaker/chainbreaker:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/chainbreaker/chainbreaker/pkgs/container/chainbreaker).
  ...
  <Step title="Complete onboarding">
    ...
    `chainbreaker-gateway` directly. `chainbreaker-cli` is for commands you run after
...
docker compose run --rm --no-deps --entrypoint node chainbreaker-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `CHAINBREAKER_EXTRA_MOUNTS`
or `CHAINBREAKER_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `chainbreaker-cli` shares `chainbreaker-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d chainbreaker-gateway`, run onboarding
and setup-time config writes through `chainbreaker-gateway` with
...
| Variable                           | Purpose                                                          |
| ---------------------------------- | ---------------------------------------------------------------- |
| `CHAINBREAKER_IMAGE`               | Use a remote image instead of building locally                   |
| `CHAINBREAKER_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `CHAINBREAKER_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `CHAINBREAKER_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `CHAINBREAKER_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `CHAINBREAKER_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `CHAINBREAKER_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `CHAINBREAKER_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `CHAINBREAKER_CONFIG_DIR` to `/home/node/.chainbreaker` and
`CHAINBREAKER_WORKSPACE_DIR` to `/home/node/.chainbreaker/workspace`, so those paths
...
and rolling file logs under `/tmp/chainbreaker/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/chainbreaker/chainbreaker/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export CHAINBREAKER_SANDBOX=1
    export CHAINBREAKER_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm chainbreaker-cli gateway probe
    docker compose run -T --rm chainbreaker-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `chainbreaker-cli` uses `network_mode: "service:chainbreaker-gateway"` so CLI
    ...
    `no-new-privileges` on `chainbreaker-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.chainbreaker`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export CHAINBREAKER_HOME_VOLUME="chainbreaker_home"`
    2. **Bake system deps**: `export CHAINBREAKER_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `CHAINBREAKER_HOME_VOLUME` or `CHAINBREAKER_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/chainbreaker/chainbreaker/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Chainbreaker runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm chainbreaker-cli config set gateway.mode local
    docker compose run --rm chainbreaker-cli config set gateway.bind lan
    docker compose run --rm chainbreaker-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Chainbreaker up to date

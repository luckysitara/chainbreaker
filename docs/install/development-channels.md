...
Chainbreaker ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/chainbreaker`, override with
  `CHAINBREAKER_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
chainbreaker update --tag 2026.4.1-beta.1
...
chainbreaker update --tag chainbreaker@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `chainbreaker update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Chainbreaker prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `chainbreaker update` would do without making changes:

```bash
chainbreaker update --dry-run
chainbreaker update --channel beta --dry-run
chainbreaker update --tag 2026.4.1-beta.1 --dry-run
chainbreaker update --dry-run --json
```
...
When you switch channels with `chainbreaker update`, Chainbreaker also syncs plugin
sources:

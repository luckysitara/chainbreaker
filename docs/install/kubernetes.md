---
summary: "Deploy Chainbreaker Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Chainbreaker on a Kubernetes cluster
  - You want to test Chainbreaker in a Kubernetes environment
title: "Kubernetes"
---

# Chainbreaker on Kubernetes

A minimal starting point for running Chainbreaker on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Chainbreaker is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/chainbreaker 18789:18789 -n chainbreaker
...
```bash
kubectl get secret chainbreaker-secrets -n chainbreaker -o jsonpath='{.data.CHAINBREAKER_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: chainbreaker (configurable via CHAINBREAKER_NAMESPACE)
├── Deployment/chainbreaker        # Single pod, init container + gateway
├── Service/chainbreaker           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/chainbreaker-config  # chainbreaker.json + AGENTS.md
└── Secret/chainbreaker-secrets    # Gateway token + API keys
```
...
Edit `chainbreaker.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret chainbreaker-secrets -n chainbreaker \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/chainbreaker -n chainbreaker
```

### Custom namespace

```bash
CHAINBREAKER_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/chainbreaker/chainbreaker:latest # or pin to a specific version from https://github.com/chainbreaker/chainbreaker/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # chainbreaker.json + AGENTS.md

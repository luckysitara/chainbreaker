---
summary: "Run Chainbreaker Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Chainbreaker running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Chainbreaker Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Chainbreaker on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Chainbreaker.
...
- Install Chainbreaker with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-chainbreaker"
    LOCATION="westus2"
    VNET_NAME="vnet-chainbreaker"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-chainbreaker-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-chainbreaker-vm"
    VM_NAME="vm-chainbreaker"
    ADMIN_USERNAME="chainbreaker"
    BASTION_NAME="bas-chainbreaker"
    BASTION_PIP_NAME="pip-chainbreaker-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Chainbreaker (in the VM shell)">
    ```bash
    curl -fsSL https://chainbreaker.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Chainbreaker, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Chainbreaker onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Chainbreaker Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Chainbreaker Azure deployment with the GitHub Copilot model provider: [Chainbreaker on Azure with GitHub Copilot](https://github.com/johnsonshi/chainbreaker-azure-github-copilot)

---
summary: "ClawDock shell helpers for Docker-based Chainbreaker installs"
read_when:
  - You run Chainbreaker with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based Chainbreaker installs.
...
| `clawdock-cli <command>`  | Run Chainbreaker CLI commands in Docker       |
...
| `clawdock-config`      | Open `~/.chainbreaker`                     |
...
- `~/.chainbreaker/.env` for provider keys and bot tokens
- `~/.chainbreaker/chainbreaker.json` for behavior config

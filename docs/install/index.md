---
summary: "Install Chainbreaker — installer script, npm/pnpm, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Chainbreaker, and launches onboarding.
...
Or skip the link and use `pnpm chainbreaker ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
chainbreaker --version      # confirm the CLI is available
chainbreaker doctor         # check for config issues
chainbreaker gateway status # verify the Gateway is running
```

## Hosting and deployment

Deploy Chainbreaker on a cloud server or VPS:
...
<CardGroup cols={3}>
  <Card title="Updating" href="/install/updating" icon="refresh-cw">
    Keep Chainbreaker up to date.
  </Card>
  <Card title="Migrating" href="/install/migrating" icon="arrow-right">
    Move to a new machine.
  </Card>
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Chainbreaker completely.
  </Card>
</CardGroup>

## Troubleshooting: `chainbreaker` not found

If the install succeeded but `chainbreaker` is not found in your terminal:

---
summary: "Host Chainbreaker on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Chainbreaker on Oracle Cloud
  - Looking for free VPS hosting for Chainbreaker
...
Run a persistent Chainbreaker Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@chainbreaker`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=chainbreaker --reset` to re-authenticate.

**Gateway will not start** -- Run `chainbreaker doctor --non-interactive` and check logs with `journalctl --user -u chainbreaker-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Chainbreaker up to date

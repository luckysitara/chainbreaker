...
2. `npm i -g chainbreaker@latest`
3. `chainbreaker onboard --install-daemon`
...
Chainbreaker installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. The full unit example and guidance
live in the [Gateway runbook](/gateway).

Minimal setup:

Create `~/.config/systemd/user/chainbreaker-gateway[-<profile>].service`:

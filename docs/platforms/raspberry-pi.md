---
summary: "Chainbreaker on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Chainbreaker on a Raspberry Pi
  - Running Chainbreaker on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Chainbreaker on Raspberry Pi

## Goal

Run a persistent, always-on Chainbreaker Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u chainbreaker -f
```

## 9) Access the Chainbreaker Dashboard
...
```bash
ssh user@gateway-host 'chainbreaker dashboard --no-open'
```
...
(or `CHAINBREAKER_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/chainbreaker-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
...
- `CHAINBREAKER_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Chainbreaker, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit chainbreaker
```

```ini
[Service]
Environment=CHAINBREAKER_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/chainbreaker-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Chainbreaker state/cache on SSD-backed storage to avoid SD-card
...
Most Chainbreaker features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled chainbreaker

# Enable if not
sudo systemctl enable chainbreaker

# Start on boot
sudo systemctl start chainbreaker
```
...
```bash
# Check logs
journalctl -u chainbreaker --no-pager -n 100

# Common fix: rebuild
cd ~/chainbreaker  # if using hackable install
npm run build
sudo systemctl restart chainbreaker
```

...
- Same path: run the app from a fixed location (for Chainbreaker, `dist/Chainbreaker.app`).
...
```bash
sudo tccutil reset Accessibility ai.chainbreaker.mac
sudo tccutil reset ScreenCapture ai.chainbreaker.mac
...
Workaround: move files into the Chainbreaker workspace (`~/.chainbreaker/workspace`) if you want to avoid per-folder grants.

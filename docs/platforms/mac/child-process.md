...
service via the external `chainbreaker` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.chainbreaker.gateway`
  (or `ai.chainbreaker.<profile>` when using `--profile`/`CHAINBREAKER_PROFILE`; legacy `com.chainbreaker.*` is supported).

...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `chainbreaker.internal.`), **or**
...
Run `chainbreaker devices list` again before approval.

4. Verify connection:

```bash
chainbreaker nodes status
chainbreaker gateway call node.list --params "{}"
```
...
- `CHAINBREAKER_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Chainbreaker iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Chainbreaker builds.
...
```bash
export CHAINBREAKER_APNS_TEAM_ID="TEAMID"
export CHAINBREAKER_APNS_KEY_ID="KEYID"
export CHAINBREAKER_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_chainbreaker-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `chainbreaker.internal.`) and Tailscale split DNS.
...
```bash
chainbreaker nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__chainbreaker__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__chainbreaker__/canvas/` and `/__chainbreaker__/a2ui/`.
...
```bash
chainbreaker nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__chainbreaker; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
chainbreaker nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `chainbreaker devices list` and approve manually.

...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Chainbreaker
...
Running Chainbreaker via Node with `tsx` fails at startup with:

```
[chainbreaker] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `chainbreaker status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch chainbreaker.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node chainbreaker.mjs status` works on Node 25.

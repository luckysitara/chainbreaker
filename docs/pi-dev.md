...
This guide summarizes a sane workflow for working on the pi integration in Chainbreaker.
...
```bash
CHAINBREAKER_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm chainbreaker agent --message "Hello" --thinking low`
...
State lives under the Chainbreaker state directory. Default is `~/.chainbreaker`. If `CHAINBREAKER_STATE_DIR` is set, use that directory instead.

To reset everything:

- `chainbreaker.json` for config

...
read_when:
  - Running Chainbreaker Gateway in WSL2 while Chrome lives on Windows
...
- Chainbreaker Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/chainbreaker/chainbreaker/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Chainbreaker and Chrome are on the same machine
...
- Chainbreaker points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Chainbreaker is not the problem yet.
...
Fix that before touching Chainbreaker config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Chainbreaker at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Chainbreaker to succeed
...
```bash
chainbreaker browser open https://example.com --browser-profile remote
chainbreaker browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `chainbreaker browser tabs` returns the target
...
3. Chainbreaker config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Chainbreaker config or Control UI auth

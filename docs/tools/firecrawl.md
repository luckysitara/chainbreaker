...
Chainbreaker can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `chainbreaker configure --section web` enables the bundled Firecrawl plugin automatically.
...
Chainbreaker always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.

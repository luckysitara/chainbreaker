...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.chainbreaker/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Chainbreaker picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    chainbreaker agent --message "give me a greeting"
    ```
...
| `metadata.chainbreaker.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.chainbreaker.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.chainbreaker.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `chainbreaker agent --message "..."` to test before sharing
...
| `~/.chainbreaker/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Chainbreaker) | Lowest     | Global                |

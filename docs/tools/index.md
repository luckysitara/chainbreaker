---
summary: "Chainbreaker tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools Chainbreaker provides
...
Chainbreaker has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). Chainbreaker ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Some plugins are **core** (shipped with Chainbreaker), others are **external**
...
These tools ship with Chainbreaker and are available without installing any plugins:
...
| `group:nodes`        | nodes                                                                                                     |
| `group:chainbreaker` | All built-in Chainbreaker tools (excludes plugin tools)                                                   |

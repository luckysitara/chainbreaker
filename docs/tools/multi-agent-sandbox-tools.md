...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `chainbreaker sandbox explain`.
...
Legacy `agent.*` configs are migrated by `chainbreaker doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=chainbreaker-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CHAINBREAKER_STATE_DIR:-$HOME/.chainbreaker}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```

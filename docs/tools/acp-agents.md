---
summary: "Use ACP runtime sessions for Codex, Claude Code, Cursor, Gemini CLI, Chainbreaker ACP, and other harness agents"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Chainbreaker run external coding harnesses (for example Pi, Claude Code, Codex, Cursor, Copilot, Chainbreaker ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses) through an ACP backend plugin.

If you ask Chainbreaker in plain language to "run this in Codex" or "start Claude Code in a thread", Chainbreaker should route that request to the ACP runtime (not the native sub-agent runtime). Each ACP session spawn is tracked as a [background task](/automation/tasks).

If you want Codex or Claude Code to connect as an external MCP client directly
to existing Chainbreaker channel conversations, use [`chainbreaker mcp serve`](/cli/mcp)
instead of ACP.
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Chainbreaker-native delegated runs.

| Area          | ACP session                           | Sub-agent run                         |
| ------------- | ------------------------------------- | ------------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Chainbreaker native sub-agent runtime |
...
- Chainbreaker keeps owning the channel transport, auth, safety, and delivery.
...
- ACP session: the durable Codex/Claude/Gemini runtime state Chainbreaker routes to
...
- `/acp spawn codex --thread auto`: Chainbreaker may create a child thread/topic and bind the ACP session there
...
- Generic current-conversation binds use the shared Chainbreaker binding store and survive normal gateway restarts.

Notes:

- `--bind here` and `--thread ...` are mutually exclusive on `/acp spawn`.
- On Discord, `--bind here` binds the current channel or thread in place. `spawnAcpSessions` is only required when Chainbreaker needs to create a child thread for `--thread auto|here`.
- If the active channel does not expose current-conversation ACP bindings, Chainbreaker returns a clear unsupported message.
...
- Chainbreaker binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Chainbreaker returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Chainbreaker agent id.
...
- Chainbreaker ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, Chainbreaker uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Chainbreaker may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Chainbreaker session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Chainbreaker sandbox.
...
If no target resolves, Chainbreaker returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Chainbreaker returns a clear unsupported-control error.
...
When Chainbreaker uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Chainbreaker `agentId` path).
...
```bash
chainbreaker plugins install acpx
chainbreaker config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
chainbreaker plugins install ./path/to/local/acpx-plugin
```
...
- Relative paths resolve from Chainbreaker workspace directory.
...
- Chainbreaker startup remains non-blocking while the backend health check runs.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install Chainbreaker globally with `npm install -g chainbreaker`, the acpx
...
normally and reports the missing dependency through `chainbreaker acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Chainbreaker plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Chainbreaker plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
chainbreaker config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `chainbreaker-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Chainbreaker
  plugins.
...
These ACPX harness permissions are separate from Chainbreaker exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
chainbreaker config set plugins.entries.acpx.config.permissionMode approve-all
chainbreaker config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Chainbreaker currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>

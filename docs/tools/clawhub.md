---
summary: "ClawHub guide: public registry, native Chainbreaker install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Chainbreaker skills and plugins**.

- Use native `chainbreaker` commands to search/install/update skills and install
...
```bash
chainbreaker plugins install clawhub:<package>
...
Native `chainbreaker` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Chainbreaker skills and plugins.
...
4. Users browse, download, and install skills in Chainbreaker.
...
If you want to add new capabilities to your Chainbreaker agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `chainbreaker skills install <skill-slug>`
3. Start a new Chainbreaker session so it picks up the new skill.
...
## How it fits into Chainbreaker

Native `chainbreaker skills install` installs into the active workspace `skills/`
directory. `chainbreaker plugins install clawhub:...` records a normal managed
...
current working directory. If an Chainbreaker workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Chainbreaker loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.chainbreaker/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Chainbreaker how to perform a
...
Interested in becoming a moderator? Ask in the Chainbreaker Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Chainbreaker workspace).
...
Code plugins must include the required Chainbreaker metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/chainbreaker/skills` and `~/.chainbreaker/skills`). This is designed to find older skill installs without extra flags.

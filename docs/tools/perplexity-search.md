...
Chainbreaker supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Chainbreaker also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `chainbreaker configure --section web`. It stores the key in
`~/.chainbreaker/chainbreaker.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.chainbreaker/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).

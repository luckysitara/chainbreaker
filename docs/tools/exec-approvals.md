...
- **gateway host** → `chainbreaker` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Chainbreaker also tries to bind
...
`~/.chainbreaker/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.chainbreaker/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, Chainbreaker treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.chainbreaker/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `chainbreaker approvals allowlist ...`).
- `chainbreaker security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `chainbreaker doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Chainbreaker still rejects the `env` builtin in safe-bin
...
`~/.chainbreaker/exec-approvals.json` directly.

CLI: `chainbreaker approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Chainbreaker sends a followup `agent` turn to the same session.
...
topics, Chainbreaker preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Chainbreaker prevents the agent from reusing

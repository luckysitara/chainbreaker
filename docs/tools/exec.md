...
- `gateway`/`node` approvals are controlled by `~/.chainbreaker/exec-approvals.json`.
...
- Chainbreaker sets `CHAINBREAKER_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Chainbreaker prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`chainbreaker security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `chainbreaker doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`chainbreaker security audit` and `chainbreaker doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.

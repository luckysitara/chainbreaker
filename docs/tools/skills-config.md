...
All skills-related configuration lives under `skills` in `~/.chainbreaker/chainbreaker.json`.
...
- Built-in skill roots always include `~/.chainbreaker/skills`, `~/.agents/skills`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.chainbreaker.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.chainbreaker/skills` → bundled skills →

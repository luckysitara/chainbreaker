...
`chainbreaker agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    chainbreaker agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    chainbreaker agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    chainbreaker agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    chainbreaker agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    chainbreaker agent --agent ops --message "Generate report" \
...
chainbreaker agent --to +15555550123 --message "Trace logs" --verbose on --json

# Turn with thinking level
chainbreaker agent --session-id 1234 --message "Summarize inbox" --thinking medium

# Deliver to a different channel than the session
chainbreaker agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```

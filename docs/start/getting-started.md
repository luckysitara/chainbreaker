---
summary: "Get Chainbreaker installed and run your first chat in minutes."
read_when:
...
Install Chainbreaker, run onboarding, and chat with your AI assistant — all in
...
<Accordion title="Advanced: environment variables">
  If you run Chainbreaker as a service account or want custom paths:

- `CHAINBREAKER_HOME` — home directory for internal path resolution
- `CHAINBREAKER_STATE_DIR` — override the state directory
- `CHAINBREAKER_CONFIG_PATH` — override the config file path

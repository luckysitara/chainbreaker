...
CLI onboarding is the **recommended** way to set up Chainbreaker on macOS,
...
`chainbreaker dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`chainbreaker configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.chainbreaker/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `chainbreaker doctor` first.
</Note>
...
Use `chainbreaker agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.chainbreaker/workspace-<agentId>`.
...
- CLI command reference: [`chainbreaker onboard`](/cli/onboard)

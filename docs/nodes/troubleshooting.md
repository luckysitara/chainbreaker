...
```bash
chainbreaker nodes describe --node <idOrNameOrIp>
chainbreaker nodes canvas snapshot --node <idOrNameOrIp>
chainbreaker logs --follow
```
...
chainbreaker nodes status
chainbreaker approvals get --node <idOrNameOrIp>
chainbreaker approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`chainbreaker approvals get --node ...`), not in the gateway pairing record.

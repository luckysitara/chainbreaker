...
read_when:
  - You are building an Chainbreaker plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (chainbreaker.plugin.json)

This page is for the **native Chainbreaker plugin manifest** only.
...
Chainbreaker auto-detects those bundle layouts too, but they are not validated
against the `chainbreaker.plugin.json` schema described here.

For compatible bundles, Chainbreaker currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Chainbreaker runtime expectations.

Every native Chainbreaker plugin **must** ship a `chainbreaker.plugin.json` file in the
**plugin root**. Chainbreaker uses this manifest to validate configuration
...
`chainbreaker.plugin.json` is the metadata Chainbreaker reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Chainbreaker can inspect without loading plugin code.                                  |
...
Chainbreaker reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Chainbreaker falls back to `choiceId`.                                    |
...
Use `contracts` only for static capability ownership metadata that Chainbreaker can
...
`imageGenerationProviders` are deprecated. Use `chainbreaker doctor --fix` to move
...
| File                       | Use it for                                                                                                             |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `chainbreaker.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs               |
| `package.json`             | npm metadata, dependency installation, and the `chainbreaker` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Chainbreaker must know it before loading plugin code, put it in `chainbreaker.plugin.json`
...
- The manifest is **required for native Chainbreaker plugins**, including local filesystem loads.

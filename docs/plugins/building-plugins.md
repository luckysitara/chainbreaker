...
summary: "Create your first Chainbreaker plugin in minutes"
read_when:
  - You want to create a new Chainbreaker plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Chainbreaker
---

# Building Plugins

Plugins extend Chainbreaker with new capabilities: channels, model providers, speech,
image generation, web search, agent tools, or any combination.

You do not need to add your plugin to the Chainbreaker repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`chainbreaker plugins install <package-name>`. Chainbreaker tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Chainbreaker to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json chainbreaker.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Chainbreaker",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Write the entry point">
    ...
    ```typescript
    // index.ts
    import { definePluginEntry } from "chainbreaker/plugin-sdk/plugin-entry";
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    chainbreaker plugins install clawhub:@myorg/chainbreaker-my-plugin
    ```

    Chainbreaker also checks ClawHub before npm for bare package specs like
    `@myorg/chainbreaker-my-plugin`.
...
Always import from focused `chainbreaker/plugin-sdk/<subpath>` paths:
...
import { ... } from "chainbreaker/plugin-sdk";
```
...
<Check>**package.json** has correct `chainbreaker` metadata</Check>
<Check>**chainbreaker.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [chainbreaker/chainbreaker](https://github.com/chainbreaker/chainbreaker/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Chainbreaker X account [@chainbreaker](https://x.com/chainbreaker) for release announcements.

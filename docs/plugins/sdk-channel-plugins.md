...
summary: "Step-by-step guide to building a messaging channel plugin for Chainbreaker"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Chainbreaker to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Chainbreaker to a
...
<Info>
  If you have not built any Chainbreaker plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Chainbreaker keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `chainbreaker/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/chainbreaker-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "chainbreaker": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Chainbreaker to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { ChainbreakerConfig } from "chainbreaker/plugin-sdk/core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "chainbreaker/plugin-sdk/core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Chainbreaker
  ...
  <Step title="Add a setup entry">
    ...
    Chainbreaker loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    Chainbreaker. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Chainbreaker.
...
├── package.json              # chainbreaker.channel metadata
├── chainbreaker.plugin.json      # Manifest with config schema

...
read_when:
  - You see the CHAINBREAKER_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the CHAINBREAKER_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Chainbreaker plugin
---

# Plugin SDK Migration

Chainbreaker has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`chainbreaker/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`chainbreaker/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`chainbreaker/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "chainbreaker/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "chainbreaker/plugin-sdk/compat";
    ...
    import { resolveControlCommandGate } from "chainbreaker/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "chainbreaker/extension-api";
...
```bash
CHAINBREAKER_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 chainbreaker gateway run
CHAINBREAKER_SUPPRESS_EXTENSION_API_WARNING=1 chainbreaker gateway run
```

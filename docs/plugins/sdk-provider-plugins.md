...
summary: "Step-by-step guide to building a model provider plugin for Chainbreaker"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Chainbreaker
...
(LLM) to Chainbreaker. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/chainbreaker-acme-ai",
      "version": "1.0.0",
      "type": "module",
      "chainbreaker": {
        "extensions": ["./index.ts"],
    ...
    The manifest declares `providerAuthEnvVars` so Chainbreaker can detect
    credentials without loading your plugin runtime. If you publish the
    provider on ClawHub, those `chainbreaker.compat` and `chainbreaker.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "chainbreaker/plugin-sdk/provider-auth";
    ...
    `chainbreaker onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "chainbreaker/plugin-sdk/provider-entry";
    ...
    `chainbreaker/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      Chainbreaker calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Chainbreaker classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # chainbreaker.providers metadata
├── chainbreaker.plugin.json      # Manifest with providerAuthEnvVars

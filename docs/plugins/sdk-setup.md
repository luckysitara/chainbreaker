...
read_when:
  ...
  - You are defining plugin config schemas or package.json chainbreaker metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`chainbreaker.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `chainbreaker` field that tells the plugin system what
...
```json
{
  "name": "@myorg/chainbreaker-my-channel",
  "version": "1.0.0",
  "type": "module",
  "chainbreaker": {
    "extensions": ["./index.ts"],
...
```json chainbreaker-clawhub-package.json
{
  "name": "@myorg/chainbreaker-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "chainbreaker": {
    "extensions": ["./index.ts"],
...
When enabled, Chainbreaker loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `chainbreaker.plugin.json` in the package root.
Chainbreaker uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Chainbreaker",
  "configSchema": {
    "type": "object",
...
Chainbreaker loads when it only needs setup surfaces (onboarding, config repair,
disabled channel inspection).

```typescript
// setup-entry.ts
import { defineSetupPluginEntry } from "chainbreaker/plugin-sdk/core";
...
**When Chainbreaker uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `chainbreaker/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Chainbreaker validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "chainbreaker/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `chainbreaker onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "chainbreaker/plugin-sdk/channel-setup";
...
helpers from `chainbreaker/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`chainbreaker/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `chainbreaker/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "chainbreaker/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/chainbreaker-my-channel",
...
```bash
chainbreaker plugins install @myorg/chainbreaker-my-plugin
```

Chainbreaker tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
chainbreaker plugins install clawhub:@myorg/chainbreaker-my-plugin   # ClawHub only
chainbreaker plugins install npm:@myorg/chainbreaker-my-plugin       # npm only
```
...
```bash
chainbreaker plugins search <query>
chainbreaker plugins install <package-name>
```

<Info>
  For npm-sourced installs, `chainbreaker plugins install` runs

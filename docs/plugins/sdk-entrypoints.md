...
**Import:** `chainbreaker/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                                 | No       | —                   |
| `configSchema` | `ChainbreakerPluginConfigSchema \| () => ChainbreakerPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: ChainbreakerPluginApi) => void`                                   | Yes      | —                   |

- `id` must match your `chainbreaker.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `chainbreaker/plugin-sdk/core`
...
| `registerCliMetadata` | `(api: ChainbreakerPluginApi) => void`                                   | No       | —                   |
| `registerFull`        | `(api: ChainbreakerPluginApi) => void`                                   | No       | —                   |
...
Chainbreaker loads this instead of the full entry when a channel is disabled,
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Chainbreaker to lazy-load the real CLI module on first invocation
...
Chainbreaker classifies loaded plugins by their registration behavior:
...
Use `chainbreaker plugins inspect <id>` to see a plugin's shape.

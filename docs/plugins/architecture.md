...
read_when:
  - Building or debugging native Chainbreaker plugins
...
This page covers the internal architecture of the Chainbreaker plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Chainbreaker. Every
native Chainbreaker plugin registers against one or more capability types:
...
Chainbreaker classifies every loaded plugin into a shape based on its actual
...
Use `chainbreaker plugins inspect <id>` to see a plugin's shape and capability
...
When you run `chainbreaker doctor` or `chainbreaker plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `chainbreaker status --all` and `chainbreaker plugins doctor`.

## Architecture overview

Chainbreaker's plugin system has four layers:

1. **Manifest + discovery**
   Chainbreaker finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `chainbreaker.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Chainbreaker plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Chainbreaker reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Chainbreaker
...
That split lets Chainbreaker validate config, explain missing/disabled plugins, and
...
normal chat actions. Chainbreaker keeps one shared `message` tool in core, and
...
Chainbreaker treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Chainbreaker-facing
  surfaces
...
So if Chainbreaker adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Chainbreaker has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { ChainbreakerPluginDefinition } from "chainbreaker/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "chainbreaker/plugin-sdk/media-understanding";

const plugin: ChainbreakerPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Chainbreaker already treats image/audio/video understanding as one shared
...
If Chainbreaker adds a new domain later, such as video generation, use the same
...
`ChainbreakerPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Chainbreaker can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Chainbreaker knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `ChainbreakerPluginApi` or
  `api.runtime`
...
Native Chainbreaker plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Chainbreaker process

Compatible bundles are safer by default because Chainbreaker currently treats them
...
name: `@chainbreaker/<id>` by default, or an approved typed suffix such as
...
Chainbreaker exports capabilities, not implementation convenience.
...
At startup, Chainbreaker does roughly this:
...
The manifest is the control-plane source of truth. Chainbreaker uses it to:
...
Chainbreaker keeps short in-process caches for:
...
- Set `CHAINBREAKER_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `CHAINBREAKER_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `CHAINBREAKER_PLUGIN_DISCOVERY_CACHE_MS` and
  `CHAINBREAKER_PLUGIN_MANIFEST_CACHE_MS`.
...
Chainbreaker still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Chainbreaker calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Chainbreaker tries the normal registry/catalog path first                                | _(not a plugin hook)_                                                                |
...
that still runs on Chainbreaker's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Chainbreaker's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Chainbreaker adds those
...
- If Chainbreaker adds a new capability such as video generation later, define the
...
- Chainbreaker only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Chainbreaker plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `chainbreaker/plugin-sdk` import when
authoring plugins:

- `chainbreaker/plugin-sdk/plugin-entry` for plugin registration primitives.
- `chainbreaker/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `chainbreaker/plugin-sdk/channel-setup`,
  `chainbreaker/plugin-sdk/channel-pairing`,
  `chainbreaker/plugin-sdk/channel-contract`,
  `chainbreaker/plugin-sdk/channel-feedback`,
  `chainbreaker/plugin-sdk/channel-inbound`,
  `chainbreaker/plugin-sdk/channel-lifecycle`,
  `chainbreaker/plugin-sdk/channel-reply-pipeline`,
  `chainbreaker/plugin-sdk/command-auth`,
  `chainbreaker/plugin-sdk/secret-input`, and
  `chainbreaker/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `chainbreaker/plugin-sdk/channel-config-helpers`,
  `chainbreaker/plugin-sdk/allow-from`,
  `chainbreaker/plugin-sdk/channel-config-schema`,
  `chainbreaker/plugin-sdk/channel-policy`,
  `chainbreaker/plugin-sdk/config-runtime`,
  `chainbreaker/plugin-sdk/infra-runtime`,
  `chainbreaker/plugin-sdk/agent-runtime`,
  `chainbreaker/plugin-sdk/lazy-runtime`,
  `chainbreaker/plugin-sdk/reply-history`,
  `chainbreaker/plugin-sdk/routing`,
  `chainbreaker/plugin-sdk/status-helpers`,
  `chainbreaker/plugin-sdk/runtime-store`, and
  `chainbreaker/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `chainbreaker/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `chainbreaker/plugin-sdk/*` subpaths. Chainbreaker core/test code may use the repo
...
- Avoid the root `chainbreaker/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `chainbreaker/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `chainbreaker/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `chainbreaker/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `chainbreaker/plugin-sdk/channel-targets`; channel-specific
...
`chainbreaker/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Chainbreaker writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Chainbreaker's
...
- if both `catalog` and `discovery` are registered, Chainbreaker uses `catalog`
...
- Read-only command paths such as `chainbreaker status`, `chainbreaker status --all`,
  `chainbreaker channels status`, `chainbreaker channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `chainbreaker.extensions`:
...
Security guardrail: every `chainbreaker.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `chainbreaker plugins install` installs plugin dependencies with
...
Optional: `chainbreaker.setupEntry` can point at a lightweight setup-only module.
When Chainbreaker needs setup surfaces for a disabled channel plugin, or
...
Optional: `chainbreaker.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Chainbreaker load the
...
Channel plugins can advertise setup/discovery metadata via `chainbreaker.channel` and
install hints via `chainbreaker.install`. This keeps the core catalog data-free.
...
Chainbreaker can also merge **external channel catalogs** (for example, an MPM
...
Or point `CHAINBREAKER_PLUGIN_CATALOG_PATHS` (or `CHAINBREAKER_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "chainbreaker": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { delegateCompactionToRuntime } from "chainbreaker/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `ChainbreakerPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Chainbreaker stays opinionated without becoming hardcoded to one

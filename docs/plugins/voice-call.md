...
read_when:
  - You want to place an outbound voice call from Chainbreaker
...
Voice calls for Chainbreaker via a plugin. Supports outbound notifications and
...
- Use `chainbreaker voicecall ...` or the `voice_call` tool
...
chainbreaker plugins install "$PLUGIN_SRC"
...
chainbreaker voicecall start --to "+15555550123"   # alias for call
...
chainbreaker voicecall latency                     # summarize turn latency from logs

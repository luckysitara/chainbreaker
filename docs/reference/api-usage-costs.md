...
Chainbreaker features that can generate provider usage or paid API calls.
...
- `chainbreaker status --usage` and `chainbreaker channels list` show provider **usage windows**
...
Chainbreaker can pick up credentials from:
...
- `chainbreaker status --usage`
- `chainbreaker models status --json`
...
`chainbreaker models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

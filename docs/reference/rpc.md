...
Chainbreaker integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Chainbreaker owns lifecycle when `channels.signal.autoStart=true`.
...
- Chainbreaker spawns `imsg rpc` as a child process (legacy iMessage integration).

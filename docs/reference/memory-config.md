...
This page lists every configuration knob for Chainbreaker memory search. For
...
`chainbreaker.json` unless noted otherwise.
...
When `provider` is not set, Chainbreaker selects the first available:
...
Remote embeddings require an API key. Chainbreaker resolves from:
...
When sqlite-vec is unavailable, Chainbreaker falls back to in-process cosine
...
| Key                   | Type     | Default                                   | Description                                 |
| --------------------- | -------- | ----------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.chainbreaker/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |

...
This document explains how Chainbreaker manages sessions end-to-end:
...
Chainbreaker is designed around a single **Gateway process** that owns session state.
...
Chainbreaker persists sessions in two layers:
...
- Transcripts: `~/.chainbreaker/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Chainbreaker resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Chainbreaker reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
chainbreaker sessions cleanup --dry-run
chainbreaker sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.chainbreaker/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Chainbreaker intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Chainbreaker consumes the events, but Pi decides when to compact).
...
Chainbreaker also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Chainbreaker bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Chainbreaker leaves it alone.
...
- `chainbreaker status` (CLI)
- `chainbreaker sessions` / `sessions --json`
...
Chainbreaker supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Chainbreaker strips/suppresses this in the delivery layer.

As of `2026.1.10`, Chainbreaker also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Chainbreaker uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Chainbreaker’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `chainbreaker status`.

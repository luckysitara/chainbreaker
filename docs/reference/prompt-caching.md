...
- With Anthropic API-key auth profiles, Chainbreaker seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Chainbreaker injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Chainbreaker exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.chainbreaker/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$CHAINBREAKER_STATE_DIR/logs/cache-trace.jsonl`
...
- `CHAINBREAKER_CACHE_TRACE=1` enables cache tracing.
- `CHAINBREAKER_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `CHAINBREAKER_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `CHAINBREAKER_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `CHAINBREAKER_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.

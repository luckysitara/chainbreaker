...
This is the full reference for `chainbreaker onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.chainbreaker/chainbreaker.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `chainbreaker doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.chainbreaker/credentials/oauth.json`; auth profiles live in `~/.chainbreaker/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.chainbreaker/credentials/oauth.json` (or `$CHAINBREAKER_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.chainbreaker/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `chainbreaker pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `chainbreaker configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `chainbreaker health`.
    - Tip: `chainbreaker status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export CHAINBREAKER_GATEWAY_TOKEN="your-token"
chainbreaker onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env CHAINBREAKER_GATEWAY_TOKEN
```
...
- Stores it under `~/.chainbreaker/tools/signal-cli/<version>/`.
...
Typical fields in `~/.chainbreaker/chainbreaker.json`:
...
`chainbreaker agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.chainbreaker/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.chainbreaker/agents/<agentId>/sessions/`.

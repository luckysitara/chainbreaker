---
summary: "How Chainbreaker builds prompt context and reports token usage + costs"
read_when:
...
Chainbreaker tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Chainbreaker assembles its own system prompt on every run. It includes:
...
For images, Chainbreaker downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Chainbreaker session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `chainbreaker status --usage` and `chainbreaker channels list` show
...
`cacheWrite`. If pricing is missing, Chainbreaker shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Chainbreaker can
...
Anthropic's 1M context window is currently beta-gated. Chainbreaker can inject the
...
Chainbreaker skips the `context-1m-*` beta header because Anthropic currently

...
read_when:
  - You want to use Qwen (Alibaba Cloud Model Studio) with Chainbreaker
...
```bash
# China endpoint
chainbreaker onboard --auth-choice modelstudio-standard-api-key-cn

# Global/Intl endpoint
chainbreaker onboard --auth-choice modelstudio-standard-api-key
```
...
```bash
# China endpoint
chainbreaker onboard --auth-choice modelstudio-api-key-cn

# Global/Intl endpoint
chainbreaker onboard --auth-choice modelstudio-api-key
```
...
`~/.chainbreaker/.env` or via `env.shellEnv`).

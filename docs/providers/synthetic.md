---
summary: "Use Synthetic's Anthropic-compatible API in Chainbreaker"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Chainbreaker registers it as the
...
```bash
chainbreaker onboard --auth-choice synthetic-api-key
```
...
Note: Chainbreaker's Anthropic client appends `/v1` to the base URL, so use

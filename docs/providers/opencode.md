---
summary: "Use OpenCode Zen and Go catalogs with Chainbreaker"
read_when:
...
OpenCode exposes two hosted catalogs in Chainbreaker:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Chainbreaker keeps the runtime provider ids
...
chainbreaker onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
chainbreaker onboard --auth-choice opencode-go
chainbreaker onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```

---
summary: "Use Amazon Bedrock (Converse API) models with Chainbreaker"
read_when:
  - You want to use Amazon Bedrock models with Chainbreaker
...
Chainbreaker can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, Chainbreaker can automatically discover Bedrock
...
When running Chainbreaker on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, Chainbreaker's credential detection currently only checks for environment
...
- Chainbreaker surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,

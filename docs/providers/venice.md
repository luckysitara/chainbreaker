---
summary: "Use Venice AI privacy-focused models in Chainbreaker"
read_when:
  - You want privacy-focused inference in Chainbreaker
...
```bash
chainbreaker onboard --auth-choice venice-api-key
```
...
```bash
chainbreaker agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Chainbreaker shows all available Venice models. Pick based on your needs:
...
```bash
chainbreaker models set venice/kimi-k2-5
chainbreaker models set venice/claude-opus-4-6
```

List all available models:

```bash
chainbreaker models list | grep venice
```

## Configure via `chainbreaker configure`

1. Run `chainbreaker configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Chainbreaker automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
chainbreaker agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
chainbreaker agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
chainbreaker agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
chainbreaker agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
chainbreaker agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `chainbreaker models list` to see currently available models. Some models may be temporarily offline.

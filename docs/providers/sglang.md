---
summary: "Run Chainbreaker with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Chainbreaker against a local SGLang server
...
Chainbreaker can connect to SGLang using the `openai-completions` API.

Chainbreaker can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Chainbreaker will query:

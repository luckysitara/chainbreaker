...
read_when:
  - You want to use Groq with Chainbreaker
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Chainbreaker connects
...
media-understanding provider, Chainbreaker uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.chainbreaker/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `chainbreaker models list | grep groq`

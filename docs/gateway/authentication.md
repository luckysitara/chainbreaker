...
Chainbreaker supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `chainbreaker gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.chainbreaker/.env` so the daemon can read it:

```bash
cat >> ~/.chainbreaker/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `chainbreaker onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.chainbreaker/.env`, systemd/launchd).
...
Then paste it into Chainbreaker:
...
```bash
chainbreaker models auth paste-token --provider anthropic
chainbreaker models auth paste-token --provider openrouter
```
...
```bash
chainbreaker models status --check
```
...
- Priority order:
  - `CHAINBREAKER_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Chainbreaker retries with the next key only for rate-limit errors (for example
...
Run `chainbreaker models status` to confirm which profile is expiring. If the profile

...
`chainbreaker doctor` is the repair + migration tool for Chainbreaker. It fixes stale
...
```bash
chainbreaker doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
chainbreaker doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
chainbreaker doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
chainbreaker doctor --non-interactive
```
...
```bash
cat ~/.chainbreaker/chainbreaker.json
```
...
- Extra workspace dir detection (`~/chainbreaker`).
...
you to run `chainbreaker doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.chainbreaker/chainbreaker.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `chainbreaker doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.chainbreaker/sessions/` to `~/.chainbreaker/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.chainbreaker/agent/` to `~/.chainbreaker/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.chainbreaker/credentials/*.json` (except `oauth.json`)
  - to `~/.chainbreaker/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `chainbreaker doctor`.
...
Doctor also checks the cron job store (`~/.chainbreaker/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.chainbreaker` folders exist across
  home directories or when `CHAINBREAKER_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.chainbreaker/chainbreaker.json` is
...
Discord plugin runtime packages) are present in the Chainbreaker install root.
If any are missing, doctor reports the packages and installs them in
`chainbreaker doctor --fix` / `chainbreaker doctor --repair` mode.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the Chainbreaker service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Chainbreaker gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`chainbreaker doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/chainbreaker` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(chainbreaker completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `chainbreaker completion --write-state` to regenerate the cache manually.
...
- `chainbreaker doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `chainbreaker doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `chainbreaker memory status --deep` to verify embedding readiness at runtime.
...
- `chainbreaker doctor` prompts before rewriting supervisor config.
- `chainbreaker doctor --yes` accepts the default repair prompts.
- `chainbreaker doctor --repair` applies recommended fixes without prompts.
- `chainbreaker doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `chainbreaker gateway install --force`.

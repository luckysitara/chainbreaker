---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Chainbreaker agents"
read_when:
...
OpenShell is a managed sandbox backend for Chainbreaker. Instead of running Docker
containers locally, Chainbreaker delegates sandbox lifecycle to the `openshell` CLI,
...
- Chainbreaker Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Chainbreaker creates an OpenShell
...
- You edit files locally outside Chainbreaker and want those changes visible in the
...
- When the sandbox is first created, Chainbreaker seeds the remote workspace from
...
- Chainbreaker does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Chainbreaker after the initial seed,
the remote sandbox does **not** see those changes. Use
`chainbreaker sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"chainbreaker"` | Sandbox source for first-time create                  |
...
1. Chainbreaker calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Chainbreaker calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `chainbreaker sandbox` commands

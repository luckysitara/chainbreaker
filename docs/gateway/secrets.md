...
Chainbreaker supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `CHAINBREAKER_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Chainbreaker runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Chainbreaker validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Chainbreaker QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/chainbreaker"],
...
- Chainbreaker resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__CHAINBREAKER_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `chainbreaker.json` refs)
...
When reload-time activation fails after a healthy state, Chainbreaker enters degraded secrets state.
...
- Strict command paths (for example `chainbreaker memory` remote-memory paths and `chainbreaker qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `chainbreaker status`, `chainbreaker status --all`, `chainbreaker channels status`, `chainbreaker channels resolve`, `chainbreaker security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `chainbreaker secrets reload`.
...
```bash
chainbreaker secrets audit --check
chainbreaker secrets configure
chainbreaker secrets audit --check
```
...
- plaintext values at rest (`chainbreaker.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `chainbreaker.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `chainbreaker secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `chainbreaker.json` plus `auth-profiles.json` for one agent scope
...
- `chainbreaker secrets configure --providers-only`
- `chainbreaker secrets configure --skip-provider-setup`
- `chainbreaker secrets configure --agent <id>`
...
```bash
chainbreaker secrets apply --from /tmp/chainbreaker-secrets-plan.json
chainbreaker secrets apply --from /tmp/chainbreaker-secrets-plan.json --allow-exec
chainbreaker secrets apply --from /tmp/chainbreaker-secrets-plan.json --dry-run
chainbreaker secrets apply --from /tmp/chainbreaker-secrets-plan.json --dry-run --allow-exec
```
...
Chainbreaker intentionally does not write rollback backups containing historical plaintext secret values.

...
summary: "Complete reference for every Chainbreaker config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.chainbreaker/chainbreaker.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Chainbreaker uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.chainbreaker/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `chainbreaker doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `chainbreaker doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["chainbreaker-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-chainbreaker",
          requireMention: false,
...
- Chainbreaker additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `chainbreaker plugins install @chainbreaker/mattermost`.
...
- `commands.callbackUrl` must resolve to the Chainbreaker gateway endpoint and be reachable from the Mattermost server.
...
Chainbreaker spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `chainbreaker channels add` (or channel onboarding) while still on a single-account top-level channel config, Chainbreaker moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `chainbreaker doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@chainbreaker", "chainbreaker"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@chainbreaker"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `chainbreaker.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Default: `~/.chainbreaker/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Chainbreaker auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/chainbreaker" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Chainbreaker assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Chainbreaker aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.chainbreaker/sandboxes",
        docker: {
          ...
          apparmorProfile: "chainbreaker-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/chainbreaker-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "chainbreaker-sandbox-browser:bookworm-slim",
          network: "chainbreaker-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Chainbreaker materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.chainbreaker/sandboxes`
...
In `remote` mode, host-local edits made outside Chainbreaker are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `chainbreaker.json`.
noVNC observer access uses VNC auth by default and Chainbreaker emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `chainbreaker-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from CHAINBREAKER_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `CHAINBREAKER_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `CHAINBREAKER_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `CHAINBREAKER_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.chainbreaker/workspace",
        agentDir: "~/.chainbreaker/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@chainbreaker"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.chainbreaker/workspace-home" },
      { id: "work", workspace: "~/.chainbreaker/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Chainbreaker resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.chainbreaker/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.chainbreaker/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.chainbreaker/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.chainbreaker/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Chainbreaker starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.chainbreaker/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Chainbreaker treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`        | `nodes`                                                                                  |
| `group:chainbreaker` | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.chainbreaker/attachments/<uuid>/` with a `.manifest.json`.
...
Chainbreaker uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.chainbreaker/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `CHAINBREAKER_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Chainbreaker forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `chainbreaker onboard --auth-choice opencode-zen` or `chainbreaker onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `chainbreaker onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `chainbreaker onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `chainbreaker onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `chainbreaker onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `chainbreaker onboard --auth-choice minimax-api`.
...
- Loaded from `~/.chainbreaker/extensions`, `<workspace>/.chainbreaker/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Chainbreaker plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Chainbreaker plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Chainbreaker applies those as sanitized agent settings, not as raw Chainbreaker config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `chainbreaker plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      chainbreaker: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or CHAINBREAKER_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `CHAINBREAKER_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `CHAINBREAKER_APNS_RELAY_BASE_URL` / `CHAINBREAKER_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `CHAINBREAKER_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
CHAINBREAKER_CONFIG_PATH=~/.chainbreaker/a.json \
CHAINBREAKER_STATE_DIR=~/.chainbreaker-a \
chainbreaker gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.chainbreaker-dev` + port `19001`), `--profile <name>` (uses `~/.chainbreaker-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/chainbreaker/tls/server.crt",
      keyPath: "/etc/chainbreaker/tls/server.key",
      caPath: "/etc/chainbreaker/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.chainbreaker/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-chainbreaker-token: <token>`.
...
```json5
{
  hooks: {
    gmail: {
      account: "chainbreaker@gmail.com",
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `CHAINBREAKER_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.chainbreaker/workspace/canvas",
    liveReload: true,
    // enabled: false, // or CHAINBREAKER_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__chainbreaker__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__chainbreaker__/a2ui/`
...
- Also serves A2UI at `/__chainbreaker__/a2ui/`.
...
- Hostname defaults to `chainbreaker`. Override with `CHAINBREAKER_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.chainbreaker/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `chainbreaker dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.chainbreaker/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${CHAINBREAKER_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `chainbreaker.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.chainbreaker/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/chainbreaker-vault-resolver",
...
- Legacy OAuth imports from `~/.chainbreaker/credentials/oauth.json`.
...
- Default log file: `/tmp/chainbreaker/chainbreaker-YYYY-MM-DD.log`.
...
```json5
{
  diagnostics: {
    ...
    otel: {
      ...
      serviceName: "chainbreaker-gateway",
      traces: true,
      metrics: true,
      logs: false,
      sampleRate: 1.0,
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Chainbreaker.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `CHAINBREAKER_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `chainbreaker doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `chainbreaker.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.

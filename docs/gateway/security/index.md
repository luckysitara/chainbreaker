...
Chainbreaker is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
**On this page:** [Trust model](#scope-first-personal-assistant-security-model) | [Quick audit](#quick-check-chainbreaker-security-audit) | [Hardened baseline](#hardened-baseline-in-60-seconds) | [DM access model](#dm-access-model-pairing--allowlist--open--disabled) | [Configuration hardening](#configuration-hardening-examples) | [Incident response](#incident-response)

## Scope first: personal assistant security model

Chainbreaker security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `chainbreaker security audit`
...
```bash
chainbreaker security audit
chainbreaker security audit --deep
chainbreaker security audit --fix
chainbreaker security audit --json
```
...
Chainbreaker is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Chainbreaker assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.chainbreaker`, including `chainbreaker.json`), treat them as a trusted operator.
...
4. Claim is not listed in [Out of Scope](https://github.com/chainbreaker/chainbreaker/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, Chainbreaker also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.chainbreaker/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.chainbreaker/credentials/<channel>-allowFrom.json` (default account)
  - `~/.chainbreaker/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.chainbreaker/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.chainbreaker/secrets.json`
- **Legacy OAuth import**: `~/.chainbreaker/credentials/oauth.json`
...
| ------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                           | critical      | Other users/processes can modify full Chainbreaker state                             | filesystem perms on `~/.chainbreaker`                                                                | yes      |
| `fs.config.perms_writable`                                    | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.chainbreaker/chainbreaker.json`                                              | yes      |
...
| `tools.exec.auto_allow_skills_enabled`                        | warn          | Exec approvals trust skill bins implicitly                                           | `~/.chainbreaker/exec-approvals.json`                                                                | no       |
...
`chainbreaker security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`chainbreaker security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Chainbreaker config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${CHAINBREAKER_GATEWAY_PASSWORD}
```
...
- Chainbreaker gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Chainbreaker responses.
...
Chainbreaker stores session transcripts on disk under `~/.chainbreaker/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.chainbreaker` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Chainbreaker cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
Chainbreaker can refresh the skills list mid-session:
...
- If you install plugins (`chainbreaker plugins install <package>`), treat it like running untrusted code:
  - The install path is the per-plugin directory under the active plugin install root.
  - Chainbreaker runs a built-in dangerous-code scan before install. `critical` findings block by default.
  - Chainbreaker uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
  ...
  - Gateway-backed skill dependency installs follow the same dangerous/suspicious split: built-in `critical` findings block unless the caller explicitly sets `dangerouslyForceUnsafeInstall`, while suspicious findings still warn only. `chainbreaker skills install` remains the separate ClawHub skill download/install flow.
...
```bash
chainbreaker pairing list <channel>
chainbreaker pairing approve <channel> <code>
```
...
By default, Chainbreaker routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Chainbreaker has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.chainbreaker/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.chainbreaker or your logs.”

## Unsafe external content bypass flags

Chainbreaker includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.chainbreaker/chainbreaker.json`: `600` (user read/write only)
- `~/.chainbreaker`: `700` (user only)

`chainbreaker doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `CHAINBREAKER_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__chainbreaker__/canvas/` and `/__chainbreaker__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Chainbreaker with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_chainbreaker-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `CHAINBREAKER_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `chainbreaker doctor --generate-gateway-token`.
...
paths, set `CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `CHAINBREAKER_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `CHAINBREAKER_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Chainbreaker
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Chainbreaker verifies the identity by resolving the
...
- On the OpenAI-compatible HTTP surface, shared-secret bearer auth restores the full default operator scopes and owner semantics for agent turns; narrower `x-chainbreaker-scopes` values do not reduce that shared-secret path.
...
- Chainbreaker will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.chainbreaker/` (or `$CHAINBREAKER_STATE_DIR/`) may contain secrets or private data:

- `chainbreaker.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `chainbreaker status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@chainbreaker", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.chainbreaker`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.chainbreaker/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `chainbreaker` profile).
...
Chainbreaker’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Chainbreaker limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `chainbreaker gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `CHAINBREAKER_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/chainbreaker/chainbreaker-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.chainbreaker/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `chainbreaker security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Chainbreaker version
...
Found a vulnerability in Chainbreaker? Please report responsibly:

1. Email: [security@chainbreaker.ai](mailto:security@chainbreaker.ai)

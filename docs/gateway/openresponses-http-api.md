...
Chainbreaker’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`chainbreaker agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-chainbreaker-scopes` values and restore the normal full operator defaults
...
- select agents with `model: "chainbreaker"`, `model: "chainbreaker/default"`, `model: "chainbreaker/<agentId>"`, or `x-chainbreaker-agent-id`
- use `x-chainbreaker-model` when you want to override the selected agent's backend model
- use `x-chainbreaker-session-key` for explicit session routing
- use `x-chainbreaker-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Chainbreaker reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
curl -sS http://127.0.0.1:18789/v1/responses \
  ...
  -H 'x-chainbreaker-agent-id: main' \
  -d '{
    "model": "chainbreaker",
    "input": "hi"
  }'

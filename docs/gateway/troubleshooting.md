...
- `chainbreaker gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `chainbreaker doctor` reports no blocking config/service issues.
- `chainbreaker channels status --probe` shows connected/ready channels.
...
chainbreaker config get agents.defaults.models
```
...
chainbreaker config get channels
chainbreaker logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `chainbreaker config get gateway.auth.token` then paste into Control UI settings.                      |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `chainbreaker devices list` then `chainbreaker devices approve <requestId>`.                                                                |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `chainbreaker configure`). If you are running Chainbreaker via Podman, the default config path is `~/.chainbreaker/chainbreaker.json`.
...
chainbreaker system heartbeat last
chainbreaker logs --follow
```
...
chainbreaker approvals get --node <idOrNameOrIp>
chainbreaker logs --follow
chainbreaker status
```
...
chainbreaker browser start --browser-profile chainbreaker
...
```bash
chainbreaker gateway status
chainbreaker config get gateway.mode
chainbreaker config get gateway.remote.url
chainbreaker config get gateway.auth.mode
```
...
```bash
chainbreaker config get gateway.bind

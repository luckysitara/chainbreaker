...
Chainbreaker can run **local AI CLIs** as a **text-only fallback** when API providers are down,
...
```bash
chainbreaker agent --message "hi" --model claude-cli/opus-4.6
```

Codex CLI also works out of the box (via the bundled OpenAI plugin):

```bash
chainbreaker agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, Chainbreaker now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Chainbreaker will
...
2. **Builds a system prompt** using the same Chainbreaker prompt + workspace context.
...
Chainbreaker will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Chainbreaker appends the
...
- **No Chainbreaker tools** (the CLI backend never receives tool calls). Some CLIs
  may still run their own agent tooling.
- **No streaming** (CLI output is collected then returned).
- **Structured outputs** depend on the CLI’s JSON format.
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. Chainbreaker sessions still work
  normally.

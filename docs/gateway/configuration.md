...
read_when:
  - Setting up Chainbreaker for the first time
...
Chainbreaker reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.chainbreaker/chainbreaker.json`.

If the file is missing, Chainbreaker uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `chainbreaker onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    chainbreaker onboard       # full onboarding flow
    chainbreaker configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    chainbreaker config get agents.defaults.workspace
    chainbreaker config set agents.defaults.heartbeat.every "2h"
    chainbreaker config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.chainbreaker/chainbreaker.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Chainbreaker only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`chainbreaker doctor`, `chainbreaker logs`, `chainbreaker health`, `chainbreaker status`)
- Run `chainbreaker doctor` to see exact issues
- Run `chainbreaker doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `chainbreaker.json`.
    ...
    ```bash
    chainbreaker config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `CHAINBREAKER_APNS_RELAY_BASE_URL` and `CHAINBREAKER_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `CHAINBREAKER_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.chainbreaker/chainbreaker.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `chainbreaker config set` for single keys.
    </Warning>
    ...
    ```bash
    chainbreaker gateway call config.get --params '{}'  # capture payload.hash
    chainbreaker gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.chainbreaker/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    chainbreaker gateway call config.patch --params '{
...
Chainbreaker reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.chainbreaker/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Chainbreaker runs your login shell and imports only the missing keys:
...
Env var equivalent: `CHAINBREAKER_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${CHAINBREAKER_GATEWAY_TOKEN}" } },

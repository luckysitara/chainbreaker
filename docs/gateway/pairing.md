...
chainbreaker nodes reject <requestId>
chainbreaker nodes status
chainbreaker nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.chainbreaker`):

- `~/.chainbreaker/nodes/paired.json`
- `~/.chainbreaker/nodes/pending.json`

If you override `CHAINBREAKER_STATE_DIR`, the `nodes/` folder moves with it.

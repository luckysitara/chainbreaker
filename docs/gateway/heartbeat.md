...
- During heartbeat runs, Chainbreaker treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `chainbreaker sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Chainbreaker skips the heartbeat run entirely (no model call).
...
```bash
chainbreaker system event --text "Check for urgent follow-ups" --mode now
```

...
read_when:
  - Running Chainbreaker behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Chainbreaker
...
- You run Chainbreaker behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Chainbreaker checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Chainbreaker extracts the user identity from the configured header
...
- Chainbreaker can stay on loopback HTTP behind the proxy.
...
If Chainbreaker itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://chainbreaker.example.com
    to: http://chainbreaker-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
chainbreaker.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy chainbreaker:18789 {
...
location / {
    ...
    proxy_pass http://chainbreaker:18789;
...
Chainbreaker rejects ambiguous configurations where both a `gateway.auth.token` (or `CHAINBREAKER_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
`chainbreaker security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Chainbreaker config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `chainbreaker security audit` and review findings

...
read_when:
  - You want to connect Chainbreaker to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

# IRC

Use IRC when you want Chainbreaker in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.chainbreaker/chainbreaker.json`.
2. Set at least:
...
```json5
{
  channels: {
    irc: {
      ...
      port: 6697,
      tls: true,
      nick: "chainbreaker-bot",
      channels: ["#chainbreaker"],
    },
  },
}
```
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Chainbreaker defaults to **mention-gating** in group contexts.

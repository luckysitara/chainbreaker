...
Tlon is a decentralized messenger built on Urbit. Chainbreaker connects to your Urbit ship and can
...
```bash
chainbreaker plugins install @chainbreaker/tlon
```

Local checkout (when running from a git repo):

```bash
chainbreaker plugins install ./path/to/local/tlon-plugin
```
...
By default, Chainbreaker blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `chainbreaker message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Chainbreaker replies in-thread.

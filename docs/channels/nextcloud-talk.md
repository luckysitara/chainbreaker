...
```bash
chainbreaker plugins install @chainbreaker/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
chainbreaker plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Chainbreaker" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `chainbreaker pairing list nextcloud-talk`
  - `chainbreaker pairing approve nextcloud-talk <CODE>`

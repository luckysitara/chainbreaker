...
read_when:
  - Setting up Twitch chat integration for Chainbreaker
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Chainbreaker connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
chainbreaker plugins install @chainbreaker/twitch
```

Local checkout (when running from a git repo):

```bash
chainbreaker plugins install ./path/to/local/twitch-plugin
```
...
4. Configure the token:
   - Env: `CHAINBREAKER_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "chainbreaker", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CHAINBREAKER_TWITCH_ACCESS_TOKEN env var)
...
```bash
CHAINBREAKER_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

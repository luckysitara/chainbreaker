---
summary: "Mattermost bot setup and Chainbreaker config"
read_when:
...
```bash
chainbreaker plugins install @chainbreaker/mattermost
```

Local checkout (when running from a git repo):

```bash
chainbreaker plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Chainbreaker and start the gateway.
...
Native slash commands are opt-in. When enabled, Chainbreaker registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Chainbreaker derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Chainbreaker.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Chainbreaker.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Chainbreaker, not `404`.
...
- Approve via:
  - `chainbreaker pairing list mattermost`
  - `chainbreaker pairing approve mattermost <CODE>`
...
Use these target formats with `chainbreaker message send` or cron/webhooks:
...
Chainbreaker resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Chainbreaker sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Chainbreaker sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Chainbreaker derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Chainbreaker run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="chainbreaker-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"chainbreaker-mattermost-interactions",
...
`chainbreaker message send` and cron/webhook deliveries.

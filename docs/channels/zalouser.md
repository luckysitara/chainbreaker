...
read_when:
  - Setting up Zalo Personal for Chainbreaker
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Chainbreaker.
...
- Install via CLI: `chainbreaker plugins install @chainbreaker/zalouser`
- Or from a source checkout: `chainbreaker plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `chainbreaker channels login --channel zalouser`
...
```bash
chainbreaker directory self --channel zalouser
chainbreaker directory peers list --channel zalouser --query "name"
chainbreaker directory groups list --channel zalouser --query "work"
```
...
- `chainbreaker pairing list zalouser`
- `chainbreaker pairing approve zalouser <code>`
...
- On startup, Chainbreaker resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Chainbreaker stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Chainbreaker state. Example:
...
- Chainbreaker sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Chainbreaker sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `chainbreaker channels logout --channel zalouser && chainbreaker channels login --channel zalouser`
...
- The channel now runs fully in Chainbreaker without external CLI binaries.

...
- Chainbreaker talks to it through its REST API (`GET /api/v1/ping`, `POST /message/text`, `POST /chat/:id/*`).
...
3. Run `chainbreaker onboard` and select BlueBubbles, or configure manually:
...
- Webhook authentication is always required. Chainbreaker rejects BlueBubbles webhook requests unless they include a password/guid that matches `channels.bluebubbles.password` (for example `?password=<password>` or `x-password`), regardless of loopback/proxy topology.
...
```
chainbreaker channels add bluebubbles --http-url http://192.168.1.100:1234 --password <password>
```
...
- Approve via:
  - `chainbreaker pairing list bluebubbles`
  - `chainbreaker pairing approve bluebubbles <CODE>`
...
- **Typing indicators**: Chainbreaker sends typing start events; BlueBubbles clears typing automatically on send or timeout (manual stop via DELETE is unreliable).
...
Chainbreaker may surface _short_ message IDs (e.g., `1`, `2`) to save tokens.
...
- Direct handles: `+15555550123`, `user@example.com`
  - If a direct handle does not have an existing DM chat, Chainbreaker will create one via `POST /api/v1/chat/new`. This requires the BlueBubbles Private API to be enabled.
...
- Pairing codes expire after one hour; use `chainbreaker pairing list bluebubbles` and `chainbreaker pairing approve bluebubbles <code>`.
- Reactions require the BlueBubbles private API (`POST /api/v1/message/react`); ensure the server version exposes it.
- Edit/unsend require macOS 13+ and a compatible BlueBubbles server version. On macOS 26 (Tahoe), edit is currently broken due to private API changes.
- Group icon updates can be flaky on macOS 26 (Tahoe): the API may return success but the new icon does not sync.
- Chainbreaker auto-hides known-broken actions based on the BlueBubbles server's macOS version. If edit still appears on macOS 26 (Tahoe), disable it manually with `channels.bluebubbles.actions.edit=false`.
- For status/health info: `chainbreaker status --all` or `chainbreaker status --deep`.

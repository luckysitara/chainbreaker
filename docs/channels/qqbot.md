...
read_when:
  - You want to connect Chainbreaker to QQ
...
QQ Bot connects to Chainbreaker via the official QQ Bot API (WebSocket gateway). The
...
## Bundled with Chainbreaker

Current Chainbreaker installs bundle QQ Bot. You do not need a separate
`chainbreaker plugins install` step for normal setup.
...
```bash
chainbreaker channels add --channel qqbot --token "AppID:AppSecret"
```
...
chainbreaker configure --section channels
```
...
- `chainbreaker channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Chainbreaker instance:
...
```bash
chainbreaker channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
| `/bot-version` | Show the Chainbreaker framework version |

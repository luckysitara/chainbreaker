...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Chainbreaker to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Chainbreaker releases, so no separate plugin install
is required.
...
```bash
chainbreaker plugins install @chainbreaker/feishu
```
...
If you just installed Chainbreaker, run onboarding:
...
2. Configuring app credentials in Chainbreaker
...
1. You already ran `chainbreaker channels add` for Feishu
2. The gateway is running (`chainbreaker gateway status`)
...
Edit `~/.chainbreaker/chainbreaker.json`:
...
```bash
chainbreaker pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  chainbreaker pairing list feishu
  chainbreaker pairing approve feishu <CODE>
  ```
...
2. Run `chainbreaker logs --follow` and look for `chat_id`
...
2. Run `chainbreaker logs --follow` and look for `open_id`
...
| Command                        | Description                   |
| ------------------------------ | ----------------------------- |
| `chainbreaker gateway status`  | Show gateway status           |
| `chainbreaker gateway install` | Install/start gateway service |
| `chainbreaker gateway stop`    | Stop gateway service          |
| `chainbreaker gateway restart` | Restart gateway service       |
| `chainbreaker logs --follow`   | Tail gateway logs             |
...
4. Check logs: `chainbreaker logs --follow`
...
5. Ensure the gateway is running: `chainbreaker gateway status`
6. Check logs: `chainbreaker logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.chainbreaker/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.chainbreaker/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [

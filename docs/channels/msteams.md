...
```bash
chainbreaker plugins install @chainbreaker/msteams
```

Local checkout (when running from a git repo):

```bash
chainbreaker plugins install ./path/to/local/msteams-plugin
```

If you choose Teams during setup and a git checkout is detected,
Chainbreaker will offer the local install path automatically.
...
3. Configure Chainbreaker with those credentials.
...
- Talk to Chainbreaker via Teams DMs, group chats, or channels.
...
- On startup, Chainbreaker resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.chainbreaker/chainbreaker.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Chainbreaker, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                        |
   | ------------------ | ------------------------------------------------------------ |
   | **Bot handle**     | Your bot name, e.g., `chainbreaker-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `chainbreaker plugins install @chainbreaker/msteams`
   - From a local checkout: `chainbreaker plugins install ./path/to/local/msteams-plugin`
...
Chainbreaker exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Chainbreaker in Teams", full: "Chainbreaker in Teams" },
...
Chainbreaker handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Chainbreaker only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ChainbreakerShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Chainbreaker sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `chainbreaker message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.chainbreaker/msteams-polls.json`.
...
```bash
chainbreaker message send --channel msteams \
...
chainbreaker message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
chainbreaker message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
chainbreaker message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
chainbreaker message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```

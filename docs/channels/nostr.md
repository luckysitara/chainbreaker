...
read_when:
  - You want Chainbreaker to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Chainbreaker to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`chainbreaker onboard`) and `chainbreaker channels add` list optional channel plugins.
...
```bash
chainbreaker plugins install @chainbreaker/nostr
```

Use a local checkout (dev workflows):

```bash
chainbreaker plugins install --link <path-to-local-nostr-plugin>
```
...
chainbreaker channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "chainbreaker",
        displayName: "Chainbreaker",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "chainbreaker@example.com",
        lud16: "chainbreaker@example.com",
      },
    },
  },
}

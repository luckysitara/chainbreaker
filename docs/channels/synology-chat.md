---
summary: "Synology Chat webhook setup and Chainbreaker config"
read_when:
  - Setting up Synology Chat with Chainbreaker
...
```bash
chainbreaker plugins install ./path/to/local/synology-chat-plugin
```
...
1. Install and enable the Synology Chat plugin.
   - `chainbreaker onboard` now shows Synology Chat in the same channel setup list as `chainbreaker channels add`.
   - Non-interactive setup: `chainbreaker channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Chainbreaker gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Chainbreaker.
   - Guided: `chainbreaker onboard`
   - Direct: `chainbreaker channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `chainbreaker pairing list synology-chat`
  - `chainbreaker pairing approve synology-chat <CODE>`
...
```bash
chainbreaker message send --channel synology-chat --target 123456 --text "Hello from Chainbreaker"
chainbreaker message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Chainbreaker now rejects duplicate exact paths

---
summary: "How Chainbreaker manages conversation sessions"
read_when:
...
Chainbreaker organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `chainbreaker security audit`.
...
Chainbreaker automatically bounds session storage over time. By default, it runs
...
Preview with `chainbreaker sessions cleanup --dry-run`.

## Inspecting sessions

- `chainbreaker status` -- session store path and recent activity.
- `chainbreaker sessions --json` -- all sessions (filter with `--active <minutes>`).

...
summary: "How Chainbreaker remembers things across sessions"
read_when:
...
Chainbreaker remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.chainbreaker/workspace`).
...
<Info>
Chainbreaker auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Chainbreaker
...
```bash
chainbreaker memory status          # Check index status and provider
chainbreaker memory search "query"  # Search from the command line
chainbreaker memory index --force   # Rebuild the index
```

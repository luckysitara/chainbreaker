...
Chainbreaker selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Chainbreaker can use (plus aliases).
...
- If you omit the provider, Chainbreaker treats the input as an alias or a model for the **default provider** (only works when there is no `/` in the model ID).
...
chainbreaker models set <provider/model>
chainbreaker models set-image <provider/model>

chainbreaker models aliases list
chainbreaker models aliases add <alias> <provider/model>
chainbreaker models aliases remove <alias>

chainbreaker models fallbacks list
chainbreaker models fallbacks add <provider/model>
chainbreaker models fallbacks remove <provider/model>
chainbreaker models fallbacks clear

chainbreaker models image-fallbacks list
chainbreaker models image-fallbacks add <provider/model>
chainbreaker models image-fallbacks remove <provider/model>
chainbreaker models image-fallbacks clear
```

`chainbreaker models` (no subcommand) is a shortcut for `models status`.
...
`chainbreaker models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.chainbreaker/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Chainbreaker writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Chainbreaker regenerates `models.json`, including command-driven paths like `chainbreaker agent`.

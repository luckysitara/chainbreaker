...
alongside Chainbreaker. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Chainbreaker falls back to the
...
Chainbreaker creates a self-contained QMD home under
`~/.chainbreaker/agents/<agentId>/qmd/` and manages the sidecar lifecycle
automatically -- collections, updates, and embedding runs are handled for you.

## How the sidecar works

- Chainbreaker creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, Chainbreaker retries with `qmd query`.
- If QMD fails entirely, Chainbreaker falls back to the builtin SQLite engine.
...
collection under `~/.chainbreaker/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Chainbreaker logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Chainbreaker
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Chainbreaker uses.
...
Chainbreaker's builtin symlink rules. Keep temporary monorepo checkouts under

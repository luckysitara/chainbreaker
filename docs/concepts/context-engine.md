...
read_when:
  - You want to understand how Chainbreaker assembles model context
...
A **context engine** controls how Chainbreaker builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Chainbreaker ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.chainbreaker/chainbreaker.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Chainbreaker plugin. Install
...
chainbreaker plugins install -l ./my-context-engine
```
...
Every time Chainbreaker runs a model prompt, the context engine participates at
...
Chainbreaker currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Chainbreaker
...
The built-in `legacy` engine preserves Chainbreaker's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Chainbreaker uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Chainbreaker disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Chainbreaker automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `chainbreaker/plugin-sdk/core` to use
  Chainbreaker's built-in compaction behavior.
...
Chainbreaker resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Chainbreaker's built-in summarization. Plugin engines can implement
...
- Use `chainbreaker doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Chainbreaker
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `chainbreaker plugins install -l ./my-engine` to link a

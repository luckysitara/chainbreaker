...
Chainbreaker runs two retrieval paths in parallel and merges the results:
...
- **Vector search** finds notes with similar meaning ("gateway host" matches
  "the machine running Chainbreaker").
...
**No results?** Run `chainbreaker memory status` to check the index. If empty, run

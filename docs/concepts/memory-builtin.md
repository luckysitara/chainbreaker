...
Chainbreaker indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.chainbreaker/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `chainbreaker memory index --force`
...
**Memory search disabled?** Check `chainbreaker memory status`. If no provider is
detected, set one explicitly or add an API key.

**Stale results?** Run `chainbreaker memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Chainbreaker falls back to in-process cosine similarity

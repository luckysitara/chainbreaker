...
[Honcho](https://honcho.dev) adds AI-native memory to Chainbreaker. It persists
...
```bash
chainbreaker plugins install @honcho-ai/chainbreaker-honcho
chainbreaker honcho setup
...
Settings live under `plugins.entries["chainbreaker-honcho"].config`:
    ...
    entries: {
      "chainbreaker-honcho": {
        config: {
          ...
          workspaceId: "chainbreaker", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `chainbreaker honcho setup` detects and
...
```bash
chainbreaker honcho setup                        # Configure API key and migrate files
chainbreaker honcho status                       # Check connection status
chainbreaker honcho ask <question>               # Query Honcho about the user
chainbreaker honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/chainbreaker-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Chainbreaker integration guide](https://docs.honcho.dev/v3/guides/integrations/chainbreaker)
- [Memory](/concepts/memory) -- Chainbreaker memory overview

...
- `/usage cost` in chats: local cost summary aggregated from Chainbreaker session logs.
- CLI: `chainbreaker status --usage` prints a full per-provider breakdown.
- CLI: `chainbreaker channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

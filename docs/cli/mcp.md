---
summary: "Expose Chainbreaker channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Chainbreaker-backed channels
  - Running `chainbreaker mcp serve`
  - Managing Chainbreaker-saved MCP server definitions
title: "mcp"
---

# mcp

`chainbreaker mcp` has two jobs:

- run Chainbreaker as an MCP server with `chainbreaker mcp serve`
- manage Chainbreaker-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Chainbreaker acting as an MCP server
- `list` / `show` / `set` / `unset` is Chainbreaker acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`chainbreaker acp`](/cli/acp) when Chainbreaker should host a coding harness
session itself and route that runtime through ACP.

## Chainbreaker as an MCP server

This is the `chainbreaker mcp serve` path.

## When to use `serve`

Use `chainbreaker mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Chainbreaker-backed channel conversations
- you already have a local or remote Chainbreaker Gateway with routed sessions
- you want one MCP server that works across Chainbreaker's channel backends instead
  of running separate per-channel bridges

Use [`chainbreaker acp`](/cli/acp) instead when Chainbreaker should host the coding
runtime itself and keep the agent session inside Chainbreaker.

## How it works

`chainbreaker mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Chainbreaker Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `chainbreaker mcp serve`
...
conversations. A conversation appears when Chainbreaker already has session state
...
```bash
# Local Gateway
chainbreaker mcp serve

# Remote Gateway
chainbreaker mcp serve --url wss://gateway-host:18789 --token-file ~/.chainbreaker/gateway.token

# Remote Gateway with password auth
chainbreaker mcp serve --url wss://gateway-host:18789 --password-file ~/.chainbreaker/gateway.password

# Enable verbose bridge logs
chainbreaker mcp serve --verbose

# Disable Claude-specific push notifications
chainbreaker mcp serve --claude-channel-mode off
```
...
Chainbreaker equivalent of a Claude Code channel adapter: standard MCP tools remain
...
`chainbreaker mcp serve` supports:
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Chainbreaker channel configuration
...
Chainbreaker ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `chainbreaker mcp serve`
...
## Chainbreaker as an MCP client registry

This is the `chainbreaker mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Chainbreaker over MCP. They manage Chainbreaker-owned MCP
server definitions under `mcp.servers` in Chainbreaker config.

Those saved definitions are for runtimes that Chainbreaker launches or configures
later, such as embedded Pi and other runtime adapters. Chainbreaker stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Chainbreaker config
...
Chainbreaker also stores a lightweight MCP server registry in config for surfaces
that want Chainbreaker-managed MCP definitions.

Commands:

- `chainbreaker mcp list`
- `chainbreaker mcp show [name]`
- `chainbreaker mcp set <name> <json>`
- `chainbreaker mcp unset <name>`

Examples:

```bash
chainbreaker mcp list
chainbreaker mcp show context7 --json
chainbreaker mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
chainbreaker mcp set docs '{"url":"https://mcp.example.com"}'
chainbreaker mcp unset context7
```

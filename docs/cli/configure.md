---
summary: "CLI reference for `chainbreaker configure` (interactive configuration prompts)"
read_when:
...
Tip: `chainbreaker config` without a subcommand opens the same wizard. Use
`chainbreaker config get|set|unset` for non-interactive edits.

For web search, `chainbreaker configure --section web` lets you choose a provider
...
chainbreaker configure --section model --section channels
```

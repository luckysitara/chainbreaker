---
summary: "CLI reference for `chainbreaker config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `chainbreaker.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `chainbreaker configure`).

## Examples

```bash
chainbreaker config file
chainbreaker config schema
chainbreaker config get browser.executablePath
...
chainbreaker config set secrets.providers.vaultfile --provider-source file --provider-path /etc/chainbreaker/secrets.json --provider-mode json
chainbreaker config unset plugins.entries.brave.config.webSearch.apiKey
chainbreaker config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
chainbreaker config validate
chainbreaker config validate --json
```

### `config schema`

Print the generated JSON schema for `chainbreaker.json` to stdout as plain text.
...
```bash
chainbreaker config schema > chainbreaker.schema.json
```
...
chainbreaker config get agents.list[0].id
```
...
chainbreaker config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
chainbreaker config set agents.defaults.heartbeat.every "0m"
chainbreaker config set gateway.port 19001 --strict-json
chainbreaker config set channels.whatsapp.groups '["*"]' --strict-json
```

## `config set` modes

`chainbreaker config set` supports four assignment styles:

1. Value mode: `chainbreaker config set <path> <value>`
2. SecretRef builder mode:

```bash
chainbreaker config set channels.discord.token \
...
```bash
chainbreaker config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/chainbreaker-vault \
  --provider-arg read \
...
```bash
chainbreaker config set --batch-json '[
  {
...
```bash
chainbreaker config set --batch-file ./config-set.batch.json --dry-run
```
...
chainbreaker config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/chainbreaker/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `chainbreaker.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.chainbreaker/chainbreaker.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
...
- `config file`: Print the active config file path (resolved from `CHAINBREAKER_CONFIG_PATH` or default location).

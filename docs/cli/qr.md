---
summary: "CLI reference for `chainbreaker qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
chainbreaker qr
chainbreaker qr --setup-code-only
chainbreaker qr --json
chainbreaker qr --remote
chainbreaker qr --url wss://gateway.example/ws
```

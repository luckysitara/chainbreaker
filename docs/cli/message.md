---
summary: "CLI reference for `chainbreaker message` (send + channel actions)"
read_when:
...
```
chainbreaker message <subcommand> [flags]
```
...
- On cache miss, Chainbreaker will attempt a live directory lookup when the provider supports it.
...
- `chainbreaker message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Chainbreaker does not force a `default` account SecretRef scope
...
```
chainbreaker message send --channel discord \
...
```
chainbreaker message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
chainbreaker message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
chainbreaker message poll --channel msteams \
...
```
chainbreaker message react --channel slack \
...
```
chainbreaker message react --channel signal \
...
```
chainbreaker message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
chainbreaker message send --channel telegram --target @mychat \

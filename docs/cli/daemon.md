---
summary: "CLI reference for `chainbreaker daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `chainbreaker daemon ...` in scripts
...
`chainbreaker daemon ...` maps to the same service control surface as `chainbreaker gateway ...` service commands.

## Usage

```bash
chainbreaker daemon status
chainbreaker daemon install
chainbreaker daemon start
chainbreaker daemon stop
chainbreaker daemon restart
chainbreaker daemon uninstall
```
...
Use [`chainbreaker gateway`](/cli/gateway) for current docs and examples.

...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Chainbreaker Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`chainbreaker acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Chainbreaker channel
conversations instead of hosting an ACP harness session, use
[`chainbreaker mcp serve`](/cli/mcp) instead.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Chainbreaker gateway or agent instead.                                                                                                                                 |
...
# Remote Gateway
chainbreaker acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
chainbreaker acp --url wss://gateway-host:18789 --token-file ~/.chainbreaker/gateway.token

# Attach to an existing session key
chainbreaker acp --session agent:main:main

# Attach by label (must already exist)
chainbreaker acp --session-label "support inbox"

# Reset the session key before the first prompt
chainbreaker acp --session agent:main:main --reset-session
```
...
```bash
chainbreaker acp client

# Point the spawned bridge at a remote Gateway
chainbreaker acp client --server-args --url wss://gateway-host:18789 --token-file ~/.chainbreaker/gateway.token

# Override the server command (default: chainbreaker)
chainbreaker acp client --server "node" --server-args chainbreaker.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Chainbreaker through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Chainbreaker Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `chainbreaker acp` over stdio.

Example config (persisted):

```bash
chainbreaker config set gateway.remote.url wss://gateway-host:18789
chainbreaker config set gateway.remote.token <token>
```
...
```bash
chainbreaker acp --session agent:main:main
chainbreaker acp --session agent:design:main
chainbreaker acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Chainbreaker plugin tools, enable the
...
Chainbreaker bot over ACP, use `acpx` with its built-in `chainbreaker` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx chainbreaker` at `chainbreaker acp`.
3. Target the Chainbreaker session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Chainbreaker ACP session
acpx chainbreaker exec "Summarize the active Chainbreaker session state."

# Persistent named session for follow-up turns
acpx chainbreaker sessions ensure --name codex-bridge
acpx chainbreaker -s codex-bridge --cwd /path/to/repo \
  "Ask my Chainbreaker work agent for recent context relevant to this repo."
```

If you want `acpx chainbreaker` to target a specific Gateway and session key every
time, override the `chainbreaker` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "chainbreaker": {
      "command": "env CHAINBREAKER_HIDE_BANNER=1 CHAINBREAKER_SUPPRESS_NOTES=1 chainbreaker acp --url ws://127.0.0.1:18789 --token-file ~/.chainbreaker/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Chainbreaker checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env CHAINBREAKER_HIDE_BANNER=1 CHAINBREAKER_SUPPRESS_NOTES=1 node chainbreaker.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Chainbreaker agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Chainbreaker ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`CHAINBREAKER_GATEWAY_TOKEN`, `CHAINBREAKER_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`CHAINBREAKER_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `CHAINBREAKER_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `chainbreaker acp client` sets `CHAINBREAKER_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `chainbreaker`).

---
summary: "CLI reference for `chainbreaker onboard` (interactive onboarding)"
read_when:
...
```bash
chainbreaker onboard
chainbreaker onboard --flow quickstart
chainbreaker onboard --flow manual
chainbreaker onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `chainbreaker gateway run`.
...
- Fastest first chat: `chainbreaker dashboard` (Control UI, no channel setup).

---
summary: "Chainbreaker CLI reference for `chainbreaker` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.chainbreaker-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.chainbreaker-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `chainbreaker update` (source installs only).
...
Chainbreaker uses a lobster palette for CLI output.
...
```
chainbreaker [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `chainbreaker voicecall`).

## Security

- `chainbreaker security audit` — audit config + local state for common security foot-guns.
- `chainbreaker security audit --deep` — best-effort live Gateway probe.
- `chainbreaker security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `chainbreaker secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `chainbreaker secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `chainbreaker secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `chainbreaker secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `chainbreaker plugins list` — discover plugins (use `--json` for machine output).
- `chainbreaker plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `chainbreaker plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `chainbreaker plugins marketplace list <marketplace>` — list marketplace entries before install.
- `chainbreaker plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `chainbreaker plugins doctor` — report plugin load errors.
...
- `chainbreaker memory status` — show index stats.
- `chainbreaker memory index` — reindex memory files.
- `chainbreaker memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.chainbreaker/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `chainbreaker config` with no
...
- `config set --dry-run`: validate assignments without writing `chainbreaker.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `chainbreaker.json`.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `chainbreaker health` or `chainbreaker status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `chainbreaker doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Chainbreaker moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
chainbreaker channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
chainbreaker channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
chainbreaker channels remove --channel discord --account work --delete
...
Tip: use `chainbreaker skills search`, `chainbreaker skills install`, and `chainbreaker skills update` for ClawHub-backed skills.
...
- `chainbreaker message send --target +15555550123 --message "Hi"`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Chainbreaker may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Chainbreaker can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `chainbreaker status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Chainbreaker services are treated as first-class and aren't flagged as "extra".
...
chainbreaker logs --limit 200
chainbreaker logs --plain
chainbreaker logs --json
chainbreaker logs --no-color
```
...
```bash
chainbreaker models auth login --provider anthropic --method cli --set-default
```
...
`chainbreaker models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `CHAINBREAKER_GATEWAY_TOKEN` / `CHAINBREAKER_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `CHAINBREAKER_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`chainbreaker browser`](/cli/browser) and the [Browser tool](/tools/browser).

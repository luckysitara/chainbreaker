---
summary: "Chainbreaker Gateway CLI (`chainbreaker gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Chainbreaker’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `chainbreaker gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.chainbreaker/chainbreaker.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
- `--token <token>`: token override (also sets `CHAINBREAKER_GATEWAY_TOKEN` for the process).
...
```bash
chainbreaker gateway health --url ws://127.0.0.1:18789
```
...
chainbreaker gateway status --require-rpc
```
...
chainbreaker gateway probe --json
```
...
```bash
chainbreaker gateway probe --ssh user@gateway-host
```
...
```bash
chainbreaker gateway call status
chainbreaker gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
- For password auth on `gateway run`, prefer `CHAINBREAKER_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `CHAINBREAKER_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
- If both `gateway.auth.token` and `gateway.auth.password` are configured and `gateway.auth.mode` is unset, install is blocked until mode is set explicitly.
- Lifecycle commands accept `--json` for scripting.

## Discover gateways (Bonjour)

`gateway discover` scans for Gateway beacons (`_chainbreaker-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `chainbreaker.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
chainbreaker gateway discover --timeout 4000
chainbreaker gateway discover --json | jq '.beacons[].wsUrl'
```

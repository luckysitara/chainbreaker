...
This hub links the core docs for how Chainbreaker connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`chainbreaker gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__chainbreaker__/canvas/`, `/__chainbreaker__/a2ui/`), protected by Gateway auth when bound beyond loopback.

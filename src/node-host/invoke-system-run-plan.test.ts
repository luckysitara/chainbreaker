...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `chainbreaker-${params.binName}-bin-`,
    run: params.run,
  });
}
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "chainbreaker-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "chainbreaker-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "chainbreaker-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "chainbreaker-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "chainbreaker-perl-preload-load-path-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-approval-hardening-"));
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "chainbreaker-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-shell-option-value-"));
    try {

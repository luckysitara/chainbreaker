...
export type PromptDefaultModelResult = { model?: string; config?: ChainbreakerConfig };
...
function resolveConfiguredModelRaw(cfg: ChainbreakerConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: ChainbreakerConfig): string[] {
...
export function applyModelAllowlist(cfg: ChainbreakerConfig, models: string[]): ChainbreakerConfig {

...
export type DoctorHealthFlowContext = {
  ...
  cfg: ChainbreakerConfig;
  cfgForPersistence: ChainbreakerConfig;
  sourceConfigValid: boolean;
  configPath: string;
...
export function resolveDoctorMode(cfg: ChainbreakerConfig): DoctorFlowMode {
  ...
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("chainbreaker config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("chainbreaker config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("chainbreaker doctor --fix")}" to apply changes.`);
  }
}

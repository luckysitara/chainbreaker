...
function rawKeyValue(config: ChainbreakerConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: ChainbreakerConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: ChainbreakerConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: ChainbreakerConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(
  ...
  return {
    ...next,
    plugins: nextPlugins as ChainbreakerConfig["plugins"],
  };
}
...
async function finalizeSearchProviderSetup(params: {
  originalConfig: ChainbreakerConfig;
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with chainbreaker configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Chainbreaker will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.chainbreaker.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Chainbreaker will store a reference instead of the API key.",

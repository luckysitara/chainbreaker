...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `chainbreaker-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.CHAINBREAKER_STATE_DIR ?? "/tmp/chainbreaker",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.CHAINBREAKER_CONFIG_PATH ?? `${stateDir}/chainbreaker.json`,
    );
  ...
  it("falls back to CHAINBREAKER_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers CHAINBREAKER_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "CHAINBREAKER_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-tui-file-secret-"));

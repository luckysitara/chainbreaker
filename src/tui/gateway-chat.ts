...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set CHAINBREAKER_GATEWAY_TOKEN/CHAINBREAKER_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "chainbreaker-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.CHAINBREAKER_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.CHAINBREAKER_GATEWAY_PASSWORD);

...
describe("resolveInitialTuiAgentId", () => {
  const cfg: ChainbreakerConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/chainbreaker" },
        { id: "ops", workspace: "/tmp/chainbreaker/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/chainbreaker/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run chainbreaker devices list");
    expect(state.pairingHint).toContain("chainbreaker devices list");
  });

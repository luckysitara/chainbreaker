...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      CHAINBREAKER_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when CHAINBREAKER_THEME=light", async () => {
    const mod = await importThemeWithEnv({ CHAINBREAKER_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when CHAINBREAKER_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ CHAINBREAKER_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats CHAINBREAKER_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ CHAINBREAKER_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("CHAINBREAKER_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      CHAINBREAKER_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ CHAINBREAKER_THEME: "light" });
    const darkMod = await importThemeWithEnv({ CHAINBREAKER_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.CHAINBREAKER_THEME = "light";

...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Chainbreaker is a hobby project and still in beta. Expect sharp edges.",
      "By default, Chainbreaker is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Chainbreaker is not a hostile multi-tenant boundary by default.",
      ...
      "",
      "Must read: https://docs.chainbreaker.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("Chainbreaker setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: ChainbreakerConfig = snapshot.valid
    ? snapshot.exists
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("chainbreaker doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("chainbreaker doctor")}`,
        `Inspect: ${formatCliCommand("chainbreaker plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("chainbreaker configure")}.`;
  ...
  let localGatewayToken = process.env.CHAINBREAKER_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.CHAINBREAKER_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: ChainbreakerConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);

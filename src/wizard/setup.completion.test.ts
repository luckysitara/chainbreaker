...
function createDeps() {
  const deps: NonNullable<Parameters<typeof setupWizardShellCompletion>[0]["deps"]> = {
    ...
    checkShellCompletionStatus: vi.fn(async (_binName: string) => ({
      ...
      cacheExists: false,
      cachePath: "/tmp/chainbreaker.zsh",
...
describe("setupWizardShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("chainbreaker");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "chainbreaker");

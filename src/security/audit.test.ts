...
const pathResolutionEnvKeys = [
  ...
  "CHAINBREAKER_BUNDLED_PLUGINS_DIR",
] as const;
...
function stubChannelPlugin(params: {
  ...
  listAccountIds?: (cfg: ChainbreakerConfig) => string[];
  isConfigured?: (account: unknown, cfg: ChainbreakerConfig) => boolean;
  isEnabled?: (account: unknown, cfg: ChainbreakerConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
async function runConfigAuditCases<T extends { cfg: ChainbreakerConfig }>(
  cases: readonly T[],
...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "chainbreaker.json"),
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "chainbreaker.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ CHAINBREAKER_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const runSharedExtensionsAudit = async (config: ChainbreakerConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "chainbreaker.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        chainbreaker: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-security-audit-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { CHAINBREAKER_HOME: isolatedHome });
  ...
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    const cases = [
          ...
          withEnvAsync(
            {
              ...
              CHAINBREAKER_GATEWAY_PASSWORD: undefined,
            },
            async () =>
              audit({
                gateway: {
              ...
              gateway: {
                ...
                auth: {
                  password: {
                    source: "env",
                    provider: "default",
                    id: "CHAINBREAKER_GATEWAY_PASSWORD",
                  },
                },
              },
            },
      ...
      },
      {
        ...
        run: async () => {
          const sourceConfig: ChainbreakerConfig = {
            gateway: {
              bind: "lan",
              auth: {
                token: {
          ...
          const resolvedConfig: ChainbreakerConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/chainbreaker-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "chainbreaker-sbx-browser-old\nchainbreaker-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "chainbreaker-sbx-browser-old") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("chainbreaker-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("chainbreaker-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "chainbreaker-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "chainbreaker-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-chainbreaker.json");
      ...
      cases
        ...
        .map((testCase) => ({
          run: async () => {
            ...
            const configPath = path.join(fixture.stateDir, "chainbreaker.json");
            await fs.writeFile(configPath, "{}\n", "utf-8");
            if (!isWindows) {
              await fs.chmod(configPath, 0o600);
            }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): ChainbreakerConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: ChainbreakerConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<ChainbreakerConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          CHAINBREAKER_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies ChainbreakerConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
      ...
      },
      {
        ...
        cfg: { hooks: requestSessionKeyHooks } satisfies ChainbreakerConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.chainbreaker",
      configPath: "/Users/test/Dropbox/.chainbreaker/chainbreaker.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: ChainbreakerConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@chainbreaker/test-hooks@1.2.3",
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@chainbreaker/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@chainbreaker/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
    ...
    const cases = [
      {
        ...
        cfg: {} satisfies ChainbreakerConfig,
        assert: (res: SecurityAuditReport) => {
          expect(res.findings).toEqual(
            expect.arrayContaining([
              expect.objectContaining({
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            ...
            JSON.stringify({
              name: "escape-plugin",
              chainbreaker: { extensions: ["../outside.js"] },
            }),
          );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.CHAINBREAKER_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.CHAINBREAKER_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };

...
import type { ChainbreakerConfig, ConfigFileSnapshot } from "../config/config.js";
...
async function listSandboxBrowserContainers(
  ...
  try {
    const result = await execDockerRawFn(
      ["ps", "-a", "--filter", "label=chainbreaker.sandboxBrowser=1", "--format", "{{.Names}}"],
      { allowFailure: true },
    );
    if (result.code !== 0) {
      return null;
  ...
  try {
    const result = await params.execDockerRawFn(
      [
        "inspect",
        "-f",
        '{{ index .Config.Labels "chainbreaker.configHash" }}\t{{ index .Config.Labels "chainbreaker.browserConfigEpoch" }}',
...
export async function collectSandboxBrowserHashLabelFindings(params?: {
  ...
  if (missingHash.length > 0) {
    findings.push({
      ...
      remediation: `${formatCliCommand("chainbreaker sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected chainbreaker.browserConfigEpoch=${SANDBOX_BROWSER_SECURITY_HASH_EPOCH}.`,
      remediation: `${formatCliCommand("chainbreaker sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (nonLoopbackPublished.length > 0) {
    findings.push({
      ...
      remediation:
        `${formatCliCommand("chainbreaker sandbox recreate --browser --all")} (add --force to skip prompt), ` +
  ...
  if (pluginDirs.length > 0) {
    ...
    if (enabledExtensionPluginIds.length > 0) {
      ...
      for (const context of contexts) {
        ...
        const broadPolicy = isToolAllowedByPolicies("__chainbreaker_plugin_probe__", policies);
  ...
  if (npmPluginInstalls.length > 0) {
    ...
    if (pluginVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `chainbreaker plugins update --all` (or reinstall affected plugins) to refresh install metadata.",
      });
    }
  }
  ...
  if (npmHookInstalls.length > 0) {
    ...
    if (hookVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `chainbreaker hooks update --all` (or reinstall affected hooks) to refresh install metadata.",
      });
    }
  }
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `chainbreaker security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all chainbreaker.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Chainbreaker extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (resolveSkillSource(entry.skill) === "chainbreaker-bundled") {
        continue;
      }
      ...
      const summary = await getCodeSafetySummary({
        ...
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `chainbreaker security audit --deep`.",
        });
        return null;
      });
      if (!summary) {

...
import type { ConfigFileSnapshot, ChainbreakerConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: ChainbreakerConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Chainbreaker state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Chainbreaker state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.CHAINBREAKER_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.CHAINBREAKER_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("chainbreaker security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.CHAINBREAKER_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.CHAINBREAKER_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: ChainbreakerConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: ChainbreakerConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: ChainbreakerConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("chainbreaker status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("chainbreaker security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set CHAINBREAKER_GATEWAY_TOKEN/CHAINBREAKER_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("chainbreaker security audit --deep")}".`,
    });
  }

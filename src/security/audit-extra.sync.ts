...
function summarizeGroupPolicy(cfg: ChainbreakerConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: ChainbreakerConfig): boolean {
...
function collectModels(cfg: ChainbreakerConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: ChainbreakerConfig): Set<string> {
  const baseCfg: ChainbreakerConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: ChainbreakerConfig, env: NodeJS.ProcessEnv): boolean {
  return hasBundledWebSearchCredential({ config: cfg, env });
}

function isWebSearchEnabled(cfg: ChainbreakerConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ChainbreakerConfig): boolean {
...
function isBrowserEnabled(cfg: ChainbreakerConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ChainbreakerConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: ChainbreakerConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: ChainbreakerConfig): {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep CHAINBREAKER_STATE_DIR on a local-only volume and re-run "${formatCliCommand("chainbreaker security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: ChainbreakerConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer CHAINBREAKER_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const chainbreakerGatewayToken =
    typeof env.CHAINBREAKER_GATEWAY_TOKEN === "string" && env.CHAINBREAKER_GATEWAY_TOKEN.trim()
      ? env.CHAINBREAKER_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-chainbreaker-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: ChainbreakerConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: ChainbreakerConfig): SecurityAuditFinding[] {
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: chainbreaker-sandbox-browser), " +
...
export function collectModelHygieneFindings(cfg: ChainbreakerConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: ChainbreakerConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];

...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "CHAINBREAKER_ENV_TEST_A";
    const keyB = "CHAINBREAKER_ENV_TEST_B";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "CHAINBREAKER_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "CHAINBREAKER_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "CHAINBREAKER_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "CHAINBREAKER_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "CHAINBREAKER_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "CHAINBREAKER_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "chainbreaker-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousChainbreakerHome = process.env.CHAINBREAKER_HOME;
    const previousStateDir = process.env.CHAINBREAKER_STATE_DIR;
    const previousBundledDir = process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
    process.env.CHAINBREAKER_HOME = "/srv/chainbreaker-home";
    process.env.CHAINBREAKER_STATE_DIR = "/srv/chainbreaker-state";
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = "/srv/chainbreaker-bundled";
    ...
    try {
      ...
      expect(env.CHAINBREAKER_HOME).toBeUndefined();
      expect(env.CHAINBREAKER_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.CHAINBREAKER_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("CHAINBREAKER_HOME", previousChainbreakerHome);
      restoreEnvKey("CHAINBREAKER_STATE_DIR", previousStateDir);
      restoreEnvKey("CHAINBREAKER_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { CHAINBREAKER_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processChainbreakerHome: process.env.CHAINBREAKER_HOME,
          processBundledDir: process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.CHAINBREAKER_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processChainbreakerHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.CHAINBREAKER_HOME).toBe("/srv/chainbreaker-home");
    } finally {
      restoreEnvKey("CHAINBREAKER_HOME", previousChainbreakerHome);
    }
  });

...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("chainbreaker-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.CHAINBREAKER_STATE_DIR).toBe(path.join(tempHome.home, ".chainbreaker"));
    ...
    expect(process.env.CHAINBREAKER_STATE_DIR).toBe(previousStateDir);

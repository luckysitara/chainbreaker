...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when CHAINBREAKER_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".chainbreaker", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when CHAINBREAKER_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", CHAINBREAKER_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".chainbreaker-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers CHAINBREAKER_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        CHAINBREAKER_PROFILE: "rescue",
        CHAINBREAKER_STATE_DIR: "C:\\State\\chainbreaker",
      },
      expected: path.join("C:\\State\\chainbreaker", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", CHAINBREAKER_PROFILE: "default" },
      expected: path.join("/home/test", ".chainbreaker", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Chainbreaker Gateway",
          "cd /d C:\\Projects\\chainbreaker",
          "set NODE_ENV=production",
          "set CHAINBREAKER_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\chainbreaker",
          environment: {
            NODE_ENV: "production",
            CHAINBREAKER_PORT: "18789",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\chainbreaker\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\chainbreaker\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Chainbreaker Share\\node.exe" "\\\\fileserver\\Chainbreaker Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Chainbreaker Share\\node.exe",
            "\\\\fileserver\\Chainbreaker Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from CHAINBREAKER_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ CHAINBREAKER_STATE_DIR: path.join(tmpDir, "custom-state") }),

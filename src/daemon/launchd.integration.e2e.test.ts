...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `chainbreaker-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      CHAINBREAKER_LAUNCHD_LABEL: `ai.chainbreaker.launchd-int-${testId}`,
      CHAINBREAKER_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });

...
const TEST_MANAGED_HOME = "/tmp/chainbreaker-test-home";
const GATEWAY_SERVICE = "chainbreaker-gateway.service";
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/chainbreaker-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "chainbreaker-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "chainbreaker-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/chainbreaker-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled chainbreaker-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled chainbreaker-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when CHAINBREAKER_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/chainbreaker-gateway.service",
    },
    {
      name: "uses profile-specific service name when CHAINBREAKER_PROFILE is set to a custom value",
      env: { HOME: "/home/test", CHAINBREAKER_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/chainbreaker-gateway-jbphoenix.service",
    },
    {
      name: "prefers CHAINBREAKER_SYSTEMD_UNIT over CHAINBREAKER_PROFILE",
      env: {
        HOME: "/home/test",
        CHAINBREAKER_PROFILE: "jbphoenix",
        CHAINBREAKER_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles CHAINBREAKER_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        CHAINBREAKER_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from CHAINBREAKER_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        CHAINBREAKER_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('chainbreaker --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["chainbreaker", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('chainbreaker --path "C:\\\\Program Files\\\\Chainbreaker"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["chainbreaker", "--path", "C:\\\\Program Files\\\\Chainbreaker"]);

    expect(
      splitArgsPreservingQuotes('chainbreaker --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["chainbreaker", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/chainbreaker gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads CHAINBREAKER_GATEWAY_TOKEN from EnvironmentFile", async () => {
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.CHAINBREAKER_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.CHAINBREAKER_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.CHAINBREAKER_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.chainbreaker/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/chainbreaker-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/chainbreaker gateway run",
          'EnvironmentFile=%h/.chainbreaker/first.env "%h/.chainbreaker/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.chainbreaker/first.env") {
        return "CHAINBREAKER_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.chainbreaker/second env.env") {
        return 'CHAINBREAKER_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      CHAINBREAKER_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "CHAINBREAKER_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "CHAINBREAKER_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "CHAINBREAKER_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      CHAINBREAKER_GATEWAY_TOKEN: "override-token",
      CHAINBREAKER_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/chainbreaker-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/chainbreaker gateway run",
          "EnvironmentFile=%h/.chainbreaker/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.chainbreaker/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'CHAINBREAKER_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "CHAINBREAKER_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      CHAINBREAKER_GATEWAY_TOKEN: "quoted token",
      CHAINBREAKER_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      CHAINBREAKER_GATEWAY_TOKEN: "file",
      CHAINBREAKER_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "chainbreaker-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ CHAINBREAKER_PROFILE: "work" });
  });

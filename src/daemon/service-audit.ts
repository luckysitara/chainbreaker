...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds CHAINBREAKER_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `chainbreaker gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service CHAINBREAKER_GATEWAY_TOKEN does not match gateway.auth.token in chainbreaker.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.CHAINBREAKER_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.CHAINBREAKER_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `chainbreaker gateway install --force` to sync the token.",
      level: "recommended",
    };
  }

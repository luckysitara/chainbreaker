...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.CHAINBREAKER_GATEWAY_PORT).toBe("18789");
    expect(env.CHAINBREAKER_GATEWAY_TOKEN).toBeUndefined();
    expect(env.CHAINBREAKER_SERVICE_MARKER).toBe("chainbreaker");
    expect(env.CHAINBREAKER_SERVICE_KIND).toBe("gateway");
    expect(typeof env.CHAINBREAKER_SERVICE_VERSION).toBe("string");
    expect(env.CHAINBREAKER_SYSTEMD_UNIT).toBe("chainbreaker-gateway.service");
    expect(env.CHAINBREAKER_WINDOWS_TASK_NAME).toBe("Chainbreaker Gateway");
    if (process.platform === "darwin") {
      expect(env.CHAINBREAKER_LAUNCHD_LABEL).toBe("ai.chainbreaker.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", CHAINBREAKER_PROFILE: "work" },
      port: 18789,
    });
    expect(env.CHAINBREAKER_SYSTEMD_UNIT).toBe("chainbreaker-gateway-work.service");
    expect(env.CHAINBREAKER_WINDOWS_TASK_NAME).toBe("Chainbreaker Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.CHAINBREAKER_LAUNCHD_LABEL).toBe("ai.chainbreaker.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through CHAINBREAKER_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CHAINBREAKER_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.CHAINBREAKER_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits CHAINBREAKER_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".chainbreaker"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", CHAINBREAKER_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".chainbreaker-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", CHAINBREAKER_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".chainbreaker"));
  });

  it("uses CHAINBREAKER_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", CHAINBREAKER_STATE_DIR: "/var/lib/chainbreaker" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/chainbreaker"));
  });

  it("expands ~ in CHAINBREAKER_STATE_DIR", () => {
    const env = { HOME: "/Users/test", CHAINBREAKER_STATE_DIR: "~/chainbreaker-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/chainbreaker-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { CHAINBREAKER_STATE_DIR: "C:\\State\\chainbreaker" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\chainbreaker");
  });
});

...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.CHAINBREAKER_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CHAINBREAKER_PROFILE: profile,
    CHAINBREAKER_GATEWAY_PORT: String(port),
    CHAINBREAKER_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CHAINBREAKER_SYSTEMD_UNIT: systemdUnit,
    CHAINBREAKER_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    CHAINBREAKER_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CHAINBREAKER_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.CHAINBREAKER_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CHAINBREAKER_GATEWAY_TOKEN: gatewayToken,
    CHAINBREAKER_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CHAINBREAKER_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CHAINBREAKER_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CHAINBREAKER_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CHAINBREAKER_LOG_PREFIX: "node",
    CHAINBREAKER_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CHAINBREAKER_SERVICE_KIND: NODE_SERVICE_KIND,
    CHAINBREAKER_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    CHAINBREAKER_STATE_DIR: sharedEnv.stateDir,
    CHAINBREAKER_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.CHAINBREAKER_STATE_DIR;
  const configPath = env.CHAINBREAKER_CONFIG_PATH;

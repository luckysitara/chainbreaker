...
// Real content from the chainbreaker-gateway.service unit file (the canonical gateway unit).
...
Restart=always
Environment=CHAINBREAKER_SERVICE_MARKER=chainbreaker
Environment=CHAINBREAKER_SERVICE_KIND=gateway
Environment=CHAINBREAKER_SERVICE_VERSION=2026.3.8

[Install]
WantedBy=default.target
`;

// Real content from the chainbreaker-test.service unit file (a non-gateway chainbreaker service).
const TEST_SERVICE_CONTENTS = `\
[Unit]
Description=Chainbreaker test service
...
describe("detectMarkerLineWithGateway", () => {
  ...
  it("returns chainbreaker for the canonical gateway unit (ExecStart has both chainbreaker and gateway)", () => {
    expect(detectMarkerLineWithGateway(GATEWAY_SERVICE_CONTENTS)).toBe("chainbreaker");
  });
  ...
  it.skipIf(!isLinux)(
    ...
    async () => {
      ...
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "chainbreaker-gateway.service"),
...
describe("findExtraGatewayServices (win32)", () => {
  ...
  it("collects only non-chainbreaker marker tasks from schtasks output", async () => {
    execSchtasksMock.mockResolvedValueOnce({
      code: 0,
      stdout: [
        "TaskName: Chainbreaker Gateway",
        "Task To Run: C:\\Program Files\\Chainbreaker\\chainbreaker.exe gateway run",
        "",

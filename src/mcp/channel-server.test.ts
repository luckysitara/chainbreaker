...
import { z } from "zod";
import { createChainbreakerChannelMcpServer, ChainbreakerChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createChainbreakerChannelMcpServer({
...
describe("chainbreaker channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new ChainbreakerChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });

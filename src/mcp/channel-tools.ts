...
import { z } from "zod";
import type { ChainbreakerChannelBridge } from "./channel-bridge.js";
import {
...
export function registerChannelMcpTools(
  ...
  server.tool(
    "conversation_get",
    "Get one Chainbreaker conversation by session key.",
  ...
  server.tool(
    "messages_read",
    "Read recent messages for one Chainbreaker conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "attachments_fetch",
    "List non-text attachments for a message in one Chainbreaker conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "events_poll",
    "Poll queued Chainbreaker conversation events since a cursor.",
    {
  ...
  server.tool(
    "events_wait",
    "Wait for the next queued Chainbreaker conversation event.",
    {
      after_cursor: z.number().int().min(0).optional(),
      session_key: z.string().optional(),
  ...
  server.tool(
    "messages_send",
    "Send a message back through the same Chainbreaker conversation route.",
    {
      session_key: z.string().min(1),
      text: z.string().min(1),
    },
  ...
  server.tool(
    ...
    "List open Chainbreaker exec or plugin approval requests visible through the Gateway.",
    {},
    async () => {
  ...
  server.tool(
    "permissions_respond",
    "Allow or deny one pending Chainbreaker exec or plugin approval request.",
    {

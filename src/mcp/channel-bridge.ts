...
export class ChainbreakerChannelBridge {
  ...
  async start(): Promise<void> {
    ...
    const gatewayUrlOverrideSource =
      connection.urlSource === "cli --url"
        ? "cli"
        : connection.urlSource === "env CHAINBREAKER_GATEWAY_URL"
          ? "env"
          : undefined;
    ...
    this.gateway = new GatewayClient({
      ...
      clientDisplayName: "Chainbreaker MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`chainbreaker mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `chainbreaker mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }

...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevChainbreakerHome = process.env.CHAINBREAKER_HOME;
    const prevHome = process.env.HOME;
    process.env.CHAINBREAKER_HOME = "/srv/chainbreaker-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/chainbreaker-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevChainbreakerHome === undefined) {
        delete process.env.CHAINBREAKER_HOME;
      } else {
        process.env.CHAINBREAKER_HOME = prevChainbreakerHome;
      }

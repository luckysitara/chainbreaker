...
export function triggerChainbreakerRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.CHAINBREAKER_SYSTEMD_UNIT,
      process.env.CHAINBREAKER_PROFILE,
    );
  ...
  const label =
    process.env.CHAINBREAKER_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CHAINBREAKER_PROFILE);

...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.CHAINBREAKER_CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_AUTH_TOKEN;
    delete process.env.CHAINBREAKER_CLAWHUB_CONFIG_PATH;
  ...
  it("checks min gateway versions with loose host labels", () => {
    ...
    expect(satisfiesGatewayMinimum("Chainbreaker 2026.3.22", "2026.3.0")).toBe(true);
  ...
  it("resolves ClawHub auth token from config.json", async () => {
    const configRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-clawhub-config-"));
    const configPath = path.join(configRoot, "clawhub", "config.json");
    process.env.CHAINBREAKER_CLAWHUB_CONFIG_PATH = configPath;
  ...
  it("resolves ClawHub auth token from the legacy config path override", async () => {
    const configRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-clawdhub-config-"));
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-clawhub-home-"));
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-clawhub-home-"));
      const xdgRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-clawhub-xdg-"));
  ...
  it("injects resolved auth token into ClawHub requests", async () => {
    process.env.CHAINBREAKER_CLAWHUB_TOKEN = "env-token-123";

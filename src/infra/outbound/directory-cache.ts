...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ChainbreakerConfig | null = null;
  ...
  get(key: string, cfg: ChainbreakerConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ChainbreakerConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: ChainbreakerConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: ChainbreakerConfig): void {

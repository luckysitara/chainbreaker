...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".chainbreaker");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".chainbreaker", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      CHAINBREAKER_STATE_DIR: path.join(home, ".chainbreaker"),
      ...env,
    };

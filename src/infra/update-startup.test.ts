...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["CHAINBREAKER_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.CHAINBREAKER_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveChainbreakerPackageRoot } = await import("./chainbreaker-root.js"));
    ...
    vi.mocked(resolveChainbreakerPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveChainbreakerPackageRoot).mockResolvedValue("/opt/chainbreaker");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/chainbreaker",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/chainbreaker/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        process.execPath,
        "/opt/chainbreaker/dist/entry.js",
        "update",
        "--yes",
        "--channel",
        "beta",

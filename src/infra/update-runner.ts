...
const DEFAULT_PACKAGE_NAME = "chainbreaker";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["chainbreaker", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    const doctorEntry = path.join(gitRoot, "chainbreaker.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "chainbreaker doctor entry",
    ...
    const doctorStep = await runStep(
      step("chainbreaker doctor", doctorArgv, gitRoot, { CHAINBREAKER_UPDATE_IN_PROGRESS: "1" }),
    );

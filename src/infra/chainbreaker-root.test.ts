...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__chainbreaker_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "chainbreaker-root");
...
function setPackageRoot(root: string, name = "chainbreaker") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./chainbreaker-root.js").resolveChainbreakerPackageRootSync,
  asyncResolver: typeof import("./chainbreaker-root.js").resolveChainbreakerPackageRoot,
  opts: Parameters<typeof import("./chainbreaker-root.js").resolveChainbreakerPackageRootSync>[0],
...
describe("resolveChainbreakerPackageRoot", () => {
  let resolveChainbreakerPackageRoot: typeof import("./chainbreaker-root.js").resolveChainbreakerPackageRoot;
  let resolveChainbreakerPackageRootSync: typeof import("./chainbreaker-root.js").resolveChainbreakerPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveChainbreakerPackageRoot, resolveChainbreakerPackageRootSync } =
      await import("./chainbreaker-root.js"));
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "chainbreaker");
        const pkgRoot = path.join(project, "node_modules", "chainbreaker");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "chainbreaker");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "chainbreaker.mjs")));
    ...
    {
      name: "falls through from a non-chainbreaker moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-chainbreaker");
    ...
    {
      name: "returns null for non-chainbreaker package roots",
      setup: () => {
        const pkgRoot = fx("not-chainbreaker");
        setPackageRoot(pkgRoot, "not-chainbreaker");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "chainbreaker.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "chainbreaker");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };

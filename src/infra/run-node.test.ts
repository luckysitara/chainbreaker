...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "chainbreaker.plugin.json");
const EXTENSION_PACKAGE = bundledPluginFile("demo", "package.json");
const EXTENSION_README = bundledPluginFile("demo", "README.md");
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "chainbreaker.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"chainbreaker-test"}\n',
  [DIST_ENTRY]: "console.log('built');\n",
  [BUILD_STAMP]: '{"head":"abc123"}\n',
} as const;

async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-run-node-"));
  try {
...
function statusCommandSpawn() {
  return [process.execPath, "chainbreaker.mjs", "status"];
}
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "chainbreaker.mjs", "--version"],
        ]);
      });
    },
  );
  ...
  it("copies bundled plugin metadata after rebuilding from a clean dist", async () => {
    await withTempDir(async (tmp) => {
      ...
      const exitCode = await runStatusCommand({
        tmp,
        spawn,
        env: { CHAINBREAKER_FORCE_BUILD: "1" },
      });

...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.CHAINBREAKER_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.CHAINBREAKER_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const keyPath = normalizeNonEmptyString(env.CHAINBREAKER_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set CHAINBREAKER_APNS_PRIVATE_KEY_P8 or CHAINBREAKER_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading CHAINBREAKER_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}

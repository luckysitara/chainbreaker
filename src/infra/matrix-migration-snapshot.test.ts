...
describe("matrix migration snapshots", () => {
  it("creates a backup marker after writing a pre-migration snapshot", async () => {
    await withTempHome(async (home) => {
      fs.writeFileSync(path.join(home, ".chainbreaker", "chainbreaker.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(home, ".chainbreaker", "state.txt"), "state\n", "utf8");
  ...
  it("reuses an existing snapshot marker when the archive still exists", async () => {
    await withTempHome(async (home) => {
      const archivePath = path.join(home, "Backups", "chainbreaker-migrations", "snapshot.tar.gz");
  ...
  it("recreates the snapshot when the marker exists but the archive is missing", async () => {
    await withTempHome(async (home) => {
      const markerPath = resolveMatrixMigrationSnapshotMarkerPath(process.env);
      fs.mkdirSync(path.dirname(markerPath), { recursive: true });
      fs.mkdirSync(path.join(home, "Backups", "chainbreaker-migrations"), { recursive: true });
      fs.writeFileSync(
        markerPath,
        JSON.stringify({
          version: 1,
          createdAt: "2026-03-10T18:00:00.000Z",
          archivePath: path.join(home, "Backups", "chainbreaker-migrations", "missing.tar.gz"),
          trigger: "older-run",
          includeWorkspace: false,
        }),
        "utf8",
      ...
      expect(result.archivePath).not.toBe(
        path.join(home, "Backups", "chainbreaker-migrations", "missing.tar.gz"),
      );
      expect(
        result.archivePath.startsWith(resolveMatrixMigrationSnapshotOutputDir(process.env)),
      ).toBe(true);
    ...
    await withTempHome(
      async (home) => {
        ...
        expect(detection.warnings).toContain(
          "Legacy Matrix encrypted state was detected, but the Matrix plugin helper is unavailable. Install or repair @chainbreaker/matrix so Chainbreaker can inspect the old rust crypto store before upgrading.",
        );
        expect(
          hasActionableMatrixMigration({
            cfg,
      ...
      {
        env: {
          CHAINBREAKER_BUNDLED_PLUGINS_DIR: (home) => path.join(home, "empty-bundled"),
        },
      },
    );
  });

...
const PRIMARY_PACKAGE_NAME = "chainbreaker";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const CHAINBREAKER_MAIN_PACKAGE_SPEC = "github:chainbreaker/chainbreaker#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Chainbreaker", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.CHAINBREAKER_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.CHAINBREAKER_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return CHAINBREAKER_MAIN_PACKAGE_SPEC;
  }

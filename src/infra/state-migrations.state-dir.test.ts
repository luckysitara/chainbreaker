...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "chainbreaker-state-dir-"));
  tempRoot = root;
  return root;
}
...
describe("legacy state dir auto-migration", () => {
  ...
  it("skips state-dir migration when CHAINBREAKER_STATE_DIR is explicitly set", async () => {
    const root = await makeTempRoot();
    const legacyDir = path.join(root, ".clawdbot");
    fs.mkdirSync(legacyDir, { recursive: true });

    const result = await autoMigrateLegacyStateDir({
      env: { CHAINBREAKER_STATE_DIR: path.join(root, "custom-state") } as NodeJS.ProcessEnv,
      homedir: () => root,
    });

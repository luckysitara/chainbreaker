...
const GATEWAY_DEVICE_ID_HEADER = "x-chainbreaker-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-chainbreaker-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-chainbreaker-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.CHAINBREAKER_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "CHAINBREAKER_APNS_RELAY_BASE_URL"
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.CHAINBREAKER_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require CHAINBREAKER_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.CHAINBREAKER_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {

...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-push-apns-auth-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("push APNs auth and helper coverage", () => {
  ...
  it("falls back to CHAINBREAKER_APNS_PRIVATE_KEY when CHAINBREAKER_APNS_PRIVATE_KEY_P8 is blank", async () => {
    const resolved = await resolveApnsAuthConfigFromEnv({
      CHAINBREAKER_APNS_TEAM_ID: "TEAM123",
      CHAINBREAKER_APNS_KEY_ID: "KEY123",
      CHAINBREAKER_APNS_PRIVATE_KEY_P8: "   ",
  ...
  it("reads APNs private keys from CHAINBREAKER_APNS_PRIVATE_KEY_PATH", async () => {
    ...
    const resolved = await resolveApnsAuthConfigFromEnv({
      CHAINBREAKER_APNS_TEAM_ID: "TEAM123",
      CHAINBREAKER_APNS_KEY_ID: "KEY123",
      CHAINBREAKER_APNS_PRIVATE_KEY_PATH: keyPath,
    } as NodeJS.ProcessEnv);
  ...
  it("reports missing auth fields and path read failures", async () => {
    ...
    await expect(resolveApnsAuthConfigFromEnv({} as NodeJS.ProcessEnv)).resolves.toEqual({
      ok: false,
      error: "APNs auth missing: set CHAINBREAKER_APNS_TEAM_ID and CHAINBREAKER_APNS_KEY_ID",
    });

    const missingKey = await resolveApnsAuthConfigFromEnv({
      CHAINBREAKER_APNS_TEAM_ID: "TEAM123",
      CHAINBREAKER_APNS_KEY_ID: "KEY123",
      CHAINBREAKER_APNS_PRIVATE_KEY_PATH: missingPath,
    } as NodeJS.ProcessEnv);

    expect(missingKey.ok).toBe(false);
    if (!missingKey.ok) {
      expect(missingKey.error).toContain(
        `failed reading CHAINBREAKER_APNS_PRIVATE_KEY_PATH (${missingPath})`,
      );
    }
  });

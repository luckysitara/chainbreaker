...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-heartbeat-suite-"));
});
...
describe("resolveHeartbeatPrompt", () => {
  it.each([
    { name: "default prompt", cfg: {} as ChainbreakerConfig, expected: HEARTBEAT_PROMPT },
    {
...
describe("runHeartbeatOnce", () => {
  ...
  it.each([
    {
      ...
      message: "Group alert",
      applyOverride: ({ cfg, sessionKey }: { cfg: ChainbreakerConfig; sessionKey: string }) => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("chainbreaker-hb");

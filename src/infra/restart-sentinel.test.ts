...
describe("restart sentinel", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["CHAINBREAKER_STATE_DIR"]);
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-sentinel-"));
  ...
  it("formats summary, distinct reason, and doctor hint together", () => {
    const payload = {
      ...
      doctorHint: "Run chainbreaker doctor",

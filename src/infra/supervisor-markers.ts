const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "CHAINBREAKER_LAUNCHD_LABEL"],
  systemd: ["CHAINBREAKER_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["CHAINBREAKER_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "CHAINBREAKER_SERVICE_MARKER",
  "CHAINBREAKER_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.CHAINBREAKER_SERVICE_MARKER?.trim();
    const serviceKind = env.CHAINBREAKER_SERVICE_KIND?.trim();

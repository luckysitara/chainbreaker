...
const TEMP_DIR_PREFIX = "chainbreaker-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("chainbreaker-install-source-utils-");
  const archivePath = path.join(cwd, "chainbreaker-plugin-1.2.3.tgz");
  ...
  const result = await packNpmSpecToArchive({
    spec: "chainbreaker-plugin@1.2.3",
    timeoutMs: 5000,
    cwd,
  });
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("chainbreaker-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-chainbreaker-archive.tgz",
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "chainbreaker-plugin@1.2.3",
          name: "chainbreaker-plugin",
          version: "1.2.3",
          filename: "chainbreaker-plugin-1.2.3.tgz",
    ...
    const result = await runPack("chainbreaker-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "chainbreaker-plugin",
        version: "1.2.3",
        resolvedSpec: "chainbreaker-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "chainbreaker-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"chainbreaker-plugin@1.2.3","name":"chainbreaker-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "chainbreaker-plugin",
        version: "1.2.3",
        resolvedSpec: "chainbreaker-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    const result = await runPack("@chainbreaker/whatsapp", cwd);
    expectPackError(result, [
      "Package not found on npm",
      "@chainbreaker/whatsapp",
      "docs.chainbreaker.ai/tools/plugin",
    ]);
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("chainbreaker-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "chainbreaker-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@chainbreaker/plugin-demo@2.0.0",
            filename: "chainbreaker-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@chainbreaker/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "chainbreaker-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@chainbreaker/plugin-demo@2.0.0",
      },
    });
  });

...
const CORE_PACKAGE_NAMES = new Set(["chainbreaker"]);
...
export async function resolveChainbreakerPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveChainbreakerPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {

...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CHAINBREAKER_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Chainbreaker";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Chainbreaker\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.CHAINBREAKER_MDNS_HOSTNAME?.trim() || "chainbreaker";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]

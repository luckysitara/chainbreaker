...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __chainbreakerDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__chainbreakerDiagnosticEventsState) {
    globalStore.__chainbreakerDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__chainbreakerDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: ChainbreakerConfig): boolean {

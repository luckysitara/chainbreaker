...
import { ensureChainbreakerCliOnPath } from "./path-env.js";
...
describe("ensureChainbreakerCliOnPath", () => {
  const envKeys = [
    "PATH",
    "CHAINBREAKER_PATH_BOOTSTRAPPED",
    "CHAINBREAKER_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/chainbreaker-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "chainbreaker");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureChainbreakerCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.CHAINBREAKER_PATH_BOOTSTRAPPED;
    delete process.env.CHAINBREAKER_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling chainbreaker exists", () => {
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "chainbreaker");
      ...
      if (envValue === undefined) {
        ...
        process.env.CHAINBREAKER_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/chainbreaker-path/case-linuxbrew");

  ...
  it("includes the cross-platform supervisor hint env vars", () => {
    expect(SUPERVISOR_HINT_ENV_VARS).toEqual(
      expect.arrayContaining([
        "LAUNCH_JOB_LABEL",
        "INVOCATION_ID",
        "CHAINBREAKER_WINDOWS_TASK_NAME",
...
describe("detectRespawnSupervisor", () => {
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ CHAINBREAKER_WINDOWS_TASK_NAME: "Chainbreaker Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
    ...
    expect(
      detectRespawnSupervisor(
        {
          CHAINBREAKER_SERVICE_MARKER: "chainbreaker",
          CHAINBREAKER_SERVICE_KIND: "worker",
        },
        "win32",
      ),
    ).toBeNull();
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.chainbreaker.gateway" }, "freebsd"),
    ).toBeNull();
  });
});

...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers CHAINBREAKER_HOME over HOME and USERPROFILE",
      env: {
      ...
      homedir: () => "/fallback",
      expected: "/srv/chainbreaker-home",
    },
    {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for CHAINBREAKER_HOME",
      env: { CHAINBREAKER_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when CHAINBREAKER_HOME is tilde-only and no fallback home exists",
      env: { CHAINBREAKER_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores CHAINBREAKER_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          CHAINBREAKER_HOME: "/srv/chainbreaker-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { CHAINBREAKER_HOME: "/srv/chainbreaker-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/chainbreaker-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/chainbreaker-home " },
      expected: "/srv/chainbreaker-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/chainbreaker-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of CHAINBREAKER_HOME", () => {
    expect(

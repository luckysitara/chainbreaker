...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/chainbreaker-state", "devices")).toEqual({
      dir: path.join("/tmp/chainbreaker-state", "devices"),
      pendingPath: path.join("/tmp/chainbreaker-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/chainbreaker-state", "devices", "paired.json"),
    });
  });

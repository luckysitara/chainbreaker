...
export const POSIX_CHAINBREAKER_TMP_DIR = "/tmp/chainbreaker";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredChainbreakerTmpDirOptions = {
...
export function resolvePreferredChainbreakerTmpDir(
  options: ResolvePreferredChainbreakerTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "chainbreaker" : `chainbreaker-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[chainbreaker] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Chainbreaker temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Chainbreaker temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_CHAINBREAKER_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_CHAINBREAKER_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_CHAINBREAKER_TMP_DIR)) {
      return POSIX_CHAINBREAKER_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_CHAINBREAKER_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_CHAINBREAKER_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_CHAINBREAKER_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_CHAINBREAKER_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_CHAINBREAKER_TMP_DIR;

...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Chainbreaker config file was found to back up."
        : "No local Chainbreaker state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-backup-"));

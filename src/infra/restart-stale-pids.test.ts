...
function createChainbreakerBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "chainbreaker-gateway" }]),
    ...overrides,
  });
}
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses chainbreaker-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "chainbreaker-gateway" },
          { pid: process.pid, cmd: "chainbreaker-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'chainbreaker'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple chainbreaker pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "chainbreaker-gateway" },
          { pid: pid2, cmd: "chainbreaker-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-chainbreaker pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks chainbreaker processes — non-chainbreaker listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty chainbreaker stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty chainbreaker stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + chainbreaker pid in stdout — container-restricted lsof reports partial results
          return createChainbreakerBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "chainbreaker-gateway" }]),
          stderr: "",
        };
      });
  ...
  // -------------------------------------------------------------------------
    it("skips a mid-loop entry when the command does not include 'chainbreaker'", () => {
      ...
      // be pushed, but the following chainbreaker entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-chainbreaker entry first, then chainbreaker entry
  ...
  // -------------------------------------------------------------------------
    it("treats status 1 + non-chainbreaker stdout as port-free (not an chainbreaker process)", () => {
      ...
      // (we only kill chainbreaker pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-chainbreaker output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no chainbreaker pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();

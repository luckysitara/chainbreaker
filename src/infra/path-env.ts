...
type EnsureChainbreakerPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureChainbreakerPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `chainbreaker` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "chainbreaker");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CHAINBREAKER_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "chainbreaker"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `chainbreaker`
...
/**
 * Best-effort PATH bootstrap so skills that require the `chainbreaker` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureChainbreakerCliOnPath(opts: EnsureChainbreakerPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CHAINBREAKER_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.CHAINBREAKER_PATH_BOOTSTRAPPED = "1";

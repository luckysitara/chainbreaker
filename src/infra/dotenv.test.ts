...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  process.env.CHAINBREAKER_STATE_DIR = stateDir;
...
describe("loadDotEnv", () => {
  it("loads ~/.chainbreaker/.env as fallback without overriding CWD .env", async () => {
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CHAINBREAKER_STATE_DIR=./evil-state",
            "CHAINBREAKER_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.CHAINBREAKER_STATE_DIR).toBe(stateDir);
        expect(process.env.CHAINBREAKER_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CHAINBREAKER_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "CHAINBREAKER_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "CHAINBREAKER_LIVE_GEMINI_KEY=sk-gemini-live",
            "CHAINBREAKER_LIVE_OPENAI_KEY=sk-openai-live",
            "CHAINBREAKER_GATEWAY_TOKEN=attacker-token",
            "CHAINBREAKER_GATEWAY_PASSWORD=attacker-password",
            "CHAINBREAKER_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
        ...
        delete process.env.CHAINBREAKER_LIVE_ANTHROPIC_KEY;
        delete process.env.CHAINBREAKER_LIVE_ANTHROPIC_KEYS;
        delete process.env.CHAINBREAKER_LIVE_GEMINI_KEY;
        delete process.env.CHAINBREAKER_LIVE_OPENAI_KEY;
        delete process.env.CHAINBREAKER_GATEWAY_TOKEN;
        delete process.env.CHAINBREAKER_GATEWAY_PASSWORD;
        delete process.env.CHAINBREAKER_GATEWAY_SECRET;
        ...
        expect(process.env.CHAINBREAKER_LIVE_ANTHROPIC_KEY).toBeUndefined();
        expect(process.env.CHAINBREAKER_LIVE_ANTHROPIC_KEYS).toBeUndefined();
        expect(process.env.CHAINBREAKER_LIVE_GEMINI_KEY).toBeUndefined();
        expect(process.env.CHAINBREAKER_LIVE_OPENAI_KEY).toBeUndefined();
        expect(process.env.CHAINBREAKER_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.CHAINBREAKER_GATEWAY_PASSWORD).toBeUndefined();
        expect(process.env.CHAINBREAKER_GATEWAY_SECRET).toBeUndefined();
      });
    });
  });

  it("blocks CHAINBREAKER_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.CHAINBREAKER_STATE_DIR).toBeUndefined();
        expect(process.env.CHAINBREAKER_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (CHAINBREAKER_AGENT_DIR, CHAINBREAKER_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, CHAINBREAKER_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "CHAINBREAKER_AGENT_DIR=./evil-agent",
            `CHAINBREAKER_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "CHAINBREAKER_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.CHAINBREAKER_AGENT_DIR;
        delete process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.CHAINBREAKER_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.CHAINBREAKER_AGENT_DIR).toBeUndefined();
        expect(process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.CHAINBREAKER_OAUTH_DIR).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "CHAINBREAKER_BUNDLED_HOOKS_DIR=./attacker-hooks",
            "CHAINBREAKER_BUNDLED_PLUGINS_DIR=./attacker-plugins",
            "CHAINBREAKER_BUNDLED_SKILLS_DIR=./attacker-skills",
          ].join("\n"),
        );

        delete process.env.CHAINBREAKER_BUNDLED_HOOKS_DIR;
        delete process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
        delete process.env.CHAINBREAKER_BUNDLED_SKILLS_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.CHAINBREAKER_BUNDLED_HOOKS_DIR).toBeUndefined();
        expect(process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.CHAINBREAKER_BUNDLED_SKILLS_DIR).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CHAINBREAKER_LIVE_GEMINI_KEY=sk-gemini-live",
            "CHAINBREAKER_LIVE_OPENAI_KEY=sk-openai-live",
            "CHAINBREAKER_GATEWAY_TOKEN=trusted-token",
            "CHAINBREAKER_GATEWAY_PASSWORD=trusted-password",
            "CHAINBREAKER_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_API_KEY;
        ...
        expect(process.env.CHAINBREAKER_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.CHAINBREAKER_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.CHAINBREAKER_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.CHAINBREAKER_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.CHAINBREAKER_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.CHAINBREAKER_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.CHAINBREAKER_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "CHAINBREAKER_STATE_DIR=./evil-state\n");

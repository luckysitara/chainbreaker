...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ChainbreakerConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ChainbreakerConfig) {
...
function resolveHeartbeatAgents(cfg: ChainbreakerConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ChainbreakerConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ChainbreakerConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ChainbreakerConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();

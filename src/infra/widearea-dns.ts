...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.CHAINBREAKER_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*chainbreaker-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "chainbreaker";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "chainbreaker");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "chainbreaker-gw");
  ...
  return `; chainbreaker-content-hash: ${contentHash}\n${contentBody}`;
}

...
vi.mock("./chainbreaker-root.js", () => ({
  resolveChainbreakerPackageRoot: vi.fn(async () => null),
  resolveChainbreakerPackageRootSync: vi.fn(() => null),
}));
...
let chainbreakerRoot: typeof import("./chainbreaker-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    chainbreakerRoot = await import("./chainbreaker-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/chainbreaker");
    const wrapperArgv1 = abs("fixtures/bin/chainbreaker");
  ...
  it("uses resolveChainbreakerPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/chainbreaker");
    (
      chainbreakerRoot.resolveChainbreakerPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/chainbreaker"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "chainbreaker" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "chainbreaker.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-chainbreaker");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/chainbreaker-bundle");
    (
      chainbreakerRoot.resolveChainbreakerPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/Chainbreaker.app/Contents/MacOS/Chainbreaker");
    const bundledUiDir = abs("fixtures/Chainbreaker.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/chainbreaker-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (

...
function listAgents(cfg: ChainbreakerConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: ChainbreakerConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<ChainbreakerConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: ChainbreakerConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: ChainbreakerConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ChainbreakerConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<ChainbreakerConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  ChainbreakerConfig,
  {
    bindingsRef: ChainbreakerConfig["bindings"];
    agentsRef: ChainbreakerConfig["agents"];
    sessionRef: ChainbreakerConfig["session"];
...
function resolveRouteCacheForConfig(cfg: ChainbreakerConfig): Map<string, ResolvedAgentRoute> {

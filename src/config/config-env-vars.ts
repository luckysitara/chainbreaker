...
function collectConfigEnvVarsByTarget(cfg?: ChainbreakerConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: ChainbreakerConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: ChainbreakerConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: ChainbreakerConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.CHAINBREAKER_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth

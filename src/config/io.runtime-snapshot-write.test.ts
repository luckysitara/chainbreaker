...
function createSourceConfig(): ChainbreakerConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): ChainbreakerConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  ...
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("chainbreaker-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("chainbreaker-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: ChainbreakerConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("chainbreaker-config-runtime-write-", async (home) => {
      const configPath = path.join(home, ".chainbreaker", "chainbreaker.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("chainbreaker-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".chainbreaker", "chainbreaker.json");
      const sourceConfig: ChainbreakerConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: ChainbreakerConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("chainbreaker-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".chainbreaker", "chainbreaker.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: ChainbreakerConfig = {
  ...
  it("notifies in-process write listeners with the refreshed runtime snapshot", async () => {
    await withTempHome("chainbreaker-config-runtime-write-listener-", async (home) => {
      const configPath = path.join(home, ".chainbreaker", "chainbreaker.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, `${JSON.stringify({ gateway: { port: 18789 } }, null, 2)}\n`);

      const seen: Array<{ configPath: string; runtimeConfig: ChainbreakerConfig }> = [];

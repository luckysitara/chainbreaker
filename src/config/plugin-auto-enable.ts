...
const ENV_CATALOG_PATHS = ["CHAINBREAKER_PLUGIN_CATALOG_PATHS", "CHAINBREAKER_MPM_CATALOG_PATHS"];
...
function collectModelRefs(cfg: ChainbreakerConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ChainbreakerConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: ChainbreakerConfig, pluginId: string): boolean {
...
function hasPluginOwnedToolConfig(cfg: ChainbreakerConfig, pluginId: string): boolean {
  if (pluginId === "xai") {
...
function collectCandidateChannelIds(cfg: ChainbreakerConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: ChainbreakerConfig): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: ChainbreakerConfig): boolean {
...
function configMayNeedPluginAutoEnable(cfg: ChainbreakerConfig, env: NodeJS.ProcessEnv): boolean {
...
function hasBrowserToolReference(cfg: ChainbreakerConfig): boolean {
...
function hasExplicitBrowserPluginEntry(cfg: ChainbreakerConfig): boolean {
  return Boolean(
    cfg.plugins?.entries && Object.prototype.hasOwnProperty.call(cfg.plugins.entries, "browser"),
  );
}

function resolveBrowserAutoEnableReason(cfg: ChainbreakerConfig): string | null {
...
function isPluginExplicitlyDisabled(cfg: ChainbreakerConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: ChainbreakerConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: ChainbreakerConfig, pluginId: string): ChainbreakerConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;

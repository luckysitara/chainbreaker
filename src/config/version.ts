...
export type ChainbreakerVersion = {
  major: number;
  minor: number;
  patch: number;
...
export function normalizeChainbreakerVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseChainbreakerVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameChainbreakerStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseChainbreakerVersion(a);
  const parsedB = parseChainbreakerVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareChainbreakerVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseChainbreakerVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseChainbreakerVersion(current);
  const parsedTouched = parseChainbreakerVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameChainbreakerStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareChainbreakerVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: ChainbreakerVersion): number {

...
import type { ChainbreakerConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type RuntimeConfigSnapshotRefreshParams = {
  sourceConfig: ChainbreakerConfig;
};
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  chainbreaker config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  chainbreaker config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: ChainbreakerConfig,
  pathSegments: string[],
): { changed: boolean; next: ChainbreakerConfig } {
...
function coerceConfig(value: unknown): ChainbreakerConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ChainbreakerConfig;
}
...
function stampConfigVersion(cfg: ChainbreakerConfig): ChainbreakerConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer Chainbreaker (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ChainbreakerConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ChainbreakerConfig {
    try {
  ...
  async function writeConfigFile(
    ...
    let cfgToWrite = persistCandidate as ChainbreakerConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ChainbreakerConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.CHAINBREAKER_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.CHAINBREAKER_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.CHAINBREAKER_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.CHAINBREAKER_WATCH_SESSION === "string" &&
        deps.env.CHAINBREAKER_WATCH_SESSION.trim().length > 0
          ? deps.env.CHAINBREAKER_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.CHAINBREAKER_WATCH_COMMAND === "string" &&
        deps.env.CHAINBREAKER_WATCH_COMMAND.trim().length > 0
          ? deps.env.CHAINBREAKER_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `CHAINBREAKER_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: ChainbreakerConfig | null = null;
let runtimeConfigSourceSnapshot: ChainbreakerConfig | null = null;
...
export function getRuntimeConfigSourceSnapshot(): ChainbreakerConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: ChainbreakerConfig;
  candidate: ChainbreakerConfig;
}): boolean {
...
export function loadConfig(): ChainbreakerConfig {
  ...
  // reparsing chainbreaker.json on hot code paths.
  setRuntimeConfigSnapshot(config);
  return runtimeConfigSnapshot ?? config;
}

export function getRuntimeConfig(): ChainbreakerConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<ChainbreakerConfig> {

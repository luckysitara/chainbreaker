...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): ChainbreakerConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: ChainbreakerConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        CHAINBREAKER_STATE_DIR: envStateDir,
      };
      const cfg: ChainbreakerConfig = {};

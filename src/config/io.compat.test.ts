...
import { parseChainbreakerVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".chainbreaker",
  port: number,
  filename: string = "chainbreaker.json",
) {
...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    expect(logger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining("Config was last written by a newer Chainbreaker"),
    );
...
describe("config io paths", () => {
  it("uses ~/.chainbreaker/chainbreaker.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".chainbreaker", 19001);
  ...
  it("defaults to ~/.chainbreaker/chainbreaker.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".chainbreaker", "chainbreaker.json"));
    });
  });

  it("uses CHAINBREAKER_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { CHAINBREAKER_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".chainbreaker", "chainbreaker.json"));
    });
  });

  it("honors explicit CHAINBREAKER_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".chainbreaker", 20002, "custom.json");
      const io = createIoForHome(home, { CHAINBREAKER_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseChainbreakerVersion(VERSION);
    if (!parsedVersion) {

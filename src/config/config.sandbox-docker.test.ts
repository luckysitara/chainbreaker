...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("chainbreaker-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "chainbreaker-sandbox-browser-global" },
      agentBrowser: { network: "chainbreaker-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("chainbreaker-sandbox-browser-agent");
  });

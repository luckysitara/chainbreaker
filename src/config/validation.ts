...
import type { ChainbreakerConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.chainbreaker.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: ChainbreakerConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: ChainbreakerConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ChainbreakerConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ChainbreakerSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as ChainbreakerConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: ChainbreakerConfig | null | undefined;

  const ensureCompatConfig = (): ChainbreakerConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Chainbreaker config schema.

...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.CHAINBREAKER_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveChainbreakerPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    CHAINBREAKER_STATE_DIR: path.join(os.tmpdir(), "chainbreaker-config-doc-baseline-state"),
    CHAINBREAKER_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };

...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "chainbreaker-shared-agentdir");
    ...
    await withTempHomeConfig(
      {
        agents: {
          list: [
            { id: "a", agentDir: "~/.chainbreaker/agents/shared/agent" },
            { id: "b", agentDir: "~/.chainbreaker/agents/shared/agent" },
          ],
        },

...
function makeTempDir() {
  return makeTrackedTempDir("chainbreaker-plugin-auto-enable", tempDirs);
}

function writePluginManifestFixture(params: { rootDir: string; id: string; channels: string[] }) {
  mkdirSafeDir(params.rootDir);
  fs.writeFileSync(
    path.join(params.rootDir, "chainbreaker.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function makeRegistry(
  ...
  return {
    plugins: plugins.map((p) => ({
      ...
      manifestPath: `/fake/${p.id}/chainbreaker.plugin.json`,
    })),
    diagnostics: [],
  };
}
...
describe("applyPluginAutoEnable", () => {
  ...
  it("uses env-scoped catalog metadata for preferOver auto-enable decisions", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@chainbreaker/env-secondary",
            chainbreaker: {
              channel: {
                id: "env-secondary",
                label: "Env Secondary",
              ...
              install: {
                npmSpec: "@chainbreaker/env-secondary",
              },
            },
          },
        ],
        ...
        plugins: [
          {
            ...
            manifestPath: "/fake/acme/chainbreaker.plugin.json",
            contracts: {
  ...
  describe("third-party channel plugins (pluginId ≠ channelId)", () => {
    it("uses the plugin manifest id, not the channel id, for plugins.entries", () => {
      // Reproduces: https://github.com/chainbreaker/chainbreaker/issues/25261

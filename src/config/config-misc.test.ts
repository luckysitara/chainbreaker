...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = ChainbreakerSchema.safeParse({
      $schema: "https://chainbreaker.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://chainbreaker.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = ChainbreakerSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = ChainbreakerSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("rejects removed legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeChainbreakerConfig(home, {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeChainbreakerConfig(home, {
        gateway: { bind: "${CHAINBREAKER_BIND}" },
      });

      const prev = process.env.CHAINBREAKER_BIND;
      process.env.CHAINBREAKER_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.CHAINBREAKER_BIND;
        } else {
          process.env.CHAINBREAKER_BIND = prev;
        }
      }
    });
  });

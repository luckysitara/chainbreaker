...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ CHAINBREAKER_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from CHAINBREAKER_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CHAINBREAKER_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when CHAINBREAKER_STATE_DIR differs from stored paths", () => {

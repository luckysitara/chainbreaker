...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "chainbreaker.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      CHAINBREAKER_STATE_DIR: path.join(suiteHome, ".chainbreaker"),
      CHAINBREAKER_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", async () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          CHAINBREAKER_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );

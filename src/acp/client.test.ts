...
const createTempDir = () => tempDirs.make("chainbreaker-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets CHAINBREAKER_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "chainbreaker",
    });

    expect(env.CHAINBREAKER_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("chainbreaker");
  });

  it("overrides pre-existing CHAINBREAKER_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      CHAINBREAKER_SHELL: "wrong",
    });
    expect(env.CHAINBREAKER_SHELL).toBe("acp-client");
  });
  ...
  it("preserves CHAINBREAKER_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        CHAINBREAKER_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["CHAINBREAKER_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Chainbreaker bridge", () => {
    ...
    expect(env.CHAINBREAKER_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.CHAINBREAKER_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "chainbreaker",
        serverArgs: ["acp"],
        defaultServerCommand: "chainbreaker",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("CHAINBREAKER_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "chainbreaker", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "chainbreaker", "dist", "entry.js");
    const shimPath = path.join(dir, "chainbreaker.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\chainbreaker\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/chainbreaker-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/chainbreaker-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/chainbreaker-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);

...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    chainbreaker: process.env.CHAINBREAKER_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.CHAINBREAKER_STATE_DIR).toBe(snapshot.chainbreaker);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips CHAINBREAKER_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/chainbreaker-state-dir-test");
    expect(process.env.CHAINBREAKER_STATE_DIR).toBe("/tmp/chainbreaker-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("chainbreaker-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("chainbreaker-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;

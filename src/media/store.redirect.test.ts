...
const HOME = path.join(os.tmpdir(), "chainbreaker-home-redirect");
...
describe("media store redirects", () => {
  ...
  beforeAll(async () => {
    ...
    process.env.CHAINBREAKER_STATE_DIR = HOME;
  });
  ...
  it("strips sensitive headers when a redirect crosses origins", async () => {
    ...
    await saveMediaSource("https://example.com/start", {
      ...
      Accept: "text/plain",
      "User-Agent": "Chainbreaker-Test/1.0",
    });
    ...
    expect(secondHeaders.get("user-agent")).toBe("Chainbreaker-Test/1.0");
  });

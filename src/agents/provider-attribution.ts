...
const CHAINBREAKER_ATTRIBUTION_PRODUCT = "Chainbreaker";
const CHAINBREAKER_ATTRIBUTION_ORIGINATOR = "chainbreaker";

export function resolveProviderAttributionIdentity(
  env: RuntimeVersionEnv = process.env as RuntimeVersionEnv,
): ProviderAttributionIdentity {
  return {
    product: CHAINBREAKER_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in Chainbreaker runtime wrapper.",
    ...identity,
    headers: {
      "HTTP-Referer": "https://chainbreaker.ai",
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: CHAINBREAKER_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": `${CHAINBREAKER_ATTRIBUTION_ORIGINATOR}/${identity.version}`,
    },
  };
}

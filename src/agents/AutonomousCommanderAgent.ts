...
import { ILLMProvider } from "../llms/ILLMProvider"; // Removed .js
import { AgentAnnouncement, Result } from "../communication/schemas"; // Removed .js
...
export class AutonomousCommanderAgent extends BaseAgent {
  ...
  private activeAgents: Map<string, AgentAnnouncement> = new Map();
  ...
  constructor(
    ...
    llmProviderName: string = "gemini", // Default LLM provider
  ) {
  ...
  async init(): Promise<void> {
    ...
    await this.communicationManager.subscribeToDiscovery((announcement) => {
      console.log(
        `${this.name} discovered agent: ${announcement.name} (${announcement.agentId}) with capabilities: ${announcement.capabilities.join(", ")}`,
      );
  ...
  private async handleAgentResult(result: Result): Promise<void> {
    console.log(
      `${this.name} received async result for task ${result.taskId}:`,
      result,
    );
  ...
  private async decideNextAction(): Promise<string> {
    const availableTools = this.toolManager
      .listTools()
      .map((tool) => ({ name: tool.name, description: tool.description }));

    const availableAgents = Array.from(this.activeAgents.values()).map(
      (agent) => ({
        name: agent.name,
...
`;
    ...
    const llmResponse = await this.llmProvider.generateChatCompletion(
    ...
    while (!completed) {
      try {
        ...
        if (action.action === "delegate_task") {
          console.log(
            `${this.name} delegating task ${action.taskType} to ${action.agentId}`,
          );
          ...
          console.log(
            `${this.name} executing tool ${action.toolName} with params:`,
            action.params,
          );
        ...
        if (this.conversationHistory.length > 20) {
          console.error(
            `${this.name} too many errors, breaking autonomous loop.`,
          );
          completed = true;
        }
      }

...
type AgentEntry = NonNullable<NonNullable<ChainbreakerConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: ChainbreakerConfig): AgentEntry[] {
...
export function listAgentIds(cfg: ChainbreakerConfig): string[] {
...
export function resolveDefaultAgentId(cfg: ChainbreakerConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: ChainbreakerConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: ChainbreakerConfig, agentId: string) {
  const id = normalizeAgentId(agentId);

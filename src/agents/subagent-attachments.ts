...
function resolveAttachmentLimits(config: ChainbreakerConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".chainbreaker", "attachments");
  const relDir = path.posix.join(".chainbreaker", "attachments", attachmentId);

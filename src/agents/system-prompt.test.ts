...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits extended sections in minimal prompt mode", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      docsPath: "/tmp/chainbreaker/docs",
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## Chainbreaker CLI Quick Reference");
    expect(prompt).toContain("chainbreaker gateway restart");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Chainbreaker docs: /tmp/chainbreaker/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Chainbreaker Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/chainbreaker. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`chainbreaker ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Chainbreaker subagents");

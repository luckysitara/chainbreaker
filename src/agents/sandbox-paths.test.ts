...
async function withOutsideHardlinkInChainbreakerTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  const openClawTmpDir = resolvePreferredChainbreakerTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Chainbreaker tmp root",
      media: path.join(openClawTmpDir, "image.png"),
      expected: path.join(openClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Chainbreaker tmp root",
      media: pathToFileURL(path.join(openClawTmpDir, "photo.png")).href,
      expected: path.join(openClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Chainbreaker tmp root",
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside chainbreaker tmp root",
      media: path.join(os.tmpdir(), "outside-chainbreaker", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Chainbreaker tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Chainbreaker tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInChainbreakerTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Chainbreaker tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInChainbreakerTmp(

...
const bundledPluginsDirSnapshot = captureEnv(["CHAINBREAKER_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
describe("createChainbreakerCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "chainbreaker-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "chainbreaker-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "chainbreaker-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "chainbreaker-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "chainbreaker-safe-bins-grep-",
        safeBins: ["grep"],

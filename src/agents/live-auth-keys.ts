...
const GOOGLE_LIVE_SINGLE_KEY = "CHAINBREAKER_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "CHAINBREAKER_LIVE_ANTHROPIC_KEY",
    listVar: "CHAINBREAKER_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "CHAINBREAKER_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `CHAINBREAKER_LIVE_${base}_KEY`;

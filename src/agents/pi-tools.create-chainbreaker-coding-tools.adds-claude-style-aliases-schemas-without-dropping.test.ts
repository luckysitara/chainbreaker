...
import { __testing, createChainbreakerCodingTools } from "./pi-tools.js";
import { createChainbreakerReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createChainbreakerCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createChainbreakerCodingTools({ senderIsOwner: true }).find(
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createChainbreakerCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createChainbreakerCodingTools({
      modelProvider: "xai",
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "chainbreaker-outside.txt");
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createChainbreakerReadTool(
      baseRead as unknown as Parameters<typeof createChainbreakerReadTool>[0],
    );

...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-cli-bundle-mcp-"));
      ...
      const pluginRoot = path.join(tempHome, ".chainbreaker", "extensions", "bundle-probe");

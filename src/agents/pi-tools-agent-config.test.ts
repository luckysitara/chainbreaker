...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: ChainbreakerConfig) {
    return createChainbreakerCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<ChainbreakerConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<ChainbreakerConfig["agents"]>["list"]>[number]["tools"];
  }): ChainbreakerConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createChainbreakerCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createChainbreakerCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createChainbreakerCodingTools({
      config: cfg,
    ...
    const defaultTools = createChainbreakerCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createChainbreakerCodingTools({
      config: cfg,
    ...
    const bobTools = createChainbreakerCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createChainbreakerCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createChainbreakerCodingTools({
      config: cfg,

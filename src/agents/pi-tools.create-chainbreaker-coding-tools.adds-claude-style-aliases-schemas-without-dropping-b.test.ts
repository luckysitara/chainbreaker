...
const defaultTools = createChainbreakerCodingTools({ senderIsOwner: true });
...
describe("createChainbreakerCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createChainbreakerCodingTools({
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    expect(openAiTools.some((tool) => tool.name === "apply_patch")).toBe(true);

    const codexTools = createChainbreakerCodingTools({
      modelProvider: "openai-codex",
      modelId: "gpt-5.4",
    });
    expect(codexTools.some((tool) => tool.name === "apply_patch")).toBe(true);

    const disabledConfig: ChainbreakerConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createChainbreakerCodingTools({
    ...
    const anthropicTools = createChainbreakerCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: ChainbreakerConfig = {
      tools: {
        exec: {
    ...
    const allowed = createChainbreakerCodingTools({
    ...
    const denied = createChainbreakerCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createChainbreakerCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });

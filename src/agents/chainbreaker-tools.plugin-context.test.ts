...
let createChainbreakerCodingTools: typeof import("./pi-tools.js").createChainbreakerCodingTools;

describe("createChainbreakerTools plugin context", () => {
  beforeEach(async () => {
    resolvePluginToolsMock.mockClear();
    vi.resetModules();
    ({ createChainbreakerTools } = await import("./chainbreaker-tools.js"));
    ({ createChainbreakerCodingTools } = await import("./pi-tools.js"));
  });
  ...
  it("does not inject ambient thread defaults into plugin tools", async () => {
    ...
    const first = createChainbreakerTools({
      config: {} as never,
      agentThreadId: "111.222",
    }).find((tool) => tool.name === "plugin-thread-default");
    const second = createChainbreakerTools({
      config: {} as never,
  ...
  it("does not inject messageThreadId defaults for missing params objects", async () => {
    ...
    const wrapped = createChainbreakerTools({
      config: {} as never,
      agentThreadId: "77",

...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.CHAINBREAKER_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".chainbreaker", `workspace-${profile}`);
  }
  return path.join(home, ".chainbreaker", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".chainbreaker";

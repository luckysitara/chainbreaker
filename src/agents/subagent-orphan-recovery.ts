 ...
 *
 * @see https://github.com/chainbreaker/chainbreaker/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /chainbreaker\.json|chainbreaker gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify chainbreaker.json or restart the gateway]"
            : undefined,
          originalRunId: runId,

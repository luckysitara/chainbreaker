...
let createChainbreakerTools: typeof import("./chainbreaker-tools.js").createChainbreakerTools;

async function loadFreshChainbreakerToolsModuleForTest() {
  vi.resetModules();
  vi.doMock("./tools/gateway.js", () => createGatewayToolModuleMocks());
  ({ createChainbreakerTools } = await import("./chainbreaker-tools.js"));
}
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshChainbreakerToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-test-"));

    try {
      await withEnvAsync(
        { CHAINBREAKER_STATE_DIR: stateDir, CHAINBREAKER_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: chainbreaker --profile isolated doctor --non-interactive",
          );
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/chainbreaker" } } }',
      }),
    ).rejects.toThrow(

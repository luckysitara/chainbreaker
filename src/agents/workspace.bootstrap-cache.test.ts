...
describe("workspace bootstrap file caching", () => {
  ...
  beforeEach(async () => {
    workspaceDir = await makeTempWorkspace("chainbreaker-bootstrap-cache-test-");
  });
  ...
  it("caches files independently by path", async () => {
    ...
    const workspace1 = await makeTempWorkspace("chainbreaker-cache-test1-");
    const workspace2 = await makeTempWorkspace("chainbreaker-cache-test2-");

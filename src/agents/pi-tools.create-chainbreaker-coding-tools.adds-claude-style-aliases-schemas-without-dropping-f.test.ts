...
describe("createChainbreakerCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-alias-"));
    try {
      const tools = createChainbreakerCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts broader file/edit alias variants", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-alias-broad-"));
    try {
      const tools = createChainbreakerCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-structured-write-"));
    try {
      const tools = createChainbreakerCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-structured-edit-"));
    try {

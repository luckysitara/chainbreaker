...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: ChainbreakerConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/chainbreaker" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: ChainbreakerConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: ChainbreakerConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: ChainbreakerConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: ChainbreakerConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: ChainbreakerConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: ChainbreakerConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: ChainbreakerConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: ChainbreakerConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/chainbreaker-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/chainbreaker");
  });

  it("uses CHAINBREAKER_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "chainbreaker-home");
    vi.stubEnv("CHAINBREAKER_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as ChainbreakerConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".chainbreaker", "workspace"));
  });

  it("uses CHAINBREAKER_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "chainbreaker-home");
    vi.stubEnv("CHAINBREAKER_HOME", home);
    // Clear state dir so it falls back to CHAINBREAKER_HOME
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/chainbreaker-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/chainbreaker-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-agent-scope-"));

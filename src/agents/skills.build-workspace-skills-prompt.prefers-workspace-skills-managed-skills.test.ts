...
const fixtureSuite = createFixtureSuite("chainbreaker-skills-prompt-suite-");
...
describe("buildWorkspaceSkillsPrompt", () => {
  ...
  it("gates by bins, config, and always", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"chainbreaker":{"requires":{"bins":["fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "anybin-skill"),
      name: "anybin-skill",
      description: "Needs any bin",
      metadata: '{"chainbreaker":{"requires":{"anyBins":["missingbin","fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "config-skill"),
      name: "config-skill",
      description: "Needs config",
      metadata: '{"chainbreaker":{"requires":{"config":["browser.enabled"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "always-skill"),
      name: "always-skill",
      description: "Always on",
      metadata: '{"chainbreaker":{"always":true,"requires":{"env":["MISSING"]}}}',
    });
    await writeSkill({
  ...
  it("uses skillKey for config lookups", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"chainbreaker":{"skillKey":"alias"}}',
    });

...
describe("tool display details", () => {
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "Chainbreaker docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "Chainbreaker docs" (top 3)');
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.chainbreaker/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.chainbreaker/workspace",
        },
      }),
    );

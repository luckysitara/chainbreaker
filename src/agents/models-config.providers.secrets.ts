...
type ModelsConfig = NonNullable<ChainbreakerConfig["models"]>;
...
function resolveConfigBackedProviderAuth(params: { provider: string; config?: ChainbreakerConfig }):
  | {
      apiKey: string;
      discoveryApiKey?: string;
      mode: "api_key";
...
function resolveXaiConfigFallbackAuth(params: { provider: string; config?: ChainbreakerConfig }):
  | {
      apiKey: string;
      source: string;
      mode: "api-key";

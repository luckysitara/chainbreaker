...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CHAINBREAKER_HOME for default workspace resolution", () => {
...
const WORKSPACE_STATE_PATH_SEGMENTS = [".chainbreaker", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("chainbreaker-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("chainbreaker-workspace-");
    await fs.mkdir(path.join(tempDir, ".chainbreaker"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-workspace-hardlink-"));
    try {

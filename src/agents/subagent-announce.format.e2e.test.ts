...
let configOverride: ChainbreakerConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: ChainbreakerConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/chainbreaker/chainbreaker/issues/31298
    previousFastTestEnv = process.env.CHAINBREAKER_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.CHAINBREAKER_TEST_FAST;
      return;
    }
    process.env.CHAINBREAKER_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // CHAINBREAKER_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Chainbreaker runtime context (internal):");

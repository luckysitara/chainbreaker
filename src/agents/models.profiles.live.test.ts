  ...
  it(
    ...
    async () => {
      ...
      await withLiveStageTimeout(
        ensureChainbreakerModelsJson(cfg),
        "[live-models] prepare models.json",
      );
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set CHAINBREAKER_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.CHAINBREAKER_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.CHAINBREAKER_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.CHAINBREAKER_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.CHAINBREAKER_LIVE_MAX_MODELS, 0);

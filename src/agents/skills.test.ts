...
const fixtureSuite = createFixtureSuite("chainbreaker-skills-suite-");
...
async function writeEnvSkill(workspaceDir: string) {
  ...
  await writeSkill({
    ...
    metadata: '{"chainbreaker":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
  });
}

beforeAll(async () => {
  await fixtureSuite.setup();
  tempHome = await createTempHomeEnv("chainbreaker-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".chainbreaker", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native Chainbreaker slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const pluginRoot = path.join(tempHome!.home, ".chainbreaker", "extensions", "compound-bundle");
    ...
    expect(
      ...
      path.join(
        installedPluginRoot(path.join(workspaceDir, ".chainbreaker"), "compound-bundle"),
        "commands",
        "workflows-review.md",
      ),
    );
...
describe("applySkillEnvOverrides", () => {
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"chainbreaker":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"chainbreaker":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("blocks override-only host env overrides in skill config", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"chainbreaker":{"requires":{"env":["HTTPS_PROXY","NODE_TLS_REJECT_UNAUTHORIZED","DOCKER_HOST"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"chainbreaker":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });

...
const buildStatusMessageMock = vi.hoisted(() => vi.fn(() => "Chainbreaker\n🧠 Model: GPT-5.4"));
...
async function loadFreshChainbreakerToolsForSessionStatusTest() {
  vi.resetModules();
...
describe("session_status tool", () => {
  beforeEach(async () => {
    buildStatusMessageMock.mockClear();
    await loadFreshChainbreakerToolsForSessionStatusTest();
  });
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("Chainbreaker");

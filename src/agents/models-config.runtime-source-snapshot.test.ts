...
let ensureChainbreakerModelsJson: typeof import("./models-config.js").ensureChainbreakerModelsJson;
...
beforeEach(async () => {
  ...
  ({ ensureChainbreakerModelsJson, resetModelsJsonReadyCacheForTest } =
...
function createOpenAiApiKeySourceConfig(): ChainbreakerConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): ChainbreakerConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): ChainbreakerConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): ChainbreakerConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: ChainbreakerConfig): ChainbreakerConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function withGeneratedModelsFromRuntimeSource(
  params: {
    ...
    candidateConfig?: ChainbreakerConfig;
  },
  ...
  await withTempHome(async () => {
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        ...
        await ensureChainbreakerModelsJson(params.candidateConfig ?? loadConfig());
        await runAssertions();
      } finally {
  ...
  it("uses non-env marker from runtime source snapshot for file refs", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const runtimeConfig: ChainbreakerConfig = {
          models: {
            providers: {
              moonshot: {
                baseUrl: "https://api.moonshot.ai/v1",
        ...
        try {
          ...
          await ensureChainbreakerModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const clonedRuntimeConfig: ChainbreakerConfig = {
          ...runtimeConfig,
          agents: {
            defaults: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureChainbreakerModelsJson(clonedRuntimeConfig);
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const sourceConfig = createOpenAiApiKeySourceConfig();
        const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
        const firstCandidate: ChainbreakerConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        const secondCandidate: ChainbreakerConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureChainbreakerModelsJson(firstCandidate);
          ...
          await ensureChainbreakerModelsJson(secondCandidate);
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const incompatibleCandidate: ChainbreakerConfig = {
          ...createOpenAiApiKeyRuntimeConfig(),
        };

        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureChainbreakerModelsJson(incompatibleCandidate);
          await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
        } finally {
          clearRuntimeConfigSnapshot();
          clearConfigCache();

...
async function resolveContext(config: ChainbreakerConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): ChainbreakerConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: ChainbreakerConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/chainbreaker-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",

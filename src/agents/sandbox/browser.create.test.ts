...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    browser: {
      ...
      containerPrefix: "chainbreaker-sbx-browser-",
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("CHAINBREAKER_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("CHAINBREAKER_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^CHAINBREAKER_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("stamps the mount format version label on browser containers", async () => {
    ...
    expect(labels).toContain(`chainbreaker.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  });
});

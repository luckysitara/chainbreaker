...
// subdirectories and the resolved Chainbreaker state directory.
...
const BLOCKED_HOME_SUBPATHS = [".aws", ".config", ".kube", ".chainbreaker", ".ssh"] as const;
  ...
  if (params.reason.kind === "reserved_target") {
    return new Error(
      `Sandbox security: bind mount "${params.bind}" targets reserved container path "${params.reason.reservedPath}" ` +
        `(resolved target: "${params.reason.targetPath}"). This can shadow Chainbreaker sandbox mounts. ` +

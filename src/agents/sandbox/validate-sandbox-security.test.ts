...
describe("getBlockedBindReason", () => {
  ...
  it("blocks sensitive home subdirectories", () => {
    ...
    expect(getBlockedBindReason("/home/tester/.chainbreaker:/mnt/state:ro")).toEqual(
      expect.objectContaining({
        kind: "targets",
        blockedPath: "/home/tester/.chainbreaker",
      }),
    );
  ...
  it("blocks sensitive home subdirectories under CHAINBREAKER_HOME", () => {
    vi.stubEnv("HOME", "/home/tester");
    vi.stubEnv("CHAINBREAKER_HOME", "/srv/chainbreaker-home");
    expect(getBlockedBindReason("/srv/chainbreaker-home/.ssh:/mnt/ssh:ro")).toEqual(
      expect.objectContaining({
        kind: "targets",
        blockedPath: "/srv/chainbreaker-home/.ssh",
      }),
    );
  });

  it("still blocks OS-home sensitive paths when CHAINBREAKER_HOME is overridden", () => {
    vi.stubEnv("HOME", "/home/tester");
    vi.stubEnv("CHAINBREAKER_HOME", "/srv/chainbreaker-home");
  ...
  it("blocks the resolved Chainbreaker state directory override", () => {
    vi.stubEnv("CHAINBREAKER_STATE_DIR", "/srv/chainbreaker-state");
    expect(getBlockedBindReason("/srv/chainbreaker-state/credentials:/mnt/state:ro")).toEqual(
      expect.objectContaining({
        kind: "targets",
        blockedPath: "/srv/chainbreaker-state",
      }),
    );
  });
});
...
describe("validateBindMounts", () => {
  ...
  it("blocks symlink escapes into blocked directories", () => {
    if (process.platform === "win32") {
      ...
      const dir = mkdtempSync(join(tmpdir(), "chainbreaker-sbx-"));
  ...
  it("blocks canonicalized sensitive paths derived from CHAINBREAKER_HOME", () => {
    if (process.platform === "win32") {
      return;
    }

    const dir = mkdtempSync(join(tmpdir(), "chainbreaker-home-"));
    ...
    vi.stubEnv("CHAINBREAKER_HOME", linkedHome);
  ...
  it("refreshes canonical blocked aliases when CHAINBREAKER_HOME symlinks retarget", () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("blocks OS-home sensitive paths when CHAINBREAKER_HOME points elsewhere", () => {
    vi.stubEnv("HOME", "/home/tester");
    vi.stubEnv("CHAINBREAKER_HOME", "/srv/chainbreaker-home");
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "chainbreaker-not-created");
    expect(() =>
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(() => validateApparmorProfile("chainbreaker-sandbox")).not.toThrow();

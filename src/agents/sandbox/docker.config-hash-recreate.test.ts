...
function spawnDockerProcess(command: string, args: string[]) {
  ...
  if (command !== "docker") {
    ...
    args[0] === "inspect" &&
    args[1] === "-f" &&
    args[2]?.includes('index .Config.Labels "chainbreaker.configHash"')
  ) {
...
function createSandboxConfig(
  ...
  return {
    ...
    docker: {
      image: "chainbreaker-sandbox:test",
    ...
    browser: {
      enabled: false,
      image: "chainbreaker-browser:test",
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall?.args).toContain(`chainbreaker.configHash=${newHash}`);
  ...
  it("applies custom binds after workspace mounts so overlapping binds can override", async () => {
    ...
    expect(createCall).toBeDefined();
    expect(createCall?.args).toContain(`chainbreaker.configHash=${expectedHash}`);
  ...
  it("stamps the mount format version label on created containers", async () => {
    ...
    expect(createCall?.args).toContain(
      `chainbreaker.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`,
    );
  });
});

...
function createConfig(): ChainbreakerConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/chainbreaker",
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "chainbreaker-test-ssh-config"),
    host: "chainbreaker-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "chainbreaker-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "chainbreaker-ssh-worker-abcd1234",
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "chainbreaker-browser",
          containerPrefix: "chainbreaker-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/chainbreaker/chainbreaker-ssh-agent-worker");

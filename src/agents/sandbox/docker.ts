...
import { markChainbreakerExecEnv } from "../../infra/chainbreaker-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "chainbreaker.sandbox=1");
  args.push("--label", `chainbreaker.sessionKey=${params.scopeKey}`);
  args.push("--label", `chainbreaker.createdAtMs=${createdAtMs}`);
  args.push("--label", `chainbreaker.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `chainbreaker.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markChainbreakerExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "chainbreaker.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`chainbreaker sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`chainbreaker sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("chainbreaker sandbox recreate --all");
}

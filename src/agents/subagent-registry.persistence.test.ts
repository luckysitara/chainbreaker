...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["CHAINBREAKER_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-subagent-"));
  ...
  it("uses isolated temp state when CHAINBREAKER_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "chainbreaker-test-state"));
  });
});

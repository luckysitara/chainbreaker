...
describe("provider attribution", () => {
  it("resolves the canonical Chainbreaker product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      headers: {
        ...
        "X-OpenRouter-Title": "Chainbreaker",
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { CHAINBREAKER_VERSION: "2026.3.22" }),
    ).toEqual({

...
function getNodesTool(options?: { modelHasVision?: boolean; allowMediaInvokeCommands?: boolean }) {
  ...
  const tool = createChainbreakerTools(toolOptions).find((candidate) => candidate.name === "nodes");
  if (!tool) {
...
beforeEach(async () => {
  ...
  vi.unstubAllGlobals();
  await loadChainbreakerToolsForTest();
});
...
describe("nodes camera_snap", () => {
  ...
  it("omits inline base64 image blocks when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/chainbreaker-camera-snap-front-.*\.jpg$/);
  });
...
async function loadChainbreakerToolsForTest(): Promise<void> {
  vi.resetModules();

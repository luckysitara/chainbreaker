    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "chainbreaker.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "chainbreaker.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "chainbreaker.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("chainbreaker-");
  const acpxRoot = await tempDirs.make("chainbreaker-acpx-plugin-");
  const helperRoot = await tempDirs.make("chainbreaker-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("chainbreaker-");
  const pluginRoot = await tempDirs.make("chainbreaker-plugin-");
  const outsideDir = await tempDirs.make("chainbreaker-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("chainbreaker-");
    const pluginRoot = await tempDirs.make("chainbreaker-claude-bundle-");

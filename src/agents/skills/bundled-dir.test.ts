...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["CHAINBREAKER_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns CHAINBREAKER_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-bundled-override-"));
    process.env.CHAINBREAKER_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "chainbreaker" }));

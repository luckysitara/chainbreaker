...
function resolveSkillsLimits(config?: ChainbreakerConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "chainbreaker-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "chainbreaker-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveChainbreakerMetadata(frontmatter),
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`chainbreaker skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`chainbreaker skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,

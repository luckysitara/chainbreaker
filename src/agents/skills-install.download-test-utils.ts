...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("chainbreaker-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-skills-install-"));
  try {

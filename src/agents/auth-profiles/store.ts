 ...
 * A common mistake — caused by the similarity with the `chainbreaker.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (chainbreaker.json uses "mode"; auth-profiles.json uses "type")
...
function shouldLogAuthStoreTiming(): boolean {
  return process.env.CHAINBREAKER_DEBUG_INGRESS_TIMING === "1";
}
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.CHAINBREAKER_AUTH_STORE_READONLY === "1";

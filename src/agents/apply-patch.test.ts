...
async function withTempDir<T>(fn: (dir: string) => Promise<T>) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-patch-"));
  try {
    return await fn(dir);
  } finally {
...
async function withWorkspaceTempDir<T>(fn: (dir: string) => Promise<T>) {
  const dir = await fs.mkdtemp(path.join(process.cwd(), "chainbreaker-patch-workspace-"));
  try {
    return await fn(dir);
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
...
*** End Patch`;
  ...
  it("rejects absolute paths outside cwd by default", async () => {
    await withTempDir(async (dir) => {
      const escapedPath = path.join(os.tmpdir(), `chainbreaker-apply-patch-${Date.now()}.txt`);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (dir) => {
        ...
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-patch-outside-"));

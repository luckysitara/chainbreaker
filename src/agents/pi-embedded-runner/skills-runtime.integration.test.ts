...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("chainbreaker-bundled-");
  const workspaceDir = await createTempDir("chainbreaker-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: ChainbreakerConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = bundledPluginsDir;

...
[media attached 2/2: /Users/tyleryust/.chainbreaker/media/IMG_6431.jpeg (image/jpeg)]
...
what is this?`);
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const ref =
      expectSingleImageReference(`[media attached: /Users/test/.chainbreaker/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`);
...
describe("loadImageFromRef", () => {
  it("allows sandbox-validated host paths outside default media roots", async () => {
    ...
    const sandboxParent = await fs.mkdtemp(path.join(homeDir, "chainbreaker-sandbox-image-"));
    try {
...
describe("detectAndLoadPromptImages", () => {
  ...
  it("blocks prompt image refs outside workspace when sandbox workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-native-image-sandbox-"));

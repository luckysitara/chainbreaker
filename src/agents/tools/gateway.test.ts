...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    chainbreaker: process.env.CHAINBREAKER_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.chainbreaker === undefined) {
      delete process.env.CHAINBREAKER_GATEWAY_TOKEN;
    } else {
      process.env.CHAINBREAKER_GATEWAY_TOKEN = envSnapshot.chainbreaker;
    }
  });
  ...
  it("uses CHAINBREAKER_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.CHAINBREAKER_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {

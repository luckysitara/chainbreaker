...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.CHAINBREAKER_TEST_FAST;
...
describe("chainbreaker-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.CHAINBREAKER_TEST_FAST = fastModeEnv.previous;
  });

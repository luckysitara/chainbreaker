...
function mockContextDeps(params: {
  ...
  const ensureChainbreakerModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveChainbreakerAgentDir: () => "/tmp/chainbreaker-agent",
  }));
  ...
  return { ensureChainbreakerModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real chainbreaker startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "chainbreaker", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "chainbreaker", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "chainbreaker", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureChainbreakerModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureChainbreakerModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {

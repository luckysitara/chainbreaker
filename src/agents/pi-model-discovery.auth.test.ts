...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.CHAINBREAKER_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.CHAINBREAKER_AUTH_STORE_READONLY;
        } else {
          process.env.CHAINBREAKER_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });

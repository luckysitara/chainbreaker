...
describe("createChainbreakerCodingTools", () => {
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-read-"));
    try {
      ...
      const contents = "Hello from chainbreaker read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "chainbreaker-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "chainbreaker-workspace"),
    ...
    const tools = createChainbreakerCodingTools({ sandbox });

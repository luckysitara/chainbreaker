...
let configuredConfig: ChainbreakerConfig | undefined;
...
function isLikelyChainbreakerCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "chainbreaker" ||
    entryBasename === "chainbreaker.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Chainbreaker CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureChainbreakerModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyChainbreakerCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): ChainbreakerConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureChainbreakerModelsJson(cfg);
    } catch {

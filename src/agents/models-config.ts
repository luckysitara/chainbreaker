...
import { planChainbreakerModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: ChainbreakerConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: ChainbreakerConfig): {
  config: ChainbreakerConfig;
  sourceConfigForSecrets: ChainbreakerConfig;
} {
...
export async function ensureChainbreakerModelsJson(
  config?: ChainbreakerConfig,
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planChainbreakerModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,

...
export const CHAINBREAKER_NODE_OPTIONS_READY = "CHAINBREAKER_NODE_OPTIONS_READY";
export const CHAINBREAKER_NODE_EXTRA_CA_CERTS_READY = "CHAINBREAKER_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.CHAINBREAKER_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[CHAINBREAKER_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[CHAINBREAKER_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[CHAINBREAKER_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[CHAINBREAKER_NODE_OPTIONS_READY] = "1";

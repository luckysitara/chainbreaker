...
describe("detectChangedScope", () => {
  ...
  it("keeps node lane off for native-only changes", () => {
    ...
    expect(detectChangedScope(["apps/shared/ChainbreakerKit/Sources/Foo.swift"])).toEqual({
      runNode: false,
      runMacos: true,
      runAndroid: true,
      runWindows: false,
  ...
  it("treats base and head as literal git args", () => {
    const markerPath = path.join(
      os.tmpdir(),
      `chainbreaker-ci-changed-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`,
    );

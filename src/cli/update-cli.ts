...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Chainbreaker and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["chainbreaker update", "Update a source checkout (git)"],
        ["chainbreaker update --channel beta", "Switch to beta channel (git + npm)"],
        ["chainbreaker update --channel dev", "Switch to dev channel (git + npm)"],
        ["chainbreaker update --tag beta", "One-off update to a dist-tag or version"],
        ["chainbreaker update --tag main", "One-off package install from GitHub main"],
        ["chainbreaker update --dry-run", "Preview actions without changing anything"],
        ["chainbreaker update --no-restart", "Update without restarting the service"],
        ["chainbreaker update --json", "Output result as JSON"],
        ["chainbreaker update --yes", "Non-interactive (accept downgrade prompts)"],
        ["chainbreaker update wizard", "Interactive update wizard"],
        ["chainbreaker --update", "Shorthand for chainbreaker update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run chainbreaker update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.chainbreaker.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chainbreaker update status", "Show channel + version status."],
          ["chainbreaker update status --json", "JSON output."],
          ["chainbreaker update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.chainbreaker.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {

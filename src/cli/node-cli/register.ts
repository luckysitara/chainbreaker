    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "chainbreaker node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["chainbreaker node status", "Check node host service status."],
          ["chainbreaker node install", "Install the node host service."],
          ["chainbreaker node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.chainbreaker.ai/cli/node")}\n`,
    );

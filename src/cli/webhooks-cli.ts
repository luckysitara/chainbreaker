...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.chainbreaker.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Chainbreaker hooks")
    ...
    .option("--hook-url <url>", "Chainbreaker hook URL")
    .option("--hook-token <token>", "Chainbreaker hook token")

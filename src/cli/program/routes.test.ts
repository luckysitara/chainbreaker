...
describe("program routes", () => {
  ...
  it("matches status route and preloads plugins only for text output", () => {
    ...
    expect(shouldLoad(["node", "chainbreaker", "status"])).toBe(true);
    expect(shouldLoad(["node", "chainbreaker", "status", "--json"])).toBe(false);
  });
  ...
  it("matches health route and preloads plugins only for text output", () => {
    ...
    expect(typeof route?.loadPlugins).toBe("function");
    const shouldLoad = route?.loadPlugins as (argv: string[]) => boolean;
    expect(shouldLoad(["node", "chainbreaker", "health"])).toBe(true);
    expect(shouldLoad(["node", "chainbreaker", "health", "--json"])).toBe(false);
  });
  ...
  it("returns false for gateway status route when option values are missing", async () => {
    ...
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "chainbreaker", "gateway", "status", "--password"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "chainbreaker", "gateway", "status", "--timeout"],
    );
  });
  ...
  it("returns false for gateway status route when probe-only flags are present", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "chainbreaker", "gateway", "status", "--ssh", "user@host"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "chainbreaker", "gateway", "status", "--ssh-identity", "~/.ssh/id_test"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "chainbreaker", "gateway", "status", "--ssh-auto"],
    );
  });
  ...
  it("returns false when status timeout flag value is missing", async () => {
    await expectRunFalse(["status"], ["node", "chainbreaker", "status", "--timeout"]);
  });
  ...
  it("returns false for sessions route when --store value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "chainbreaker", "sessions", "--store"]);
  });

  it("returns false for sessions route when --active value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "chainbreaker", "sessions", "--active"]);
  });

  it("returns false for sessions route when --agent value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "chainbreaker", "sessions", "--agent"]);
  });
  ...
  it("returns false for config get route when path argument is missing", async () => {
    await expectRunFalse(["config", "get"], ["node", "chainbreaker", "config", "get", "--json"]);
  });

  it("returns false for config unset route when path argument is missing", async () => {
    await expectRunFalse(["config", "unset"], ["node", "chainbreaker", "config", "unset"]);
  });
  ...
  it("returns false for models status route when probe flags are missing values", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "chainbreaker", "models", "status", "--probe-provider"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "chainbreaker", "models", "status", "--probe-timeout"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "chainbreaker", "models", "status", "--probe-concurrency"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "chainbreaker", "models", "status", "--probe-max-tokens"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "chainbreaker", "models", "status", "--probe-provider", "openai", "--agent"],
    );
  });
  ...
  it("returns false for models status route when --probe-profile has no value", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "chainbreaker", "models", "status", "--probe-profile"],
    );
  });

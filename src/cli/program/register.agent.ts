...
${formatHelpExamples([
  ['chainbreaker agent --to +15555550123 --message "status update"', "Start a new session."],
  ['chainbreaker agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.chainbreaker.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.chainbreaker.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "chainbreaker agents set-identity --identity-file ~/.chainbreaker/workspace/IDENTITY.md --agent main",

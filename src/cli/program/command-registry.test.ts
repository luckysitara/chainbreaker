...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "chainbreaker", "doctor"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "chainbreaker", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "chainbreaker", "status"], async () => {
      await program.parseAsync(["node", "chainbreaker", "status"]);
    });

...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chainbreaker status", "Show channel health + session summary."],
          ["chainbreaker status --all", "Full diagnosis (read-only)."],
          ["chainbreaker status --json", "Machine-readable output."],
          ["chainbreaker status --usage", "Show model provider usage/quota snapshots."],
          [
            "chainbreaker status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["chainbreaker status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.chainbreaker.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.chainbreaker.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chainbreaker sessions", "List all sessions."],
          ["chainbreaker sessions --agent work", "List sessions for one agent."],
          ["chainbreaker sessions --all-agents", "Aggregate sessions across agents."],
          ["chainbreaker sessions --active 120", "Only last 2 hours."],
          ["chainbreaker sessions --json", "Machine-readable output."],
          ["chainbreaker sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.chainbreaker.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chainbreaker sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "chainbreaker sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["chainbreaker sessions cleanup --enforce", "Apply maintenance now."],
          ["chainbreaker sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["chainbreaker sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "chainbreaker sessions cleanup --enforce --store ./tmp/sessions.json",

...
const EXAMPLES = [
  ["chainbreaker models --help", "Show detailed help for the models command."],
  [
    "chainbreaker channels login --verbose",
    "Link personal WhatsApp Web and show QR + connection logs.",
  ],
  [
    'chainbreaker message send --target +15555550123 --message "Hi" --json',
  ...
  [
    'chainbreaker message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env CHAINBREAKER_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.chainbreaker-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates CHAINBREAKER_STATE_DIR/CHAINBREAKER_CONFIG_PATH under ~/.chainbreaker-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.chainbreaker.ai/cli");

...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "chainbreaker", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "chainbreaker", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "chainbreaker", "status"]);
  });

...
beforeEach(() => {
  ...
  originalHideBanner = process.env.CHAINBREAKER_HIDE_BANNER;
  ...
  delete process.env.CHAINBREAKER_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.CHAINBREAKER_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("chainbreaker");
    program
      .command("status")
      .option("--json")
      .action(() => {});
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "chainbreaker", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("chainbreaker-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "chainbreaker", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "chainbreaker", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "channels", "add"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    ...
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "plugins", "install", MATRIX_REPO_INSTALL_SPEC],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "chainbreaker", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.CHAINBREAKER_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "chainbreaker", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "chainbreaker", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "agents", "list", "--json"],
    });
    ...
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "chainbreaker", "agents", "list"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "chainbreaker", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "chainbreaker", "backup", "create", "--json"],
    });

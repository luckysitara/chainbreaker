...
const mocks = vi.hoisted(() => {
  ...
  return {
    runtime,
    serveChainbreakerChannelMcp: vi.fn(),
  };
});
...
const serveChainbreakerChannelMcp = mocks.serveChainbreakerChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveChainbreakerChannelMcp: mocks.serveChainbreakerChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("chainbreaker-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("chainbreaker-cli-mcp-home-", async () => {
      ...
      expect(serveChainbreakerChannelMcp).toHaveBeenCalledWith({

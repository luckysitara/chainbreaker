...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "chainbreaker";
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.chainbreaker.ai/cli/plugins")}\n`,
    );
      ...
      if (!opts.verbose) {
        ...
        const rows = list.map((plugin) => {
          ...
          return {
            ...
            Format: plugin.format ?? "chainbreaker",
            Status:
              plugin.status === "loaded"
                ? theme.success("loaded")
                : plugin.status === "disabled"
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      const cfg = (snapshot.sourceConfig ?? snapshot.config) as ChainbreakerConfig;
      const enableResult = enablePluginInConfig(cfg, id);
      let next: ChainbreakerConfig = enableResult.config;

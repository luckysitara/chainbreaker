...
export function resolveMissingBrowserCommandMessage(config?: ChainbreakerConfig): string | null {
  const allow =
  ...
  if (allow.length > 0 && !allow.includes("browser")) {
    return (
      'The `chainbreaker browser` command is unavailable because `plugins.allow` excludes "browser". ' +
      'Add "browser" to `plugins.allow` if you want the bundled browser CLI and tool.'
    );
  }
  if (config?.plugins?.entries?.browser?.enabled === false) {
    return (
      "The `chainbreaker browser` command is unavailable because `plugins.entries.browser.enabled=false`. " +
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? process.env.CHAINBREAKER_CONTAINER?.trim() ?? null;

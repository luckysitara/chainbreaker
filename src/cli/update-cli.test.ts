...
import type { ChainbreakerConfig, ConfigFileSnapshot } from "../config/types.chainbreaker.js";
...
vi.mock("../infra/chainbreaker-root.js", () => ({
  resolveChainbreakerPackageRoot: vi.fn(),
}));
...
const { resolveChainbreakerPackageRoot } = await import("../infra/chainbreaker-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/chainbreaker-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as ChainbreakerConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/chainbreaker-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveChainbreakerPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("chainbreaker-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("chainbreaker-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveChainbreakerPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("chainbreaker");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/chainbreaker-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "chainbreaker-gateway" }],
      hints: [],
    });
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("Chainbreaker update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "chainbreaker", "update", "status", "--json"]);
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ChainbreakerConfig,
        });
      },
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    mockPackageInstallStatus(createCaseDir("chainbreaker-update"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "latest",
      version: "2026.3.23-2",
      nodeEngine: ">=22.14.0",
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g chainbreaker` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    {
      ...
      expectedSpec: "chainbreaker@next",
    },
    {
      name: "main shorthand",
      run: async () => {
      ...
      expectedSpec: "github:chainbreaker/chainbreaker#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("chainbreaker-update"));
        await updateCommand({ yes: true, tag: "github:chainbreaker/chainbreaker#main" });
      },
      expectedSpec: "github:chainbreaker/chainbreaker#main",
    },
    {
      name: "CHAINBREAKER_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("chainbreaker-update"));
        await withEnvAsync(
          { CHAINBREAKER_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/chainbreaker-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/chainbreaker-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "chainbreaker");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "chainbreaker", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("chainbreaker-update");
    const localAppData = createCaseDir("chainbreaker-localappdata");
        ...
        await withEnvAsync(
          {
            ...
            CHAINBREAKER_CONFIG_PATH: "./config/chainbreaker.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            CHAINBREAKER_STATE_DIR: path.resolve("./state"),
            CHAINBREAKER_CONFIG_PATH: path.resolve("./config/chainbreaker.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            CHAINBREAKER_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ CHAINBREAKER_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.CHAINBREAKER_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("chainbreaker-update-wizard");
    await withEnvAsync({ CHAINBREAKER_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/chainbreaker as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ CHAINBREAKER_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "chainbreaker"));
    });

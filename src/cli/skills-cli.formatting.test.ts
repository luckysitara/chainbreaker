...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-bundled-skills-test-"));
    process.env.CHAINBREAKER_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

...
vi.mock("../infra/path-env.js", () => ({
  ensureChainbreakerCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "chainbreaker", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "chainbreaker", "--help"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "chainbreaker", "--help"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "chainbreaker", "--container", "demo", "status"]);

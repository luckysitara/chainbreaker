...
vi.mock("../plugins/loader.js", () => ({
  loadChainbreakerPlugins: mocks.loadChainbreakerPlugins,
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("reloads when escalating from configured-channels to channels", async () => {
    ...
    expect(mocks.loadChainbreakerPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadChainbreakerPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not treat a pre-seeded partial registry as all scope", async () => {
    ...
    expect(mocks.loadChainbreakerPlugins).toHaveBeenCalledTimes(1);
    expect(mocks.loadChainbreakerPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config,
        throwOnLoadError: true,

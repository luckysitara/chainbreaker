...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("chainbreaker-daemon-install-int-");
    configPath = path.join(tempHome, "chainbreaker.json");
    process.env.HOME = tempHome;
    process.env.CHAINBREAKER_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.CHAINBREAKER_GATEWAY_TOKEN).toBeUndefined();
  });
});

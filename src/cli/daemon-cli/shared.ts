...
export function renderRuntimeHints(
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.CHAINBREAKER_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.CHAINBREAKER_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const hints = buildPlatformServiceStartHints({
    installCommand: formatCliCommand("chainbreaker gateway install", env),
    startCommand: formatCliCommand("chainbreaker gateway", env),

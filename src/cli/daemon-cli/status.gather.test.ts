...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    CHAINBREAKER_STATE_DIR: "/tmp/chainbreaker-daemon",
    CHAINBREAKER_CONFIG_PATH: "/tmp/chainbreaker-daemon/chainbreaker.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.CHAINBREAKER_STATE_DIR ?? "/tmp/chainbreaker-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.CHAINBREAKER_CONFIG_PATH ?? `${stateDir}/chainbreaker.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/chainbreaker-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CHAINBREAKER_STATE_DIR = "/tmp/chainbreaker-cli";
    process.env.CHAINBREAKER_CONFIG_PATH = "/tmp/chainbreaker-cli/chainbreaker.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/chainbreaker-daemon/chainbreaker.json",
      }),
    );
  });
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.CHAINBREAKER_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.CHAINBREAKER_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.CHAINBREAKER_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "chainbreaker-gateway" }],
        hints: [],
      },
      healthy: false,

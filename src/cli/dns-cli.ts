...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. chainbreaker.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($CHAINBREAKER_CONFIG_PATH, default ~/.chainbreaker/chainbreaker.json):",
        ),
      );
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by chainbreaker dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active Chainbreaker config ($CHAINBREAKER_CONFIG_PATH, default ~/.chainbreaker/chainbreaker.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });

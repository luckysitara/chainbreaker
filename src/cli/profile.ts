...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.chainbreaker${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.CHAINBREAKER_PROFILE = profile;

  const stateDir = env.CHAINBREAKER_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.CHAINBREAKER_STATE_DIR?.trim()) {
    env.CHAINBREAKER_STATE_DIR = stateDir;
  }

  if (!env.CHAINBREAKER_CONFIG_PATH?.trim()) {
    env.CHAINBREAKER_CONFIG_PATH = path.join(stateDir, "chainbreaker.json");
  }

  if (profile === "dev" && !env.CHAINBREAKER_GATEWAY_PORT?.trim()) {
    env.CHAINBREAKER_GATEWAY_PORT = "19001";
  }
}

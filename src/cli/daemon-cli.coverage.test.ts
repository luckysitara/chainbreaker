...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      CHAINBREAKER_GATEWAY_PORT: String(params.port),
      ...(params.token ? { CHAINBREAKER_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CHAINBREAKER_STATE_DIR = "/tmp/chainbreaker-cli-state";
    process.env.CHAINBREAKER_CONFIG_PATH = "/tmp/chainbreaker-cli-state/chainbreaker.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        CHAINBREAKER_STATE_DIR: "/tmp/chainbreaker-daemon-state",
        CHAINBREAKER_CONFIG_PATH: "/tmp/chainbreaker-daemon-state/chainbreaker.json",
        CHAINBREAKER_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.chainbreaker.gateway.plist",
    });

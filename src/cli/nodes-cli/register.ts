    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chainbreaker nodes status", "List known nodes with live status."],
          ["chainbreaker nodes pairing pending", "Show pending node pairing requests."],
          [
            'chainbreaker nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["chainbreaker nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.chainbreaker.ai/cli/nodes")}\n`,
    );

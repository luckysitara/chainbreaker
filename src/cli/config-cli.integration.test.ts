...
async function withExecDryRunConfigHarness(
  ...
  const configPath = path.join(tempDir, "chainbreaker.json");
  const batchPath = path.join(tempDir, "batch.json");
  const markerPath = path.join(tempDir, "marker.txt");
  const envSnapshot = captureEnv(["CHAINBREAKER_CONFIG_PATH", "CHAINBREAKER_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
...
describe("config cli integration", () => {
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-config-cli-int-"));
    const configPath = path.join(tempDir, "chainbreaker.json");
    const batchPath = path.join(tempDir, "batch.json");
    const envSnapshot = captureEnv([
      "CHAINBREAKER_CONFIG_PATH",
      "CHAINBREAKER_TEST_FAST",
      "DISCORD_BOT_TOKEN",
    ]);
    try {
      fs.writeFileSync(
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "chainbreaker.json");
    const envSnapshot = captureEnv([
      "CHAINBREAKER_CONFIG_PATH",
      "CHAINBREAKER_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    await withExecDryRunConfigHarness("chainbreaker-config-cli-int-exec-skip-", async (params) => {
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    await withExecDryRunConfigHarness("chainbreaker-config-cli-int-exec-allow-", async (params) => {
      const before = fs.readFileSync(params.configPath, "utf8");
      await runConfigSet({
        cliOptions: {
          batchFile: params.batchPath,

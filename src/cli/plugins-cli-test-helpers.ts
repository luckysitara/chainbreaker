...
export const loadConfig = vi.fn<() => ChainbreakerConfig>(() => ({}) as ChainbreakerConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: ChainbreakerConfig) => Promise<void>>(
  async () => undefined,
);
export const replaceConfigFile = vi.fn(
  async (params: { nextConfig: ChainbreakerConfig }) => await writeConfigFile(params.nextConfig),
);
export const resolveStateDir = vi.fn(() => "/tmp/chainbreaker-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: ChainbreakerConfig) => writeConfigFile(config),
  replaceConfigFile: (params: { nextConfig: ChainbreakerConfig }) => replaceConfigFile(params),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as ChainbreakerConfig);
  readConfigFileSnapshot.mockImplementation(async () => {
    const config = loadConfig();
    return {
      path: "/tmp/chainbreaker-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  resolveStateDir.mockReturnValue("/tmp/chainbreaker-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: ChainbreakerConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: ChainbreakerConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: ChainbreakerConfig }) => ({
    config,
    warnings: [],
  }));
  ...
  recordHookInstall.mockImplementation((cfg: ChainbreakerConfig) => cfg);
}

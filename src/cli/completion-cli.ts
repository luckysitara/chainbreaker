...
function updateCompletionProfile(
  ...
  const block = `# Chainbreaker Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(chainbreaker completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.chainbreaker.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $CHAINBREAKER_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "chainbreaker") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";

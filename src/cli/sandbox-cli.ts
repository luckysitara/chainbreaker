...
const SANDBOX_EXAMPLES = {
  main: [
    ["chainbreaker sandbox list", "List all sandbox containers."],
    ...
    ["chainbreaker sandbox recreate --agent mybot", "Recreate agent containers."],
    ["chainbreaker sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["chainbreaker sandbox list", "List all sandbox containers."],
    ["chainbreaker sandbox list --browser", "List only browser containers."],
  ...
  explain: [
    ["chainbreaker sandbox explain", "Show effective sandbox config."],
    ["chainbreaker sandbox explain --session agent:main:main", "Explain a specific session."],
    ["chainbreaker sandbox explain --agent work", "Explain an agent sandbox."],
    ["chainbreaker sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.chainbreaker.ai/cli/sandbox")}\n`,
    )
    .action(() => {

      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["chainbreaker gateway run", "Run the gateway in the foreground."],
            ["chainbreaker gateway status", "Show service status and probe reachability."],
            ["chainbreaker gateway discover", "Find local and wide-area gateway beacons."],
            ["chainbreaker gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.chainbreaker.ai/cli/gateway")}\n`,
      ),
  );

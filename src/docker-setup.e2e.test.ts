...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "chainbreaker-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    CHAINBREAKER_GATEWAY_TOKEN: "test-token",
    CHAINBREAKER_CONFIG_DIR: join(sandbox.rootDir, "config"),
    CHAINBREAKER_WORKSPACE_DIR: join(sandbox.rootDir, "chainbreaker"),
  };
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    CHAINBREAKER_GATEWAY_TOKEN: undefined,
    CHAINBREAKER_CONFIG_DIR: configDir,
    CHAINBREAKER_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      CHAINBREAKER_EXTRA_MOUNTS: undefined,
      CHAINBREAKER_HOME_VOLUME: "chainbreaker-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CHAINBREAKER_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("CHAINBREAKER_EXTRA_MOUNTS=");
    expect(envFile).toContain("CHAINBREAKER_HOME_VOLUME=chainbreaker-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("chainbreaker-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("chainbreaker-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg CHAINBREAKER_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node chainbreaker-gateway dist/index.js onboard --mode local --no-install-daemon",
    );
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node chainbreaker-gateway dist/index.js config set gateway.mode local",
    );
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node chainbreaker-gateway dist/index.js config set gateway.bind lan",
    );
    expect(log).toContain(
      'run --rm --no-deps --entrypoint node chainbreaker-gateway dist/index.js config set gateway.controlUi.allowedOrigins ["http://localhost:18789","http://127.0.0.1:18789"] --strict-json',
    );
    expect(log).not.toContain("run --rm chainbreaker-cli onboard --mode local --no-install-daemon");
  });

  it("avoids shared-network chainbreaker-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
  ...
  it("writes CHAINBREAKER_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CHAINBREAKER_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh chainbreaker-gateway -c");
  });

  it("reuses existing config token when CHAINBREAKER_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      ...
      async (configDir) => {
        await writeFile(
          join(configDir, "chainbreaker.json"),
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("CHAINBREAKER_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when CHAINBREAKER_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "CHAINBREAKER_GATEWAY_TOKEN=dotenv-token-123\nCHAINBREAKER_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("CHAINBREAKER_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "CHAINBREAKER_GATEWAY_TOKEN=first-token",
        "CHAINBREAKER_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("CHAINBREAKER_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("CHAINBREAKER_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats CHAINBREAKER_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_SANDBOX: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CHAINBREAKER_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg CHAINBREAKER_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg CHAINBREAKER_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker chainbreaker-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        CHAINBREAKER_SANDBOX: "1",
        CHAINBREAKER_DOCKER_SOCKET: socketPath,
      ...
      expect(log).toContain(
        "run --rm --no-deps chainbreaker-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
  ...
  it("rejects injected multiline CHAINBREAKER_EXTRA_MOUNTS values", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("CHAINBREAKER_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid CHAINBREAKER_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid CHAINBREAKER_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("CHAINBREAKER_HOME_VOLUME must match");
  });

  it("rejects CHAINBREAKER_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      CHAINBREAKER_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("CHAINBREAKER_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:chainbreaker-gateway"');
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{CHAINBREAKER_TZ:-UTC\}/g)).toHaveLength(2);
  });
});

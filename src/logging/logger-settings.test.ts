...
let originalChainbreakerLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.CHAINBREAKER_TEST_FILE_LOG;
  originalChainbreakerLogLevel = process.env.CHAINBREAKER_LOG_LEVEL;
  delete process.env.CHAINBREAKER_TEST_FILE_LOG;
  delete process.env.CHAINBREAKER_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.CHAINBREAKER_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalChainbreakerLogLevel === undefined) {
    delete process.env.CHAINBREAKER_LOG_LEVEL;
  } else {
    process.env.CHAINBREAKER_LOG_LEVEL = originalChainbreakerLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();

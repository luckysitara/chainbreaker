...
const testLogPath = path.join(os.tmpdir(), "chainbreaker-test-env-log-level.log");
const defaultMaxFileBytes = 500 * 1024 * 1024;

describe("CHAINBREAKER_LOG_LEVEL", () => {
  let originalEnv: string | undefined;

  beforeEach(() => {
    originalEnv = process.env.CHAINBREAKER_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.CHAINBREAKER_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.CHAINBREAKER_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.CHAINBREAKER_LOG_LEVEL = "nope";

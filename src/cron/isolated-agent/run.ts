...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.CHAINBREAKER_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.chainbreaker/agents/<agentId>/agent/).
  const agentId = normalizedRequested ?? defaultAgentId;
  const agentCfg = buildCronAgentDefaultsConfig({
    defaults: params.cfg.agents?.defaults,
    agentConfigOverride,
  });
  const cfgWithAgentDefaults: ChainbreakerConfig = {
    ...params.cfg,
  ...
  try {
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: async (providerOverride, modelOverride, runOptions) => {
          ...
          if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
            ...
            // the configured timeoutSeconds. See: https://github.com/chainbreaker/chainbreaker/issues/29774
          ...
          const result = await runEmbeddedPiAgent({
            ...
            // inherit owner-only tooling like local `chainbreaker agent` runs.
            senderIsOwner: true,
            messageChannel,
    ...
    // See: https://github.com/chainbreaker/chainbreaker/issues/57206
    //

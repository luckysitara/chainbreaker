...
const { makeStorePath } = createCronStoreHarness({ prefix: "chainbreaker-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses CHAINBREAKER_HOME for tilde expansion", () => {
    vi.stubEnv("CHAINBREAKER_HOME", "/srv/chainbreaker-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/chainbreaker-home", "cron", "jobs.json"));
  });
});

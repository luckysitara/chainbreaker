...
describe("cron run log", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("chainbreaker-cron-log-perms-", async (dir) => {
        const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("chainbreaker-cron-log-dir-perms-", async (dir) => {
  ...
  it("reads newest entries and filters by jobId", async () => {
    await withRunLogDir("chainbreaker-cron-log-read-", async (dir) => {
  ...
  it("reads telemetry fields", async () => {
    await withRunLogDir("chainbreaker-cron-log-telemetry-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("cleans up pending-write bookkeeping after appends complete", async () => {
    await withRunLogDir("chainbreaker-cron-log-pending-", async (dir) => {
  ...
  it("read drains pending fire-and-forget writes", async () => {
    await withRunLogDir("chainbreaker-cron-log-drain-", async (dir) => {

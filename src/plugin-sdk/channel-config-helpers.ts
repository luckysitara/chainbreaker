...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`chainbreaker pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`chainbreaker pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends ChainbreakerConfig = ChainbreakerConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: ChainbreakerConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends ChainbreakerConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends ChainbreakerConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends ChainbreakerConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {

...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.CHAINBREAKER_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function onDiagnosticEvent(listener) {
  ...
  if (!diagnosticEvents || typeof diagnosticEvents.onDiagnosticEvent !== "function") {
    throw new Error("chainbreaker/plugin-sdk root alias could not resolve onDiagnosticEvent");
  }
...
function buildPluginSdkAliasMap(useDist) {
  ...
  const aliasMap = {
    "chainbreaker/plugin-sdk": __filename,
  };
...
function getJiti(tryNative) {
  ...
  const jitiLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled Chainbreaker core graph.
    tryNative,

...
const shouldWarnCompatImport =
  ...
  process.env.CHAINBREAKER_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING !== "1";

if (shouldWarnCompatImport) {
  process.emitWarning(
    "chainbreaker/plugin-sdk/compat is deprecated for new plugins. Migrate to focused chainbreaker/plugin-sdk/<subpath> imports. See https://docs.chainbreaker.ai/plugins/sdk-migration",
    {
      code: "CHAINBREAKER_PLUGIN_SDK_COMPAT_DEPRECATED",
      detail:
        "Bundled plugins must use scoped plugin-sdk subpaths. External plugins may keep compat temporarily while migrating. Migration guide: https://docs.chainbreaker.ai/plugins/sdk-migration",
    },
  );
}

...
const originalBundledPluginsDir = process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
...
describe("plugin-sdk facade runtime", () => {
  it("honors bundled plugin dir overrides outside the package root", () => {
    const overrideA = createBundledPluginDir("chainbreaker-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("chainbreaker-facade-runtime-b-", "override-b");

    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = overrideA;
    ...
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = overrideB;
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("chainbreaker-facade-identity-", "identity-check");
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = dir;
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createCircularPluginDir("chainbreaker-facade-circular-");
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = dir;
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("chainbreaker-facade-throw-");
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = dir;

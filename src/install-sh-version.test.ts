...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-install-sh-"));
  const cliPath = path.join(root, "chainbreaker");
...
CHAINBREAKER_BIN="$FAKE_CHAINBREAKER_BIN"
resolve_chainbreaker_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_CHAINBREAKER_BIN: cliPath,
        CHAINBREAKER_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Chainbreaker 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Chainbreaker dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Chainbreaker dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-install-stdin-"));

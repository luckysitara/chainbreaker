...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        CHAINBREAKER_BASE_ENV: "base",
        CHAINBREAKER_TO_REMOVE: undefined,
      },
      env: {
        CHAINBREAKER_TEST_ENV: "ok",
      },
    });

    expect(resolved.CHAINBREAKER_BASE_ENV).toBe("base");
    expect(resolved.CHAINBREAKER_TEST_ENV).toBe("ok");
    expect(resolved.CHAINBREAKER_TO_REMOVE).toBeUndefined();
    expect(resolved.CHAINBREAKER_CLI).toBe(CHAINBREAKER_CLI_ENV_VALUE);
  });

...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "chainbreaker", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "chainbreaker", version: "2.3.4" });
  ...
  it("ignores non-chainbreaker package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers CHAINBREAKER_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        CHAINBREAKER_VERSION: "9.9.9",
        CHAINBREAKER_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale CHAINBREAKER_VERSION for compatibility checks", () => {
    const previous = process.env.CHAINBREAKER_VERSION;
    const previousService = process.env.CHAINBREAKER_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.CHAINBREAKER_VERSION = "2026.3.25";
      process.env.CHAINBREAKER_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.CHAINBREAKER_VERSION = previous;
      process.env.CHAINBREAKER_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        CHAINBREAKER_VERSION: "2026.3.99",
        CHAINBREAKER_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        ...
        CHAINBREAKER_SERVICE_VERSION: "  2.0.0  ",

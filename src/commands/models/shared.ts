...
export async function loadValidConfigOrThrow(): Promise<ChainbreakerConfig> {
...
export async function updateConfig(
  mutator: (cfg: ChainbreakerConfig) => ChainbreakerConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ChainbreakerConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ChainbreakerConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("chainbreaker agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

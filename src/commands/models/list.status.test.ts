...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveChainbreakerAgentDir: vi.fn().mockReturnValue("/tmp/chainbreaker-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/chainbreaker-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/chainbreaker-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/chainbreaker-dev/chainbreaker.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveChainbreakerAgentDir: mocks.resolveChainbreakerAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/chainbreaker-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveChainbreakerAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/chainbreaker-dev/chainbreaker.json");
    expect(payload.auth.storePath).toBe("/tmp/chainbreaker-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/chainbreaker-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/chainbreaker-agent-custom");

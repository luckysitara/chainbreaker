...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: ChainbreakerConfig;
  let lastUpdatedConfig: ChainbreakerConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/chainbreaker/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/chainbreaker/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/chainbreaker/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: ChainbreakerConfig) => ChainbreakerConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/chainbreaker/agents/main",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with chainbreaker configure --section web (recommended mode: cached). Docs: https://docs.chainbreaker.ai/tools/web",
    );
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `chainbreaker plugins list --json`.',
    );
  });

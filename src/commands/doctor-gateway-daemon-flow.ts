...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.CHAINBREAKER_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("chainbreaker gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }

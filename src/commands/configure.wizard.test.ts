...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.chainbreaker/chainbreaker.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.chainbreaker/workspace",
  applyWizardMetadata: (cfg: ChainbreakerConfig) => cfg,
...
function setupBaseWizardState(config: ChainbreakerConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: ChainbreakerConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: ChainbreakerConfig) =>
  ...
  it("still supports keyless web search providers through the shared setup flow", async () => {
    ...
    mocks.resolveSearchProviderOptions.mockReturnValue([
      createSearchProviderOption({
        ...
        docsUrl: "https://docs.chainbreaker.ai/tools/web",
        credentialPath: "",
      }),
    ]);
    mocks.setupSearch.mockImplementation(async (cfg: ChainbreakerConfig) =>

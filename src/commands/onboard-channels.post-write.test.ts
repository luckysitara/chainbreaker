    ...
    createTestRegistry([
      {
        ...
        plugin: {
          ...
          setup: {
            applyAccountConfig: ({ cfg }: { cfg: ChainbreakerConfig }) => cfg,
          },
          setupWizard: {
            channel: "telegram",
            status: {
...
describe("setupChannels post-write hooks", () => {
  ...
  it("collects onboarding post-write hooks and runs them against the final config", async () => {
    ...
    const restore = patchChannelOnboardingAdapterForTest({
      configureInteractive,
      afterConfigWritten,
      getStatus: vi.fn(async ({ cfg }: { cfg: ChainbreakerConfig }) => ({
        channel: "telegram",
    ...
    try {
      const cfg = await setupChannels({} as ChainbreakerConfig, runtime, prompter, {
      ...
      expect(afterConfigWritten).toHaveBeenCalledWith({
        previousCfg: {} as ChainbreakerConfig,
        cfg,
        accountId: "acct-1",
        runtime,
      });

...
describe("readServiceStatusSummary", () => {
  it("marks Chainbreaker-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["chainbreaker", "gateway", "run"] })),
    ...
    expect(summary.managedByChainbreaker).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByChainbreaker).toBe(false);
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["chainbreaker", "gateway", "run", "--port", "18789"],
          environment: { CHAINBREAKER_GATEWAY_PORT: "18789" },
        })),
        readRuntime,
      }),
      "Daemon",

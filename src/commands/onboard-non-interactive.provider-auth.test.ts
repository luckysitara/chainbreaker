...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "chainbreaker.json");
  ...
  try {
    await withEnvAsync(
      {
        HOME: tempHome,
        CHAINBREAKER_STATE_DIR: tempHome,
        CHAINBREAKER_CONFIG_PATH: configPath,
        CHAINBREAKER_SKIP_CHANNELS: "1",
        CHAINBREAKER_SKIP_GMAIL_WATCHER: "1",
        CHAINBREAKER_SKIP_CRON: "1",
        CHAINBREAKER_SKIP_CANVAS_HOST: "1",
        CHAINBREAKER_GATEWAY_TOKEN: undefined,
        CHAINBREAKER_GATEWAY_PASSWORD: undefined,
        CUSTOM_API_KEY: undefined,
        CHAINBREAKER_DISABLE_CONFIG_CACHE: "1",
        CHAINBREAKER_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
        CHAINBREAKER_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
      },
      async () => {
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores xAI API key in the default auth profile", async () => {
    await withOnboardEnv("chainbreaker-onboard-xai-", async (env) => {
  ...
  it("infers Mistral auth choice from --mistral-api-key", async () => {
    await withOnboardEnv("chainbreaker-onboard-mistral-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Volcano Engine API key and sets default model", async () => {
    await withOnboardEnv("chainbreaker-onboard-volcengine-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers BytePlus auth choice from --byteplus-api-key and sets default model", async () => {
    await withOnboardEnv("chainbreaker-onboard-byteplus-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("chainbreaker-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("chainbreaker-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("chainbreaker-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      name: "anthropic",
      prefix: "chainbreaker-onboard-ref-flag-anthropic-",
      authChoice: "apiKey",
      optionKey: "anthropicApiKey",
      flagName: "--anthropic-api-key",
      envVar: "ANTHROPIC_API_KEY",
    ...
    {
      name: "openai",
      prefix: "chainbreaker-onboard-ref-flag-openai-",
      authChoice: "openai-api-key",
      optionKey: "openaiApiKey",
      flagName: "--openai-api-key",
      envVar: "OPENAI_API_KEY",
    ...
    {
      name: "openrouter",
      prefix: "chainbreaker-onboard-ref-flag-openrouter-",
    ...
    {
      name: "xai",
      prefix: "chainbreaker-onboard-ref-flag-xai-",
      authChoice: "xai-api-key",
      optionKey: "xaiApiKey",
      flagName: "--xai-api-key",
      envVar: "XAI_API_KEY",
    ...
    {
      name: "volcengine",
      prefix: "chainbreaker-onboard-ref-flag-volcengine-",
      authChoice: "volcengine-api-key",
      optionKey: "volcengineApiKey",
      flagName: "--volcengine-api-key",
      envVar: "VOLCANO_ENGINE_API_KEY",
    ...
    {
      name: "byteplus",
      prefix: "chainbreaker-onboard-ref-flag-byteplus-",
  ...
  it("stores the detected env alias as keyRef for both OpenCode runtime providers", async () => {
    await withOnboardEnv("chainbreaker-onboard-ref-opencode-alias-", async ({ runtime }) => {
      await withEnvAsync(
        {
  ...
  it("configures vLLM via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("chainbreaker-onboard-vllm-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "vllm",
  ...
  it("configures SGLang via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("chainbreaker-onboard-sglang-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "sglang",
  ...
  it("stores LiteLLM API key in the default auth profile", async () => {
    await withOnboardEnv("chainbreaker-onboard-litellm-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      ...
      prefix: "chainbreaker-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "chainbreaker-onboard-cf-gateway-infer-",
      options: {},
    },
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("chainbreaker-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("chainbreaker-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers Model Studio auth choice from --modelstudio-api-key and sets default model", async () => {
    await withOnboardEnv("chainbreaker-onboard-modelstudio-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "chainbreaker-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "chainbreaker-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("stores CUSTOM_API_KEY env ref for non-interactive custom provider auth in ref mode", async () => {
    await withOnboardEnv(
      "chainbreaker-onboard-custom-provider-env-ref-",
      async ({ configPath, runtime }) => {
        process.env.CUSTOM_API_KEY = "custom-env-key"; // pragma: allowlist secret
        await runCustomLocalNonInteractive(runtime, {
          secretInputMode: "ref", // pragma: allowlist secret
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "chainbreaker-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
        upsertAuthProfile({
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "chainbreaker-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "chainbreaker-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveSetupWithDefaults(runtime, {
            customApiKey: "custom-test-key", // pragma: allowlist secret

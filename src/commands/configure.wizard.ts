...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.CHAINBREAKER_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.CHAINBREAKER_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from chainbreaker.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<ChainbreakerConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Chainbreaker update wizard" : "Chainbreaker configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: ChainbreakerConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.chainbreaker.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("chainbreaker doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.CHAINBREAKER_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.CHAINBREAKER_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });

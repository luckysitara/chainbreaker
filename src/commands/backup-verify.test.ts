...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-chainbreaker-backup";
...
describe("backupVerifyCommand", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".chainbreaker"), { recursive: true });
    delete process.env.CHAINBREAKER_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("chainbreaker-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    ...
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-backup-verify-out-"));
    try {
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-chainbreaker-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.chainbreaker",
            archivePath: `${rootName}/payload/posix/tmp/.chainbreaker`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "chainbreaker-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "chainbreaker-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".chainbreaker");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.chainbreaker/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "chainbreaker-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "chainbreaker-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [

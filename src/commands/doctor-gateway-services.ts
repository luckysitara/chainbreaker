...
import { writeConfigFile, type ChainbreakerConfig } from "../config/config.js";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service CHAINBREAKER_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (
    ...
    const nextCfg: ChainbreakerConfig = {
      ...cfg,
      gateway: {
        ...cfg.gateway,
        auth: {

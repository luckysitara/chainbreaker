...
type EnvSnapshot = {
  HOME?: string;
  CHAINBREAKER_HOME?: string;
  CHAINBREAKER_STATE_DIR?: string;
  CHAINBREAKER_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    CHAINBREAKER_HOME: process.env.CHAINBREAKER_HOME,
    CHAINBREAKER_STATE_DIR: process.env.CHAINBREAKER_STATE_DIR,
    CHAINBREAKER_OAUTH_DIR: process.env.CHAINBREAKER_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: ChainbreakerConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: ChainbreakerConfig) {
...
async function runStateIntegrityText(cfg: ChainbreakerConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.CHAINBREAKER_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when CHAINBREAKER_OAUTH_DIR is explicitly configured", async () => {
    process.env.CHAINBREAKER_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: ChainbreakerConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints chainbreaker-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: ChainbreakerConfig = {};
    ...
    expect(text).toMatch(/chainbreaker sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/chainbreaker sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /chainbreaker sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );

...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "chainbreaker-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("chainbreaker plugins install @chainbreaker/matrix");
    expect(warnings[2]).toContain("chainbreaker plugins install ");
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), "chainbreaker-matrix-stale-cleanup-test-" + Date.now());
    await fs.rm(missingPath, { recursive: true, force: true });

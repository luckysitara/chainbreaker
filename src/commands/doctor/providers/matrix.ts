  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "chainbreaker doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: ChainbreakerConfig): Promise<string[]> {
...
function hasConfiguredMatrixChannel(cfg: ChainbreakerConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: ChainbreakerConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: ChainbreakerConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export async function applyMatrixDoctorRepair(params: {
  ...
  if (actionableMatrixMigration) {
    try {
      ...
      warnings.push(
        '- Skipping Matrix migration changes for now. Resolve the snapshot failure, then rerun "chainbreaker doctor --fix".',
      );
    }

...
const ensureChainbreakerModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveChainbreakerAgentDir = vi.fn().mockReturnValue("/tmp/chainbreaker-agent");
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    STATE_DIR: "/tmp/chainbreaker-state",
    loadConfig,
...
beforeEach(() => {
  ...
  ensureChainbreakerModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureChainbreakerModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureChainbreakerModelsJson).toHaveBeenCalled();
    expect(ensureChainbreakerModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });

...
beforeAll(() => {
  envSnapshot = captureEnv(["CHAINBREAKER_PROFILE"]);
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.chainbreaker.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.chainbreaker.node.plist",
    }),
  }),
}));
...
vi.mock("../infra/chainbreaker-root.js", () => ({
  resolveChainbreakerPackageRoot: vi.fn().mockResolvedValue("/tmp/chainbreaker"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/chainbreaker/pnpm-lock.yaml",
      markerPath: "/tmp/chainbreaker/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("chainbreaker --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("CHAINBREAKER_GATEWAY_TOKEN", "abcd1234", async () => {

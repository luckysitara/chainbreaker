...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ChainbreakerConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: ChainbreakerConfig) => { changed: boolean; next: ChainbreakerConfig };
  defaultModel: string;
  overrideConfig: ChainbreakerConfig;
  alreadyDefaultConfig: ChainbreakerConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: ChainbreakerConfig) => config,
      applyDefaultConfig: (config: ChainbreakerConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: ChainbreakerConfig = { agents: { defaults: {} } };

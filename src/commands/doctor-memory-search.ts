 ...
 * Runs as part of `chainbreaker doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("chainbreaker config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("chainbreaker config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("chainbreaker memory status --deep")}`,
        ]
          .filter(Boolean)
          .join("\n"),
        "Memory search",
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`chainbreaker config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("chainbreaker configure --section model")}`,
        `- To disable: ${formatCliCommand("chainbreaker config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("chainbreaker memory status --deep")}`,
      ].join("\n"),
      "Memory search",

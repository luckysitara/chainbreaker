...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "CHAINBREAKER_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "CHAINBREAKER_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under CHAINBREAKER_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("chainbreaker-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-oauth-sync-"));
    ...
    process.env.CHAINBREAKER_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-oauth-nosync-"));
    process.env.CHAINBREAKER_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside CHAINBREAKER_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-oauth-external-"));
    process.env.CHAINBREAKER_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* CHAINBREAKER_STATE_DIR

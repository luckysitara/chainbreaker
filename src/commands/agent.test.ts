...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/chainbreaker-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/chainbreaker-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "chainbreaker-agent-" });
}
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<ChainbreakerConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<ChainbreakerConfig["channels"]>["telegram"]>>,
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as ChainbreakerConfig },
    writeOptions: {},

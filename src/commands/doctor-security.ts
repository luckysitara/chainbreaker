...
import type { ChainbreakerConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: ChainbreakerConfig): string[] {
...
function resolveHostExecPolicy(params: {
  ...
  const basePath = "~/.chainbreaker/exec-approvals.json";
  const agentEntry =
...
function collectExecPolicyConflictWarnings(cfg: ChainbreakerConfig): string[] {
  const warnings: string[] = [];
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("chainbreaker approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: ChainbreakerConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: ChainbreakerConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("chainbreaker security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.chainbreaker/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("chainbreaker approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.chainbreaker.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("chainbreaker configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("chainbreaker config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("chainbreaker doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("chainbreaker config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('chainbreaker config set session.dmScope "per-channel-peer"') +

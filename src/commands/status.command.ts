...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByChainbreaker ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByChainbreaker ? "installed · " : "";
  ...
  runtime.log(theme.heading("Chainbreaker status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`chainbreaker devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("chainbreaker security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("chainbreaker security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.chainbreaker.ai/faq");
  runtime.log("Troubleshooting: https://docs.chainbreaker.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("chainbreaker status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("chainbreaker logs --follow")}`);
  if (nodeOnlyGateway) {
    runtime.log(`  Need node service?  ${formatCliCommand("chainbreaker node status")}`);
  } else if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("chainbreaker status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("chainbreaker gateway probe")}`);
  }
}

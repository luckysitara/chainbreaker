...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.CHAINBREAKER_GATEWAY_TOKEN;
    prevPassword = process.env.CHAINBREAKER_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.CHAINBREAKER_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".chainbreaker"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".chainbreaker", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as ChainbreakerConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.CHAINBREAKER_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as ChainbreakerConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "CHAINBREAKER_GATEWAY_TOKEN" },
        },
      },
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as ChainbreakerConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as ChainbreakerConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("chainbreaker approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as ChainbreakerConfig);
    const message = lastMessage();
    expect(message).toContain("[secrets]");
    expect(message).toContain("failed to resolve account");
    expect(message).toContain("Run: chainbreaker security audit --deep");
  });

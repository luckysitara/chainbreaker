...
export const resolveChainbreakerPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadChainbreakerPlugins: () => createEmptyPluginRegistry(),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/chainbreaker.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveChainbreakerPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.CHAINBREAKER_STATE_DIR;
  originalUpdateInProgress = process.env.CHAINBREAKER_UPDATE_IN_PROGRESS;
  process.env.CHAINBREAKER_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-doctor-state-"));

...
export async function runNonInteractiveLocalSetup(params: {
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("chainbreaker gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("chainbreaker configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.chainbreaker.ai/tools/web`,
    );
  }
}

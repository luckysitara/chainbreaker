...
const originalUpdateInProgress = process.env.CHAINBREAKER_UPDATE_IN_PROGRESS;
...
async function runRepair(cfg: ChainbreakerConfig) {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      CHAINBREAKER_GATEWAY_TOKEN: "stale-token",
    },
  });
  mocks.auditGatewayServiceConfig.mockResolvedValue({
    ok: false,
    issues: [
      {
        ...
        message: "Gateway service CHAINBREAKER_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  afterEach(() => {
    ...
    if (originalUpdateInProgress === undefined) {
      ...
      process.env.CHAINBREAKER_UPDATE_IN_PROGRESS = originalUpdateInProgress;
    }
  });
  ...
  it("uses CHAINBREAKER_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ CHAINBREAKER_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      ...
      realpath: async (value: string) => {
        if (value.includes("/global/5/node_modules/chainbreaker/")) {
          return value.replace(
            "/global/5/node_modules/chainbreaker/",
            "/global/5/node_modules/.pnpm/chainbreaker@2026.3.12/node_modules/chainbreaker/",
          );
        }
        return value;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/chainbreaker/../chainbreaker/dist/index.js",
      installEntrypoint: "/opt/chainbreaker/dist/index.js",
  ...
  it("repairs entrypoint mismatch in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/chainbreaker/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/chainbreaker/dist/index.js",
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            CHAINBREAKER_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Chainbreaker gateway next.",
    );
  });
});

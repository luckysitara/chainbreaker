...
function resolveAgentName(cfg: ChainbreakerConfig, agentId: string) {
...
function resolveAgentModel(cfg: ChainbreakerConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ChainbreakerConfig): AgentSummary[] {

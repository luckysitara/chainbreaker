...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "CHAINBREAKER_GATEWAY_TOKEN",
          placeholder: "CHAINBREAKER_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like CHAINBREAKER_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Chainbreaker will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Chainbreaker trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.chainbreaker.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );

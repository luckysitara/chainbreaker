...
import { ChainbreakerSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: ChainbreakerConfig): {
  config: ChainbreakerConfig;
  removed: string[];
} {
  const parsed = ChainbreakerSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: ChainbreakerConfig): void {

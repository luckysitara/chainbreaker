...
import { type ChainbreakerConfig, createConfigIO, writeConfigFile } from "../config/config.js";
...
async function readConfigFileRaw(configPath: string): Promise<{
  exists: boolean;
  parsed: ChainbreakerConfig;
}> {
  try {
    const raw = await fs.readFile(configPath, "utf-8");
    const parsed = safeParseWithSchema(JsonRecordSchema, JSON5.parse(raw));
    return { exists: true, parsed: (parsed ?? {}) as ChainbreakerConfig };
  } catch {

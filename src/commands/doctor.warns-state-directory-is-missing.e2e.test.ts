...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CHAINBREAKER_STATE_DIR = missingDir;
  ...
  it("skips gateway auth warning when CHAINBREAKER_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.CHAINBREAKER_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.CHAINBREAKER_GATEWAY_TOKEN;
    delete process.env.CHAINBREAKER_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.CHAINBREAKER_GATEWAY_TOKEN = previousToken;
      }
    }

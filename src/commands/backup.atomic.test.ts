...
describe("backupCreateCommand atomic archive write", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("chainbreaker-backup-atomic-test-");
  });
  ...
  it("does not leave a partial final archive behind when tar creation fails", async () => {
    ...
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-backup-failure-"));
    try {
  ...
  it("does not overwrite an archive created after readiness checks complete", async () => {
    const stateDir = path.join(tempHome.home, ".chainbreaker");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-backup-race-"));

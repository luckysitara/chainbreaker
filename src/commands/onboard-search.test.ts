...
function createPerplexityConfig(apiKey: string, enabled?: boolean): ChainbreakerConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: ChainbreakerConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): ChainbreakerConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: ChainbreakerConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: ChainbreakerConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);

...
const CHAINBREAKER_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
  (entry) => entry.configFile === "chainbreaker.json",
);
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of CHAINBREAKER_COMPILED_SECRET_TARGETS) {
...
const CHAINBREAKER_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: CHAINBREAKER_COMPILED_SECRET_TARGETS,
    entriesById: CHAINBREAKER_TARGETS_BY_ID,
  });

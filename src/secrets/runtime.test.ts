...
function createOpenAiFileModelsConfig(): NonNullable<ChainbreakerConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("can skip auth-profile SecretRef resolution when includeAuthStoreRefs is false", async () => {
    const missingEnvVar = `CHAINBREAKER_MISSING_AUTH_PROFILE_SECRET_${Date.now()}`;
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-secrets-file-provider-"));
  ...
  it("does not write inherited auth stores during runtime secret activation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-secrets-runtime-"));
    ...
    const prevStateDir = process.env.CHAINBREAKER_STATE_DIR;
    ...
    try {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.CHAINBREAKER_STATE_DIR = prevStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });

...
export type ConfigureCandidate = {
  ...
  pathSegments: string[];
  label: string;
  configFile: "chainbreaker.json" | "auth-profiles.json";
...
function getSecretProviders(config: ChainbreakerConfig): Record<string, SecretProviderConfig> {
...
export function buildConfigureCandidates(config: ChainbreakerConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `chainbreaker:${candidate.path}`;
}
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredChainbreakerConfig?: ChainbreakerConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredChainbreakerConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const chainbreakerCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "chainbreaker.json" as const,
  ...
  return [...chainbreakerCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "chainbreaker secrets configure",

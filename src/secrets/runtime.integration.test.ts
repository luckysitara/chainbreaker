...
function asConfig(value: unknown): ChainbreakerConfig {
...
function createOpenAIFileRuntimeConfig(secretFile: string): ChainbreakerConfig {
  return asConfig({
    secrets: {
      providers: {
        default: {
...
describe("secrets runtime snapshot integration", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "CHAINBREAKER_DISABLE_PLUGIN_DISCOVERY_CACHE",
      "CHAINBREAKER_VERSION",
    ]);
    delete process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
    process.env.CHAINBREAKER_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
    delete process.env.CHAINBREAKER_VERSION;
  });
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    await withEnvAsync(
      {
        CHAINBREAKER_BUNDLED_PLUGINS_DIR: undefined,
        CHAINBREAKER_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
        CHAINBREAKER_VERSION: undefined,
      },
      async () => {
        ...
        expect(
          ensureAuthProfileStore("/tmp/chainbreaker-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("chainbreaker-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("chainbreaker-secrets-runtime-refresh-fail-", async (home) => {
      const { secretFile, agentDir } = await createOpenAIFileRuntimeFixture(home);
  ...
  it(
    ...
    async () => {
      await withTempHome("chainbreaker-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".chainbreaker", "chainbreaker.json"), "utf8"),
  ...
  it(
    ...
    async () => {
      await withTempHome("chainbreaker-secrets-runtime-web-reload-lkg-", async (home) => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          config: asConfig({
            tools: {
              web: {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("chainbreaker-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".chainbreaker", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".chainbreaker", "agents", "ops", "agent");

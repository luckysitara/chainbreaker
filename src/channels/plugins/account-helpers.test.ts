...
function cfg(
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as ChainbreakerConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: ChainbreakerConfig) => string[];
  input: ChainbreakerConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: ChainbreakerConfig, expected: string) {

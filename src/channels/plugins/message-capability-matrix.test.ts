...
describe("channel action capability matrix", () => {
  ...
  function getCapabilities(plugin: Pick<ChannelPlugin, "actions">, cfg: ChainbreakerConfig) {
  ...
  it("forwards Telegram action capabilities through the channel wrapper", () => {
    ...
    const result = getCapabilities(telegramPlugin, {} as ChainbreakerConfig);
    ...
    const discordResult = getCapabilities(discordPlugin, {} as ChainbreakerConfig);

...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: ChainbreakerConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: ChainbreakerConfig }>(
  collector: WarningCollector<{ cfg: ChainbreakerConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: ChainbreakerConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: ChainbreakerConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: ChainbreakerConfig },
>(
  projectCfg: (cfg: ChainbreakerConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: ChainbreakerConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: ChainbreakerConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: ChainbreakerConfig },
>(params: {
  providerConfigPresent: (cfg: ChainbreakerConfig) => boolean;

...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: ChainbreakerConfig, policy: ChannelAccessPolicy) => ChainbreakerConfig;
  resolveAllowlist: (params: { cfg: ChainbreakerConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: ChainbreakerConfig; resolved: TResolved }) => ChainbreakerConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: ChainbreakerConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: ChainbreakerConfig }) => params.cfg);

...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as ChainbreakerConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as ChainbreakerConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as ChainbreakerConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as ChainbreakerConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as ChainbreakerConfig, channel: "demo-buttons" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as ChainbreakerConfig, channel: "demo-cards" },
        "interactive",
      ),
    ).toBe(false);
    expect(
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as ChainbreakerConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as ChainbreakerConfig)).toEqual([]);

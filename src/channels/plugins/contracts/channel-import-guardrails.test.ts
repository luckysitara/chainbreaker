...
const SAME_CHANNEL_SDK_GUARDS: GuardedSource[] = [
  ...
  },
  {
    ...
    forbiddenPatterns: [/["']chainbreaker\/plugin-sdk\/account-resolution["']/],
  },
  {
...
const CHANNEL_CONFIG_SCHEMA_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']chainbreaker\/plugin-sdk\/core["']/],
  },
];
...
function expectNoCrossPluginSdkFacadeImports(file: string, imports: string[]): void {
  ...
  for (const specifier of imports) {
    if (!specifier.startsWith("chainbreaker/plugin-sdk/")) {
      continue;
    }
    const targetSubpath = specifier.slice("chainbreaker/plugin-sdk/".length);
...
describe("channel import guardrails", () => {
  ...
  it("keeps bundled extension source files off root and compat plugin-sdk imports", () => {
    for (const file of collectExtensionSourceFiles()) {
      ...
      expect(text, `${file} should not import chainbreaker/plugin-sdk root`).not.toMatch(
        /["']chainbreaker\/plugin-sdk["']/,
      );
      expect(text, `${file} should not import chainbreaker/plugin-sdk/compat`).not.toMatch(
        /["']chainbreaker\/plugin-sdk\/compat["']/,
      );
    }
  });
  ...
  it("keeps internalized extension helper surfaces behind local api barrels", () => {
    for (const extensionId of LOCAL_EXTENSION_API_BARREL_GUARDS) {
      for (const file of collectExtensionFiles(extensionId)) {
        ...
        ).not.toMatch(new RegExp(`["']chainbreaker/plugin-sdk/${extensionId}(?:["'/])`, "u"));
      }
    }
  });
});

...
type SetupContractEntry = {
  ...
  cases: Array<{
    ...
    beforeTest?: () => void;
    assertPatchedConfig?: (cfg: ChainbreakerConfig) => void;
    assertResolvedAccount?: (account: unknown, cfg: ChainbreakerConfig) => void;
  }>;
};
...
setBundledChannelRuntime("line", {
  channel: {
    line: {
      listLineAccountIds,
      resolveDefaultLineAccountId,
      resolveLineAccount: ({ cfg, accountId }: { cfg: ChainbreakerConfig; accountId?: string }) =>
...
const matrixSessionBindingStateDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "chainbreaker-matrix-session-binding-contract-"),
);

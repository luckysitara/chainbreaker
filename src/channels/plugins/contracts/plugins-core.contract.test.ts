  ...
  it.each([
    {
      ...
      setup: () => {
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-catalog-"));
        const catalogPath = path.join(dir, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@chainbreaker/demo-channel",
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-catalog-state-"));
  ...
  it.each([
    {
      ...
      setup: () => {
        const home = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-catalog-home-"));
        const catalogPath = path.join(home, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@chainbreaker/env-demo-channel",
        ...
        return {
          env: {
            ...process.env,
            CHAINBREAKER_PLUGIN_CATALOG_PATHS: "~/catalog.json",
    ...
    },
    {
      ...
      setup: () => {
        ...
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@chainbreaker/default-env-demo",

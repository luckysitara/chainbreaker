...
import type { ChainbreakerConfig } from "../../config/types.chainbreaker.js";
import {
...
describe("group policy warning builders", () => {
  ...
  it("projects cfg-only warning collector inputs", () => {
    const collect = projectConfigWarningCollector<{ cfg: ChainbreakerConfig; accountId: string }>(
  ...
  it("projects account+cfg warning collector inputs", () => {
    const collect = projectAccountConfigWarningCollector<
      ...
      { account: { accountId: string }; cfg: ChainbreakerConfig }
    >(
      (cfg: ChainbreakerConfig) => cfg.channels ?? {},

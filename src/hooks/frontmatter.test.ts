...
`;
    ...
    expect(parsed.chainbreaker.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.chainbreaker.emoji).toBe("📝");
    expect(parsed.chainbreaker.events).toEqual(["command"]);
    expect(parsed.chainbreaker.requires.config).toEqual(["workspace.dir"]);
...
describe("resolveChainbreakerMetadata", () => {
  it("extracts chainbreaker metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveChainbreakerMetadata(frontmatter);
  ...
  it("returns undefined when chainbreaker key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        chainbreaker: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Chainbreaker" },
            { id: "npm", kind: "npm", package: "@chainbreaker/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@chainbreaker/hook");
  });
    ...
    "chainbreaker":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Chainbreaker" }],
      },
  }
---
...
`;
    ...
    const chainbreaker = resolveChainbreakerMetadata(frontmatter);
    expect(chainbreaker).toBeDefined();
    expect(chainbreaker?.emoji).toBe("💾");
    expect(chainbreaker?.events).toEqual(["command:new", "command:reset"]);
    expect(chainbreaker?.requires?.config).toEqual(["workspace.dir"]);
    expect(chainbreaker?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const chainbreaker = resolveChainbreakerMetadata(frontmatter);
    expect(chainbreaker?.emoji).toBe("disk");
    expect(chainbreaker?.events).toEqual(["command:new"]);
  });
});

...
async function ensureChainbreakerHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing chainbreaker.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json chainbreaker.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensureChainbreakerHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `chainbreaker.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `chainbreaker.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "chainbreaker-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "chainbreaker-hook-pack-",
    spec,
    timeoutMs,

...
This directory contains hooks that ship with Chainbreaker. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.chainbreaker/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.chainbreaker/chainbreaker.json`:
...
2. Restart gateway: `pkill -9 -f 'chainbreaker.*gateway' && pnpm chainbreaker gateway`
3. Enable the hook: `chainbreaker hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.chainbreaker.ai/automation/hooks

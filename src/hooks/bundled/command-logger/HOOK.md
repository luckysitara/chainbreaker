...
homepage: https://docs.chainbreaker.ai/automation/hooks#command-logger
metadata:
  {
    "chainbreaker":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.chainbreaker/logs/commands.log`
...
`~/.chainbreaker/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.chainbreaker/logs/commands.log ~/.chainbreaker/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/chainbreaker`:
   ```
   /home/username/.chainbreaker/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

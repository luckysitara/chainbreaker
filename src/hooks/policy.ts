import type { ChainbreakerConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "chainbreaker-bundled": {
    ...
    canOverride: ["chainbreaker-bundled"],
    canBeOverriddenBy: ["chainbreaker-managed", "chainbreaker-plugin"],
  },
  "chainbreaker-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["chainbreaker-bundled", "chainbreaker-plugin"],
    canBeOverriddenBy: ["chainbreaker-managed"],
  },
  "chainbreaker-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["chainbreaker-bundled", "chainbreaker-managed", "chainbreaker-plugin"],
    canBeOverriddenBy: ["chainbreaker-managed"],
  },
  "chainbreaker-workspace": {
    ...
    canOverride: ["chainbreaker-workspace"],
    canBeOverriddenBy: ["chainbreaker-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "chainbreaker-plugin") {
    return { enabled: true };
  }

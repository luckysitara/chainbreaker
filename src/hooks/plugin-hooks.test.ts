...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "chainbreaker-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.CHAINBREAKER_BUNDLED_HOOKS_DIR;
    process.env.CHAINBREAKER_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      ...
      process.env.CHAINBREAKER_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  function createConfig(enabled: boolean): ChainbreakerConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("chainbreaker-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Chainbreaker hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".chainbreaker", "extensions", "claude-bundle");

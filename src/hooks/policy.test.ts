...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "chainbreaker-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "chainbreaker-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "chainbreaker-bundled");
      const plugin = makeHookEntry("shared", "chainbreaker-plugin");
      const managed = makeHookEntry("shared", "chainbreaker-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("chainbreaker-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "chainbreaker-managed");
      const workspace = makeHookEntry("shared", "chainbreaker-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "chainbreaker-workspace");
      const second = makeHookEntry("shared", "chainbreaker-workspace");
      second.hook.handlerPath = "/tmp/chainbreaker-workspace/shared/handler-2.js";

...
export function resolveBundledHooksDir(): string | undefined {
  const override = process.env.CHAINBREAKER_BUNDLED_HOOKS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  // This path works when installed via npm: node_modules/chainbreaker/dist/hooks/bundled-dir.js
  try {

...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".chainbreaker");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CHAINBREAKER_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$CHAINBREAKER_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CHAINBREAKER_STATE_DIR.

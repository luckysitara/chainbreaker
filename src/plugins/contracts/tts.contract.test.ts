...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): ChainbreakerConfig {
  return {
    messages: {
      tts: {
        provider: "openai",
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.CHAINBREAKER_TTS_PREFS;
      process.env.CHAINBREAKER_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.CHAINBREAKER_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: ChainbreakerConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,

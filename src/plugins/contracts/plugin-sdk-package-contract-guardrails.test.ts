...
const PLUGIN_SDK_SUBPATH_PATTERN = /chainbreaker\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function packChainbreakerToTempDir(packDir: string): string {
...
async function readPackedRootPackageJson(archivePath: string): Promise<{
  ...
  const extractDir = mkdtempSync(join(os.tmpdir(), "chainbreaker-packed-root-package-json-"));
  try {
    await tar.x({
      file: archivePath,
      cwd: extractDir,
...
function buildLegacyPluginSourceAlias(): string {
  return ["chainbreaker", ["plugin", "source"].join("-")].join("/") + "/";
}
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references chainbreaker/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }

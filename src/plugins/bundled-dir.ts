...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.CHAINBREAKER_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveChainbreakerPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveChainbreakerPackageRootSync({ argv1: process.argv[1] }),

...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing chainbreaker.extensions; update the plugin package to include chainbreaker.extensions (for example ["./dist/index.js"]). See https://docs.chainbreaker.ai/help/troubleshooting#plugin-install-fails-with-missing-chainbreaker-extensions';
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_CHAINBREAKER_EXTENSIONS: "missing_chainbreaker_extensions",
  EMPTY_CHAINBREAKER_EXTENSIONS: "empty_chainbreaker_extensions",
...
function ensureChainbreakerExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_CHAINBREAKER_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json chainbreaker.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_CHAINBREAKER_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    return {
      ok: false,
      error: `Bundle "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "chainbreaker security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureChainbreakerExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureChainbreakerExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from chainbreaker.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-chainbreaker"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json chainbreaker.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Chainbreaker >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set CHAINBREAKER_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Chainbreaker >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Chainbreaker and retry.`,
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "chainbreaker security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
export async function installPluginFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "chainbreaker-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromFile(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin file "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "chainbreaker security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "chainbreaker-npm-pack-",
    spec,
    timeoutMs,

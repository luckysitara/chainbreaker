...
let loadChainbreakerPluginsMock: ReturnType<typeof vi.fn>;
...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    CHAINBREAKER_HOME: "/tmp/chainbreaker-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/chainbreaker.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/chainbreaker.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectRuntimeProviderResolution(
  ...
  expect(loadChainbreakerPluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadChainbreakerPluginsMock = vi
      .spyOn(loaderModule, "loadChainbreakerPlugins")
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: (_config: { plugins?: Record<string, unknown> }, env: NodeJS.ProcessEnv) => {
        env.CHAINBREAKER_HOME = "/tmp/chainbreaker-home-b";
      },
    },
  ] as const)("$name", ({ mutate }) => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({ CHAINBREAKER_HOME: "/tmp/chainbreaker-home-a" });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        CHAINBREAKER_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadChainbreakerPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      CHAINBREAKER_PLUGIN_DISCOVERY_CACHE_MS: "5",
      CHAINBREAKER_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadChainbreakerPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      CHAINBREAKER_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.CHAINBREAKER_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });

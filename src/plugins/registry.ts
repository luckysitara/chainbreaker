...
import type {
  ...
  ChainbreakerPluginChannelRegistration,
  ChainbreakerPluginCliCommandDescriptor,
  ChainbreakerPluginCliRegistrar,
  ChainbreakerPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  ChainbreakerPluginHttpRouteAuth,
  ChainbreakerPluginHttpRouteMatch,
  ChainbreakerPluginHttpRouteHandler,
  ChainbreakerPluginHttpRouteParams,
  ChainbreakerPluginHookOptions,
...
export type PluginToolRegistration = {
  pluginId: string;
  pluginName?: string;
  factory: ChainbreakerPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
  rootDir?: string;
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: ChainbreakerPluginCliRegistrar;
  commands: string[];
...
export type PluginServiceRegistration = {
  pluginId: string;
  pluginName?: string;
  service: ChainbreakerPluginService;
  source: string;
  rootDir?: string;
};
...
export type PluginCommandRegistration = {
  pluginId: string;
  pluginName?: string;
  command: ChainbreakerPluginCommandDefinition;
  source: string;
  rootDir?: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: ChainbreakerPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: ChainbreakerPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: ChainbreakerPluginHookOptions | undefined,
    config: ChainbreakerPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: ChainbreakerPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: ChainbreakerPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as ChainbreakerPluginChannelRegistration).plugin === "object"
        ? (registration as ChainbreakerPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: ChainbreakerPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerService = (record: PluginRecord, service: ChainbreakerPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: ChainbreakerPluginCommandDefinition) => {

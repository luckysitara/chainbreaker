...
function createBundledWhatsAppRuntimeFixture() {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-whatsapp-boundary-"));
  ...
  for (const [relativePath, value] of Object.entries({
    ...
    "dist/active-listener.js": [
      'const key = Symbol.for("chainbreaker.whatsapp.activeListenerState");',
      "const g = globalThis;",
      "if (!g[key]) {",

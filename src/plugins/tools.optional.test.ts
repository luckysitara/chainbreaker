...
const loadChainbreakerPluginsMock = vi.fn();
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadChainbreakerPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
...
describe("resolvePluginTools optional tools", () => {
  beforeEach(async () => {
    vi.resetModules();
    loadChainbreakerPluginsMock.mockClear();
    resolveRuntimePluginRegistryMock.mockReset();
    resolveRuntimePluginRegistryMock.mockImplementation((params) =>
      loadChainbreakerPluginsMock(params),
    );
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { CHAINBREAKER_HOME: "/srv/chainbreaker-home" },
      },
    },
    {

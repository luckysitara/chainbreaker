...
const mocks = vi.hoisted(() => ({
  ...
  loadChainbreakerPluginCliRegistry: vi.fn(),
  loadChainbreakerPlugins: vi.fn(),
  applyPluginAutoEnable: vi.fn(),
}));

vi.mock("./loader.js", () => ({
  loadChainbreakerPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadChainbreakerPluginCliRegistry(...args),
  loadChainbreakerPlugins: (...args: unknown[]) => mocks.loadChainbreakerPlugins(...args),
}));
...
function expectPluginLoaderConfig(config: ChainbreakerConfig) {
  expect(mocks.loadChainbreakerPlugins).toHaveBeenCalledWith(
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: ChainbreakerConfig;
  autoEnabledConfig: ChainbreakerConfig;
}) {
...
describe("registerPluginCliCommands", () => {
  beforeEach(() => {
    ...
    mocks.loadChainbreakerPluginCliRegistry.mockReset();
    mocks.loadChainbreakerPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadChainbreakerPlugins.mockReset();
    mocks.loadChainbreakerPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as ChainbreakerConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as ChainbreakerConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    expect(mocks.loadChainbreakerPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
      }),
    );
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadChainbreakerPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to awaited CLI metadata collection when runtime loading ignored async registration", async () => {
    ...
    await registerPluginCliCommands(program, {} as ChainbreakerConfig, undefined, undefined, {
      mode: "lazy",
    });

    expect(mocks.loadChainbreakerPluginCliRegistry).toHaveBeenCalledTimes(1);

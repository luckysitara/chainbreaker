...
const discoverChainbreakerPluginsMock = vi.fn();
const loadPluginManifestMock = vi.fn();

vi.mock("./discovery.js", () => ({
  discoverChainbreakerPlugins: (...args: unknown[]) => discoverChainbreakerPluginsMock(...args),
}));
...
function setBundledDiscoveryCandidates(candidates: unknown[]) {
  discoverChainbreakerPluginsMock.mockReturnValue({
    candidates,
    diagnostics: [],
  });
}
  ...
  loadPluginManifestMock.mockImplementation((rootDir: string) =>
    rootDir in manifestIds
      ...
      : {
          ...
          manifestPath: `${rootDir}/chainbreaker.plugin.json`,
        },
  );
}
...
function setBundledLookupFixture() {
  setBundledDiscoveryCandidates([
    createBundledCandidate({
      ...
      packageName: "@chainbreaker/feishu",
    }),
    createBundledCandidate({
...
function createResolvedBundledSource(params: {
  ...
  return {
    ...
    npmSpec: params.npmSpec ?? `@chainbreaker/${params.pluginId}`,
  };
}
...
describe("bundled plugin sources", () => {
  beforeEach(() => {
    discoverChainbreakerPluginsMock.mockReset();
  ...
  it("resolves bundled sources keyed by plugin id", () => {
    setBundledDiscoveryCandidates([
      ...
      createBundledCandidate({
        ...
        packageName: "@chainbreaker/msteams",
      }),
    ]);
    setBundledManifestIdsByRoot({
      [appBundledPluginRoot("feishu")]: "feishu",
  ...
  it.each([
    [
      ...
      { kind: "npmSpec", value: "@chainbreaker/feishu" } as const,
      { pluginId: "feishu", localPath: appBundledPluginRoot("feishu") },
    ],
    [
      "returns undefined for missing npm spec",
      { kind: "npmSpec", value: "@chainbreaker/not-found" } as const,
      undefined,
    ],
    [
  ...
  it("forwards an explicit env to bundled discovery helpers", () => {
    ...
    const env = { HOME: "/tmp/chainbreaker-home" } as NodeJS.ProcessEnv;
    ...
    expect(discoverChainbreakerPluginsMock).toHaveBeenNthCalledWith(1, {
      workspaceDir: "/workspace",
      env,
    });
    expect(discoverChainbreakerPluginsMock).toHaveBeenNthCalledWith(2, {
      workspaceDir: "/workspace",
      env,
    });
  });
  ...
  it("reuses a pre-resolved bundled map for repeated lookups", () => {
    ...
    expect(
      findBundledPluginSourceInMap({
        bundled,
        lookup: { kind: "npmSpec", value: "@chainbreaker/feishu" },
      })?.pluginId,
    ).toBe("feishu");
  });
});

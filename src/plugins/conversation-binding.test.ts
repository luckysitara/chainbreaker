...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-plugin-binding-"));
...
vi.mock("../infra/home-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    expandHomePrefix: (value: string) => {
      if (value === "~/.chainbreaker/plugin-binding-approvals.json") {
  ...
  it.each([
    ...
    },
    {
      ...
      existingRecord: {
        ...
        targetSessionKey: "chainbreaker-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
        targetKind: "session" as const,
        conversation: {
          channel: "telegram",
          accountId: "default",

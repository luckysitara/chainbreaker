...
const excludeOptionalEnv = { CHAINBREAKER_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  params: {
    ...
    packageChainbreaker?: Record<string, unknown>;
  },
) {
  const pluginDir = path.join(repoRoot, "extensions", params.id);
  fs.mkdirSync(pluginDir, { recursive: true });
  writeJson(path.join(pluginDir, "chainbreaker.plugin.json"), {
    id: params.id,
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageChainbreaker ? { chainbreaker: params.packageChainbreaker } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string) {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "chainbreaker.plugin.json"),
      "utf8",
    ),
  ) as { skills?: string[] };
}
...
function readBundledPackageJson(repoRoot: string, pluginId: string) {
  ...
  ) as { chainbreaker?: { extensions?: string[] } };
}
...
function createTlonSkillPlugin(repoRoot: string, skillPath = "node_modules/@tloncorp/tlon-skill") {
  return createPlugin(repoRoot, {
    id: "tlon",
    packageName: "@chainbreaker/tlon",
    manifest: { skills: [skillPath] },
    packageChainbreaker: { extensions: ["./index.ts"] },
  });
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-plugin-meta-");
    const pluginDir = createPlugin(repoRoot, {
      id: "acpx",
      packageName: "@chainbreaker/acpx",
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "chainbreaker.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    expect(packageJson.chainbreaker?.extensions).toEqual(["./index.js"]);
  });

  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-plugin-node-modules-");
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-plugin-hoisted-skill-");
    const pluginDir = createTlonSkillPlugin(repoRoot);
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-plugin-missing-skill-");
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-plugin-retry-");
    const pluginDir = createPlugin(repoRoot, {
      id: "diffs",
      packageName: "@chainbreaker/diffs",
      manifest: { skills: ["./skills"] },
      packageChainbreaker: { extensions: ["./index.ts"] },
    });
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "chainbreaker.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@chainbreaker/google-gemini-cli-auth",
    });
  ...
  it.each([
    {
      ...
      pluginId: "acpx",
      packageName: "@chainbreaker/acpx-plugin",
      packageChainbreaker: { extensions: ["./index.ts"] },
    ...
    },
    {
      ...
      pluginId: "whatsapp",
      packageName: "@chainbreaker/whatsapp",
      packageChainbreaker: {
        extensions: ["./index.ts"],
        install: { npmSpec: "@chainbreaker/whatsapp" },
      },
      env: {},
      expectedExists: true,
    },
  ] as const)("$name", ({ pluginId, packageName, packageChainbreaker, env, expectedExists }) => {
    const repoRoot = makeRepoRoot(`chainbreaker-bundled-plugin-${pluginId}-`);
  ...
  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    const repoRoot = makeRepoRoot("chainbreaker-bundled-runtime-support-");
    const pluginDir = path.join(repoRoot, "extensions", "image-generation-core");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@chainbreaker/image-generation-core",
      version: "0.0.1",
      private: true,
      type: "module",
    });

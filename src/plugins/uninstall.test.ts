...
type PluginConfig = NonNullable<ChainbreakerConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: ChainbreakerConfig["channels"];
}): ChainbreakerConfig {
...
function createSinglePluginWithEmptySlotsConfig(): ChainbreakerConfig {
...
function createSingleNpmInstallConfig(installPath: string): ChainbreakerConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as ChainbreakerConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "chainbreaker-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-chainbreaker-plugin-install/my-plugin",
      },
      extensionsDir,
    });

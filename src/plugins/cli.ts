...
import type { ChainbreakerPluginCliCommandDescriptor } from "./types.js";
...
function resolvePluginCliLoadContext(cfg?: ChainbreakerConfig, env?: NodeJS.ProcessEnv) {
...
async function loadPluginCliMetadataRegistry(
  ...
  return {
    ...context,
    registry: await loadChainbreakerPluginCliRegistry({
...
async function loadPluginCliCommandRegistry(
  ...
  const context = resolvePluginCliLoadContext(cfg, env);
  const runtimeRegistry = loadChainbreakerPlugins({
    config: context.config,
    workspaceDir: context.workspaceDir,
    env,
    logger: context.logger,
  ...
  try {
    const metadataRegistry = await loadChainbreakerPluginCliRegistry({
      config: context.config,
      workspaceDir: context.workspaceDir,
      env,
      logger: context.logger,
...
export async function getPluginCliCommandDescriptors(
  cfg?: ChainbreakerConfig,
  env?: NodeJS.ProcessEnv,
): Promise<ChainbreakerPluginCliCommandDescriptor[]> {
  try {
    const { registry } = await loadPluginCliMetadataRegistry(cfg, env);
    const seen = new Set<string>();
    const descriptors: ChainbreakerPluginCliCommandDescriptor[] = [];

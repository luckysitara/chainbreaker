...
type LoadChainbreakerPlugins = typeof import("./loader.js").loadChainbreakerPlugins;
...
const loadChainbreakerPluginsMock = vi.fn<LoadChainbreakerPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/chainbreaker.plugin.json`,
  };
}
...
function getLastLoadPluginsCall(): Record<string, unknown> {
  const call = loadChainbreakerPluginsMock.mock.calls.at(-1)?.[0];
...
function createAutoEnabledProviderConfig() {
  const rawConfig: ChainbreakerConfig = {
    plugins: {},
  };
  const autoEnabledConfig: ChainbreakerConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    vi.resetModules();
    vi.doMock("./loader.js", () => ({
      loadChainbreakerPlugins: (...args: Parameters<LoadChainbreakerPlugins>) =>
        loadChainbreakerPluginsMock(...args),
    }));
  ...
  beforeEach(() => {
    loadChainbreakerPluginsMock.mockReset();
    ...
    loadChainbreakerPluginsMock.mockReturnValue(registry);

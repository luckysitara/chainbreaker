...
const CHAINBREAKER_PACKAGE_ROOT =
  resolveLoaderPackageRoot({
...
function resolvePackageChannelMeta(
  packageManifest: ChainbreakerPackageManifest | undefined,
  channelId: string,
): ChainbreakerPackageManifest["channel"] | undefined {
...
export function listBundledPluginMetadata(params?: {
  ...
  const rootDir = path.resolve(params?.rootDir ?? CHAINBREAKER_PACKAGE_ROOT);

...
const originalBundledDir = process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
...
function createChainbreakerRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "chainbreaker" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "chainbreaker-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "chainbreaker-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "chainbreaker-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "chainbreaker-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    const repoRoot = createChainbreakerRoot(layout);
    expectResolvedBundledDirFromRoot({
      repoRoot,
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createChainbreakerRoot({
          prefix: "chainbreaker-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        const cwdRepoRoot = createChainbreakerRoot({
          prefix: "chainbreaker-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "chainbreaker.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createChainbreakerRoot({
          prefix: "chainbreaker-bundled-dir-override-",
          hasDistExtensions: true,
        });
        return {
          installedRoot,

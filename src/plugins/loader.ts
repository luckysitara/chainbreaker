...
import { discoverChainbreakerPlugins } from "./discovery.js";
...
export function resolveRuntimePluginRegistry(
  ...
  return getCompatibleActivePluginRegistry(options) ?? loadChainbreakerPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ChainbreakerPluginDefinition;
  register?: ChainbreakerPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ChainbreakerPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ChainbreakerPluginDefinition;
...
function createPluginRecord(params: {
  ...
  return {
    ...
    format: params.format ?? "chainbreaker",
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.CHAINBREAKER_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverChainbreakerPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverChainbreakerPlugins>["candidates"][number];
  right: ReturnType<typeof discoverChainbreakerPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadChainbreakerPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadChainbreakerPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  const discovery = discoverChainbreakerPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Chainbreaker yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: ChainbreakerPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as ChainbreakerPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
...
export async function loadChainbreakerPluginCliRegistry(
  options: PluginLoadOptions = {},

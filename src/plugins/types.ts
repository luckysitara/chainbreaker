 ...
 */
export type ChainbreakerPluginConfigSchema = {
...
export type ChainbreakerPluginToolContext = {
  config?: ChainbreakerConfig;
  /** Active runtime-resolved config snapshot when one is available. */
  runtimeConfig?: ChainbreakerConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
  sessionKey?: string;
...
export type ChainbreakerPluginToolFactory = (
  ctx: ChainbreakerPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type ChainbreakerPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type ChainbreakerPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<ChainbreakerConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Chainbreaker uses this to expose
  ...
  ) => Promise<ChainbreakerConfig | null>;
};
 ...
 *
 * Runs after Chainbreaker resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Chainbreaker auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Chainbreaker wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Chainbreaker builds its generic
...
/**
 * Provider-owned stream wrapper hook after Chainbreaker applies its generic
 ...
 *
 * Return `true` or `false` to override Chainbreaker's built-in provider cache TTL
 ...
 *
 * Runs only after Chainbreaker exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Chainbreaker should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Chainbreaker loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Chainbreaker falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Chainbreaker calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Chainbreaker wrappers.
   *
   ...
   *
   * Called after Chainbreaker resolves the raw configured credential but before the
   ...
   * hint than Chainbreaker's generic auth-store guidance.
   */
   ...
   * Chainbreaker's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Chainbreaker should surface a provider-specific hint for
   ...
   * merging. Chainbreaker deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Chainbreaker uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `chainbreaker doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `chainbreaker doctor` should offer.
   *
   ...
   *
   * Chainbreaker calls this before falling back to the shared `pi-ai` OAuth
...
export type WebSearchProviderPlugin = {
   ...
   * when Chainbreaker has no config-aware runtime context yet.
   *
...
export type ChainbreakerPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current Chainbreaker configuration */
  config: ChainbreakerConfig;
 ...
 */
export type ChainbreakerPluginCommandDefinition = {
...
export type ChainbreakerPluginHttpRouteAuth = "gateway" | "plugin";
export type ChainbreakerPluginHttpRouteMatch = "exact" | "prefix";

export type ChainbreakerPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type ChainbreakerPluginHttpRouteParams = {
  path: string;
  handler: ChainbreakerPluginHttpRouteHandler;
  auth: ChainbreakerPluginHttpRouteAuth;
  match?: ChainbreakerPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type ChainbreakerPluginCliContext = {
  program: Command;
  config: ChainbreakerConfig;
  workspaceDir?: string;
  logger: PluginLogger;
 ...
 * If you want Chainbreaker to keep a plugin command lazy-loaded while still
 ...
 */
export type ChainbreakerPluginCliCommandDescriptor = {
  name: string;
  description: string;
...
export type ChainbreakerPluginServiceContext = {
  config: ChainbreakerConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type ChainbreakerPluginService = {
  id: string;
  start: (ctx: ChainbreakerPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: ChainbreakerPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Chainbreaker should inject bundle MCP config for this backend.
   *
...
export type ChainbreakerPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type ChainbreakerPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: ChainbreakerPluginApi) => void | Promise<void>;
  activate?: (api: ChainbreakerPluginApi) => void | Promise<void>;
};

export type ChainbreakerPluginModule =
  | ChainbreakerPluginDefinition
  | ((api: ChainbreakerPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

/** Main registration API injected into native plugin entry files. */
export type ChainbreakerPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | ChainbreakerPluginToolFactory,
    opts?: ChainbreakerPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: ChainbreakerPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: ChainbreakerPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: ChainbreakerPluginChannelRegistration | ChannelPlugin) => void;
  ...
  registerCli: (
    registrar: ChainbreakerPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Chainbreaker can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: ChainbreakerPluginCliCommandDescriptor[];
    },
  ) => void;
  registerService: (service: ChainbreakerPluginService) => void;
   ...
   */
  registerCommand: (command: ChainbreakerPluginCommandDefinition) => void;
...
export type PluginFormat = "chainbreaker" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * Chainbreaker does not await this callback before allowing or denying the tool call.
     */

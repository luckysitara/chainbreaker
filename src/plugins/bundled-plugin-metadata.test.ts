...
describe("bundled plugin metadata", () => {
  ...
  it("prefers built generated paths when present and falls back to source paths", () => {
    const tempRoot = createGeneratedPluginTempRoot("chainbreaker-bundled-plugin-metadata-");
  ...
  it("merges runtime channel schema metadata with manifest-owned channel config fields", () => {
    const tempRoot = createGeneratedPluginTempRoot("chainbreaker-bundled-plugin-channel-configs-");

    writeJson(path.join(tempRoot, "extensions", "alpha", "package.json"), {
      name: "@chainbreaker/alpha",
      version: "0.0.1",
      chainbreaker: {
        extensions: ["./index.ts"],
        channel: {
    ...
    writeJson(path.join(tempRoot, "extensions", "alpha", "chainbreaker.plugin.json"), {
      id: "alpha",
      channels: ["alpha"],
  ...
  it("captures top-level public surface artifacts without duplicating the primary entrypoints", () => {
    const tempRoot = createGeneratedPluginTempRoot("chainbreaker-bundled-plugin-public-artifacts-");
  ...
  it("loads channel config metadata from built public surfaces in dist-only roots", () => {
    const tempRoot = createGeneratedPluginTempRoot("chainbreaker-bundled-plugin-dist-config-");
    ...
    writeJson(path.join(distRoot, "extensions", "alpha", "chainbreaker.plugin.json"), {
      id: "alpha",
      configSchema: {
        type: "object",
        properties: {},

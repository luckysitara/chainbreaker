...
import type { ChainbreakerPluginService, ChainbreakerPluginServiceContext } from "./types.js";
...
function createRegistry(services: ChainbreakerPluginService[]) {
...
function expectServiceContext(
  ctx: ChainbreakerPluginServiceContext,
...
function expectServiceLogger(ctx: ChainbreakerPluginServiceContext) {
...
function expectServiceContexts(
  contexts: ChainbreakerPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: ChainbreakerPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: ChainbreakerPluginServiceContext[] = [];

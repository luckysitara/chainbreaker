...
import type { ChainbreakerPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("chainbreaker-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "chainbreaker.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "chainbreaker" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageManifest?: ChainbreakerPackageManifest;
  packageDir?: string;
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "chainbreaker.plugin.json");
  const linkedManifest = path.join(rootDir, "chainbreaker.plugin.json");
  ...
  return loadPluginManifestRegistry({
    ...
    candidates: [
      createPluginCandidate({
        ...
        packageManifest: {
          install: {
            npmSpec: "@chainbreaker/synology-chat",
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "chainbreaker.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { CHAINBREAKER_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires Chainbreaker >=2026.3.22, but this host is 2026.3.21",
      expectWarn: false,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | chainbreaker.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { CHAINBREAKER_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        CHAINBREAKER_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: hermeticEnv({
        HOME: homeB,
        CHAINBREAKER_HOME: undefined,
        CHAINBREAKER_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
  ...
  it("does not reuse cached manifests across host version changes", () => {
    ...
    const olderHost = loadPluginManifestRegistry({
      ...
      candidates,
      env: hermeticEnv({
        CHAINBREAKER_VERSION: "2026.3.21",
      }),
    });
    const newerHost = loadPluginManifestRegistry({
      ...
      candidates,
      env: hermeticEnv({
        CHAINBREAKER_VERSION: "2026.3.22",
      }),
    });

...
import {
  ...
  loadChainbreakerPluginCliRegistry,
...
type PluginLoadConfig = NonNullable<Parameters<typeof loadChainbreakerPlugins>[0]>["config"];
type PluginRegistry = ReturnType<typeof loadChainbreakerPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "chainbreaker-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "chainbreaker.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function writeBundledPlugin(params: {
  ...
  process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".chainbreaker", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ CHAINBREAKER_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
      ...
      JSON.stringify(
        {
          ...
          chainbreaker: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadChainbreakerPlugins>) {
...
function useNoBundledPlugins() {
  process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadChainbreakerPlugins>[0], "cache" | "config">,
) {
  return loadChainbreakerPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "chainbreaker.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function loadBundleFixture(params: {
  ...
  const stateDir = makeTempDir();
  const bundleRoot = path.join(workspaceDir, ".chainbreaker", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ CHAINBREAKER_STATE_DIR: stateDir, ...params.env }, () =>
    loadChainbreakerPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadChainbreakerPlugins>,
  pluginId: string,
) {
  expect(
    registry.diagnostics.some(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadChainbreakerPlugins>;
  loadSecond: () => ReturnType<typeof loadChainbreakerPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadChainbreakerPlugins>;
  loadVariant: () => ReturnType<typeof loadChainbreakerPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const chainbreakerHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(chainbreakerHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    CHAINBREAKER_HOME: chainbreakerHome,
    HOME: ignoredHome,
    CHAINBREAKER_STATE_DIR: stateDir,
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadChainbreakerPlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".chainbreaker", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ CHAINBREAKER_STATE_DIR: stateDir }, () =>
      loadChainbreakerPlugins({
        workspaceDir,
...
describe("loadChainbreakerPlugins", () => {
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadChainbreakerPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadChainbreakerPlugins(options);
        const scoped = loadChainbreakerPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadChainbreakerPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadChainbreakerPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadChainbreakerPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadChainbreakerPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadChainbreakerPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadChainbreakerPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadChainbreakerPlugins({
              ...
              env: {
                ...process.env,
                CHAINBREAKER_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadChainbreakerPlugins({
              ...
              env: {
                ...process.env,
                CHAINBREAKER_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadChainbreakerPlugins({
              ...
              env: {
                ...
                CHAINBREAKER_STATE_DIR: stateDir,
                CHAINBREAKER_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadChainbreakerPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(chainbreakerHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadChainbreakerPlugins(options),
          loadVariant: () =>
            loadChainbreakerPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadChainbreakerPlugins({
      env: {
        ...
        HOME: homeDir,
        CHAINBREAKER_HOME: undefined,
        CHAINBREAKER_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers CHAINBREAKER_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const chainbreakerHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "chainbreaker-home-demo",
      dir: path.join(chainbreakerHome, "plugins", "chainbreaker-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "chainbreaker-home-demo", register() {} };`,
    });
    ...
    const registry = loadChainbreakerPlugins({
      ...
      config: {
        plugins: {
          allow: ["chainbreaker-home-demo"],
          entries: {
            "chainbreaker-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/chainbreaker-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "chainbreaker-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadChainbreakerPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadChainbreakerPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "chainbreaker.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadChainbreakerPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@chainbreaker/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@chainbreaker/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@chainbreaker/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@chainbreaker/setup-runtime-not-preferred-test",
  ...
  register(api) {
    ...
    const registry = await loadChainbreakerPluginCliRegistry({
      config: {
        plugins: {
          load: { paths: [plugin.file] },
          allow: ["config-cli"],
  ...
  it("uses the real channel entry in cli-metadata mode for CLI metadata capture", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@chainbreaker/cli-metadata-channel",
          chainbreaker: { extensions: ["./index.cjs"], setupEntry: "./setup-entry.cjs" },
        },
        null,
        2,
      ),
    ...
    fs.writeFileSync(
      path.join(pluginDir, "index.cjs"),
      `const { defineChannelPluginEntry } = require("chainbreaker/plugin-sdk/core");
require("node:fs").writeFileSync(${JSON.stringify(fullMarker)}, "loaded", "utf-8");
module.exports = {
  ...
  it("collects channel CLI metadata during full plugin loads", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@chainbreaker/full-cli-metadata-channel",
          chainbreaker: { extensions: ["./index.cjs"] },
        },
        null,
        2,
      ),
...
module.exports = {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "chainbreaker.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "chainbreaker.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ CHAINBREAKER_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("chainbreaker/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__chainbreakerLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("chainbreaker/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("chainbreaker/plugin-sdk");

...
import { resolveChainbreakerPackageRootSync } from "../infra/chainbreaker-root.js";
...
function hasTrustedChainbreakerRootIndicator(params: {
  packageRoot: string;
  ...
  const hasChainbreakerBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("chainbreaker")) ||
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedChainbreakerRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedChainbreakerRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveChainbreakerPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedChainbreakerRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveChainbreakerPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedChainbreakerRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`chainbreaker/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias ? { "chainbreaker/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "chainbreaker/plugin-sdk": pluginSdkAlias } : {}),

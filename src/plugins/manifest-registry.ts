...
import { discoverChainbreakerPlugins, type PluginCandidate } from "./discovery.js";
import {
  loadPluginManifest,
  type ChainbreakerPackageManifest,
  type PluginManifest,
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CHAINBREAKER_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.CHAINBREAKER_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  channelConfigs?: Record<string, PluginManifestChannelConfig>;
  packageChannel?: ChainbreakerPackageManifest["channel"];
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "chainbreaker",
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "chainbreaker") === "bundle";
    const manifestRes:
        ...
        message:
          minHostVersionCheck.kind === "invalid"
            ...
            : minHostVersionCheck.kind === "unknown_host_version"
              ? `plugin requires Chainbreaker >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
              : `plugin requires Chainbreaker >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
      });
      continue;
    }

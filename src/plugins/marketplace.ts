...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-marketplace-"));
...
> {
  ...
  try {
    ...
    try {
      ...
      tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-marketplace-download-"));

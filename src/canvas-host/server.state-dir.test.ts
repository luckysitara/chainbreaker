...
describe("canvas host state dir defaults", () => {
  ...
  it("uses CHAINBREAKER_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("chainbreaker-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Chainbreaker Canvas");
      } finally {
        await handler.close();
      }
    });

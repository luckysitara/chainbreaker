...
<title>Chainbreaker Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.chainbreakerCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.chainbreakerCanvasA2UIAction &&
        typeof window.chainbreakerCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.chainbreakerSendUserAction === "function";
  ...
  window.addEventListener("chainbreaker:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Chainbreaker node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.chainbreakerSendUserAction === "function"
        ? window.chainbreakerSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CHAINBREAKER_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.CHAINBREAKER_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Chainbreaker Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

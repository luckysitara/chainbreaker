...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "chainbreaker"),
    sessionKey,
    remoteCacheDir: join(home, ".chainbreaker", "media", "remote-cache", sessionKey),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("chainbreaker-triggers-", async (home) => {

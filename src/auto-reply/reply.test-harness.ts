...
type HomeEnvSnapshot = {
  ...
  CHAINBREAKER_STATE_DIR: string | undefined;
  CHAINBREAKER_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    CHAINBREAKER_AGENT_DIR: process.env.CHAINBREAKER_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".chainbreaker", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.CHAINBREAKER_STATE_DIR = path.join(home, ".chainbreaker");
    process.env.CHAINBREAKER_AGENT_DIR = path.join(home, ".chainbreaker", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".chainbreaker", "agent");

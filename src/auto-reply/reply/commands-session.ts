...
import { scheduleGatewaySigusr1Restart, triggerChainbreakerRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Chainbreaker in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerChainbreakerRestart();
  if (!restartMethod.ok) {

...
const workspaceHarness = createCommandWorkspaceHarness("chainbreaker-command-plugins-install-");
...
describe("handleCommands /plugins install", () => {
  ...
  it("installs a plugin from a local path", async () => {
    ...
    await withTempHome("chainbreaker-command-plugins-home-", async () => {
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@chainbreaker/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@chainbreaker/clawhub-demo",
    ...
    await withTempHome("chainbreaker-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildCommandTestParams(
        "/plugins install clawhub:@chainbreaker/clawhub-demo@1.2.3",
        {
          commands: {
            text: true,
            plugins: true,
      ...
      expect(installPluginFromClawHubMock).toHaveBeenCalledWith(
        expect.objectContaining({
          spec: "clawhub:@chainbreaker/clawhub-demo@1.2.3",
        }),
      );
      expect(persistPluginInstallMock).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@chainbreaker/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@chainbreaker/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("chainbreaker-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildCommandTestParams(
        "/plugin add clawhub:@chainbreaker/alias-demo@1.0.0",
        {
          commands: {
            text: true,
            plugins: true,
      ...
      expect(installPluginFromClawHubMock).toHaveBeenCalledWith(
        expect.objectContaining({
          spec: "clawhub:@chainbreaker/alias-demo@1.0.0",
        }),
      );
    });
  });

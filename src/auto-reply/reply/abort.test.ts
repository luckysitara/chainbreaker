...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop chainbreaker!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@chainbreaker_bot", { botUsername: "chainbreaker_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@chainbreaker_bot", { botUsername: "chainbreaker_bot" })).toBe(true);

...
export function resolveConfiguredAcpBackendId(cfg: ChainbreakerConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: ChainbreakerConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (workspaceLocalPath && existsSync(workspaceLocalPath)) {
      return `chainbreaker plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^chainbreaker plugins install /u, "");
    ...
    return "chainbreaker plugins install acpx";
  }

...
const { createChainbreakerCodingToolsMock } = vi.hoisted(() => ({
  createChainbreakerCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(async () => {
    vi.restoreAllMocks();
    vi.resetModules();
    createChainbreakerCodingToolsMock.mockClear();
    createChainbreakerCodingToolsMock.mockReturnValue([]);
    const piTools = await import("../../agents/pi-tools.js");
    vi.spyOn(piTools, "createChainbreakerCodingTools").mockImplementation(
      createChainbreakerCodingToolsMock,
    );
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createChainbreakerCodingToolsMock).toHaveBeenCalledWith(

...
function listConfiguredTelegramAccountIds(cfg: ChainbreakerConfig): string[] {
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-commands-"));
...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-commands-config-"));
  const configPath = path.join(dir, "chainbreaker.json");
  const previous = process.env.CHAINBREAKER_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CHAINBREAKER_CONFIG_PATH = previous;
    }
    await fs.rm(dir, {
      recursive: true,
      force: true,
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/chainbreaker-session-store.json" },
    } as ChainbreakerConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/chainbreaker-agent-compact";
      ...
      },
      {
        ...
        run: async () => {
          await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
            ...
            const written = await readJsonFile<ChainbreakerConfig>(configPath);

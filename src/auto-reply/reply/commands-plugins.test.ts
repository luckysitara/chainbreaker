...
const workspaceHarness = createCommandWorkspaceHarness("chainbreaker-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".chainbreaker", "extensions", params.pluginId);
...
function buildCfg(): ChainbreakerConfig {
  return {
    plugins: {
      enabled: true,
    },

...
beforeAll(async () => {
  suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-session-suite-"));
});
...
describe("initSessionState thread forking", () => {
  it("forks a new session from the parent session file", async () => {
    ...
    const root = await makeCaseDir("chainbreaker-thread-session-");
  ...
  it("forks from parent when thread session key already exists but was not forked yet", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const root = await makeCaseDir("chainbreaker-thread-session-existing-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("skips fork and creates fresh session when parent tokens exceed threshold", async () => {
    const root = await makeCaseDir("chainbreaker-thread-session-overflow-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("respects session.parentForkMaxTokens override", async () => {
    const root = await makeCaseDir("chainbreaker-thread-session-overflow-override-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("records topic-specific session files when MessageThreadId is present", async () => {
    const root = await makeCaseDir("chainbreaker-topic-session-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState RawBody", () => {
  it("uses RawBody for command extraction and reset triggers when Body contains wrapped context", async () => {
    const root = await makeCaseDir("chainbreaker-rawbody-");
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as ChainbreakerConfig;
  ...
  it("preserves argument casing while still matching reset triggers case-insensitively", async () => {
    const root = await makeCaseDir("chainbreaker-rawbody-reset-case-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("uses the default per-agent sessions store when config store is unset", async () => {
    const root = await makeCaseDir("chainbreaker-session-store-default-");
    const stateDir = path.join(root, ".chainbreaker");
...
describe("initSessionState reset policy", () => {
  ...
  it("defaults to daily reset at 4am local time", async () => {
    ...
    const root = await makeCaseDir("chainbreaker-reset-daily-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("treats sessions as stale before the daily reset when updated before yesterday's boundary", async () => {
    ...
    const root = await makeCaseDir("chainbreaker-reset-daily-edge-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("expires sessions when idle timeout wins over daily reset", async () => {
    ...
    const root = await makeCaseDir("chainbreaker-reset-idle-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("defaults to daily resets when only resetByType is configured", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("chainbreaker-reset-type-default-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps legacy idleMinutes behavior without reset config", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("chainbreaker-reset-legacy-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState reset triggers in WhatsApp groups", () => {
  ...
  function makeCfg(params: { storePath: string; allowFrom: string[] }): ChainbreakerConfig {
    return {
  ...
  it("applies WhatsApp group reset authorization across sender variants", async () => {
    ...
    const storePath = await createStorePath("chainbreaker-group-reset");
    const cases = [
      {
...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("preserves behavior overrides across /new and /reset", async () => {
    const storePath = await createStorePath("chainbreaker-reset-overrides-");
  ...
  it("preserves selected auth profile overrides across /new and /reset", async () => {
    const storePath = await createStorePath("chainbreaker-reset-model-auth-");
  ...
  it("preserves spawned session ownership metadata across /new and /reset", async () => {
    const storePath = await createStorePath("chainbreaker-reset-spawned-metadata-");
  ...
  it("requires operator.admin when Provider is internal even if Surface carries external metadata", async () => {
    const storePath = await createStorePath("chainbreaker-internal-reset-provider-authoritative-");
  ...
  it("archives the old session store entry on /new", async () => {
    const storePath = await createStorePath("chainbreaker-archive-old-");
  ...
  it("archives the old session transcript on daily/scheduled reset (stale session)", async () => {
    ...
    try {
      ...
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("chainbreaker-stale-archive-");
  ...
  it("disposes the previous bundle MCP runtime on session rollover", async () => {
    const storePath = await createStorePath("chainbreaker-stale-runtime-dispose-");
  ...
  it("idle-based new session does NOT preserve overrides (no entry to read)", async () => {
    const storePath = await createStorePath("chainbreaker-idle-no-preserve-");
...
describe("drainFormattedSystemEvents", () => {
  ...
  it("keeps channel summary lines prefixed as trusted system output on new main sessions", async () => {
    ...
    const result = await drainFormattedSystemEvents({
      cfg: { channels: {} } as ChainbreakerConfig,
      sessionKey: "agent:main:main",
      isMainSession: true,
      isNewSession: true,
    });
...
describe("persistSessionUsageUpdate", () => {
  ...
  it("uses lastCallUsage for totalTokens when provided", async () => {
    const storePath = await createStorePath("chainbreaker-usage-");
  ...
  it("uses lastCallUsage cache counters when available", async () => {
    const storePath = await createStorePath("chainbreaker-usage-cache-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("treats CLI usage as a fresh context snapshot when requested", async () => {
    const storePath = await createStorePath("chainbreaker-usage-cli-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("accumulates estimatedCostUsd across persisted usage updates", async () => {
    const storePath = await createStorePath("chainbreaker-usage-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("persists zero estimatedCostUsd for free priced models", async () => {
    const storePath = await createStorePath("chainbreaker-usage-free-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,

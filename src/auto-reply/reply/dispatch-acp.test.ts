...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: ChainbreakerConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
function createAcpConfigWithVisibleToolTags(): ChainbreakerConfig {
...
describe("tryDispatchAcpReply", () => {
  beforeEach(async () => {
    ...
    vi.doMock("../../acp/policy.js", () => ({
      resolveAcpDispatchPolicyError: (cfg: ChainbreakerConfig) =>
        policyMocks.resolveAcpDispatchPolicyError(cfg),
      resolveAcpAgentPolicyError: (cfg: ChainbreakerConfig, agent: string) =>
    ...
    vi.doMock("../../acp/runtime/session-meta.js", () => ({
      readAcpSessionEntry: (params: { sessionKey: string; cfg?: ChainbreakerConfig }) =>
  ...
  it("uses canonical session keys for bound-session identity notices", async () => {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: ChainbreakerConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/chainbreaker-session-store.json",
              sessionKey: canonicalSessionKey,

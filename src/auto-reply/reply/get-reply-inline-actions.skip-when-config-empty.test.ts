...
const createChainbreakerToolsMock = vi.fn();
...
async function loadFreshInlineActionsModuleForTest() {
  ...
  vi.doMock("../../agents/chainbreaker-tools.runtime.js", () => ({
    createChainbreakerTools: (...args: unknown[]) => createChainbreakerToolsMock(...args),
  }));
...
describe("handleInlineActions", () => {
  beforeEach(async () => {
    ...
    createChainbreakerToolsMock.mockReset();
    buildStatusReplyMock.mockReset();
    buildStatusReplyMock.mockResolvedValue({ text: "status" });
    createChainbreakerToolsMock.mockReturnValue([]);
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createChainbreakerToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({

...
const piEmbeddedMocks = getSharedMocks("chainbreaker.trigger-handling.pi-embedded-mocks", () => ({
...
const webSessionMocks = getSharedMocks("chainbreaker.trigger-handling.web-session-mocks", () => ({
...
type TempHomeEnvSnapshot = {
  ...
  chainbreakerHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    chainbreakerHome: process.env.CHAINBREAKER_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("CHAINBREAKER_HOME", snapshot.chainbreakerHome);
  restoreKey("CHAINBREAKER_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.CHAINBREAKER_STATE_DIR = join(home, ".chainbreaker");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "chainbreaker-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".chainbreaker", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): ChainbreakerConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "chainbreaker"),

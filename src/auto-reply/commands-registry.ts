...
export function isCommandEnabled(cfg: ChainbreakerConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ChainbreakerConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ChainbreakerConfig),
...
export function getCommandDetection(_cfg?: ChainbreakerConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ChainbreakerConfig) {

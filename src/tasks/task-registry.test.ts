...
const ORIGINAL_STATE_DIR = process.env.CHAINBREAKER_STATE_DIR;
...
async function withTaskRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withTempDir({ prefix: "chainbreaker-task-registry-" }, async (root) => {
...
describe("task-registry", () => {
  afterEach(() => {
    ...
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.CHAINBREAKER_STATE_DIR = ORIGINAL_STATE_DIR;
    }
  ...
  it("cancels ACP-backed tasks through the ACP session manager", async () => {
    await withTempDir({ prefix: "chainbreaker-task-registry-" }, async (root) => {

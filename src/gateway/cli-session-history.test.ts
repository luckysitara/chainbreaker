...
async function withClaudeProjectsDir<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-claude-history-"));
  ...
  it("augments chat history when a session has a claude-cli binding", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      const messages = augmentChatHistoryWithCliSessionImports({
        entry: {
          sessionId: "chainbreaker-session",
          updatedAt: Date.now(),
          cliSessionBindings: {
            "claude-cli": {
              sessionId,
      ...
      expect(messages[0]).toMatchObject({
        role: "user",
        __chainbreaker: { cliSessionId: sessionId },
      });
    });
  });

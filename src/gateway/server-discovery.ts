...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/chainbreaker/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (Chainbreaker)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.CHAINBREAKER_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "chainbreaker");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.CHAINBREAKER_TAILNET_DNS?.trim();

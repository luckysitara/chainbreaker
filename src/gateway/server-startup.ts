...
import type { loadChainbreakerPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureChainbreakerModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadChainbreakerPlugins>;
  ...
  // Tests can opt out via CHAINBREAKER_SKIP_CHANNELS (or legacy CHAINBREAKER_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.CHAINBREAKER_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.CHAINBREAKER_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (CHAINBREAKER_SKIP_CHANNELS=1 or CHAINBREAKER_SKIP_PROVIDERS=1)",
    );
  }

...
import { ensureChainbreakerCliOnPath } from "../infra/path-env.js";
import {
...
import {
  attachChainbreakerTranscriptMeta,
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("chainbreaker doctor")}" to repair, then retry.`
    : "";
...
async function prepareGatewayStartupConfig(params: {
  ...
  ) => Promise<{ config: ChainbreakerConfig }>;
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.CHAINBREAKER_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.CHAINBREAKER_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "CHAINBREAKER_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "CHAINBREAKER_RAW_STREAM_PATH",
  ...
  let cfgAtStart: ChainbreakerConfig;
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `chainbreaker config set gateway.auth.mode token` and `chainbreaker config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (matrixInstallPathIssue) {
    const lines = formatPluginInstallPathIssue({
      ...
      pluginLabel: "Matrix",
      defaultInstallCommand: "chainbreaker plugins install @chainbreaker/matrix",
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachChainbreakerTranscriptMeta(update.message, {

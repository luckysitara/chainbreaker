...
const ensureChainbreakerModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveChainbreakerAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureChainbreakerModelsJson: (config: unknown, agentDir: unknown) =>
    ensureChainbreakerModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  ...
  beforeEach(() => {
    ensureChainbreakerModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureChainbreakerModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureChainbreakerModelsJsonMock).not.toHaveBeenCalled();

...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers CHAINBREAKER_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        CHAINBREAKER_HANDSHAKE_TIMEOUT_MS: "75",
        CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        CHAINBREAKER_HANDSHAKE_TIMEOUT_MS: "",
        CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);

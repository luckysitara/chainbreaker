...
function loadGatewayConfig(): ChainbreakerConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: ChainbreakerConfig, env?: NodeJS.ProcessEnv): number {
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__CHAINBREAKER_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;

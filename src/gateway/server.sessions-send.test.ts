...
const { createChainbreakerTools } = await import("../agents/chainbreaker-tools.js");
...
type SessionSendTool = ReturnType<typeof createChainbreakerTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createChainbreakerTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["CHAINBREAKER_GATEWAY_PORT", "CHAINBREAKER_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.CHAINBREAKER_GATEWAY_PORT = String(gatewayPort);
  process.env.CHAINBREAKER_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("CHAINBREAKER_CONFIG_PATH missing in gateway test environment");
      }

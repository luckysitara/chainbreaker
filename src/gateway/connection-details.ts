...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: ChainbreakerConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.CHAINBREAKER_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: chainbreaker doctor --fix",

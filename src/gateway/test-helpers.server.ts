...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "CHAINBREAKER_SKIP_BROWSER_CONTROL_SERVER",
  "CHAINBREAKER_SKIP_GMAIL_WATCHER",
  "CHAINBREAKER_SKIP_CANVAS_HOST",
  "CHAINBREAKER_BUNDLED_PLUGINS_DIR",
  "CHAINBREAKER_SKIP_CHANNELS",
  "CHAINBREAKER_SKIP_PROVIDERS",
  "CHAINBREAKER_SKIP_CRON",
  "CHAINBREAKER_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.CHAINBREAKER_CONFIG_PATH) {
    configPaths.add(process.env.CHAINBREAKER_CONFIG_PATH);
  }
  if (process.env.CHAINBREAKER_STATE_DIR) {
    configPaths.add(path.join(process.env.CHAINBREAKER_STATE_DIR, "chainbreaker.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.CHAINBREAKER_SKIP_BROWSER_CONTROL_SERVER = "1";
  ...
  process.env.CHAINBREAKER_SKIP_PROVIDERS = "1";
  process.env.CHAINBREAKER_SKIP_CRON = "1";
  process.env.CHAINBREAKER_TEST_MINIMAL_GATEWAY = "1";
  process.env.CHAINBREAKER_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "chainbreaker-test-no-bundled-extensions")
    : "chainbreaker-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.CHAINBREAKER_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".chainbreaker-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".chainbreaker-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".chainbreaker-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>chainbreaker-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__chainbreakerTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__chainbreakerTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["CHAINBREAKER_GATEWAY_TOKEN"]);
  const prev = process.env.CHAINBREAKER_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CHAINBREAKER_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.CHAINBREAKER_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

...
let lastCreateChainbreakerToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/chainbreaker-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateChainbreakerToolsContext?.agentTo,
          agentThreadId: lastCreateChainbreakerToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createChainbreakerTools: (ctx: Record<string, unknown>) => {
      lastCreateChainbreakerToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateChainbreakerToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-chainbreaker-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-chainbreaker-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateChainbreakerToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateChainbreakerToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-chainbreaker-message-to": "channel:24514",
        "x-chainbreaker-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as owner on /tools/invoke", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-chainbreaker-scopes": "operator.approvals",
      },
      tool: "owner_only_test",
      sessionKey: "main",
    });

...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".chainbreaker", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "chainbreaker");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".chainbreaker");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "chainbreaker.json");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.CHAINBREAKER_STATE_DIR = path.join(tempHome, ".chainbreaker");
      delete process.env.CHAINBREAKER_CONFIG_PATH;
      process.env.CHAINBREAKER_SKIP_CHANNELS = "1";
...
`.trimStart(),
      ...
      process.env.CHAINBREAKER_CONFIG_PATH = configPath;
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-wizard-home-"));
      process.env.HOME = tempHome;

...
export function resolveGatewayReloadSettings(cfg: ChainbreakerConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: ChainbreakerConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ChainbreakerConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ChainbreakerConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: ChainbreakerConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: ChainbreakerConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: ChainbreakerConfig) => {

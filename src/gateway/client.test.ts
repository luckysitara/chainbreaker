...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("chainbreaker doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.CHAINBREAKER_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://chainbreaker-gateway.ai:18789",
      onConnectError,
    });

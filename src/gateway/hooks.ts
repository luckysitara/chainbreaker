...
export function resolveHooksConfig(cfg: ChainbreakerConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ChainbreakerConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-chainbreaker-token"] === "string"
      ? req.headers["x-chainbreaker-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-chainbreaker-idempotency-key"]) ||

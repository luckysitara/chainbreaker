...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/chainbreaker.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/chainbreaker.json"],
    });
  });

  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/chainbreaker.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/chainbreaker.json"],
    });
  });
...
describe("config.openFile", () => {
  ...
  it("opens the configured file without shell interpolation", async () => {
    process.env.CHAINBREAKER_CONFIG_PATH = "/tmp/config $(touch pwned).json";
  ...
  it("returns a generic error and logs details when the opener fails", async () => {
    process.env.CHAINBREAKER_CONFIG_PATH = "/tmp/config.json";
    vi.mocked(execFile).mockImplementation(((...args: unknown[]) => {

...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("CHAINBREAKER_CONFIG_PATH is required for gateway config tests");
  }
...
async function postResponses(port: number, body: unknown, headers?: Record<string, string>) {
  const res = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
    ...
    headers: {
      ...
      "x-chainbreaker-scopes": "operator.write",
      ...headers,
    },
...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "chainbreaker", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `chainbreaker` or `chainbreaker/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "chainbreaker", input: "hi" },
        { "x-chainbreaker-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "chainbreaker/beta", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "chainbreaker", input: "hi" },
        { "x-chainbreaker-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-chainbreaker-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats HTTP callers as non-owner regardless of requested scopes", async () => {
    ...
    const adminScopeResponse = await postResponses(
      port,
      { model: "chainbreaker", input: "hi" },
      { "x-chainbreaker-scopes": "operator.admin, operator.write" },
    );
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "chainbreaker", input: "hi" },
      { "x-chainbreaker-scopes": "operator.admin, operator.write" },
    );

...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.CHAINBREAKER_TAILNET_DNS;
    delete process.env.CHAINBREAKER_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.CHAINBREAKER_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.CHAINBREAKER_TAILNET_DNS = "studio.tailnet.ts.net.";

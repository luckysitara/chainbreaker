...
type GatewaySecretDefaults = NonNullable<ChainbreakerConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${CHAINBREAKER_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.CHAINBREAKER_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.CHAINBREAKER_GATEWAY_PASSWORD);
}

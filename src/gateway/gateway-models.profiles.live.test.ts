...
import { resolveChainbreakerAgentDir } from "../agents/agent-paths.js";
...
import { ensureChainbreakerModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, ChainbreakerConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.CHAINBREAKER_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const PROVIDERS = parseFilter(process.env.CHAINBREAKER_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.CHAINBREAKER_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.CHAINBREAKER_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.CHAINBREAKER_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["CHAINBREAKER_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.CHAINBREAKER_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.CHAINBREAKER_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: ChainbreakerConfig;
  agentDir: string;
}): ChainbreakerConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ChainbreakerConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.CHAINBREAKER_CONFIG_PATH,
    token: process.env.CHAINBREAKER_GATEWAY_TOKEN,
    skipChannels: process.env.CHAINBREAKER_SKIP_CHANNELS,
    skipGmail: process.env.CHAINBREAKER_SKIP_GMAIL_WATCHER,
    skipCron: process.env.CHAINBREAKER_SKIP_CRON,
    skipCanvas: process.env.CHAINBREAKER_SKIP_CANVAS_HOST,
    disableBonjour: process.env.CHAINBREAKER_DISABLE_BONJOUR,
    logLevel: process.env.CHAINBREAKER_LOG_LEVEL,
    agentDir: process.env.CHAINBREAKER_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.CHAINBREAKER_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.CHAINBREAKER_SKIP_CHANNELS = "1";
  process.env.CHAINBREAKER_SKIP_GMAIL_WATCHER = "1";
  process.env.CHAINBREAKER_SKIP_CRON = "1";
  process.env.CHAINBREAKER_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.CHAINBREAKER_DISABLE_BONJOUR = "1";
    process.env.CHAINBREAKER_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.CHAINBREAKER_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveChainbreakerAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-live-state-"));
  process.env.CHAINBREAKER_STATE_DIR = tempStateDir;
  ...
  process.env.CHAINBREAKER_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.chainbreaker-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveChainbreakerAgentDir();
  const sanitizedCfg: ChainbreakerConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-live-"));
  const tempConfigPath = path.join(tempDir, "chainbreaker.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.CHAINBREAKER_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              modelKey,
              message: strictReply
                ? "Chainbreaker live tool probe (local, safe): " +
    ...
    process.env.CHAINBREAKER_CONFIG_PATH = previous.configPath;
    process.env.CHAINBREAKER_GATEWAY_TOKEN = previous.token;
    process.env.CHAINBREAKER_SKIP_CHANNELS = previous.skipChannels;
    process.env.CHAINBREAKER_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.CHAINBREAKER_SKIP_CRON = previous.skipCron;
    process.env.CHAINBREAKER_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.CHAINBREAKER_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.CHAINBREAKER_LOG_LEVEL = previous.logLevel;
    process.env.CHAINBREAKER_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CHAINBREAKER_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.CHAINBREAKER_LIVE_GATEWAY_MODELS?.trim();

...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-Chainbreaker-Token header (query parameters are not allowed).",
      );
      return true;
    }
        ...
        wss.handleUpgrade(req, socket, head, (ws) => {
          (
            ws as unknown as import("ws").WebSocket & {
              __chainbreakerPreauthBudgetClaimed?: boolean;
              __chainbreakerPreauthBudgetKey?: string;
            }
          ).__chainbreakerPreauthBudgetKey = preauthBudgetKey;
          ...
          const budgetClaimed = Boolean(
            ...
            ).__chainbreakerPreauthBudgetClaimed,
          );
          if (budgetClaimed) {
            budgetTransferred = true;

...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers CHAINBREAKER_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.CHAINBREAKER_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CHAINBREAKER_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.CHAINBREAKER_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CHAINBREAKER_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CHAINBREAKER_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CHAINBREAKER_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            ...
            CHAINBREAKER_SERVICE_VERSION: "2.4.6-service",
        ...
        {
          env: {
            CHAINBREAKER_VERSION: "9.9.9-cli",
            CHAINBREAKER_SERVICE_VERSION: "2.4.6-service",
            npm_package_version: "1.0.0-package",
          },
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `chainbreaker-test-device-${randomUUID()}.json`),
        nonce,
      });

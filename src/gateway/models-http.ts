...
import {
  CHAINBREAKER_DEFAULT_MODEL_ID,
  CHAINBREAKER_MODEL_ID,
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([CHAINBREAKER_MODEL_ID, CHAINBREAKER_DEFAULT_MODEL_ID]);
  ids.add(`chainbreaker/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`chainbreaker/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== CHAINBREAKER_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {

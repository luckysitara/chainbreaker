...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== CHAINBREAKER_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
  ...
  const overrideModel = getHeader(req, "x-chainbreaker-model")?.trim() || memorySearch?.model || "";

...
import { createChainbreakerTools } from "../agents/chainbreaker-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-chainbreaker-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-chainbreaker-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-chainbreaker-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-chainbreaker-thread-id")?.trim() || undefined;
  ...
  const allTools = createChainbreakerTools({

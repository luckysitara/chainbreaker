...
const WRITE_SCOPE_HEADER = { "x-chainbreaker-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-chainbreaker-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("chainbreaker/default");
  ...
  it("ignores narrower declared scopes for shared-secret bearer auth", async () => {
    const res = await postEmbeddings(
      ...
      { "x-chainbreaker-scopes": "operator.read" },
    );
    expect(res.status).toBe(200);
  ...
  it("rejects invalid agent targets", async () => {
    ...
    expect(json.error).toEqual({
      ...
      message: "Invalid `model`. Use `chainbreaker` or `chainbreaker/<agentId>`.",
    });
  });

  it("rejects disallowed x-chainbreaker-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "chainbreaker/default",
        input: "hello",
      },
      { "x-chainbreaker-model": "ollama/nomic-embed-text" },
    );
    expect(res.status).toBe(400);
    const json = (await res.json()) as { error?: { type?: string; message?: string } };
    expect(json.error).toEqual({

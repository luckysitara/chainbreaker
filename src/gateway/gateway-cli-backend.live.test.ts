...
const CLI_LIVE = isTruthyEnvValue(process.env.CHAINBREAKER_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.CHAINBREAKER_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.CHAINBREAKER_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("CHAINBREAKER_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
    const preservedEnv = new Set(
      parseJsonStringArray(
        ...
        process.env.CHAINBREAKER_LIVE_CLI_BACKEND_PRESERVE_ENV,
      ) ?? [],
    );
    ...
    const rawModel = process.env.CHAINBREAKER_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `CHAINBREAKER_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const baseCliArgs =
      parseJsonStringArray(
        "CHAINBREAKER_LIVE_CLI_BACKEND_ARGS",
    ...
    const cliClearEnv =
      parseJsonStringArray(
        "CHAINBREAKER_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.CHAINBREAKER_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const filteredCliClearEnv = cliClearEnv.filter((name) => !preservedEnv.has(name));
    const cliImageArg = process.env.CHAINBREAKER_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.CHAINBREAKER_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "CHAINBREAKER_LIVE_CLI_BACKEND_IMAGE_MODE requires CHAINBREAKER_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-live-cli-"));
    const disableMcpConfig = process.env.CHAINBREAKER_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";

...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-ui-"));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-ui-root-"));
    try {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/chainbreaker");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/chainbreaker/avatar/main");
        expect(parsed).not.toHaveProperty("assistantAgentId");
        expect(parsed).not.toHaveProperty("serverVersion");
      },
    });
  ...
  it("serves local avatar bytes through hardened avatar handler", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-avatar-http-"));
    try {
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-avatar-http-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-avatar-http-outside-"));
    try {
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-ui-outside-"));
        try {
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = handleControlUiHttpRequest(
          ...
          res,
          { basePath: "/chainbreaker", root: { kind: "resolved", path: tmp } },
        );
  ...
  it("falls through POST requests under configured basePath (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const route of ["/chainbreaker", "/chainbreaker/", "/chainbreaker/some-page"]) {
          const { handled, end } = runControlUiRequest({
            url: route,
            method: "POST",
            rootPath: tmp,
  ...
  it("rejects absolute-path escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: `/chainbreaker/${absolutePathUrl}`,
          method: "GET",
          rootPath: root,
          basePath: "/chainbreaker",
        });
  ...
  it("rejects symlink escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: "/chainbreaker/assets/leak.txt",
          method: "GET",
          rootPath: root,
          basePath: "/chainbreaker",
        });

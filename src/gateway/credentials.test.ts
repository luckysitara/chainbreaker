...
function cfg(input: Partial<ChainbreakerConfig>): ChainbreakerConfig {
  return input as ChainbreakerConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<ChainbreakerConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  CHAINBREAKER_GATEWAY_TOKEN: "env-token",
...
describe("resolveGatewayCredentialsFromValues", () => {
  ...
  it("rejects unresolved env var placeholders in config credentials", () => {
    const resolved = resolveGatewayCredentialsFromValues({
      configToken: "${CHAINBREAKER_GATEWAY_TOKEN}",
      configPassword: "${CHAINBREAKER_GATEWAY_PASSWORD}",
      env: {} as NodeJS.ProcessEnv,
      tokenPrecedence: "config-first",
      passwordPrecedence: "config-first", // pragma: allowlist secret
    });

...
describe("runCapability video provider wiring", () => {
  it("merges video baseUrl and headers with entry precedence", async () => {
    ...
    await withVideoFixture("chainbreaker-video-merge", async ({ ctx, media, cache }) => {
      const cfg = {
        models: {
          providers: {
            moonshot: {
  ...
  it("auto-selects moonshot for video when google is unavailable", async () => {
    const isolatedAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-video-agent-"));
    try {
      await withEnvAsync(
        {

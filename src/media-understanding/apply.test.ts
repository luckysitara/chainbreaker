...
const TEMP_MEDIA_PREFIX = "chainbreaker-media-";
...
function createGroqAudioConfig(): ChainbreakerConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ChainbreakerConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): ChainbreakerConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: ChainbreakerConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredChainbreakerTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        CHAINBREAKER_AGENT_DIR: isolatedAgentDir,

...
} from "../scripts/chainbreaker-npm-release-check.ts";
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"chainbreaker.tgz","files":[]}]')).toEqual([
      { filename: "chainbreaker.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> chainbreaker@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"chainbreaker.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");

    expect(parseNpmPackJsonOutput(stdout)).toEqual([
      {
        filename: "chainbreaker.tgz",
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> chainbreaker@2026.3.23 prepack")).toBeNull();
  });
});
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/chainbreaker/chainbreaker.git" },
        bin: { chainbreaker: "chainbreaker.mjs" },

...
function installDiscoveryHooks(state: DiscoveryState) {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("chainbreaker/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("chainbreaker/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("chainbreaker/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,

...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked CHAINBREAKER_HOME.
  delete process.env.CHAINBREAKER_HOME;
  process.env.CHAINBREAKER_STATE_DIR = path.join(base, ".chainbreaker");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "chainbreaker-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".chainbreaker", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {

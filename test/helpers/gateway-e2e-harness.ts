...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `chainbreaker-e2e-${name}-`));
  const configDir = path.join(homeDir, ".chainbreaker");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          CHAINBREAKER_SKIP_PROVIDERS: "1",
          CHAINBREAKER_SKIP_GMAIL_WATCHER: "1",
          CHAINBREAKER_SKIP_CRON: "1",
          CHAINBREAKER_SKIP_BROWSER_CONTROL_SERVER: "1",
          CHAINBREAKER_SKIP_CANVAS_HOST: "1",
          CHAINBREAKER_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },

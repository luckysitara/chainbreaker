...
export function requireChainbreakerAgentDir(): string {
  const agentDir = process.env.CHAINBREAKER_AGENT_DIR;
  if (!agentDir) {
    throw new Error("CHAINBREAKER_AGENT_DIR not set");
  }
  return agentDir;
}

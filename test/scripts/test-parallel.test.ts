...
const clearPlannerShardEnv = (env) => {
  const nextEnv = { ...env };
  delete nextEnv.CHAINBREAKER_TEST_SHARDS;
  delete nextEnv.CHAINBREAKER_TEST_SHARD_INDEX;
  delete nextEnv.CHAINBREAKER_TEST_FORCE_THREADS;
  delete nextEnv.CHAINBREAKER_TEST_FORCE_FORKS;
  delete nextEnv.CHAINBREAKER_TEST_DISABLE_THREAD_EXPANSION;
  delete nextEnv.CHAINBREAKER_TEST_SHOW_POOL_DECISION;
  delete nextEnv.CHAINBREAKER_TEST_PROFILE;
  delete nextEnv.CHAINBREAKER_TEST_WORKERS;
  delete nextEnv.CHAINBREAKER_TEST_SKIP_DEFAULT;
  delete nextEnv.CHAINBREAKER_TEST_INCLUDE_EXTENSIONS;
  delete nextEnv.CHAINBREAKER_TEST_INCLUDE_CHANNELS;
  delete nextEnv.CHAINBREAKER_TEST_INCLUDE_GATEWAY;
  return nextEnv;
};
...
const HIGH_MEMORY_LOCAL_PLANNER_ENV = {
  RUNNER_OS: "macOS",
  CHAINBREAKER_TEST_HOST_CPU_COUNT: "12",
  CHAINBREAKER_TEST_HOST_MEMORY_GIB: "128",
} as const;
...
function createLocalPlannerEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return createPlannerEnv({
    CI: "",
    GITHUB_ACTIONS: "",
    CHAINBREAKER_TEST_LOAD_AWARE: "0",
    ...overrides,
  });
}
...
function runManifestOutputWriter(workflow: string, envOverrides: NodeJS.ProcessEnv = {}): string {
  const outputPath = path.join(os.tmpdir(), `chainbreaker-${workflow}-output-${Date.now()}.txt`);
  try {
...
describe("scripts/test-parallel lane planning", () => {
  it("keeps serial profile on split unit lanes instead of one giant unit worker", () => {
    const output = runPlannerPlan(["--plan"], {
      CHAINBREAKER_TEST_PROFILE: "serial",
    });
  ...
  it("recycles default local unit-fast runs into bounded batches", () => {
    const output = runPlannerPlan(["--plan"], {
      CI: "",
      CHAINBREAKER_TEST_UNIT_FAST_LANES: "1",
      CHAINBREAKER_TEST_UNIT_FAST_BATCH_TARGET_MS: "1",
    });
  ...
  it("reports capability-derived output for mid-memory local macOS hosts", () => {
    const output = runPlannerPlan(
      ...
      createLocalPlannerEnv({
        RUNNER_OS: "macOS",
        CHAINBREAKER_TEST_HOST_CPU_COUNT: "10",
        CHAINBREAKER_TEST_HOST_MEMORY_GIB: "64",
      }),
    );
  ...
  it("prints the planner-backed CI manifest as JSON", () => {
    const output = runPlannerPlan(["--ci-manifest"], {
      ...
      CHAINBREAKER_CI_DOCS_ONLY: "false",
      CHAINBREAKER_CI_DOCS_CHANGED: "false",
      CHAINBREAKER_CI_RUN_NODE: "true",
      CHAINBREAKER_CI_RUN_MACOS: "true",
      CHAINBREAKER_CI_RUN_ANDROID: "false",
      CHAINBREAKER_CI_RUN_WINDOWS: "true",
      CHAINBREAKER_CI_RUN_SKILLS_PYTHON: "false",
      CHAINBREAKER_CI_HAS_CHANGED_EXTENSIONS: "false",
      CHAINBREAKER_CI_CHANGED_EXTENSIONS_MATRIX: '{"include":[]}',
    });
  ...
  it("writes CI workflow outputs in ci mode", () => {
    const outputs = runManifestOutputWriter("ci", {
      ...
      CHAINBREAKER_CI_RUN_NODE: "true",
      CHAINBREAKER_CI_RUN_MACOS: "true",
      CHAINBREAKER_CI_RUN_ANDROID: "true",
      CHAINBREAKER_CI_RUN_WINDOWS: "true",
      CHAINBREAKER_CI_RUN_SKILLS_PYTHON: "false",
      CHAINBREAKER_CI_HAS_CHANGED_EXTENSIONS: "false",
      CHAINBREAKER_CI_CHANGED_EXTENSIONS_MATRIX: '{"include":[]}',
  ...
  it("writes install-smoke outputs in install-smoke mode", () => {
    const outputs = runManifestOutputWriter("install-smoke", {
      CHAINBREAKER_CI_DOCS_ONLY: "false",
      CHAINBREAKER_CI_RUN_CHANGED_SMOKE: "true",
    });
  ...
  it("passes through vitest --mode values that are not wrapper runtime overrides", () => {
    const output = runPlannerPlan(
      ...
      createLocalPlannerEnv({
        RUNNER_OS: "Linux",
        CHAINBREAKER_TEST_HOST_CPU_COUNT: "16",
        CHAINBREAKER_TEST_HOST_MEMORY_GIB: "128",
      }),
    );
  ...
  it("rejects explicit existing files that are not known test files", () => {
    const tempFilePath = path.join(os.tmpdir(), `chainbreaker-non-test-${Date.now()}.ts`);

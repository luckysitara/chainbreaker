...
describe("stageBundledPluginRuntimeDeps", () => {
  function createBundledPluginFixture(params: {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-runtime-deps-"));
  ...
  it("skips restaging when runtime deps stamp matches the sanitized manifest", () => {
    const { pluginDir, repoRoot } = createBundledPluginFixture({
      packageJson: {
        name: "@chainbreaker/fixture-plugin",
        version: "1.0.0",
        dependencies: { "left-pad": "1.3.0" },
        peerDependencies: { chainbreaker: "^1.0.0" },
        peerDependenciesMeta: { chainbreaker: { optional: true } },
        devDependencies: { chainbreaker: "^1.0.0" },
        chainbreaker: { bundle: { stageRuntimeDependencies: true } },
      },
    });
    ...
    stageBundledPluginRuntimeDeps({
      ...
      installPluginRuntimeDepsImpl: ({ fingerprint }: { fingerprint: string }) => {
        installCount += 1;
        fs.writeFileSync(
          path.join(pluginDir, ".chainbreaker-runtime-deps-stamp.json"),
  ...
  it("stages runtime deps from the root node_modules when already installed", () => {
    ...
    expect(fs.existsSync(path.join(pluginDir, ".chainbreaker-runtime-deps-stamp.json"))).toBe(true);
  });

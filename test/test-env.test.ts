...
function createTempHome(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-test-env-real-home-"));
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-chainbreaker.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.chainbreaker/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.chainbreaker/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".chainbreaker", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".chainbreaker", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.CHAINBREAKER_LIVE_TEST_QUIET = "1";
    process.env.CHAINBREAKER_CONFIG_PATH = "~/custom-chainbreaker.json5";
    process.env.CHAINBREAKER_TEST_HOME = priorIsolatedHome;
    process.env.CHAINBREAKER_STATE_DIR = path.join(priorIsolatedHome, ".chainbreaker");
    ...
    expect(process.env.CHAINBREAKER_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".chainbreaker", "chainbreaker.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".chainbreaker", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.CHAINBREAKER_LIVE_TEST = "1";
    process.env.CHAINBREAKER_LIVE_USE_REAL_HOME = "1";
    process.env.CHAINBREAKER_LIVE_TEST_QUIET = "1";

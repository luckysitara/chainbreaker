...
process.env.CHAINBREAKER_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
import type { ChainbreakerConfig } from "../src/config/config.js";
...
// Set HOME/state isolation before importing any runtime Chainbreaker modules.
const testEnv = withIsolatedTestHome();

installProcessWarningFilter();

const REGISTRY_STATE = Symbol.for("chainbreaker.pluginRegistryState");
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: ChainbreakerConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: ChainbreakerConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;

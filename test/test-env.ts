...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.CHAINBREAKER_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "CHAINBREAKER_TEST_FAST", value: process.env.CHAINBREAKER_TEST_FAST },
    ...
    { key: "CHAINBREAKER_STATE_DIR", value: process.env.CHAINBREAKER_STATE_DIR },
    { key: "CHAINBREAKER_CONFIG_PATH", value: process.env.CHAINBREAKER_CONFIG_PATH },
    { key: "CHAINBREAKER_GATEWAY_PORT", value: process.env.CHAINBREAKER_GATEWAY_PORT },
    { key: "CHAINBREAKER_BRIDGE_ENABLED", value: process.env.CHAINBREAKER_BRIDGE_ENABLED },
    { key: "CHAINBREAKER_BRIDGE_HOST", value: process.env.CHAINBREAKER_BRIDGE_HOST },
    { key: "CHAINBREAKER_BRIDGE_PORT", value: process.env.CHAINBREAKER_BRIDGE_PORT },
    { key: "CHAINBREAKER_CANVAS_HOST_PORT", value: process.env.CHAINBREAKER_CANVAS_HOST_PORT },
    { key: "CHAINBREAKER_TEST_HOME", value: process.env.CHAINBREAKER_TEST_HOME },
    { key: "CHAINBREAKER_AGENT_DIR", value: process.env.CHAINBREAKER_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CHAINBREAKER_TEST_HOME = tempHome;
  process.env.CHAINBREAKER_TEST_FAST = "1";
  ...
  delete process.env.CHAINBREAKER_BRIDGE_ENABLED;
  delete process.env.CHAINBREAKER_BRIDGE_HOST;
  delete process.env.CHAINBREAKER_BRIDGE_PORT;
  delete process.env.CHAINBREAKER_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.CHAINBREAKER_STATE_DIR = path.join(tempHome, ".chainbreaker");
  }
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.CHAINBREAKER_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".chainbreaker");
  const priorIsolatedHome = params.env.CHAINBREAKER_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".chainbreaker")
  ) {
    realStateDir = path.join(params.realHome, ".chainbreaker");
  }
  const tempStateDir = path.join(params.tempHome, ".chainbreaker");
  fs.mkdirSync(tempStateDir, { recursive: true });

  const realConfigPath = params.env.CHAINBREAKER_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.CHAINBREAKER_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "chainbreaker.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "chainbreaker.json"),
...
export function installTestEnv(): { cleanup: () => void; tempHome: string } {
  const live =
    process.env.LIVE === "1" ||
    process.env.CHAINBREAKER_LIVE_TEST === "1" ||
    process.env.CHAINBREAKER_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.CHAINBREAKER_LIVE_USE_REAL_HOME);

...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("passes through the filesystem module cache path when provided", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CHAINBREAKER_VITEST_FS_MODULE_CACHE_PATH: "/tmp/chainbreaker-vitest-cache",
        },
        "linux",
      ),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
        fsModuleCachePath: "/tmp/chainbreaker-vitest-cache",
      },
    });
  });
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CHAINBREAKER_VITEST_IMPORT_DURATIONS: "true",
          CHAINBREAKER_VITEST_PRINT_IMPORT_BREAKDOWN: "1",
        },
        "linux",
      ),
    ).toEqual({

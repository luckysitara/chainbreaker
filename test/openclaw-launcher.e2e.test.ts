...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-launcher-"));
  fixtureRoots.push(fixtureRoot);
  await fs.copyFile(
    path.resolve(process.cwd(), "chainbreaker.mjs"),
    path.join(fixtureRoot, "chainbreaker.mjs"),
  );
...
describe("chainbreaker launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      ...
      'import "missing-chainbreaker-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "chainbreaker.mjs"), "--help"],
      {
        cwd: fixtureRoot,
        encoding: "utf8",
      },
    ...
    expect(result.stderr).toContain("missing-chainbreaker-launcher-dep");
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:chainbreaker/chainbreaker#<ref>");
  });
});

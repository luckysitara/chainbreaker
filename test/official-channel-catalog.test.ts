...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("chainbreaker-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@chainbreaker/whatsapp",
      version: "2026.3.23",
      description: "Chainbreaker WhatsApp channel plugin",
      chainbreaker: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("chainbreaker-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@chainbreaker/whatsapp",
      chainbreaker: {
        channel: {

...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `chainbreaker/plugin-sdk` alias is a prefix match.
    alias: [
      {
        find: "chainbreaker/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `chainbreaker/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "chainbreaker/plugin-sdk",

---
name: node-connect
description: Diagnose Chainbreaker node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Chainbreaker is advertising that route, then fix pairing/auth.
...
- whether `chainbreaker devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `chainbreaker qr --json`. It uses the same setup-code payload Android scans.

```bash
chainbreaker config get gateway.mode
chainbreaker config get gateway.bind
chainbreaker config get gateway.tailscale.mode
chainbreaker config get gateway.remote.url
chainbreaker config get gateway.auth.mode
chainbreaker config get gateway.auth.allowTailscale
chainbreaker config get plugins.entries.device-pair.config.publicUrl
chainbreaker qr --json
chainbreaker devices list
chainbreaker nodes status
```

If this Chainbreaker instance is pointed at a remote gateway, also run:

```bash
chainbreaker qr --remote --json
```
...
`chainbreaker qr --json` success means:
...
If `chainbreaker qr --json` says `Gateway is only bound to loopback`:
...
- `chainbreaker devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run chainbreaker qr again, rescan, then approve the pending device pairing.`

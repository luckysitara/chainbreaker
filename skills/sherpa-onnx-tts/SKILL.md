...
1. Download the runtime for your OS (extracts into `$CHAINBREAKER_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.chainbreaker/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$CHAINBREAKER_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.chainbreaker/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Chainbreaker config file (`$CHAINBREAKER_CONFIG_PATH`, default `~/.chainbreaker/chainbreaker.json`):

...
**⚠️ CRITICAL: Never review PRs in Chainbreaker's own project folder!**
...
8. **NEVER start Codex inside your Chainbreaker state directory** (`$CHAINBREAKER_STATE_DIR`, default `~/.chainbreaker`) - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/chainbreaker/** - that's the LIVE Chainbreaker instance!
...
For long-running background tasks, append a wake trigger to your prompt so Chainbreaker gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
chainbreaker system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: chainbreaker system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```

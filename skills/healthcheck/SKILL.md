---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Chainbreaker deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Chainbreaker cron scheduling for periodic checks, or version status checks on a machine running Chainbreaker (laptop, workstation, Pi, VPS).
---

# Chainbreaker Host Hardening

## Overview

Assess and harden the host running Chainbreaker, then align it to a user-defined risk tolerance without breaking access. Use Chainbreaker security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Chainbreaker changes the host firewall, SSH, or OS updates; it does not.
...
5. Chainbreaker gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Chainbreaker can access sensitive data.
...
### 2) Run Chainbreaker security audits (read-only)

As part of the default read-only checks, run `chainbreaker security audit --deep`. Only offer alternatives if the user requests them:

1. `chainbreaker security audit` (faster, non-probing)
2. `chainbreaker security audit --json` (structured output)

Offer to apply Chainbreaker safe defaults (numbered):

1. `chainbreaker security audit --fix`

Be explicit that `--fix` only tightens Chainbreaker defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Chainbreaker version/update status (read-only)

As part of the default read-only checks, run `chainbreaker update status`.
...
- Credential hygiene notes (location of Chainbreaker creds, prefer disk encryption)
...
- Chainbreaker security audit (re-run)
...
After Chainbreaker install or first hardening pass, run at least one baseline audit and version check:

- `chainbreaker security audit`
- `chainbreaker security audit --deep`
- `chainbreaker update status`

Ongoing monitoring is recommended. Use the Chainbreaker cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `chainbreaker cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `chainbreaker update status`
...
Before creating, `chainbreaker cron list` and match on exact `name`. If found, `chainbreaker cron edit <id> ...`.
If not found, `chainbreaker cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `chainbreaker update status` (preferred for source checkouts and channels)
2. `npm view chainbreaker version` (published npm version)

## Chainbreaker command accuracy

Use only supported commands and flags:

- `chainbreaker security audit [--deep] [--fix] [--json]`
- `chainbreaker status` / `chainbreaker status --deep`
- `chainbreaker health --json`
- `chainbreaker update status`
- `chainbreaker cron add|list|runs|run`

Do not invent CLI flags or imply Chainbreaker enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Chainbreaker compaction:

---
name: chainbreaker-release-maintainer
description: Maintainer workflow for Chainbreaker releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Chainbreaker Release Maintainer
...
- Core `chainbreaker` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/Chainbreaker/Resources/Info.plist`
...
- Every Chainbreaker release ships the npm package and macOS app together.
- The production Sparkle feed lives at `https://raw.githubusercontent.com/chainbreaker/chainbreaker/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `chainbreaker` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `chainbreaker YYYY.M.D-beta.N`
...
```bash
  CHAINBREAKER_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/chainbreaker-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the Chainbreaker npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `chainbreaker/chainbreaker` and the real mac preflight in
  `chainbreaker/releases-private` for every release.
...
- Chainbreaker publish uses GitHub trusted publishing.
...
- `.github/workflows/macos-release.yml` in `chainbreaker/chainbreaker` is now a
...
- Real mac preflight and real mac publish both use
  `chainbreaker/releases-private/.github/workflows/chainbreaker-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@chainbreaker/chainbreaker-release-managers` before publish continues.
- Mac publish uses
  `chainbreaker/releases-private/.github/workflows/chainbreaker-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `chainbreaker/chainbreaker`
  automatically when `CHAINBREAKER_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for Chainbreaker releases.
- `@chainbreaker/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/chainbreaker-npm-release.yml` with `preflight_only=true`
   and wait for it to pass. Save that run id if you want the real publish to
   reuse the prepared npm tarball.
10. Start `.github/workflows/macos-release.yml` in `chainbreaker/chainbreaker` and wait
...
13. Start `.github/workflows/chainbreaker-npm-release.yml` with the same tag for
...
15. Wait for `npm-release` approval from `@chainbreaker/chainbreaker-release-managers`.
16. Start
    `chainbreaker/releases-private/.github/workflows/chainbreaker-macos-publish.yml`
...
- Use `chainbreaker-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.

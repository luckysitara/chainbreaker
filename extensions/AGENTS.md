...
- Extension production code should import from `chainbreaker/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `chainbreaker.plugin.json` and the package
  `chainbreaker` block so discovery and setup work without executing plugin code.

...
function normalizeDenyList(cfg: ChainbreakerPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ChainbreakerPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: ChainbreakerPluginApi) {
    ...
    const timerService: ChainbreakerPluginService = {

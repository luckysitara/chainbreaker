...
} from "chainbreaker/plugin-sdk/memory-core-host-runtime-files";
import { buildAgentSessionKey } from "chainbreaker/plugin-sdk/routing";
...
function resolveAgent(cfg: ChainbreakerConfig, agent?: string) {
...
function resolveAgentIds(cfg: ChainbreakerConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }

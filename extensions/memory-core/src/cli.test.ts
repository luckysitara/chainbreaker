...
import {
  ...
  spyRuntimeJson,
  spyRuntimeLogs,
} from "chainbreaker/plugin-sdk/testing";
...
vi.mock("chainbreaker/plugin-sdk/memory-core-host-runtime-core", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("chainbreaker/plugin-sdk/memory-core-host-runtime-core")>();
  return {
    ...actual,
    loadConfig,
...
vi.mock("chainbreaker/plugin-sdk/memory-core-host-runtime-cli", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("chainbreaker/plugin-sdk/memory-core-host-runtime-cli")>();
  return {
    ...actual,
...
let defaultRuntime: typeof import("chainbreaker/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("chainbreaker/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("chainbreaker/plugin-sdk/memory-core-host-runtime-cli").setVerbose;

beforeAll(async () => {
  ({ registerMemoryCli } = await import("./cli.js"));
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("chainbreaker/plugin-sdk/memory-core-host-runtime-cli"));
});
...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("chainbreaker memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('chainbreaker memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('chainbreaker memory search --query "deployment" --max-results 20');

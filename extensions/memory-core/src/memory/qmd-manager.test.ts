...
const MCPORTER_STATE_KEY = Symbol.for("chainbreaker.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("chainbreaker.qmdEmbedQueueTail");
...
vi.mock("chainbreaker/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("chainbreaker/plugin-sdk/memory-core-host-engine-foundation")
  >("chainbreaker/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("chainbreaker/plugin-sdk/file-lock", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/file-lock")>();
  return {
    ...actual,
...
import type { ChainbreakerConfig } from "chainbreaker/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: ChainbreakerConfig }) {

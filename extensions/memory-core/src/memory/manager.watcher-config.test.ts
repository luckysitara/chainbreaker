...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): ChainbreakerConfig {
    const defaults: NonNullable<NonNullable<ChainbreakerConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: ChainbreakerConfig) {

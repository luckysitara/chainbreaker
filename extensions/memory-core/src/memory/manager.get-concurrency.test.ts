...
describe("memory manager cache hydration", () => {
  ...
  beforeEach(async () => {
    ...
    vi.clearAllMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-mem-concurrent-"));
  ...
  function createMemoryConcurrencyConfig(indexPath: string): ChainbreakerConfig {
    return {
      agents: {
        defaults: {

...
import { readMemoryFile } from "chainbreaker/plugin-sdk/memory-core-host-engine-storage";
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeAll(async () => {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-mem-read-"));

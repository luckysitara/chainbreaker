...
import {
  ...
  formatHelpExamples,
  theme,
} from "chainbreaker/plugin-sdk/memory-core-host-runtime-cli";
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["chainbreaker memory status", "Show index and provider status."],
          ["chainbreaker memory status --deep", "Probe embedding provider readiness."],
          ["chainbreaker memory index --force", "Force a full reindex."],
          ['chainbreaker memory search "meeting notes"', "Quick search using positional query."],
          [
            'chainbreaker memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["chainbreaker memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.chainbreaker.ai/cli/memory")}\n`,
    );

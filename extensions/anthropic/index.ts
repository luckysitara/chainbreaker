...
import { parseDurationMs } from "chainbreaker/plugin-sdk/cli-runtime";
import {
  definePluginEntry,
...
import { cloneFirstTemplateModel } from "chainbreaker/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "chainbreaker/plugin-sdk/provider-usage";
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("chainbreaker doctor --yes")}"`,
  ].join("\n");
}

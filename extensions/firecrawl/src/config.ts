...
} from "chainbreaker/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<ChainbreakerConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<ChainbreakerConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ChainbreakerConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: ChainbreakerConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: ChainbreakerConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: ChainbreakerConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: ChainbreakerConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: ChainbreakerConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlMaxAgeMs(cfg?: ChainbreakerConfig, override?: number): number {

import type { ImageGenerationProvider } from "chainbreaker/plugin-sdk/image-generation";
import type { MediaUnderstandingProvider } from "chainbreaker/plugin-sdk/media-understanding";
import {
  definePluginEntry,
...
import { createProviderApiKeyAuthMethod } from "chainbreaker/plugin-sdk/provider-auth-api-key";
import type { ProviderPlugin } from "chainbreaker/plugin-sdk/provider-model-shared";
import { createGoogleThinkingPayloadWrapper } from "chainbreaker/plugin-sdk/provider-stream";
import {
...
const GOOGLE_GEMINI_CLI_ENV_VARS = [
  "CHAINBREAKER_GEMINI_OAUTH_CLIENT_ID",
  "CHAINBREAKER_GEMINI_OAUTH_CLIENT_SECRET",
...
async function loadGoogleGeminiCliProvider(): Promise<ProviderPlugin> {
  if (!googleGeminiCliProviderPromise) {
    googleGeminiCliProviderPromise = import("./gemini-cli-provider.js").then((mod) => {
      ...
      } as Pick<ChainbreakerPluginApi, "registerProvider"> as ChainbreakerPluginApi);
      if (!provider) {

...
import type { ChainbreakerConfig } from "chainbreaker/plugin-sdk/plugin-entry";
import {
  jsonResult,
  readCache,
...
type XSearchConfig = NonNullable<ChainbreakerConfig["tools"]>["web"] extends infer Web
...
const X_SEARCH_CACHE_KEY = Symbol.for("chainbreaker.xai.x-search.cache");
...
function readLegacyGrokApiKey(cfg?: ChainbreakerConfig): string | undefined {
...
function readPluginXaiWebSearchApiKey(cfg?: ChainbreakerConfig): string | undefined {
  return readConfiguredSecretString(
...
function resolveFallbackXaiApiKey(cfg?: ChainbreakerConfig): string | undefined {
  return readPluginXaiWebSearchApiKey(cfg) ?? readLegacyGrokApiKey(cfg);
}

function resolveXSearchConfig(cfg?: ChainbreakerConfig): XSearchConfig {
...
export function createXSearchTool(options?: {
  ...
  runtimeConfig?: ChainbreakerConfig | null;
}) {

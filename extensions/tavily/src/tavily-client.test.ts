...
vi.mock("chainbreaker/plugin-sdk/provider-web-search", () => ({
...
vi.mock("chainbreaker/plugin-sdk/security-runtime", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: chainbreaker", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "chainbreaker" });
  });

  it("runTavilyExtract sends X-Client-Source: chainbreaker", async () => {

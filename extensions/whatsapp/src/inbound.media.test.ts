...
vi.mock("chainbreaker/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("chainbreaker/plugin-sdk/config-runtime")>(
...
vi.mock("chainbreaker/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("chainbreaker/plugin-sdk/media-runtime")>(
    "chainbreaker/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `chainbreaker-inbound-media-${crypto.randomUUID()}`);

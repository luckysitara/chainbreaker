...
import { info, success } from "chainbreaker/plugin-sdk/runtime-env";
import { getChildLogger } from "chainbreaker/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "chainbreaker/plugin-sdk/runtime-env";
import { resolveOAuthDir } from "chainbreaker/plugin-sdk/state-paths";
import type { WebChannel } from "chainbreaker/plugin-sdk/text-runtime";
import { resolveUserPath } from "chainbreaker/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("chainbreaker channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}

...
import { danger, info, success } from "chainbreaker/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "chainbreaker/plugin-sdk/runtime-env";
import { logInfo } from "chainbreaker/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (code === LOGGED_OUT_STATUS) {
      ...
      console.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("chainbreaker channels login")} and scan the QR again.`,
        ),
      );

...
import { VERSION } from "chainbreaker/plugin-sdk/cli-runtime";
import { danger, success } from "chainbreaker/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "chainbreaker/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "chainbreaker/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("chainbreaker channels login")}`,
              ),
            );
          }
        }

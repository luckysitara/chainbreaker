...
import { optimizeImageToPng } from "chainbreaker/plugin-sdk/media-runtime";
import { resolveStateDir } from "chainbreaker/plugin-sdk/state-paths";
import { resolvePreferredChainbreakerTmpDir } from "chainbreaker/plugin-sdk/temp-path";
import { mockPinnedHostnameResolution } from "chainbreaker/plugin-sdk/testing";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredChainbreakerTmpDir(), "chainbreaker-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub CHAINBREAKER_STATE_DIR.
    // Also keep it outside the Chainbreaker temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["CHAINBREAKER_STATE_DIR"]);
    process.env.CHAINBREAKER_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "chainbreaker-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredChainbreakerTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Chainbreaker state workspace and sandbox roots", async () => {
  ...
  it("rejects default Chainbreaker state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

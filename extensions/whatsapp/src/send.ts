import { loadConfig, type ChainbreakerConfig } from "chainbreaker/plugin-sdk/config-runtime";
import { resolveMarkdownTableMode } from "chainbreaker/plugin-sdk/config-runtime";
import { generateSecureUuid } from "chainbreaker/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "chainbreaker/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "chainbreaker/plugin-sdk/runtime-env";
import { getChildLogger } from "chainbreaker/plugin-sdk/text-runtime";
import { redactIdentifier } from "chainbreaker/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "chainbreaker/plugin-sdk/text-runtime";
import { markdownToWhatsApp } from "chainbreaker/plugin-sdk/text-runtime";
import { toWhatsappJid } from "chainbreaker/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: ChainbreakerConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);

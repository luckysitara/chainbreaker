...
import type { PollInput } from "chainbreaker/plugin-sdk/media-runtime";
...
export function requireActiveWebListener(accountId?: string | null): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${id}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`chainbreaker channels login --channel whatsapp --account ${id}`)}.`,
    );
  }

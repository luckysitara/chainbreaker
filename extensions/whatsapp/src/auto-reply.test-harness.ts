...
import { createPinnedLookup } from "chainbreaker/plugin-sdk/fetch-runtime";
import { resetInboundDedupe } from "chainbreaker/plugin-sdk/reply-runtime";
import { resetLogger, setLoggerOverride } from "chainbreaker/plugin-sdk/runtime-env";
import * as ssrf from "chainbreaker/plugin-sdk/ssrf-runtime";
...
vi.mock("chainbreaker/plugin-sdk/agent-runtime", async (importOriginal) => {
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-session-"));

import { DEFAULT_ACCOUNT_ID } from "chainbreaker/plugin-sdk/routing";
import type { RuntimeEnv } from "chainbreaker/plugin-sdk/runtime-env";
import { createWhatsAppPollFixture, expectWhatsAppPollSent } from "chainbreaker/plugin-sdk/testing";
...
import type { ChainbreakerConfig } from "./runtime-api.js";
...
const hoisted = vi.hoisted(() => ({
  ...
  listWhatsAppAccountIds: vi.fn((cfg: ChainbreakerConfig) => {
  ...
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/chainbreaker-whatsapp-test",
  })),
}));
...
vi.mock("chainbreaker/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("chainbreaker/plugin-sdk/setup")>(
    "chainbreaker/plugin-sdk/setup",
  );
  return {
    ...actual,
...
describe("whatsapp setup wizard", () => {
  ...
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/chainbreaker-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("chainbreaker channels login"),
      "WhatsApp",
    );
  });

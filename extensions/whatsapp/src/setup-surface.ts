...
} from "chainbreaker/plugin-sdk/setup";
import type { ChannelSetupWizard } from "chainbreaker/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "chainbreaker/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  ...
  patch: Partial<NonNullable<NonNullable<ChainbreakerConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: ChainbreakerConfig, dmPolicy: DmPolicy): ChainbreakerConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: ChainbreakerConfig, allowFrom?: string[]): ChainbreakerConfig {
...
async function detectWhatsAppLinked(cfg: ChainbreakerConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Chainbreaker can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Chainbreaker" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("chainbreaker channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }

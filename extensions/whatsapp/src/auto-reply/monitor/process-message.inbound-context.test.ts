...
describe("web processMessage inbound context", () => {
  ...
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-process-message-"));
  ...
  it("does not force an [chainbreaker] response prefix in self-chats when identity is unset", async () => {
    await processSelfDirectMessage({
      messages: {},
      session: { store: sessionStorePath },
    } as unknown as ReturnType<typeof import("chainbreaker/plugin-sdk/config-runtime").loadConfig>);

import { resolveInboundDebounceMs } from "chainbreaker/plugin-sdk/channel-inbound";
import { enqueueSystemEvent } from "chainbreaker/plugin-sdk/channel-runtime";
import { formatCliCommand } from "chainbreaker/plugin-sdk/cli-runtime";
import { waitForever } from "chainbreaker/plugin-sdk/cli-runtime";
import { hasControlCommand } from "chainbreaker/plugin-sdk/command-auth";
import { loadConfig } from "chainbreaker/plugin-sdk/config-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "chainbreaker/plugin-sdk/reply-history";
import { getReplyFromConfig } from "chainbreaker/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "chainbreaker/plugin-sdk/routing";
import { logVerbose } from "chainbreaker/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "chainbreaker/plugin-sdk/runtime-env";
import { getChildLogger } from "chainbreaker/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "chainbreaker/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("chainbreaker channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("chainbreaker channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }

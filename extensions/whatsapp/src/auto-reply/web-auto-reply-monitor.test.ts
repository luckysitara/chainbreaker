...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-group-gating-"));
...
function runGroupGating(params: {
  cfg: ReturnType<typeof import("chainbreaker/plugin-sdk/config-runtime").loadConfig>;
...
function makeInboundCfg(messagePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/chainbreaker" } },

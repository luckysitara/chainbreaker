...
import { saveSessionStore } from "chainbreaker/plugin-sdk/config-runtime";
...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\bchainbreaker\b/i] };
  ...
  it("falls back to regex when no mentions are present", () => {
    const msg = makeMsg({
      body: "chainbreaker can you help?",
      selfE164: "+15551234567",
      selfJid: "15551234567@s.whatsapp.net",
    });
    expectMentioned(msg, mentionCfg, true);
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\bchainbreaker\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("uses @lid reverse mapping for mentions and self identity", async () => {
    await withTempDir("chainbreaker-lid-mapping-", async (authDir) => {
      await fs.writeFile(
...
describe("getSessionSnapshot", () => {
  it("uses channel reset overrides when configured", async () => {
    ...
    try {
      await withTempDir("chainbreaker-snapshot-", async (root) => {
...
describe("web auto-reply util", () => {
  describe("mentions diagnostics", () => {
    it("returns normalized debug fields and mention outcome", () => {
      ...
      const result = debugMention(msg, { mentionRegexes: [/\bchainbreaker\b/i] });
      expect(result.wasMentioned).toBe(true);
      expect(result.details.bodyClean).toBe("chainbreaker ping");

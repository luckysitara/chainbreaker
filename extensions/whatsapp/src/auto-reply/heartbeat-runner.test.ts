import { redactIdentifier } from "chainbreaker/plugin-sdk/logging-core";
import type { getReplyFromConfig } from "chainbreaker/plugin-sdk/reply-runtime";
import { HEARTBEAT_TOKEN } from "chainbreaker/plugin-sdk/reply-runtime";
...
vi.mock("chainbreaker/plugin-sdk/agent-runtime", () => ({
...
vi.mock("chainbreaker/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.mock("chainbreaker/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/routing")>();
  return {
    ...actual,
...
vi.mock("chainbreaker/plugin-sdk/channel-runtime", async (importOriginal) => {
...
vi.mock("chainbreaker/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/runtime-env")>();
  const logger = {
    child: () => logger,
...
vi.mock("chainbreaker/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    redactIdentifier,
  };
...
vi.mock("chainbreaker/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.mock("../auth-store.js", () => ({
  WA_WEB_AUTH_DIR: "/tmp/chainbreaker-oauth/whatsapp/default",
  resolveDefaultWebAuthDir: () => "/tmp/chainbreaker-oauth/whatsapp/default",

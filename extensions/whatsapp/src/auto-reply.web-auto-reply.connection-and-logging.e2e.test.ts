...
import { setLoggerOverride } from "chainbreaker/plugin-sdk/runtime-env";
...
describe("web auto-reply connection", () => {
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/chainbreaker-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/chainbreaker-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
  ...
  it("marks dispatch idle after replies flush", async () => {
    ...
    const mockConfig: ChainbreakerConfig = {

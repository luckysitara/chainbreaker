...
const CONFIG_KEY = Symbol.for("chainbreaker:testConfigMock");
...
vi.mock("chainbreaker/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/media-runtime")>();
  const mockModule = Object.create(null) as Record<string, unknown>;
  Object.defineProperties(mockModule, Object.getOwnPropertyDescriptors(actual));
...
vi.mock("chainbreaker/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/chainbreaker-oauth",
  };
});
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("chainbreaker:lastSocket")] =
    created.lastSocket;
  return {
    ...actual,
    ...created.mod,
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("chainbreaker:lastSocket")];
  if (typeof getter === "function") {

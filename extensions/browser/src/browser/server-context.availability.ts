...
import {
  isChromeCdpReady,
  isChromeReachable,
  launchChainbreakerChrome,
  stopChainbreakerChrome,
} from "./chrome.js";
...
export function createProfileAvailability({
  ...
  const reconcileProfileRuntime = async (): Promise<void> => {
    ...
    if (profileState.running) {
      await stopChainbreakerChrome(profileState.running).catch(() => {});
      setProfileRunning(null);
    }
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchChainbreakerChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      // Browser control service can restart while a loopback Chainbreaker browser is still
      ...
      const launched = await launchChainbreakerChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopChainbreakerChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    if (!profileState.running) {
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by chainbreaker. ` +
          `Run action=reset-profile profile=${profile.name} to kill the process.`,
      );
    }

    await stopChainbreakerChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchChainbreakerChrome(current.resolved, profile);

...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredChainbreakerTmpDir: vi.fn(() => "/tmp/chainbreaker"),
}));
...
describe("pw-tools-core", () => {
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredChainbreakerTmpDir.mockReturnValue("/tmp/chainbreaker");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".chainbreaker-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredChainbreakerTmpDir.mockReturnValue("/tmp/chainbreaker-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "chainbreaker-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "chainbreaker-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredChainbreakerTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "chainbreaker-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "chainbreaker-preferred", "downloads")}${path.sep}`),
    );
  });

...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".chainbreaker-profile-decorated");
}
 ...
 */
export function decorateChainbreakerProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CHAINBREAKER_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CHAINBREAKER_BROWSER_COLOR).toUpperCase();

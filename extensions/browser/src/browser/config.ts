import type { BrowserConfig, BrowserProfileConfig, ChainbreakerConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_CHAINBREAKER_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_CHAINBREAKER_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "chainbreaker" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CHAINBREAKER_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CHAINBREAKER_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ChainbreakerConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_CHAINBREAKER_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_CHAINBREAKER_BROWSER_PROFILE_NAME]
        ? DEFAULT_CHAINBREAKER_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "chainbreaker";

...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "chainbreaker" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
const state: HarnessState = {
  ...
  cfgDefaultProfile: "chainbreaker",
  cfgProfiles: {},
  createTargetId: null,
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.CHAINBREAKER_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "chainbreaker",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/chainbreaker" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    chainbreaker: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchChainbreakerChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "chainbreaker";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.CHAINBREAKER_GATEWAY_PORT;
  process.env.CHAINBREAKER_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.CHAINBREAKER_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.CHAINBREAKER_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CHAINBREAKER_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.CHAINBREAKER_GATEWAY_PASSWORD;
  } else {
    process.env.CHAINBREAKER_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}

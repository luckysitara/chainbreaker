...
function localChainbreakerProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "chainbreaker",
...
function createLocalChainbreakerResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localChainbreakerProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveChainbreakerUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-reset-"));
    const profileDir = path.join(tempRoot, "chainbreaker");
    ...
    const ops = createLocalChainbreakerResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveChainbreakerUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-reset-no-own-"));
    const profileDir = path.join(tempRoot, "chainbreaker");
    fs.mkdirSync(profileDir, { recursive: true });

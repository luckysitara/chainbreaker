...
const { stopChainbreakerChromeMock } = vi.hoisted(() => ({
  stopChainbreakerChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopChainbreakerChrome: stopChainbreakerChromeMock,
}));
...
beforeEach(async () => {
  ...
  stopChainbreakerChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["chainbreaker", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      chainbreaker: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.chainbreaker).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopChainbreakerChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("chainbreaker browser stop failed: Error: oops");
  });
});

  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          chainbreaker: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.CHAINBREAKER_GATEWAY_PORT;
    process.env.CHAINBREAKER_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.CHAINBREAKER_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.CHAINBREAKER_GATEWAY_PASSWORD;

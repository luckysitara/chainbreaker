...
export function buildChainbreakerChromeLaunchArgs(params: {
...
export async function launchChainbreakerChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveChainbreakerUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_CHAINBREAKER_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildChainbreakerChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateChainbreakerProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 chainbreaker browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`chainbreaker browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`chainbreaker browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 chainbreaker browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopChainbreakerChrome(

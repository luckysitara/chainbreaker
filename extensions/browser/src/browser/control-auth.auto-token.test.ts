...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => ChainbreakerConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<ChainbreakerConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: ChainbreakerConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,

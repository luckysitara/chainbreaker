...
export function registerBrowserFilesAndDownloadsCommands(
  ...
  browser
    ...
    .argument(
      "<paths...>",
      "File paths to upload (must be within Chainbreaker temp uploads dir, e.g. /tmp/chainbreaker/uploads/file.pdf)",
    )
  ...
  browser
    ...
    .argument(
      "[path]",
      "Save path within chainbreaker temp downloads dir (default: /tmp/chainbreaker/downloads/...; fallback: os.tmpdir()/chainbreaker/downloads/...)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",
  ...
  browser
    ...
    .argument(
      "<path>",
      "Save path within chainbreaker temp downloads dir (e.g. report.pdf or /tmp/chainbreaker/downloads/report.pdf)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",

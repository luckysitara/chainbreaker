...
Read-only diff viewer plugin for **Chainbreaker** agents.
...
Set plugin-wide defaults in `~/.chainbreaker/chainbreaker.json`:
...
Before:
Chainbreaker supports plugins.

After:
Chainbreaker supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/chainbreaker-diffs`).

...
vi.mock("chainbreaker/plugin-sdk/provider-model-shared", () => ({
...
vi.mock("chainbreaker/plugin-sdk/json-store", () => ({
  loadJsonFile,
  saveJsonFile,
}));

vi.mock("chainbreaker/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/chainbreaker-state",
}));

import type { ProviderResolveDynamicModelContext } from "chainbreaker/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/chainbreaker-state/credentials/github-copilot.token.json";

...
import { resetInboundDedupe } from "chainbreaker/plugin-sdk/reply-runtime";
import type { MsgContext } from "chainbreaker/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "chainbreaker/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "chainbreaker/plugin-sdk/reply-runtime";
import type { MockFn } from "chainbreaker/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("chainbreaker/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("chainbreaker/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("chainbreaker/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("chainbreaker/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/chainbreaker-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("chainbreaker/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("chainbreaker/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("chainbreaker/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
const replySpyHoisted = vi.hoisted(() => ({
  replySpy: vi.fn(async (_ctx: MsgContext, opts?: GetReplyOptions) => {
    ...
    (
      ...
      configOverride?: ChainbreakerConfig,
...
function createModelsProviderDataFromConfig(cfg: ChainbreakerConfig): {
...
vi.doMock("chainbreaker/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("chainbreaker/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("chainbreaker/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("chainbreaker/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("chainbreaker/plugin-sdk/channel-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("chainbreaker/plugin-sdk/channel-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: ChainbreakerConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: ChainbreakerConfig) => {

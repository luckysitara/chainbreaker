import type { TelegramNetworkConfig } from "chainbreaker/plugin-sdk/config-runtime";
...
let isWSL2Sync: typeof import("chainbreaker/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  vi.resetModules();
  ({ isWSL2Sync } = await import("chainbreaker/plugin-sdk/runtime-env"));
  ({
  ...
  it.each([
    {
      ...
      expected: {
        value: true,
        source: "env:CHAINBREAKER_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { CHAINBREAKER_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:CHAINBREAKER_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { CHAINBREAKER_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { CHAINBREAKER_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:CHAINBREAKER_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { CHAINBREAKER_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:CHAINBREAKER_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { CHAINBREAKER_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },

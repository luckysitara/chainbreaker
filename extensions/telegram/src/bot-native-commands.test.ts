...
import { TELEGRAM_COMMAND_NAME_PATTERN } from "chainbreaker/plugin-sdk/config-runtime";
...
import { STATE_DIR } from "chainbreaker/plugin-sdk/state-paths";
    ...
    expect(firstDeliverRepliesCall?.[0]).toEqual(
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([
          expect.stringMatching(/[\\/]\.chainbreaker[\\/]workspace-work$/),
        ]),
      }),
    );

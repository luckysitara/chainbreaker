...
} from "chainbreaker/plugin-sdk/command-auth-native";
import {
...
import type { ChannelGroupPolicy } from "chainbreaker/plugin-sdk/config-runtime";
import { getRuntimeConfigSnapshot } from "chainbreaker/plugin-sdk/config-runtime";
...
import {
  ...
  resolveChunkMode,
} from "chainbreaker/plugin-sdk/reply-dispatch-runtime";
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): ChainbreakerConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: ChainbreakerConfig): TelegramAccountConfig => {
    try {

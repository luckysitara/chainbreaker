...
import type { PluginRuntime } from "chainbreaker/plugin-sdk/testing";
...
function createCfg(): ChainbreakerConfig {
  return {
    channels: {
      telegram: {
        enabled: true,
...
function resolveAccount(cfg: ChainbreakerConfig, accountId: string): ResolvedTelegramAccount {
  return telegramPlugin.config.resolveAccount(cfg, accountId) as ResolvedTelegramAccount;
}

function createStartTelegramContext(cfg: ChainbreakerConfig, accountId: string) {
...
function startTelegramAccount(cfg: ChainbreakerConfig, accountId: string) {
...
function configureOpsProxyNetwork(cfg: ChainbreakerConfig) {
...
describe("telegramPlugin duplicate token guard", () => {
  ...
  // Regression: https://github.com/chainbreaker/chainbreaker/issues/53876

...
import type { PluginInteractiveTelegramHandlerContext } from "chainbreaker/plugin-sdk/core";
import {
  clearPluginInteractiveHandlers,
  registerPluginInteractiveHandler,
} from "chainbreaker/plugin-sdk/plugin-runtime";
import { expectChannelInboundContextContract as expectInboundContextContract } from "chainbreaker/plugin-sdk/testing";
...
let normalizeTelegramCommandName: typeof import("chainbreaker/plugin-sdk/config-runtime").normalizeTelegramCommandName;
...
describe("createTelegramBot", () => {
  beforeAll(async () => {
    ...
    ({ normalizeTelegramCommandName } = await import("chainbreaker/plugin-sdk/config-runtime"));
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/chainbreaker-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/chainbreaker-telegram-model-compact-${process.pid}-${Date.now()}.json`;
    const config = {
      agents: {
        defaults: {
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/chainbreaker-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Chainbreaker" },
        },
      },
      me: { id: 999, username: "chainbreaker_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });

...
describe("handleTelegramAction", () => {
  ...
  function telegramConfig(overrides?: Record<string, unknown>): ChainbreakerConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as ChainbreakerConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: ChainbreakerConfig, accountId = "media") {
    await handleTelegramAction(

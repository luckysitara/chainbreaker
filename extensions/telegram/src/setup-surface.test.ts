...
import { DEFAULT_ACCOUNT_ID } from "chainbreaker/plugin-sdk/setup";
...
async function runPrepare(cfg: ChainbreakerConfig, accountId: string) {
...
async function runFinalize(cfg: ChainbreakerConfig, accountId: string) {
...
function expectPreparedResult(
...
): { cfg: ChainbreakerConfig } & Exclude<Awaited<ReturnType<typeof runPrepare>>, void | undefined> {
  ...
  return prepared as { cfg: ChainbreakerConfig } & Exclude<
...
describe("telegramSetupWizard.finalize", () => {
  it("shows global config commands for the default account", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining('chainbreaker config set channels.telegram.dmPolicy "allowlist"'),
  ...
  it("shows account-scoped config commands for named accounts", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        'chainbreaker config set channels.telegram.accounts.alerts.dmPolicy "allowlist"',
      ),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        `chainbreaker config set channels.telegram.accounts.alerts.allowFrom '["YOUR_USER_ID"]'`,
      ),
      "Telegram DM access warning",
    );
  });

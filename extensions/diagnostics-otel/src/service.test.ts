...
import type { ChainbreakerPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/chainbreaker-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): ChainbreakerPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("chainbreaker.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("chainbreaker.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("chainbreaker.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("chainbreaker.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("chainbreaker.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("chainbreaker.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("chainbreaker.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("chainbreaker.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("chainbreaker.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("chainbreaker.webhook.processed");
    expect(spanNames).toContain("chainbreaker.message.processed");
    expect(spanNames).toContain("chainbreaker.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["chainbreaker.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("chainbreaker.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "chainbreaker.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["chainbreaker.reason"]).toBe("string");
    expect(String(attrs?.["chainbreaker.reason"])).not.toContain(

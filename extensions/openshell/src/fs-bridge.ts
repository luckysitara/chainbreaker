...
import { createWritableRenameTargetResolver } from "chainbreaker/plugin-sdk/sandbox";
...
class OpenShellFsBridge implements SandboxFsBridge {
  ...
  async writeFile(params: {
    ...
    const tempPath = path.join(
      parentDir,
      `.chainbreaker-openshell-write-${path.basename(hostPath)}-${process.pid}-${Date.now()}`,
    );

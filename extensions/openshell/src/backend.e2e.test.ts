...
const CHAINBREAKER_OPENSHELL_E2E = process.env.CHAINBREAKER_E2E_OPENSHELL === "1";
const CHAINBREAKER_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const CHAINBREAKER_OPENSHELL_COMMAND =
  process.env.CHAINBREAKER_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "chainbreaker-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && CHAINBREAKER_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: CHAINBREAKER_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(CHAINBREAKER_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "chainbreaker-openshell-e2e-"));
      ...
      const gatewayName = `chainbreaker-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `chainbreaker-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/chainbreaker-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: CHAINBREAKER_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("chainbreaker-openshell-e2e");

...
import type { ChainbreakerPluginApi } from "./api.js";
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredChainbreakerTmpDir: pluginApiMocks.resolvePreferredChainbreakerTmpDir,
...
function createApi(params?: {
  runtime?: ChainbreakerPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as ChainbreakerPluginApi["runtime"],
    registerCommand: params?.registerCommand,
  }) as ChainbreakerPluginApi;
}
...
function registerPairCommand(params?: {
  ...
  let command: ChainbreakerPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as ChainbreakerPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredChainbreakerTmpDir(), { recursive: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Chainbreaker iOS app:");
    expect(text).toContain("![Chainbreaker pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Chainbreaker iOS app:");

...
async function writeQrPngTempFile(data: string): Promise<string> {
  const pngBase64 = await renderQrPngBase64(data);
  const tmpRoot = resolvePreferredChainbreakerTmpDir();
...
function resolveScheme(
  cfg: ChainbreakerPluginApi["config"],
...
async function resolveGatewayUrl(api: ChainbreakerPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for Chainbreaker devices",
          ...
          if (channel === "webchat") {
            ...
            return {
              text: [
                ...
                "",
                `![Chainbreaker pairing QR](${qrDataUrl})`,
              ].join("\n"),
            };
          }

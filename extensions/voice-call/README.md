...
Official Voice Call plugin for **Chainbreaker**.
...
Docs: `https://docs.chainbreaker.ai/plugins/voice-call`
Plugin system: `https://docs.chainbreaker.ai/plugin`

## Install (local dev)

### Option A: install via Chainbreaker (recommended)
...
```bash
PLUGIN_HOME=~/.chainbreaker/extensions
mkdir -p "$PLUGIN_HOME"
...
- advanced webhook, streaming, and tunnel notes: `https://docs.chainbreaker.ai/plugins/voice-call`

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.chainbreaker.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.chainbreaker.ai/plugins/voice-call#tts-for-calls`

## CLI

```bash
chainbreaker voicecall call --to "+15555550123" --message "Hello from Chainbreaker"
chainbreaker voicecall continue --call-id <id> --message "Any questions?"
chainbreaker voicecall speak --call-id <id> --message "One moment"
chainbreaker voicecall end --call-id <id>
chainbreaker voicecall status --call-id <id>
chainbreaker voicecall tail

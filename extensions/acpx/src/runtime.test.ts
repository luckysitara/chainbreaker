...
import { runAcpRuntimeAdapterContract } from "chainbreaker/plugin-sdk/testing";
...
describe("AcpxRuntime", () => {
  ...
  it("ensures sessions and streams prompt events", async () => {
    ...
    expect(prompt?.chainbreakerShell).toBe("acp");
  ...
  it("routes ACPX commands through the built-in plugin-tools bridge only when explicitly enabled", async () => {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-acpx-plugin-tools-"));
    ...
    try {
      ...
      fs.writeFileSync(path.join(pluginRoot, "chainbreaker.plugin.json"), "{}\n", "utf8");
      ...
      expect(payload.mcpServers).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            name: "chainbreaker-plugin-tools",
  ...
  it("does not mark backend unhealthy when a per-session cwd is missing", async () => {
    const { runtime } = await createMockRuntimeFixture();
    const missingCwd = path.join(os.tmpdir(), "chainbreaker-acpx-runtime-test-missing-cwd");

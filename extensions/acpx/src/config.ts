...
import { buildPluginConfigSchema } from "chainbreaker/plugin-sdk/core";
import { z } from "chainbreaker/plugin-sdk/zod";
import type { ChainbreakerPluginConfigSchema } from "../runtime-api.js";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "chainbreaker-plugin-tools";
const ACPX_BIN_NAME = process.platform === "win32" ? "acpx.cmd" : "acpx";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "chainbreaker.plugin.json")) &&
...
function resolveChainbreakerRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openClawRoot = resolveChainbreakerRoot(pluginRoot);
...
export function createAcpxPluginConfigSchema(): ChainbreakerPluginConfigSchema {

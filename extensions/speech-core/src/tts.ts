...
import { normalizeChannelId, type ChannelId } from "chainbreaker/plugin-sdk/channel-runtime";
import type {
  ChainbreakerConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "chainbreaker/plugin-sdk/logging-core";
...
import { isVerbose, logVerbose } from "chainbreaker/plugin-sdk/runtime-env";
import { resolvePreferredChainbreakerTmpDir } from "chainbreaker/plugin-sdk/sandbox";
import { CONFIG_DIR, resolveUserPath, stripMarkdown } from "chainbreaker/plugin-sdk/text-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.CHAINBREAKER_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: ChainbreakerConfig) {
...
function resolveRegistryDefaultSpeechProviderId(cfg?: ChainbreakerConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: ChainbreakerConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: ChainbreakerConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: ChainbreakerConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: ChainbreakerConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredChainbreakerTmpDir();

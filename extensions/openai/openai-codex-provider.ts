...
} from "chainbreaker/plugin-sdk/provider-auth";
import { buildOauthProviderAuthResult } from "chainbreaker/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "chainbreaker/plugin-sdk/provider-auth-login";
import {
...
import { createOpenAIAttributionHeadersWrapper } from "chainbreaker/plugin-sdk/provider-stream";
import { fetchCodexUsage } from "chainbreaker/plugin-sdk/provider-usage";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `chainbreaker models auth login --provider openai-codex` or `chainbreaker configure`.";
}

...
`chainbreaker.invoke --each`) without adding new Chainbreaker code per workflow.
...
This extension depends on Chainbreaker internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Chainbreaker extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.chainbreaker/extensions` as a standalone plugin directory.

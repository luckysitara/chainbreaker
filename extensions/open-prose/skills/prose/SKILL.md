...
metadata: { "chainbreaker": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Chainbreaker `sessions_spawn`
- **File I/O** == Chainbreaker `read`/`write`
- **Remote fetch** == Chainbreaker `web_fetch` (or `exec` with curl when POST is required)

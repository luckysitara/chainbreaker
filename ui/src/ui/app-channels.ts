...
export async function handleWhatsAppStart(host: ChainbreakerApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ChainbreakerApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ChainbreakerApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ChainbreakerApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ChainbreakerApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ChainbreakerApp): string {
...
function resolveGatewayHttpAuthHeader(host: ChainbreakerApp): string | null {
...
function buildGatewayHttpHeaders(host: ChainbreakerApp): Record<string, string> {
  const authorization = resolveGatewayHttpAuthHeader(host);
  return authorization ? { Authorization: authorization } : {};
}

export function handleNostrProfileEdit(
  host: ChainbreakerApp,
  accountId: string,
...
export function handleNostrProfileCancel(host: ChainbreakerApp) {
...
export function handleNostrProfileToggleAdvanced(host: ChainbreakerApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ChainbreakerApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ChainbreakerApp) {
  const state = host.nostrProfileFormState;

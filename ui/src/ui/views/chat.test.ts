...
describe("chat view", () => {
  ...
  it("keeps the welcome logo fallback under the mounted base path", () => {
    ...
    expect(logoImage).not.toBeNull();
    expect(logoImage?.getAttribute("src")).toBe("/chainbreaker/favicon.svg");
  });
  ...
  it("keeps grouped assistant avatar fallbacks under the mounted base path", () => {
    ...
    expect(groupedLogo?.getAttribute("src")).toBe("/chainbreaker/favicon.svg");
  });

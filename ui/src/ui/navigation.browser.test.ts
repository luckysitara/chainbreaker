...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/chainbreaker");
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/chainbreaker/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/chainbreaker");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/chainbreaker/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CHAINBREAKER_CONTROL_UI_BASE_PATH__ = "/chainbreaker";
    const app = mountApp("/chainbreaker/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/chainbreaker");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/chainbreaker/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("chainbreaker.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "chainbreaker.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/chainbreaker" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;
    ...
    expect(app.settings.token).toBe("abc123");
    expect(
      JSON.parse(localStorage.getItem("chainbreaker.control.settings.v1") ?? "{}"),
    ).toMatchObject({
      gatewayUrl: "wss://gateway.example/chainbreaker",
    });
    expect(JSON.parse(localStorage.getItem("chainbreaker.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/chainbreaker";
  ...
  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/chainbreaker#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/chainbreaker");
    expect(app.settings.token).toBe("");

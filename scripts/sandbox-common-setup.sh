...
BASE_IMAGE="${BASE_IMAGE:-chainbreaker-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-chainbreaker-sandbox-common:bookworm-slim}"
...
CHAINBREAKER_DOCKER_BUILD_USE_BUILDX="${CHAINBREAKER_DOCKER_BUILD_USE_BUILDX:-0}"
CHAINBREAKER_DOCKER_BUILD_CACHE_FROM="${CHAINBREAKER_DOCKER_BUILD_CACHE_FROM:-}"
CHAINBREAKER_DOCKER_BUILD_CACHE_TO="${CHAINBREAKER_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${CHAINBREAKER_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${CHAINBREAKER_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${CHAINBREAKER_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${CHAINBREAKER_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${CHAINBREAKER_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=chainbreaker.sandbox=1)
NOTE

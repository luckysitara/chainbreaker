...
# Chainbreaker Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Chainbreaker."
...
GUM_VERSION="${CHAINBREAKER_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Chainbreaker Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://chainbreaker.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$CHAINBREAKER_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.chainbreaker.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_chainbreaker_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.chainbreaker-* "$npm_root"/chainbreaker 2>/dev/null || true
}

extract_chainbreaker_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_chainbreaker_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/chainbreaker" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/chainbreaker"|"/usr/local/bin/chainbreaker")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/chainbreaker/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale chainbreaker symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing chainbreaker binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Chainbreaker package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_chainbreaker_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Chainbreaker npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*chainbreaker" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_chainbreaker_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_chainbreaker_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Chainbreaker npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an chainbreaker binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${CHAINBREAKER_TAGLINE_INDEX:-}" ]]; then
        if [[ "${CHAINBREAKER_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((CHAINBREAKER_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${CHAINBREAKER_NO_ONBOARD:-0}
NO_PROMPT=${CHAINBREAKER_NO_PROMPT:-0}
DRY_RUN=${CHAINBREAKER_DRY_RUN:-0}
INSTALL_METHOD=${CHAINBREAKER_INSTALL_METHOD:-}
CHAINBREAKER_VERSION=${CHAINBREAKER_VERSION:-latest}
USE_BETA=${CHAINBREAKER_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/chainbreaker"
GIT_DIR=${CHAINBREAKER_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${CHAINBREAKER_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${CHAINBREAKER_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${CHAINBREAKER_VERBOSE:-0}"
VERIFY_INSTALL="${CHAINBREAKER_VERIFY_INSTALL:-0}"
CHAINBREAKER_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Chainbreaker installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/chainbreaker)
...
Environment variables:
  CHAINBREAKER_INSTALL_METHOD=git|npm
  CHAINBREAKER_VERSION=latest|next|main|<semver>|<spec>
  CHAINBREAKER_BETA=0|1
  CHAINBREAKER_GIT_DIR=...
  CHAINBREAKER_GIT_UPDATE=0|1
  CHAINBREAKER_NO_PROMPT=1
  CHAINBREAKER_VERIFY_INSTALL=1
  CHAINBREAKER_DRY_RUN=1
  CHAINBREAKER_NO_ONBOARD=1
  CHAINBREAKER_VERBOSE=1
  CHAINBREAKER_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Chainbreaker checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Chainbreaker source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_chainbreaker_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"chainbreaker"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://chainbreaker.ai/install.sh | bash"
}
...
ensure_chainbreaker_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/chainbreaker" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/chainbreaker" ]]; then
        ln -sf "$npm_root/chainbreaker/dist/entry.js" "${npm_bin}/chainbreaker"
        ui_info "Created chainbreaker bin link at ${npm_bin}/chainbreaker"
    fi
    return 0
}

# Check for existing Chainbreaker installation
check_existing_chainbreaker() {
    if [[ -n "$(type -P chainbreaker 2>/dev/null || true)" ]]; then
        ui_info "Existing Chainbreaker installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make chainbreaker show as \"command not found\" in new terminals."
...
warn_chainbreaker_not_found() {
    ui_warn "Installed, but chainbreaker is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t chainbreaker 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named chainbreaker; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_chainbreaker_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P chainbreaker 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/chainbreaker" ]]; then
        echo "${npm_bin}/chainbreaker"
        return 0
    fi
...
install_chainbreaker_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/chainbreaker/chainbreaker.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Chainbreaker from git checkout: ${repo_dir}"
    else
        ui_info "Installing Chainbreaker from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Chainbreaker" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Chainbreaker" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/chainbreaker" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/chainbreaker"
    ui_success "Chainbreaker wrapper installed to \$HOME/.local/bin/chainbreaker"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view chainbreaker dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:chainbreaker/chainbreaker#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_chainbreaker() {
    local package_name="chainbreaker"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            CHAINBREAKER_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="chainbreaker"
        else
            CHAINBREAKER_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${CHAINBREAKER_VERSION}" ]]; then
        CHAINBREAKER_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${CHAINBREAKER_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${CHAINBREAKER_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Chainbreaker v${resolved_version}"
    else
        ui_info "Installing Chainbreaker (${CHAINBREAKER_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${CHAINBREAKER_VERSION}")"

    if ! install_chainbreaker_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_chainbreaker_paths
        install_chainbreaker_npm "${install_spec}"
    fi

    if [[ "${CHAINBREAKER_VERSION}" == "latest" && "${package_name}" == "chainbreaker" ]]; then
        if ! resolve_chainbreaker_bin &> /dev/null; then
            ui_warn "npm install chainbreaker@latest failed; retrying chainbreaker@next"
            cleanup_npm_chainbreaker_paths
            install_chainbreaker_npm "chainbreaker@next"
        fi
    fi

    ensure_chainbreaker_bin_link || true

    ui_success "Chainbreaker installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${CHAINBREAKER_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_chainbreaker_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (chainbreaker not on PATH yet)"
        warn_chainbreaker_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${CHAINBREAKER_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.chainbreaker/workspace-${profile}"
    else
        echo "${HOME}/.chainbreaker/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${CHAINBREAKER_CONFIG_PATH:-$HOME/.chainbreaker/chainbreaker.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run chainbreaker onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but chainbreaker not on PATH; skipping onboarding"
        warn_chainbreaker_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run chainbreaker onboard to retry"
        return
    }
}
...
if ! declare -F extract_chainbreaker_semver >/dev/null 2>&1; then
# Inline fallback when version-parse.sh could not be sourced (for example, stdin install).
extract_chainbreaker_semver() {
    local raw="${1:-}"
    local parsed=""
    parsed="$(
        printf '%s\n' "$raw" \
...
resolve_chainbreaker_version() {
    local version=""
    local raw_version_output=""
    local claw="${CHAINBREAKER_BIN:-}"
    if [[ -z "$claw" ]] && command -v chainbreaker &> /dev/null; then
        claw="$(command -v chainbreaker)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_chainbreaker_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/chainbreaker/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/chainbreaker/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: chainbreaker not on PATH yet"
        warn_chainbreaker_not_found
        return 1
    fi

    run_quiet_step "Checking Chainbreaker version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: chainbreaker gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_chainbreaker_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Chainbreaker checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set CHAINBREAKER_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_chainbreaker; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Chainbreaker"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g chainbreaker &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g chainbreaker 2>/dev/null || true
        ...
        install_chainbreaker_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/chainbreaker" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/chainbreaker"
    ...
    CHAINBREAKER_BIN="$(resolve_chainbreaker_bin || true)"
    ...
    installed_version=$(resolve_chainbreaker_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Chainbreaker installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Chainbreaker installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/chainbreaker"
        ui_kv "Update command" "chainbreaker update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://chainbreaker.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running chainbreaker doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                CHAINBREAKER_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                CHAINBREAKER_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                CHAINBREAKER_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run chainbreaker doctor and chainbreaker plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run chainbreaker onboard later"
        else
            local config_path="${CHAINBREAKER_CONFIG_PATH:-$HOME/.chainbreaker/chainbreaker.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (chainbreaker not on PATH yet)"
                    warn_chainbreaker_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run chainbreaker onboard to finish setup"
            return 0
        fi
    fi

    if command -v chainbreaker &> /dev/null; then
        local claw="${CHAINBREAKER_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_chainbreaker_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (chainbreaker daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if CHAINBREAKER_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: chainbreaker daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${CHAINBREAKER_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi

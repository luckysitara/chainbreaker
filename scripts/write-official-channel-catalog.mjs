...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.chainbreaker) ? packageJson.chainbreaker : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.chainbreaker?.channel?.id) || left.name;
    const rightId = trimString(right.chainbreaker?.channel?.id) || right.name;

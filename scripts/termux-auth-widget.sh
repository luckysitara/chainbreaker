...
# Chainbreaker Auth Widget for Termux
...
SERVER="${CHAINBREAKER_SERVER:-l36}"

# Check auth status
termux-toast "Checking Chainbreaker auth..."

STATUS=$(ssh "$SERVER" '$HOME/chainbreaker/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/chainbreaker/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CHAINBREAKER_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/chainbreaker/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CHAINBREAKER_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Chainbreaker won't work until re-auth"
                ;;
        esac
        ;;

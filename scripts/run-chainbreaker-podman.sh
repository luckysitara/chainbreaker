#!/usr/bin/env bash
# Rootless Chainbreaker in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-chainbreaker-podman.sh launch        # Start gateway
#   ./scripts/run-chainbreaker-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   chainbreaker --container chainbreaker dashboard --no-open
#   chainbreaker --container chainbreaker channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      CHAINBREAKER_GATEWAY_TOKEN|CHAINBREAKER_PODMAN_CONTAINER|CHAINBREAKER_PODMAN_IMAGE|CHAINBREAKER_IMAGE|CHAINBREAKER_PODMAN_PULL|CHAINBREAKER_PODMAN_GATEWAY_HOST_PORT|CHAINBREAKER_GATEWAY_PORT|CHAINBREAKER_PODMAN_BRIDGE_HOST_PORT|CHAINBREAKER_BRIDGE_PORT|CHAINBREAKER_GATEWAY_BIND|CHAINBREAKER_PODMAN_USERNS|CHAINBREAKER_BIND_MOUNT_OPTIONS|CHAINBREAKER_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-chainbreaker-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${CHAINBREAKER_CONFIG_DIR:-$EFFECTIVE_HOME/.chainbreaker}"
ENV_FILE="${CHAINBREAKER_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${CHAINBREAKER_CONFIG_DIR:-$EFFECTIVE_HOME/.chainbreaker}"
ENV_FILE="${CHAINBREAKER_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${CHAINBREAKER_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${CHAINBREAKER_PODMAN_CONTAINER:-chainbreaker}"
CHAINBREAKER_IMAGE="${CHAINBREAKER_PODMAN_IMAGE:-${CHAINBREAKER_IMAGE:-chainbreaker:local}}"
PODMAN_PULL="${CHAINBREAKER_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${CHAINBREAKER_PODMAN_GATEWAY_HOST_PORT:-${CHAINBREAKER_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${CHAINBREAKER_PODMAN_BRIDGE_HOST_PORT:-${CHAINBREAKER_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${CHAINBREAKER_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$CHAINBREAKER_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  CHAINBREAKER_CONTAINER="" CHAINBREAKER_CONFIG_DIR="$config_dir" \
    chainbreaker config get gateway.bind 2>/dev/null || true
}
...
# CHAINBREAKER_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${CHAINBREAKER_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'CHAINBREAKER_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v chainbreaker >/dev/null 2>&1; then
    echo "Warning: chainbreaker not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    CHAINBREAKER_CONTAINER="" CHAINBREAKER_CONFIG_DIR="$config_dir" \
      chainbreaker config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    CHAINBREAKER_CONTAINER="" CHAINBREAKER_CONFIG_DIR="$config_dir" \
      chainbreaker config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  CHAINBREAKER_CONTAINER="" CHAINBREAKER_CONFIG_DIR="$config_dir" \
    chainbreaker config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${CHAINBREAKER_GATEWAY_TOKEN:-}" ]]; then
  export CHAINBREAKER_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "CHAINBREAKER_GATEWAY_TOKEN" "$CHAINBREAKER_GATEWAY_TOKEN"
  echo "Generated CHAINBREAKER_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/chainbreaker.json"
...
PODMAN_USERNS="${CHAINBREAKER_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported CHAINBREAKER_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (CHAINBREAKER_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${CHAINBREAKER_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${CHAINBREAKER_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$CHAINBREAKER_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.chainbreaker/.npm \
    -e CHAINBREAKER_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.chainbreaker:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.chainbreaker/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$CHAINBREAKER_IMAGE" \
...
echo "Host CLI: chainbreaker --container $CONTAINER_NAME dashboard --no-open"
...
if [[ "$PLATFORM_NAME" == "Darwin" ]]; then
  ...
  echo "  CHAINBREAKER_CONTAINER=$CONTAINER_NAME chainbreaker dashboard --no-open"
  echo "  One-time setup:"
  echo "    CHAINBREAKER_CONTAINER=$CONTAINER_NAME chainbreaker config set gateway.controlUi.allowedOrigins '[\"http://127.0.0.1:18789\",\"http://localhost:18789\",\"http://127.0.0.1:28889\",\"http://localhost:28889\"]' --strict-json"

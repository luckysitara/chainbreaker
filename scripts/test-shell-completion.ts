 ...
 * `chainbreaker update`. Use it to verify the completion installation flow
...
\`chainbreaker update\`, \`chainbreaker doctor\`, and \`chainbreaker onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Chainbreaker Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`chainbreaker completion --install\` later to enable.`));
    return;
  }

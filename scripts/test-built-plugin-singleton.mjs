...
const warningFilterKey = Symbol.for("chainbreaker.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadChainbreakerPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadChainbreakerPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@chainbreaker/build-smoke-plugin",
      type: "module",
      chainbreaker: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "chainbreaker.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'chainbreaker/plugin-sdk';",
...
const registry = loadChainbreakerPlugins({
  ...
  env: {
    ...process.env,
    CHAINBREAKER_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),

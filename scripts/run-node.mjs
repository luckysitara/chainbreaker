...
const extensionRestartMetadataFiles = new Set(["chainbreaker.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.CHAINBREAKER_FORCE_BUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by CHAINBREAKER_FORCE_BUILD",
...
const logRunner = (message, deps) => {
  if (deps.env.CHAINBREAKER_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[chainbreaker] ${message}\n`);
};

const runChainbreaker = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["chainbreaker.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!buildRequirement.shouldBuild) {
    ...
    return await runChainbreaker(deps);
  }

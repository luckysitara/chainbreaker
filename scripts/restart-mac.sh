#!/usr/bin/env bash
# Reset Chainbreaker like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CHAINBREAKER_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Chainbreaker.app/Contents/MacOS/Chainbreaker"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Chainbreaker"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Chainbreaker"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Chainbreaker"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.chainbreaker.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/chainbreaker-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CHAINBREAKER_RESTART_LOG:-/tmp/chainbreaker-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CHAINBREAKER_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.chainbreaker/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  CHAINBREAKER_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node chainbreaker.mjs daemon install --force --runtime node"
      log "  node chainbreaker.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.chainbreaker/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.chainbreaker.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Chainbreaker instances"
kill_all_chainbreaker
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Chainbreaker"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Chainbreaker.app" ]]; then
    APP_BUNDLE="/Applications/Chainbreaker.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Chainbreaker.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Chainbreaker.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Chainbreaker.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CHAINBREAKER_APP_BUNDLE to your installed Chainbreaker.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node chainbreaker.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node chainbreaker.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".chainbreaker", "chainbreaker.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Chainbreaker is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.chainbreaker.gateway.plist' | head -n 40 || true"
fi

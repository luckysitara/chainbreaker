...
show_usage() {
    cat << EOF
clawlog - Chainbreaker Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Chainbreaker logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Chainbreaker logs flow through the macOS unified log (subsystem: ai.chainbreaker).

#!/usr/bin/env bash
# ClawDock - Docker helpers for Chainbreaker
# Inspired by Simon Willison's "Running Chainbreaker in Docker"
# https://til.simonwillison.net/llms/chainbreaker-docker
#
# Installation:
...
# Common paths to check for Chainbreaker
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/chainbreaker"
  "${HOME}/workspace/chainbreaker"
  "${HOME}/projects/chainbreaker"
  "${HOME}/dev/chainbreaker"
  "${HOME}/code/chainbreaker"
  "${HOME}/src/chainbreaker"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Chainbreaker at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/chainbreaker"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Chainbreaker not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/chainbreaker/chainbreaker.git ~/chainbreaker"
    echo "  cd ~/chainbreaker && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^CHAINBREAKER_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d chainbreaker-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart chainbreaker-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f chainbreaker-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.chainbreaker"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show chainbreaker.json
  if [[ -f "${config_dir}/chainbreaker.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/chainbreaker.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/chainbreaker.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No chainbreaker.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.chainbreaker/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec chainbreaker-gateway \
    bash -c 'echo "alias chainbreaker=\"./chainbreaker.mjs\"" > /tmp/.bashrc_chainbreaker && bash --rcfile /tmp/.bashrc_chainbreaker'
}
...
clawdock-cli() {
  _clawdock_compose run --rm chainbreaker-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build chainbreaker-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d chainbreaker-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build chainbreaker-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "CHAINBREAKER_GATEWAY_TOKEN=$token" chainbreaker-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" chainbreaker-gateway \
    bash -c './chainbreaker.mjs config set gateway.remote.token "$TOKEN" && ./chainbreaker.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec chainbreaker-gateway \
    bash -c "./chainbreaker.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart chainbreaker-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm chainbreaker-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec chainbreaker-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'chainbreaker config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Chainbreaker${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (chainbreaker alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to chainbreaker project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.chainbreaker)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'chainbreaker channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'chainbreaker status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.chainbreaker.ai${_CLR_RESET}"
  echo ""
}

...
const buildRequestedSurfaces = (request, env) => {
  ...
  const surfaces = [];
  const skipDefaultRuns = env.CHAINBREAKER_TEST_SKIP_DEFAULT === "1";
  if (!skipDefaultRuns) {
    surfaces.push("unit");
  }
  if (env.CHAINBREAKER_TEST_INCLUDE_EXTENSIONS === "1") {
    surfaces.push("extensions");
  }
  if (env.CHAINBREAKER_TEST_INCLUDE_CHANNELS === "1") {
    surfaces.push("channels");
  }
  if (env.CHAINBREAKER_TEST_INCLUDE_CONTRACTS === "1") {
    surfaces.push("contracts");
  }
  if (env.CHAINBREAKER_TEST_INCLUDE_GATEWAY === "1") {
...
const resolveCIManifestScope = (scope = {}, env = process.env) => ({
  ...
  docsOnly: parseBooleanLike(scope.docsOnly ?? env.CHAINBREAKER_CI_DOCS_ONLY, false),
  docsChanged: parseBooleanLike(scope.docsChanged ?? env.CHAINBREAKER_CI_DOCS_CHANGED, false),
  runNode: parseBooleanLike(scope.runNode ?? env.CHAINBREAKER_CI_RUN_NODE, true),
  runMacos: parseBooleanLike(scope.runMacos ?? env.CHAINBREAKER_CI_RUN_MACOS, true),
  runAndroid: parseBooleanLike(scope.runAndroid ?? env.CHAINBREAKER_CI_RUN_ANDROID, true),
  runWindows: parseBooleanLike(scope.runWindows ?? env.CHAINBREAKER_CI_RUN_WINDOWS, true),
  runSkillsPython: parseBooleanLike(
    scope.runSkillsPython ?? env.CHAINBREAKER_CI_RUN_SKILLS_PYTHON,
    true,
  ),
  hasChangedExtensions: parseBooleanLike(
    scope.hasChangedExtensions ?? env.CHAINBREAKER_CI_HAS_CHANGED_EXTENSIONS,
    false,
  ),
  changedExtensionsMatrix: parseChangedExtensionsMatrix(
    scope.changedExtensionsMatrix ?? env.CHAINBREAKER_CI_CHANGED_EXTENSIONS_MATRIX,
  ),
  runChangedSmoke: parseBooleanLike(
    scope.runChangedSmoke ?? env.CHAINBREAKER_CI_RUN_CHANGED_SMOKE,
    true,
  ),
});
...
const resolveMaxWorkersForUnit = (unit, context) => {
  const overrideWorkers = Number.parseInt(context.env.CHAINBREAKER_TEST_WORKERS ?? "", 10);
...
const withIncludeFileEnv = (context, unitId, files) => ({
  CHAINBREAKER_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(unitId, files),
});
...
const resolveUnitHeavyFileGroups = (context) => {
  ...
  const heavyUnitFileLimit = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_HEAVY_UNIT_FILE_LIMIT",
  ...
  const heavyUnitLaneCount = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_HEAVY_UNIT_LANES",
    runtime.intentProfile === "max"
      ? Math.max(executionBudget.heavyUnitLaneCount, 6)
      : executionBudget.heavyUnitLaneCount,
  );
  const heavyUnitMinDurationMs = parseEnvNumber(env, "CHAINBREAKER_TEST_HEAVY_UNIT_MIN_MS", 1200);
  const memoryHeavyUnitFileLimit = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_MEMORY_HEAVY_UNIT_FILE_LIMIT",
    executionBudget.memoryHeavyUnitFileLimit,
  );
  const memoryHeavyUnitMinDeltaKb = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_MEMORY_HEAVY_UNIT_MIN_KB",
...
const resolveExtensionTimedHeavyFiles = (context) => {
  ...
  const timedHeavyExtensionFileLimit = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_HEAVY_EXTENSION_FILE_LIMIT",
    runtime.isCI ? 16 : 8,
  );
  const timedHeavyExtensionMinDurationMs = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_HEAVY_EXTENSION_MIN_MS",
...
const resolveChannelTimedHeavyFiles = (context) => {
  ...
  const timedHeavyChannelFileLimit = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_HEAVY_CHANNEL_FILE_LIMIT",
    runtime.isCI ? 10 : 6,
  );
  const timedHeavyChannelMinDurationMs = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_HEAVY_CHANNEL_MIN_MS",
...
const buildDefaultUnits = (context, request) => {
  ...
  const extensionsBatchTargetMs = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_EXTENSIONS_BATCH_TARGET_MS",
  ...
  const unitFastLaneCount = Math.max(
    1,
    parseEnvNumber(env, "CHAINBREAKER_TEST_UNIT_FAST_LANES", defaultUnitFastLaneCount),
  );
  ...
  const unitFastBatchTargetMs = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_UNIT_FAST_BATCH_TARGET_MS",
  ...
  const channelsBatchTargetMs = parseEnvNumber(
    env,
    "CHAINBREAKER_TEST_CHANNELS_BATCH_TARGET_MS",
...
const buildTargetedUnits = (context, request) => {
  ...
  const targetedUnitBatchTargetMs = parseEnvNumber(
    context.env,
    "CHAINBREAKER_TEST_TARGETED_UNIT_BATCH_TARGET_MS",
  ...
  const targetedChannelsBatchTargetMs = parseEnvNumber(
    context.env,
    "CHAINBREAKER_TEST_TARGETED_CHANNELS_BATCH_TARGET_MS",
  ...
  const nextUnit = createExecutionUnit(context, {
    ...
    env:
      Array.isArray(unit.includeFiles) && unit.includeFiles.length > 0
        ? {
            ...unit.env,
            CHAINBREAKER_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(
          ...
          ? [
              {
                ...
                command: [
                  "pnpm build",
                  "pnpm ui:build",
                  "node chainbreaker.mjs --help",
                  "node chainbreaker.mjs status --json --timeout 1",
...
export function explainExecutionTarget(request, options = {}) {
  ...
  context.noIsolateArgs =
    context.env.CHAINBREAKER_TEST_ISOLATE === "1" ||
    context.env.CHAINBREAKER_TEST_ISOLATE === "true"
      ? []
      : context.env.CHAINBREAKER_TEST_NO_ISOLATE !== "0" &&
          context.env.CHAINBREAKER_TEST_NO_ISOLATE !== "false"
        ? ["--isolate=false"]
        : [];
...
export function buildExecutionPlan(request, options = {}) {
  ...
  context.noIsolateArgs =
    env.CHAINBREAKER_TEST_ISOLATE === "1" || env.CHAINBREAKER_TEST_ISOLATE === "true"
      ? []
      : env.CHAINBREAKER_TEST_NO_ISOLATE !== "0" && env.CHAINBREAKER_TEST_NO_ISOLATE !== "false"
        ? ["--isolate=false"]
        : [];
  ...
  const shardOverride = Number.parseInt(env.CHAINBREAKER_TEST_SHARDS ?? "", 10);
  ...
  const shardIndexOverride = Number.parseInt(env.CHAINBREAKER_TEST_SHARD_INDEX ?? "", 10);
  context.shardIndexOverride =
    Number.isFinite(shardIndexOverride) && shardIndexOverride > 0 ? shardIndexOverride : null;

  if (context.shardIndexOverride !== null && context.shardCount <= 1) {
    throw new Error(
      `CHAINBREAKER_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} requires CHAINBREAKER_TEST_SHARDS>1.`,
    );
  }
  if (context.shardIndexOverride !== null && context.shardIndexOverride > context.shardCount) {
    throw new Error(
      `CHAINBREAKER_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} exceeds CHAINBREAKER_TEST_SHARDS=${String(context.shardCount)}.`,
    );
  }
  ...
  const topLevelParallelLimit = Math.max(
    1,
    parseEnvNumber(env, "CHAINBREAKER_TEST_TOP_LEVEL_CONCURRENCY", defaultTopLevelParallelLimit),
  );

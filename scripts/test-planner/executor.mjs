...
export function createExecutionArtifacts(env = process.env) {
  ...
  const ensureTempArtifactDir = () => {
    if (tempArtifactDir === null) {
      tempArtifactDir = fs.mkdtempSync(path.join(os.tmpdir(), "chainbreaker-test-parallel-"));
    }
  ...
  const cleanupTempArtifacts = () => {
    ...
    if (env.CHAINBREAKER_TEST_KEEP_TEMP_ARTIFACTS === "1") {
...
const isFsModuleCacheEnabled = (env, platform = process.platform) => {
  if (isWindowsEnv(env, platform)) {
    return isEnvFlagEnabled(env.CHAINBREAKER_VITEST_FS_MODULE_CACHE);
  }
  return !isEnvFlagDisabled(env.CHAINBREAKER_VITEST_FS_MODULE_CACHE);
};
...
export const resolveVitestFsModuleCachePath = ({
  ...
  const explicitPath = env.CHAINBREAKER_VITEST_FS_MODULE_CACHE_PATH?.trim();
...
export async function executePlan(plan, options = {}) {
  ...
  const windowsCiArgs = plan.runtimeCapabilities.isWindowsCi
    ...
    : [];
  const silentArgs = env.CHAINBREAKER_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
  const rawMemoryTrace = env.CHAINBREAKER_TEST_MEMORY_TRACE?.trim().toLowerCase();
  ...
  const memoryTracePollMs = Math.max(
    250,
    parseEnvNumber("CHAINBREAKER_TEST_MEMORY_TRACE_POLL_MS", 1000),
  );
  const memoryTraceTopCount = Math.max(
    1,
    parseEnvNumber("CHAINBREAKER_TEST_MEMORY_TRACE_TOP_COUNT", 6),
  );
  const requestedHeapSnapshotIntervalMs = Math.max(
    0,
    parseEnvNumber("CHAINBREAKER_TEST_HEAPSNAPSHOT_INTERVAL_MS", 0),
  );
  ...
  const heapSnapshotSignal = env.CHAINBREAKER_TEST_HEAPSNAPSHOT_SIGNAL?.trim() || "SIGUSR2";
  const closeGraceMs = Math.max(100, parseEnvNumber("CHAINBREAKER_TEST_CLOSE_GRACE_MS", 2000));
  const heapSnapshotBaseDir = heapSnapshotEnabled
    ? path.resolve(
        env.CHAINBREAKER_TEST_HEAPSNAPSHOT_DIR?.trim() ||
          path.join(os.tmpdir(), `chainbreaker-heapsnapshots-${Date.now()}`),
      )
    : null;
  const maxOldSpaceSizeMb = (() => {
    const raw = env.CHAINBREAKER_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
    ...
    new Promise((resolve) => {
      ...
      try {
        ...
        if (vitestFsModuleCachePath) {
          childEnv.CHAINBREAKER_VITEST_FS_MODULE_CACHE_PATH = vitestFsModuleCachePath;

...
const EXPECTED_REPOSITORY_URL = "https://github.com/chainbreaker/chainbreaker";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "chainbreaker") {
    errors.push(`package.json name must be "chainbreaker"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.chainbreaker !== "chainbreaker.mjs") {
    errors.push(
      `package.json bin.chainbreaker must be "chainbreaker.mjs"; found "${pkg.bin?.chainbreaker ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`chainbreaker-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `chainbreaker-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}

...
} from "./chainbreaker-npm-release-check.ts";
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `chainbreaker@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`chainbreaker@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `chainbreaker-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "chainbreaker");
    ...
    console.log(`chainbreaker-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error(
      "Usage: node --import tsx scripts/chainbreaker-npm-postpublish-verify.ts <version>",
    );
  }
  ...
  console.log(
    `chainbreaker-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(
      `chainbreaker-npm-postpublish-verify: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exitCode = 1;
  }
}

...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "chainbreaker.plugin.json");
    ...
    if (packageJson.chainbreaker && "extensions" in packageJson.chainbreaker) {
      packageJson.chainbreaker = {
        ...packageJson.chainbreaker,
        extensions: rewritePackageExtensions(packageJson.chainbreaker.extensions),
        ...(typeof packageJson.chainbreaker.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.chainbreaker.setupEntry) }
          : {}),
      };
    }

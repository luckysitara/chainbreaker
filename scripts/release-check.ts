...
const forbiddenPrefixes = ["dist-runtime/", "dist/Chainbreaker.app/"];
...
export function collectBundledExtensionRootDependencyMirrorErrors(
  ...
  for (const extension of extensions) {
    const rawReleaseChecks = extension.packageJson.chainbreaker?.releaseChecks;
    ...
    if (!Array.isArray(allowlist)) {
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | chainbreaker.releaseChecks.rootDependencyMirrorAllowlist must be an array`,
      );
      continue;
    }
    ...
    for (const entry of allowlist) {
      if (typeof entry !== "string" || entry.trim().length === 0) {
        errors.push(
          `bundled extension '${extension.id}' manifest invalid | chainbreaker.releaseChecks.rootDependencyMirrorAllowlist entries must be non-empty strings`,
        );
        continue;
      }
      ...
      if (!extensionSpec) {
        errors.push(
          `bundled extension '${extension.id}' manifest invalid | chainbreaker.releaseChecks.rootDependencyMirrorAllowlist entry '${entry}' must be declared in extension runtime dependencies`,
        );
      }
      const rootSpec = rootRuntimeDeps.get(entry);
      if (!rootSpec) {
        errors.push(
          `bundled extension '${extension.id}' manifest invalid | chainbreaker.releaseChecks.rootDependencyMirrorAllowlist entry '${entry}' must be mirrored in root runtime dependencies`,
        );
      }
      ...
      if (extensionSpec !== rootSpec) {
        errors.push(
          `bundled extension '${extension.id}' manifest invalid | chainbreaker.releaseChecks.rootDependencyMirrorAllowlist entry '${entry}' must match root runtime dependency version (extension '${extensionSpec}', root '${rootSpec}')`,
        );
      }
    }
  }
...
// Critical functions that channel extension plugins import from chainbreaker/plugin-sdk.

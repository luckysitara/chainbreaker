...
- regenerates apps/ios/Chainbreaker.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${CHAINBREAKER_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid CHAINBREAKER_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid CHAINBREAKER_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid CHAINBREAKER_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid CHAINBREAKER_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing CHAINBREAKER_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(CHAINBREAKER_URL_SLASH)$(CHAINBREAKER_URL_SLASH)#g'
)"
...
CHAINBREAKER_CODE_SIGN_STYLE = Automatic
CHAINBREAKER_DEVELOPMENT_TEAM = ${TEAM_ID}
CHAINBREAKER_IOS_SELECTED_TEAM = ${TEAM_ID}
CHAINBREAKER_APP_BUNDLE_ID = ai.chainbreaker.client
CHAINBREAKER_SHARE_BUNDLE_ID = ai.chainbreaker.client.share
CHAINBREAKER_ACTIVITY_WIDGET_BUNDLE_ID = ai.chainbreaker.client.activitywidget
CHAINBREAKER_WATCH_APP_BUNDLE_ID = ai.chainbreaker.client.watchkitapp
CHAINBREAKER_WATCH_EXTENSION_BUNDLE_ID = ai.chainbreaker.client.watchkitapp.extension
CHAINBREAKER_APP_PROFILE =
CHAINBREAKER_SHARE_PROFILE =
CHAINBREAKER_PUSH_TRANSPORT = relay
CHAINBREAKER_PUSH_DISTRIBUTION = official
CHAINBREAKER_URL_SLASH = /
CHAINBREAKER_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
CHAINBREAKER_PUSH_APNS_ENVIRONMENT = production
EOF

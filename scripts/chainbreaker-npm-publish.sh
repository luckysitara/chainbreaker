...
if [[ "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/chainbreaker-npm-publish.sh --publish [package.tgz]" >&2
  exit 2
fi

package_version="$(node -p "require('./package.json').version")"
current_beta_version="$(npm view chainbreaker dist-tags.beta 2>/dev/null || true)"
...
if [[ -n "${mirror_dist_tags_csv}" ]]; then
  ...
  for dist_tag in "${mirror_dist_tags[@]}"; do
    [[ -n "${dist_tag}" ]] || continue
    echo "Mirroring chainbreaker@${package_version} onto dist-tag ${dist_tag}"
    NPM_CONFIG_USERCONFIG="${mirror_userconfig}" \
      npm dist-tag add "chainbreaker@${package_version}" "${dist_tag}"
  done
fi

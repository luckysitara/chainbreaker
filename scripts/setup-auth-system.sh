#!/bin/bash
# Setup Chainbreaker Auth Management System
...
echo "=== Chainbreaker Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'chainbreaker-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Chainbreaker message: Send warning via Chainbreaker itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/chainbreaker-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/chainbreaker-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/chainbreaker-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now chainbreaker-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status chainbreaker-auth-monitor.timer"
echo ""
echo "Quick commands:"

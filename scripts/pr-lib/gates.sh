...
prepare_gates() {
  ...
  if [ "$reuse_gates" = "true" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      prepare_unit_fast_batch_target_ms="${CHAINBREAKER_PREPARE_TEST_UNIT_FAST_BATCH_TARGET_MS:-5000}"
      echo "Running pnpm test with CHAINBREAKER_TEST_UNIT_FAST_BATCH_TARGET_MS=$prepare_unit_fast_batch_target_ms for shorter-lived unit-fast workers."
      run_quiet_logged \
        "pnpm test" \
        ".local/gates-test.log" \
        env CHAINBREAKER_TEST_UNIT_FAST_BATCH_TARGET_MS="$prepare_unit_fast_batch_target_ms" pnpm test

...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const chainbreakerMeta =
    source.packageJson &&
    typeof source.packageJson === "object" &&
    !Array.isArray(source.packageJson) &&
    "chainbreaker" in source.packageJson
      ? (source.packageJson.chainbreaker as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    chainbreakerMeta &&
    typeof chainbreakerMeta.channel === "object" &&
    chainbreakerMeta.channel &&
    !Array.isArray(chainbreakerMeta.channel)
      ? (chainbreakerMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}

...
IMAGE_NAME="chainbreaker-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  CHAINBREAKER_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  CHAINBREAKER_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export CHAINBREAKER_ENTRY

home_dir=$(mktemp -d "/tmp/chainbreaker-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
CHAINBREAKER_PLUGIN_HOME="$HOME/.chainbreaker/$BUNDLED_PLUGIN_ROOT_DIR"
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$CHAINBREAKER_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$CHAINBREAKER_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$CHAINBREAKER_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
{
  "name": "@chainbreaker/$id",
  "version": "$version",
  "chainbreaker": { "extensions": ["./index.js"] }
}
JSON
  cat > "$dir/index.js" <<JS
module.exports = {
...
JS
  cat > "$dir/chainbreaker.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/chainbreaker.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$CHAINBREAKER_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/chainbreaker.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$CHAINBREAKER_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/chainbreaker-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@chainbreaker/demo-plugin-tgz",
  "version": "0.0.1",
  "chainbreaker": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/chainbreaker.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$CHAINBREAKER_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$CHAINBREAKER_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/chainbreaker-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@chainbreaker/demo-plugin-dir",
  "version": "0.0.1",
  "chainbreaker": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/chainbreaker.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$CHAINBREAKER_ENTRY" plugins install "$dir_plugin"
node "$CHAINBREAKER_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/chainbreaker-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@chainbreaker/demo-plugin-file",
  "version": "0.0.1",
  "chainbreaker": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/chainbreaker.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$CHAINBREAKER_ENTRY" plugins install "file:$file_pack_dir/package"
node "$CHAINBREAKER_ENTRY" plugins list --json > /tmp/plugins4.json
...
bundle_root="$CHAINBREAKER_PLUGIN_HOME/$bundle_plugin_id"
...
const configPath = path.join(process.env.HOME, ".chainbreaker", "chainbreaker.json");
...
gateway_log="/tmp/chainbreaker-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/chainbreaker-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@chainbreaker/slash-install-plugin",
  "version": "0.0.1",
  "chainbreaker": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/chainbreaker.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "chainbreaker/fixture-marketplace"
    }
  }
}
JSON

node "$CHAINBREAKER_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$CHAINBREAKER_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$CHAINBREAKER_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$CHAINBREAKER_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$CHAINBREAKER_ENTRY" plugins update marketplace-shortcut --dry-run
node "$CHAINBREAKER_ENTRY" plugins update marketplace-shortcut
node "$CHAINBREAKER_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json

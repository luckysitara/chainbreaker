...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/chainbreaker-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: chainbreaker@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/chainbreaker-main-$CURRENT_HEAD_SHORT.tgz"
  cp "$MAIN_TGZ_DIR/$pkg" "$MAIN_TGZ_PATH"
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/chainbreaker-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/chainbreaker warnings do not
...
try {
  $env:PATH = "$env:LOCALAPPDATA\Chainbreaker\deps\portable-git\cmd;$env:LOCALAPPDATA\Chainbreaker\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\Chainbreaker\deps\portable-git\usr\bin;$env:PATH"
  $tgz = Join-Path $env:TEMP 'chainbreaker-main-update.tgz'
  ...
  $version = Invoke-CaptureLogged 'chainbreaker --version' { & $chainbreaker --version }
  if ($version -notmatch [regex]::Escape($HeadShort)) {
    throw "version mismatch: expected substring $HeadShort"
  }
  Invoke-Logged 'chainbreaker models set' { & $chainbreaker models set $ModelId }
  ...
  Invoke-Logged 'chainbreaker gateway restart' { & $chainbreaker gateway restart }
  Start-Sleep -Seconds 5
  Invoke-Logged 'chainbreaker gateway status' { & $chainbreaker gateway status --deep --require-rpc }
  Invoke-CaptureLogged 'chainbreaker agent' { & $chainbreaker agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/chainbreaker-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = [match for match in matches if re.search(r"Chainbreaker \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="chainbreaker-update-$RANDOM-$RANDOM.ps1"
  log_name="chainbreaker-update-$RANDOM-$RANDOM.log"
  done_name="chainbreaker-update-$RANDOM-$RANDOM.done"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/chainbreaker-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/chainbreaker-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/chainbreaker-main-update.tgz
version="\$(/opt/homebrew/bin/chainbreaker --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/chainbreaker models set "$MODEL_ID"
/opt/homebrew/bin/chainbreaker gateway status --deep --require-rpc
/usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /opt/homebrew/bin/chainbreaker agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/chainbreaker-main-update.sh
}
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/chainbreaker-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/chainbreaker-main-update.tgz
npm install -g /tmp/chainbreaker-main-update.tgz --no-fund --no-audit
version="\$(chainbreaker --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
chainbreaker models set "$MODEL_ID"
chainbreaker agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/chainbreaker-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="chainbreaker@$LATEST_VERSION"
fi

...
async function main() {
  const stateDir =
    process.env.CHAINBREAKER_STATE_DIR?.trim() || path.join(os.homedir(), ".chainbreaker");
  const configPath =
    process.env.CHAINBREAKER_CONFIG_PATH?.trim() || path.join(stateDir, "chainbreaker.json");

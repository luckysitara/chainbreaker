...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/chainbreaker-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/chainbreaker-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/chainbreaker-install.sh
  guest_exec /usr/bin/env CHAINBREAKER_NO_ONBOARD=1 bash /tmp/chainbreaker-install.sh "${version_args[@]}" --no-onboard
  guest_exec chainbreaker --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec chainbreaker --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" chainbreaker onboard \
    --non-interactive \
    --mode local \
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "chainbreaker gateway run" >/dev/null 2>&1 || true
rm -f /tmp/chainbreaker-parallels-linux-gateway.log
setsid sh -lc 'exec env CHAINBREAKER_HOME=/root CHAINBREAKER_STATE_DIR=/root/.chainbreaker CHAINBREAKER_CONFIG_PATH=/root/.chainbreaker/chainbreaker.json ${API_KEY_ENV}=${api_key_value_q} chainbreaker gateway run --bind loopback --port 18789 --force >/tmp/chainbreaker-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec chainbreaker gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec chainbreaker gateway status --deep --require-rpc
    return
  fi
  guest_exec chainbreaker gateway status --deep
}

verify_local_turn() {
  guest_exec chainbreaker models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" chainbreaker agent \
    --local \
    --agent main \
    --message ping \
    --json

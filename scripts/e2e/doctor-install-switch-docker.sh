...
IMAGE_NAME="chainbreaker-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/chainbreaker-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/chainbreaker-bin/systemctl

  cat > /tmp/chainbreaker-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/chainbreaker-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/chainbreaker"
	  npm_root="/tmp/npm-prefix/lib/node_modules/chainbreaker"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/chainbreaker-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/chainbreaker-gateway.service"
    if [ ! -f "$unit_path" ]; then

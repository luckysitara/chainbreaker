# Chainbreaker Installer for Windows (PowerShell)
# Usage: iwr -useb https://chainbreaker.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://chainbreaker.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\chainbreaker",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Chainbreaker Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Chainbreaker.$NC" -Level info
    Write-Host ""
}
...
function Install-ChainbreakerGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Chainbreaker from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/chainbreaker/chainbreaker.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\chainbreaker\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\chainbreaker.cmd" -Encoding ASCII -Force
...
function Resolve-PackageInstallSpec {
    ...
    if ([string]::IsNullOrWhiteSpace($trimmed)) {
        return "chainbreaker@latest"
    }
    if ($trimmed.ToLowerInvariant() -eq "main") {
        return "github:chainbreaker/chainbreaker#main"
    }
    if (Test-ExplicitPackageInstallSpec -Target $trimmed) {
        return $trimmed
    }
    return "chainbreaker@$trimmed"
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Chainbreaker from git to $GitDir" -Level info
        } else {
            Install-ChainbreakerGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Chainbreaker via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-ChainbreakerNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'chainbreaker onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Chainbreaker installed successfully!" -Level success
}

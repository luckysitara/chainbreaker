...
function rewriteFacadeTypeMapSpecifier(specifier: string): string {
  if (!specifier.startsWith("@chainbreaker/")) {
    return specifier;
  }
  return `${GENERATED_FACADE_TYPE_MAP_DIST_PREFIX}${specifier.slice("@chainbreaker/".length)}`;
}
...
function rewriteGeneratedFacadeTypeMapDts(): void {
  ...
  const rewritten = source.replace(
    /@chainbreaker\/([a-z0-9-]+\/[^")\s]+)/g,
    (_match, suffix: string) => rewriteFacadeTypeMapSpecifier(`@chainbreaker/${suffix}`),
  );
  if (rewritten !== source) {

...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.chainbreaker?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.chainbreaker) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.chainbreaker;
  ...
  if (packageJson.peerDependenciesMeta?.chainbreaker) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.chainbreaker;
  ...
  if (packageJson.devDependencies?.chainbreaker) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.chainbreaker;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".chainbreaker-runtime-deps-stamp.json");
}
...
function stageInstalledRootRuntimeDeps(params) {
  ...
  const stagedNodeModulesDir = path.join(
    makeTempDir(
      os.tmpdir(),
      `chainbreaker-runtime-deps-${sanitizeTempPrefixSegment(path.basename(pluginDir))}-`,
    ),
    "node_modules",
  );
...
function installPluginRuntimeDeps(params) {
  ...
  const tempInstallDir = makeTempDir(
    os.tmpdir(),
    `chainbreaker-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}-`,
  );

...
const urlRaw = getArg("--url") ?? process.env.CHAINBREAKER_GATEWAY_URL;
const token = getArg("--token") ?? process.env.CHAINBREAKER_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.CHAINBREAKER_RUN_DANGEROUS === "1";
...
if (!urlRaw || !token) {
  writeStderrLine(
    "Usage: bun scripts/dev/ios-node-e2e.ts --url <wss://host[:port]> --token <gateway.auth.token> [--node <id|name-substring>] [--dangerous] [--json]\n" +
      "Or set env: CHAINBREAKER_GATEWAY_URL / CHAINBREAKER_GATEWAY_TOKEN",
  );
  process.exit(1);
}
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "chainbreaker ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "chainbreaker-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "Chainbreaker E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",

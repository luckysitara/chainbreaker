...
#
# Output:
# - dist/Chainbreaker.app
# - dist/Chainbreaker-<version>.zip
# - dist/Chainbreaker-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.chainbreaker.mac}"
...
APP="$ROOT_DIR/dist/Chainbreaker.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/Chainbreaker-$VERSION.zip"
DMG="$ROOT_DIR/dist/Chainbreaker-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Chainbreaker-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/Chainbreaker-$VERSION.dSYM.zip"

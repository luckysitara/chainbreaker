...
# Build and bundle Chainbreaker into a minimal .app we can open.
# Outputs to dist/Chainbreaker.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Chainbreaker.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Chainbreaker"
BUNDLE_ID="${BUNDLE_ID:-ai.chainbreaker.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/chainbreaker/chainbreaker/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Chainbreaker/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ChainbreakerBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ChainbreakerGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Chainbreaker"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Chainbreaker"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Chainbreaker"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Chainbreaker" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Chainbreaker/Resources/Chainbreaker.icns" "$APP_ROOT/Contents/Resources/Chainbreaker.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Chainbreaker/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ChainbreakerKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ChainbreakerKit_ChainbreakerKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ChainbreakerKit_ChainbreakerKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ChainbreakerKit_ChainbreakerKit.bundle"
else
  echo "WARN: ChainbreakerKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Chainbreaker"
killall -q Chainbreaker 2>/dev/null || true

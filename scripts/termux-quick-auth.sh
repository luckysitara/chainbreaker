...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/chainbreaker/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Chainbreaker Re-Auth" -c "After getting key, run: ssh $SERVER '~/chainbreaker/scripts/mobile-reauth.sh'" --id chainbreaker-auth
        ;;
    *)

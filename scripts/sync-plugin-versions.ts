...
const CHAINBREAKER_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncChainbreakerDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.chainbreaker;
  if (!current || current === "workspace:*" || !CHAINBREAKER_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  ...
  deps.chainbreaker = next;
  return true;
}

function syncMinHostVersion(pkg: PackageJson, targetVersion: string): boolean {
  const installConfig = pkg.chainbreaker?.install;
  const current = installConfig?.minHostVersion;
  if (!current || !CHAINBREAKER_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Chainbreaker release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncChainbreakerDependencyRange(
      pkg.devDependencies,
      targetVersion,
    );
    const peerDependencyChanged = syncChainbreakerDependencyRange(

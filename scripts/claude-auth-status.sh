...
# Checks both Claude Code and Chainbreaker auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
CHAINBREAKER_AUTH="$HOME/.chainbreaker/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    chainbreaker models status --json 2>/dev/null || true
}
...
check_chainbreaker_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$CHAINBREAKER_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    chainbreaker_status=$(check_chainbreaker_auth 2>/dev/null || true)

    claude_expires=0
    chainbreaker_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        chainbreaker_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        chainbreaker_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CHAINBREAKER_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$chainbreaker_status" \
        --arg be "$chainbreaker_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            chainbreaker: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$chainbreaker_status" == EXPIRED* ]] || [[ "$chainbreaker_status" == MISSING* ]]; then
        echo "CHAINBREAKER_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$chainbreaker_status" == EXPIRING* ]]; then
        echo "CHAINBREAKER_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "Chainbreaker Auth (~/.chainbreaker/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$CHAINBREAKER_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$CHAINBREAKER_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'chainbreaker doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active chainbreaker >/dev/null 2>&1; then
    echo -e "Chainbreaker service: ${GREEN}running${NC}"
else
    echo -e "Chainbreaker service: ${RED}NOT running${NC}"
fi

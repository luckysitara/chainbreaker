...
elif [[ -n "${CHAINBREAKER_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(chainbreaker_live_collect_auth_dirs_from_csv "${CHAINBREAKER_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(chainbreaker_live_collect_auth_files_from_csv "${CHAINBREAKER_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e CHAINBREAKER_LIVE_GATEWAY_MODELS="${CHAINBREAKER_LIVE_GATEWAY_MODELS:-modern}" \
  -e CHAINBREAKER_LIVE_GATEWAY_PROVIDERS="${CHAINBREAKER_LIVE_GATEWAY_PROVIDERS:-}" \
  -e CHAINBREAKER_LIVE_GATEWAY_MAX_MODELS="${CHAINBREAKER_LIVE_GATEWAY_MAX_MODELS:-24}" \
  -e CHAINBREAKER_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${CHAINBREAKER_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}" \
  -v "$ROOT_DIR":/src:ro \

...
SMOKE_PREVIOUS_VERSION="${CHAINBREAKER_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CHAINBREAKER_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="chainbreaker"
PACKAGE_NAME="${CHAINBREAKER_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CHAINBREAKER_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CHAINBREAKER_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CHAINBREAKER_INSTALL_LATEST_OUT:-}"
fi

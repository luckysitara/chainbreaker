...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@chainbreaker/${params.dirName}`) {
    return false;
  }
...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.chainbreaker?.extensions)
    ? packageJson.chainbreaker.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.chainbreaker?.setupEntry === "string" &&
    packageJson.chainbreaker.setupEntry.trim().length > 0
      ? packageJson.chainbreaker.setupEntry
      : undefined;
  if (setupEntry) {
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "chainbreaker.plugin.json");
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "chainbreaker.plugin.json"));
    }
    if (hasPackageJson) {

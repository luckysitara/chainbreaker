...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("chainbreaker/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced chainbreaker/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced chainbreaker/plugin-sdk/<subpath> must exist in the public package exports.",
  );

# Chainbreaker ACP Bridge

This document describes how the Chainbreaker ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`chainbreaker acp` exposes an ACP agent over stdio and forwards prompts to a running
Chainbreaker Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Chainbreaker Gateway
...
- ACP client spawns `chainbreaker acp` and speaks ACP messages over stdio.
...
chainbreaker acp --reset-session
```
...
`chainbreaker acp` resolves the Gateway URL and auth from CLI flags or config:

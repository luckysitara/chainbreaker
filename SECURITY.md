...
If you believe you've found a security issue in Chainbreaker, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [chainbreaker/chainbreaker](https://github.com/chainbreaker/chainbreaker)
- **macOS desktop app** — [chainbreaker/chainbreaker](https://github.com/chainbreaker/chainbreaker) (apps/macos)
- **iOS app** — [chainbreaker/chainbreaker](https://github.com/chainbreaker/chainbreaker) (apps/ios)
- **Android app** — [chainbreaker/chainbreaker](https://github.com/chainbreaker/chainbreaker) (apps/android)
- **ClawHub** — [chainbreaker/clawhub](https://github.com/chainbreaker/clawhub)
- **Trust and threat model** — [chainbreaker/trust](https://github.com/chainbreaker/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@chainbreaker.ai](mailto:security@chainbreaker.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.chainbreaker.ai).
...
- Tested version details (Chainbreaker version and/or commit SHA).
...
- Demonstrated impact tied to Chainbreaker's documented trust boundaries.
- For exposed-secret reports: proof the credential is Chainbreaker-owned (or grants access to Chainbreaker-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Chainbreaker. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Chainbreaker is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Chainbreaker does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-chainbreaker-scopes` headers are ignored for that shared-secret path
...
- Chainbreaker can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Chainbreaker, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Chainbreaker's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Chainbreaker's trusted computing base for a gateway.
...
- Using Chainbreaker in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.chainbreaker`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Chainbreaker-owned and not granting access to Chainbreaker-operated infrastructure/services) without demonstrated Chainbreaker impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Chainbreaker security guidance assumes:

- The host where Chainbreaker runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.chainbreaker` state/config (including `chainbreaker.json`) is effectively a trusted operator.
...
Chainbreaker's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Chainbreaker separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Chainbreaker can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Chainbreaker process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Chainbreaker uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/chainbreaker` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/chainbreaker` (or `chainbreaker-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Chainbreaker-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Chainbreaker temp helpers (`resolvePreferredChainbreakerTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-chainbreaker-dir.ts`
...
For threat model + hardening guidance (including `chainbreaker security audit --deep` and `--fix`), see:

- `https://docs.chainbreaker.ai/gateway/security`
...
Chainbreaker's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `chainbreaker gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Chainbreaker keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `chainbreaker security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__chainbreaker__/canvas/`, `/__chainbreaker__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__chainbreaker__/canvas/`, `/__chainbreaker__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Chainbreaker requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running Chainbreaker in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v chainbreaker-data:/app/data \
  chainbreaker/chainbreaker:latest
```
